...
import React, { useState, useEffect, useMemo, useRef, useCallback, ReactNode, MouseEvent } from 'react';
...
import { Sliders, Compass, Printer, X, Share2, Download, FileText, UserCheck, Move, Maximize2, RotateCw, Send, TreePine, Building2, Mountain, Handshake, SlidersHorizontal } from 'lucide-react';
...
const AdminDashboard = (typeof import.meta !== 'undefined' && import.meta?.env?.DEV)
  ? React.lazy(() => import('./components/AdminDashboard').then(m => ({ default: m.AdminDashboard })))
  : (() => null) as unknown as React.ComponentType<any>;

const AdminAccessDialog = (typeof import.meta !== 'undefined' && import.meta?.env?.DEV)
...
export default function App() {
  ...
  const handleStudioEmblemTap = useCallback(() => {
    const now = Date.now();
    if (now - studioLastTapTimeRef.current > 5000) {
      studioTapCountRef.current = 1;
    } else {
      studioTapCountRef.current += 1;
    }
    studioLastTapTimeRef.current = now;
  ...
  const [editorialStatuses, setEditorialStatuses] = useState<Record<string, 'CANDIDATE' | 'NEEDS RESEARCH' | 'APPROVED' | 'MERGE CANDIDATE' | 'RETIRED'>>(() => {
    ...
    INITIAL_RECOMMENDATIONS.forEach(r => {
      if (r.publicationStatus === 'CANONICAL' || r.publicationStatus === 'PUBLISHED') {
        initial[r.id] = 'APPROVED';
      } else if (r.publicationStatus === 'NEEDS_ADDITIONAL_RESEARCH') {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      const isPreviewParam = qParams.get('preview') === 'true' || qParams.get('showPreview') === 'true' || qParams.get('includeDrafts') === 'true' || qParams.get('includeResearchCandidates') === 'true';
      if (isPreviewParam) {
        setIsPreviewMode(true);
      }
  ...
  const [customOrbit, setCustomOrbit] = useState<{ orbitX: number; orbitY: number } | null>(() => {
    try {
      const saved = safeStorage.getItem('idemo_custom_orbit_v1');
      if (saved) {
        const parsed = JSON.parse(saved);
  ...
  const [selectedCats, setSelectedCats] = useState<Category[]>(() => {
    try {
      const saved = safeStorage.getItem('idemo_selected_cats_v1');
      if (saved) {
  ...
  useEffect(() => {
    try {
      safeStorage.setItem('idemo_selected_cats_v1', JSON.stringify(selectedCats));
    } catch (e) {}
  }, [selectedCats]);
  ...
  const selectedRec = React.useMemo(() => {
    ...
    let found = userFacingRecommendations.find(r => String(r.id).trim() === normId);
    if (!found && (isPreviewMode || isAdmin || currentScreen === 'studio')) {
      found = allRecommendations.find(r => String(r.id).trim() === normId) || null;
    }
    return found || null;
  }, [selectedRecId, userFacingRecommendations, allRecommendations, isPreviewMode, isAdmin, currentScreen]);
  ...
  return (
      ...
      <AnimatePresence mode="wait">
        ...
        {currentScreen === 'profile' && (
          <ProfileScreen 
            ...
            orbitX={orbitX}
            orbitY={orbitY}
            onOrbitChange={(x: number, y: number, b?: number, t?: number) => {
              setCustomOrbit({ orbitX: x, orbitY: y });
              if (typeof b === 'number') setBudget(b);
              ...
              try {
                ...
                console.warn('Failed to persist custom orbit:', err);
              }
            }}
    ...
    <AnimatePresence>
      {currentScreen === 'studio' && (
        <React.Suspense fallback={<div className="min-h-screen bg-[#FAF9F5] flex items-center justify-center font-mono text-xs text-[#1E2E20]">Loading IDEMO Studio...</div>}>
          <div className="fixed inset-0 z-[200] bg-[#FAF9F5] overflow-auto flex flex-col w-screen h-screen">
            <IdemoStudio
              onReturnToApp={() => setCurrentScreen('home')}
              customRecommendations={customRecommendations}
              editorialStatuses={editorialStatuses}
              onUpdateEditorialStatuses={handleUpdateEditorialStatuses}
              onPreviewInTravelerApp={handlePreviewInTravelerApp}
            />
          </div>
        </React.Suspense>
      )}
  ...
  return (
    ...
    >
      <header className="flex justify-between items-start">
        <div className="space-y-1 cursor-pointer select-none" onClick={onEmblemTap}>
...
export const CONCIERGE_T: Record<string, any> = {
  en: {
    ...
    confirmed_msg: "Your inquiry has been registered in the IDEMO ecosystem. Verified partner proposals and updates will appear directly in My Travel Plan.",
    ...
    warning: "IDEMO Concierge desk support is optional. All inquiries are fulfilled through our verified partner network.",
    temp_gastronomy: [
  ...
  sr: {
    ...
    confirmed_msg: "Ваш упит је евидентиран у IDEMO систему. Понуде верификованих партнера стићи ће директно у Ваш План путовања.",
    ...
    warning: "Подршка консијерж службе је опциона. Сви упити се обрађују кроз IDEMO мрежу верификованих партнера.",
    temp_gastronomy: [
  ...
  es: {
    ...
    confirmed_msg: "Su solicitud ha sido registrada en el sistema IDEMO. Las ofertas de socios verificados aparecerán directamente en Su Plan de Viaje.",
    ...
    warning: "El soporte de Concierge es opcional. Todas las solicitudes se gestionan a través de nuestra red de socios verificados en IDEMO.",
    temp_gastronomy: [
  ...
  de: {
    ...
    confirmed_msg: "Ihre Anfrage wurde im IDEMO-System erfasst. Angebote verifizierter Partner erscheinen direkt in Ihrem Reiseplan.",
    ...
    warning: "Concierge-Unterstützung ist optional. Alle Anfragen werden über unser verifiziertes Partnernetzwerk in IDEMO bearbeitet.",
    temp_gastronomy: [
  ...
  ru: {
    ...
    confirmed_msg: "Ваш запрос зарегистрирован в системе IDEMO. Предложения проверенных партнеров появятся непосредственно в Моем Плане Путешествия.",
    via_whatsapp: "Написать в WhatsApp",
    via_instagram: "Написать в Instagram",
    via_call: "Перейти к звонку",
    close: "Вернуться в гид",
    warning: "Поддержка консьерж-службы опциональна. Все запросы обрабатываются через проверенную сеть партнеров IDEMO.",
    temp_gastronomy: [
  ...
  zh: {
    ...
    confirmed_msg: "您的需求已在 IDEMO 系统中成功登记。认证本土合作伙伴的服务方案与更新将直接呈现在“我的旅行计划”中。",
    via_whatsapp: "通过 WhatsApp 专线发送",
    via_instagram: "通过 Instagram 私信发送",
    via_call: "发起私密电话确认",
    close: "返回浏览",
    warning: "礼宾专线为可选支持服务。所有需求均通过 IDEMO 验证合作伙伴网络进行自动撮合与确认。",
    temp_gastronomy: [
      "申请贵宾专属桌位预订",
      "安排定制主厨品鉴菜单",
      "咨询奢华VIP专座空位"
