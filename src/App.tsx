...
import { AppScreen, Recommendation, Category } from "./types";
import {
...
import {
  VibeSettings,
  DEFAULT_VIBE_SETTINGS,
  calculateVibeMatch,
  VibeCalibrationDashboard,
} from "./components/VibeCalibration";
import {
  REGIONS,
  LocalTransitCard,
  isLocationInRegion,
  ...
  BASE_HUBS,
  getTaxiEstimation,
} from "./components/AreaAndTransit";
...
const AdminAccessDialog = import.meta.env.DEV
  ? React.lazy(() =>
      import("./components/AdminDashboard").then((m) => ({
        default: m.AdminAccessDialog,
      })),
    )
  : ((() => null) as unknown as React.ComponentType<any>);

const IdemoStudio = React.lazy(() =>
  import("./components/IdemoStudio").then((m) => ({ default: m.IdemoStudio })),
);
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
        active
          ? "text-brand-charcoal font-bold scale-105"
          : isQuiet
  ...
  const content = (
    <div className="bg-[#F7F6F0] p-2.5 rounded-xl border border-[#E2DFC2]/60 flex items-center gap-2 text-left">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[8.5px] font-mono uppercase tracking-wider text-brand-charcoal/50 font-bold">
          {label}
        </p>
        <p className="text-[11px] font-sans font-bold text-brand-charcoal truncate">
          {value}
        </p>
      </div>
    </div>
...
export const CYRILLIC_DICTIONARY: Record<
  string,
  { cyrillic: string; phonetic: string; tip: string }
> = {
  "1": {
...
export function getDynamicStyle(
  ...
  if (
    selectedCats.includes(Category.GASTRONOMY) ||
    selectedCats.includes("Gastronomy")
  ) {
    styleKey = "gourmet";
  } else if (
    selectedCats.includes(Category.HISTORY) ||
    selectedCats.includes("History") ||
    selectedCats.includes(Category.WELLBEING) ||
    selectedCats.includes("Wellbeing")
  ) {
    styleKey = "cultural";
  } else if (
    selectedCats.includes(Category.NATURE) ||
    selectedCats.includes("Nature")
  ) {
    styleKey = "hidden";
  } else if (
    selectedCats.includes(Category.CLUBBING) ||
    selectedCats.includes("Clubbing")
  ) {
    styleKey = "beats";
  ...
  if (hr >= 5 && hr < 12) {
    weatherStr =
      language === "sr"
        ...
        : language === "zh"
          ? "清凉的夏日早晨"
          : language === "es"
            ? "mañana fresca de verano"
            : language === "de"
              ? "frischer Sommermorgen"
              : language === "ru"
    ...
    weatherStr =
      language === "sr"
        ? "sunčano letnje popodne"
        : language === "zh"
          ? "阳光明媚的夏日午后"
          : language === "es"
            ? "tarde soleada de verano"
            : language === "de"
              ? "sonniger Nachmittag im Sommer"
              : language === "ru"
    ...
    weatherStr =
      language === "sr"
        ? "vedro letnje noćno nebo"
        : language === "zh"
          ? "晴朗的夏夜星空"
          : language === "es"
            ? "noche despejada de verano"
            : language === "de"
              ? "klarer Sommernachthimmel"
              : language === "ru"
                ? "ясная летняя ночь"
  ...
  if (time <= 4) {
    timeStr =
      language === "sr"
        ? `raspoloživo ${time} sata (mikro-izlet)`
        : language === "zh"
          ? `仅有 ${time} 小时可用 (微旅行)`
          : language === "es"
            ? `${time} horas disponibles (micro-viaje)`
            : language === "de"
              ? `${time} Stunden verfügbar (Mikro-Trip)`
              : language === "ru"
                ? `доступно ${time} ч. (микро-поездка)`
                : `${time} hours available (micro-trip)`;
  } else if (time <= 8) {
    timeStr =
      language === "sr"
        ? `raspoloživo ${time} sati (poludnevni doživljaj)`
        : language === "zh"
          ? `${time} 小时可用 (半日深度体验)`
          : language === "es"
            ? `${time} horas disponibles (inmersión de medio día)`
            : language === "de"
              ? `${time} Stunden verfügbar (Halbtages-Erlebnis)`
              : language === "ru"
                ? `доступно ${time} ч. (на poludnevni doživljaj)`
                : `${time} hours available (half-day immersion)`;
  } else {
    timeStr =
      language === "sr"
        ? `raspoloživo ${time} sati (celodnevno putovanje)`
        : language === "zh"
          ? `${time} 小时可用 (全天深度之旅)`
          : language === "es"
            ? `${time} horas disponibles (viaje de día completo)`
            : language === "de"
              ? `${time} Stunden verfügbar (Tagesausflug)`
              : language === "ru"
                ? `доступно ${time} ч. (полный день)`
  ...
  if (selectedCats.length > 0) {
    ...
    selectedCats.forEach((cat: string) => {
      ...
      let interestStr = "";
      if (language === "sr")
        interestStr = `sklonost ka: ${label.toLowerCase()}`;
      else if (language === "zh") interestStr = `偏好：${label}`;
      else if (language === "es")
        interestStr = `preferencia por ${label.toLowerCase()}`;
      else if (language === "de")
        interestStr = `Vorliebe für ${label.toLowerCase()}`;
      else if (language === "ru")
        interestStr = `предпочтение: ${label.toLowerCase()}`;
    ...
    if (language === "sr") generalStr = "prilagođeno opštem istraživanju";
      ...
      generalStr = "adaptado para exploración general";
    else if (language === "de")
      generalStr = "angepasst an allgemeine Erkundung";
    else if (language === "ru")
      generalStr = "настроено для общего исследования";
  ...
  if (
    ...
    locationStr =
      language === "sr"
        ? "blizu slikovitog ušća Save i Dunava"
        : language === "zh"
          ? "邻近萨瓦河与多瑙河的壮丽交汇处"
          : language === "es"
            ? "cerca de los escénicos ríos Sava y Danubio"
            : language === "de"
              ? "nahe der malerischen Save und Donau"
              : language === "ru"
                ? "близ живописного слияния Савы и Дуная"
    ...
    locationStr =
      language === "sr"
        ? "pogodno za kulinarski centar Beograda"
        : language === "zh"
          ? "便捷通达贝尔格莱德美食中心"
          : language === "es"
            ? "conveniente para el núcleo culinario de Belgrado"
            : language === "de"
              ? "ideal für das kulinarische Herz Belgrads"
              : language === "ru"
                ? "удобно для гастрономического central'nogo kulinarnogo centra"
    ...
    locationStr =
      language === "sr"
        ? "savršeno pozicionirano blizu splavova na Savi"
        : language === "zh"
          ? "完美契合萨瓦河畔的浮船俱乐部 (Splavovi)"
          : language === "es"
            ? "perfectamente alineado con los splavovi del río Sava"
            : language === "de"
              ? "perfekt gelegen nahe den Save-Splavovi"
              : language === "ru"
                ? "отлично подходит для сплавов на реке Сава"
                : "perfectly aligned with Sava River splavovi";
  } else {
    locationStr =
      language === "sr"
        ? "izuzetno blizu kompleksa EXPO 2027"
        : language === "zh"
          ? "高效便捷通达世博会 (EXPO 2027) 展馆"
          : language === "es"
            ? "muy accesible al complejo EXPO 2027"
            : language === "de"
              ? "hervorragende Anbindung zum EXPO 2027 Gelände"
              : language === "ru"
                ? "отличная доступность до комплекса ЭКСПО-2027"
                : "highly accessible to EXPO 2027 complex";
  }

  const whyBullets = [weatherStr, timeStr, ...interestBullets, locationStr];
...
function CalendarMonthView({
  ...
  const monthName =
    language === "sr" ? monthNamesSr[month - 1] : monthNamesEn[month - 1];
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      const validDate =
        ...
        /^\d{4}-\d{2}-\d{2}$/.test(item.scheduledDate)
          ? item.scheduledDate
          : undefined;
...
function resolvePlannerItems(
  storedItems: Recommendation[],
  userFacingRecs: Recommendation[],
): Recommendation[] {
...
export default function App() {
  ...
  const [modifiedRecommendations, setModifiedRecommendations] = useState<
  ...
  const [deletedRecommendationIds, setDeletedRecommendationIds] = useState<
    string[]
  >(() => {
    try {
  ...
  const baseRecommendationsList =
    liveRecommendations ?? INITIAL_RECOMMENDATIONS;
  ...
  const handleUpdateEditorialStatuses = (
    nextStatuses: Record<
      string,
      | "CANDIDATE"
      | "NEEDS RESEARCH"
  ...
  const allRecommendations = useMemo(() => {
    ...
    const approvedDrafts = draftExpansionPool.filter(
      (r) => editorialStatuses[r.id] === "APPROVED",
    );

    const baseItems = [...baseRecommendationsList, ...approvedDrafts].map(
      (item) => {
  ...
  const adminAllRecommendations = useMemo(() => {
    ...
    const baseItems = [...baseRecommendationsList, ...draftExpansionPool].map(
      (item) => {
        if (modifiedRecommendations[item.id]) {
          return { ...item, ...modifiedRecommendations[item.id] };
        }
  ...
  const [scheduledItems, setScheduledItems] = useState<Recommendation[]>(() =>
    getInitialSchedule(),
  );
  const [language, setLanguage] = useState("en");
  const [translationNonce, setTranslationNonce] = useState(0);

  const [confirmedAccuracyRecs, setConfirmedAccuracyRecs] = useState<
    Record<
      string,
      {
        timestamp: string;
  ...
  const [accuracySelectedItem, setAccuracySelectedItem] = useState<any | null>(
    null,
  );
  const [accuracyQuestionStep, setAccuracyQuestionStep] = useState<
    1 | 2 | "success"
  >(1);
  const [accuracyAnswers, setAccuracyAnswers] = useState<{
    accurate: "Yes" | "Mostly" | "NeedsUpdate" | null;
    categories: string[];
    note: string;
  }>({
    accurate: null,
  ...
  const handleConfirmAccuracySubmit = () => {
    ...
    try {
      const savedSubmissions = safeStorage.getItem(
    ...
    const isDuplicate = submissionsList.some(
      (s: any) =>
        s.recId === recId &&
    ...
    try {
      safeStorage.setItem(
        "idemo_accuracy_confirmations_v1",
        JSON.stringify(updatedConfirmations),
      );
    } catch (e) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      logSystemError(
        "Failed to parse query string campaign/deep links",
        "URL_PARSER_ERROR",
      );
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    actionEvents.forEach((ev) =>
      window.addEventListener(ev, handleInactivityTouch),
    );

    const checkLockStatusInterval = setInterval(() => {
      if (Date.now() - absoluteLastActive > 15 * 60 * 1000) {
        // 15 Minute absolute timeout
        setIsAdmin(false);
        logSystemError(
          "Administrative session expired due to inactivity",
          "SECURITY_LOCKOUT",
        );
      }
    ...
    return () => {
      actionEvents.forEach((ev) =>
        window.removeEventListener(ev, handleInactivityTouch),
      );
  ...
  const [networkToast, setNetworkToast] = useState<"online" | "offline" | null>(
    null,
  );
  ...
  const handlePurgeMemories = () => {
    ...
    try {
      safeStorage.getAllKeys().forEach((key) => {
        if (
          key.startsWith("tr_") ||
          key.startsWith("idemo_") ||
          key.startsWith("serbiabre_")
        ) {
  ...
  React.useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("click", handleGlobalClick as any, {
        capture: true,
      });
      ...
      if (conn) {
        ...
        return () => {
          ...
          window.removeEventListener("click", handleGlobalClick as any, {
            capture: true,
          });
  ...
  const [proximityReference, setProximityReference] = useState<
    "expo" | "hotel" | "zemun" | "none"
  >("none");
  ...
  const currentArchetype = React.useMemo(() => {
    ...
    for (const arch of ARCHETYPES) {
      ...
      const totalDivergence =
        catDivergence * 0.5 + budgetDiff * 0.25 + timeDiff * 0.25;
  ...
  const [ratings, setRatings] = useState<
    Record<string, { vibe: "like" | "intrigue" | "dislike"; tags: string[] }>
  >(() => {
    try {
  ...
  const rankedRecommendations = getRankedRecommendations(
    userFacingRecommendations,
    prefs,
  );
  ...
  const toggleLike = (id: string) => {
    ...
    const rec = userFacingRecommendations.find(
      (r) => String(r.id).trim() === normId,
    );
    if (rec) {
  ...
  const scheduleItem = (
    ...
    if (scheduledItems.find((item) => String(item.id).trim() === normId))
      return;
    trackRecSave(normId);
    ...
    const updated = [
      ...scheduledItems,
      { ...rec, id: normId, scheduledDate: validDate },
    ];
  ...
  const updateScheduledDate = (id: string, date: string) => {
    ...
    setScheduledItems((prev) => {
      const updated = prev.map((item) =>
        String(item.id).trim() === normId
          ? { ...item, scheduledDate: validDate }
          : item,
      );
      try {
        safeStorage.setItem("idemo_schedule", JSON.stringify(updated));
  ...
  const selectedRec = React.useMemo(() => {
    ...
    return (
      userFacingRecommendations.find((r) => String(r.id).trim() === normId) ||
      null
    );
  ...
  return (
    <>
      ...
      >
        <AnimatePresence mode="wait">
          ...
          {currentScreen === "details" && selectedRec && (
            <DetailsScreen
              ...
              onSchedule={(date, preventRedirect) =>
                scheduleItem(selectedRec, date, preventRedirect)
              }
              onNavigate={(screen: string) => setCurrentScreen(screen)}
              onRemove={() => removeScheduledItem(selectedRec.id)}
              rating={ratings[selectedRec.id]}
              onSaveRating={(vibe: any, tags: string[]) =>
                saveRating(selectedRec.id, vibe, tags)
              }
              vibeSettings={currentArchetype.targetVibe}
              onSelectRec={handleSelectRec}
              lowSignalMode={lowSignalMode}
            ...
            <ProfileScreen
              ...
              onAddCustomRecommendations={(recs: Recommendation[]) => {
                setCustomRecommendations((prev) => {
                  ...
                  try {
                    safeStorage.setItem(
                      "idemo_custom_recommendations_v1",
                    ...
                    <div className="space-y-4 animate-fade-in">
                      ...
                      <div className="p-4 bg-white border border-border-main/40 rounded-2xl space-y-1">
                        <h5 className="text-[12.5px] font-serif font-bold text-brand-charcoal leading-snug">
                          {language === "sr"
                            ? "Da li je ova preporuka i dalje tačna?"
                            : language === "zh"
                              ? "该推荐地点的信息目前依然准确吗？"
                              : "Is this recommendation still accurate?"}
                        </h5>
                      ...
                      <div className="space-y-2 pt-2">
                        <button
                          onClick={() => {
                            ...
                            const existingConf = confirmedAccuracyRecs[recId];
                            if (existingConf && existingConf.timestamp) {
                              const confTime = new Date(
                                existingConf.timestamp,
                              ).getTime();
                              const diffHrs =
                                (now - confTime) / (1000 * 60 * 60);
                              if (diffHrs < 24) {
                                alert(
                                  language === "sr"
                                    ? "Već ste poslali potvrdu za ovu lokaciju u poslednja 24 sata."
                            ...
                            if (isDuplicate) {
                              console.log(
                                "Ignored identical duplicate submission",
                              );
                              setAccuracyQuestionStep("success");
                              setTimeout(() => {
                                setAccuracyModalOpen(false);
                        ...
                        >
                          <div className="space-y-0.5">
                            ...
                            <span className="text-[10px] text-brand-charcoal/50 block font-medium">
                              {language === "sr"
                                ? "Potvrdite detalje lokacije bez izmena"
                                : language === "zh"
                                  ? "验证位置、价格、营业时间等完整信息"
                                  : "Confirm hours, location & details as is"}
                            </span>
                          </div>
                          <span className="text-brand-charcoal/30 group-hover:text-brand-charcoal transition-colors">
                            ➔
                          </span>
                        </button>
                        ...
                        >
                          <div className="space-y-0.5">
                            ...
                            <span className="text-[10px] text-brand-charcoal/50 block font-medium">
                              {language === "sr"
                                ? "Baza je dobra, ali mali detalji se razlikuju"
                                : language === "zh"
                                  ? "基本信息符合，但个别微小信息有变"
                                  : "The core is right, but minor detail differs"}
                            </span>
                          </div>
                          <span className="text-brand-charcoal/30 group-hover:text-brand-charcoal transition-colors">
                            ➔
                        ...
                        >
                          <div className="space-y-0.5">
                            ...
                            <span className="text-[10px] text-brand-charcoal/50 block font-medium">
                              {language === "sr"
                                ? "Važna promena radnog vremena, cena ili lokacije"
                                : language === "zh"
                                  ? "营业时间、消费情况或位置有较明显变化"
                                  : "Critical updates needed for hours, price, etc."}
                            </span>
                          </div>
                          <span className="text-brand-charcoal/30 group-hover:text-brand-charcoal transition-colors">
                            ➔
                    ...
                    <div className="space-y-4 animate-fade-in">
                        ...
                        {[
                          {
                            ...
                            label_sr: "Radno vreme",
                            label_zh: "营业时间",
                          },
                          {
                            id: "price",
                            label_en: "Price details",
                            label_sr: "Cene",
                            label_zh: "价格变动",
                          },
                          {
                            ...
                            label_sr: "Kontakt podaci",
                            label_zh: "联系方式",
                          },
                          {
                            id: "location",
                            label_en: "Location / Address",
                            label_sr: "Adresa / Lokacija",
                            label_zh: "地理位置",
                          },
                          {
                            id: "perm_closed",
                            label_en: "Closed permanently",
                            label_sr: "Trajno zatvoreno",
                            label_zh: "永久停业",
                          },
                          {
                            id: "temp_closed",
                            label_en: "Temporarily closed",
                            label_sr: "Privremeno zatvoreno",
                            label_zh: "暂停营业",
                          },
                          {
                            id: "desc",
                          ...
                          },
                          {
                            id: "photos",
                            label_en: "Photos",
                            label_sr: "Slike / Galerija",
                            label_zh: "实景照片",
                          },
                          {
                            id: "other",
                            label_en: "Other",
                            label_sr: "Drugo / Ostalo",
                            label_zh: "其它",
                          },
                        ].map((chip) => {
                          const chipLabel =
                            language === "sr"
                              ...
                              : language === "zh"
                                ...
                                : chip.label_en;
                          const isSelected =
                            accuracyAnswers.categories.includes(chipLabel);
                          return (
                            <button
                              key={chip.id}
                              onClick={() => {
                                triggerHaptic(3);
                                setAccuracyAnswers((prev) => {
                                  const updated = prev.categories.includes(
                                    chipLabel,
                                  )
                      ...
                      {!showAccuracyNoteField ? (
                        ...
                        >
                          +{" "}
                          {language === "sr"
                            ? "Dodaj belešku kustosu"
                            : language === "zh"
                              ? "添加附加备注说明"
                              : "Add custom note to editors"}
                        </button>
                      ) : (
                        <div className="space-y-1.5 animate-fade-in pt-1">
                          <label className="text-[9.5px] uppercase tracking-wider text-brand-charcoal/50 font-extrabold block">
                            {language === "sr"
                              ? "Detalji (maksimalno 200 karaktera)"
                              : language === "zh"
                                ? "详细备注（最多200字）"
                  ...
                  {accuracyQuestionStep === 2 && (
                    <>
                      ...
                      <button
                        ...
                        disabled={
                          accuracyAnswers.categories.length === 0 &&
                          accuracyAnswers.note.trim().length === 0
                        }
        ...
        {/* IDEMO Studio Operations Shell */}
        <AnimatePresence>
          {currentScreen === "studio" && (
            <React.Suspense
              fallback={
                <div className="min-h-screen bg-[#FAF9F5] flex items-center justify-center font-mono text-xs text-[#1E2E20]">
                  Loading IDEMO Studio...
                </div>
              }
            >
              <div className="fixed inset-0 z-[200] bg-[#FAF9F5] overflow-auto">
                <IdemoStudio
            ...
            <React.Suspense fallback={null}>
              <AdminDashboard
                ...
                onUpdateModifiedRecommendations={(mods) => {
                  ...
                  try {
                    safeStorage.setItem(
                      "idemo_modified_recommendations_v1",
                      JSON.stringify(mods),
                    );
                  } catch (e) {
                ...
                onUpdateDeletedRecommendationIds={(deleted) => {
                  ...
                  try {
                    safeStorage.setItem(
                      "idemo_deleted_recommendations_v1",
                    ...
                    >
                      {lowSignalMode ? (
                        <div className="w-full h-full bg-[#FAF9F5] border-b border-[#E5E3DB] flex flex-col items-center justify-center p-6 text-center select-none">
                          <span className="text-[24px] mb-2 font-serif opacity-35 text-brand-charcoal/80">
                            ✦
                          </span>
                          <span className="text-xs uppercase tracking-[0.2em] font-black text-brand-charcoal leading-snug px-6">
                            {getLocalizedValue(item, "title", language)}
                          </span>
                          <span className="text-[12px] font-mono tracking-wider text-[#5C5A4D] uppercase mt-4 bg-[#EDEBDF] px-3.5 py-1.5 rounded-full border border-border-main/40 font-bold">
                            Bandwidth Optimized
                          </span>
                        </div>
                      ) : (
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent p-6 flex flex-col justify-end text-white text-left">
                        <div className="flex justify-between items-start mb-auto w-full">
                          <div className="flex flex-col items-start gap-1.5 animate-fade-in-slow">
                            <span className="text-[12px] uppercase tracking-widest text-[#FFF] bg-[#8A1F1F] px-2.5 py-1 rounded-md font-bold leading-none">
                        ...
                        <div className="grid grid-cols-[1fr_auto] gap-4 items-end w-full">
                          <div className="min-w-0 flex flex-col justify-end">
                            <h3 className="text-xl font-serif leading-tight line-clamp-2 text-white">
                              {getLocalizedValue(item, "title", language)}
                            </h3>
                            <div className="mt-2.5 flex items-center gap-2 opacity-85">
                              <div className="h-[1.5px] w-4 bg-white" />
                              <span className="text-[12px] uppercase tracking-widest truncate font-extrabold">
                                {t.explore_narrative}
                              </span>
                            </div>
                          </div>
                ...
                renderDetailsScreen={(rec: any, onBack: () => void) => (
                  <DetailsScreen
                    ...
                    onBack={onBack}
                    onSchedule={(date, preventRedirect) =>
                      scheduleItem(rec, date, preventRedirect)
                    }
                    onNavigate={(screen: string) => setCurrentScreen(screen)}
                    onRemove={() => removeScheduledItem(rec.id)}
                    rating={ratings[rec.id]}
                    onSaveRating={(vibe: any, tags: string[]) =>
                      saveRating(rec.id, vibe, tags)
                    }
                    vibeSettings={currentArchetype.targetVibe}
                    onSelectRec={handleSelectRec}
                    lowSignalMode={lowSignalMode}
        ...
        <AnimatePresence>
          {networkToast && (
            ...
            >
              <div
                className={`px-4 py-2 w-full max-w-[340px] text-[10px] font-mono tracking-wider uppercase font-semibold text-center border rounded-full backdrop-blur-md shadow-[0_10px_25px_rgba(0,0,0,0.1)] flex items-center justify-center gap-2 ${
                  networkToast === "online"
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${networkToast === "online" ? "bg-emerald-400 animate-pulse" : "bg-amber-400 animate-pulse"}`}
                />
                <span>
                  {networkToast === "online"
                    ? language === "sr"
                      ? "Veza uspostavljena • Online"
                      : language === "zh"
                        ? "网络已连接 • 在线模式"
                        : "Network link secured • Online"
                    : language === "sr"
                      ? "Sistem u lokalu • Ofline"
                      : language === "zh"
                        ? "本地缓存运行 • 离线模式"
                        : "Operating in local cache • Offline"}
                </span>
              </div>
            </motion.div>
          )}
      ...
      <div
        id="print-portfolio-element"
        className="relative p-[1.6cm] bg-[#FAF9F5] text-[#2F3126] font-sans antialiased text-[11px] leading-relaxed select-text"
      >
        ...
        <div className="absolute top-[12%] left-[4%] opacity-[0.035] pointer-events-none z-[-10] w-[140px]">
          ...
          >
            ...
            <path
              d="M45,110 L50,115 L55,110 M40,120 L50,127 L60,120 M35,130 L50,140 L65,130"
              strokeWidth="0.4"
            />
          </svg>
        </div>
        ...
        <div className="absolute top-[42%] right-[4%] opacity-[0.035] pointer-events-none z-[-10] w-[170px]">
          ...
          >
            ...
            <path
              d="M10,100 Q 32.5,70 55,100 T 100,100 T 145,100 T 190,100"
              strokeDasharray="2,2"
            />
          </svg>
        </div>
        ...
        <div className="absolute bottom-[10%] left-[8%] opacity-[0.035] pointer-events-none z-[-10] w-[150px]">
          ...
          >
            ...
            <text fontSize="5.5" fontFamily="serif" letterSpacing="1">
              <textPath href="#roman_txt_print" startOffset="5%">
        ...
        <div className="bg-white border border-[#E0DDD5] border-b-[3px] border-b-[#1E2E20] p-6 mb-6 rounded-lg relative">
          <div className="text-[8px] uppercase tracking-[0.4em] text-[#8F8B73] font-bold mb-1.5">
            {language === "sr"
              ? "ЕКСПО БЕОГРАД • ОФИЦИЈЕЛНИ ДЕЛЕГАТСКИ ПЛАН"
              : "EXPO BELGRADE • OFFICIAL DELEGATE PORTFOLIO"}
          </div>
          <h1 className="font-serif text-[26px] font-bold text-[#1E2E20] tracking-tight leading-none mb-2">
            {language === "sr"
              ? "ИТИНЕРАР ЛИЧНИ СЛУЖБЕНИ ПУТ"
              : "ELEGANT VISITOR ITINERARY"}
          </h1>
          ...
          <div className="col-span-5 bg-white border-2 border-[#D5D3C8] rounded-xl p-5 flex flex-col justify-between shadow-xs">
            <div>
              <h2 className="font-serif text-[11.5px] font-extrabold text-[#5C5A4D] uppercase tracking-[0.18em] border-b-2 border-[#FAF9F5] pb-1.5 mb-2.5">
                {language === "sr"
                  ? "УСКЛАЂЕН ПРОФИЛ ПОСЕТИОЦА"
                  : "CALIBRATED VISITOR PROFILE"}
              </h2>
              <h3 className="font-serif text-[16px] font-black text-[#1B5E20] leading-snug mb-3">
                {
                  getDynamicStyle(language, selectedCats, days, budget, time)
                    .styleName
                }
              </h3>
              <div className="space-y-2">
                {getDynamicStyle(
                  ...
                  <div
                    key={idx}
                    className="flex items-start gap-2 text-[11.5px] text-brand-charcoal font-semibold leading-relaxed"
                  >
            ...
            <div className="flex-1 flex flex-col justify-center gap-1.5">
              ...
              {sortedMonths.length > 1 && (
                <p className="text-[7.5px] text-[#8C8A7D] text-center italic mt-1">
                  {language === "sr"
                    ? "*Додатни месеци планирани у наставку"
                    : "*Additional months scheduled below"}
                </p>
              )}
            </div>
          </div>
            ...
            <table className="w-full text-[9px]">
              <tbody>
                ...
                <tr className="border-b border-[#F3F1ED]">
                  ...
                  <td className="py-1 text-[#2F3126]">
                    {language === "sr" ? "Ћирилица" : "Cyrillic & Latin"}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            {[...scheduledItems]
              ...
              .map((item: any, idx: number) => {
                const dateFormatted = item.scheduledDate
                  ? new Date(
                      item.scheduledDate.split("T")[0].replace(/-/g, "/"),
                    ).toLocaleDateString(
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-white border border-[#E5E3DB] rounded-xl overflow-hidden itinerary-card"
                  >
                    <div className="bg-[#F8F7F4] border-b border-[#E5E3DB] px-4 py-2 flex justify-between items-center">
                      <span className="font-mono font-bold text-[#2E7D32] text-[10px]">
                        #{idx + 1}
                      </span>
                      <span className="font-bold text-[10px] text-[#1E2E20] uppercase tracking-wide">
                        {dateFormatted}
                      </span>
                    </div>
                    <div className="p-4 flex gap-5 animate-none">
                      <div className="flex-[1.2]">
                        <h4 className="font-serif text-[15px] font-bold text-[#1E2E20] mb-0.5">
                          {getLocalizedValue(item, "title", language)}
                        </h4>
                        <p className="text-[8px] uppercase tracking-widest text-[#2E7D32] font-black mb-2">
                          {formatCategory(item.category, t)}
                        </p>
                        <p className="text-[9.5px] text-[#8C8A7D] mb-2.5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {item.duration && (
                            <span className="bg-[#F3F1ED] border border-[#E5E3DB] rounded px-2 py-0.5 text-[8.5px] font-medium text-[#5C5E54]">
                              🕒 {language === "sr" ? "Трајање" : "Duration"}:{" "}
                              {item.duration}
                            </span>
                          )}
                          {item.travelTime && (
                            <span className="bg-[#F3F1ED] border border-[#E5E3DB] rounded px-2 py-0.5 text-[8.5px] font-medium text-[#5C5E54]">
                              ⚡ {language === "sr" ? "Транзит" : "Transit"}:{" "}
                              {item.travelTime}
                            </span>
                          )}
                          {item.preferredTransport && (
                            <span className="bg-[#F3F1ED] border border-[#E5E3DB] rounded px-2 py-0.5 text-[8.5px] font-medium text-[#5C5E54]">
                              🚗 {language === "sr" ? "Транспорт" : "Transport"}
                              : {item.preferredTransport}
                            </span>
                          )}
                          {item.estimatedCost && (
                            <span className="bg-[#F3F1ED] border border-[#E5E3DB] rounded px-2 py-0.5 text-[8.5px] font-medium text-[#5C5E54]">
                              💶 {language === "sr" ? "Буџет" : "Cost"}:{" "}
                              {item.estimatedCost}
                            </span>
                          )}
                        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex flex-col justify-center items-center gap-y-3.5 my-1 w-full">
        ...
        <div
          className="relative flex justify-center items-center w-[210px] h-[52px]"
        ...
        >
          ...
          <motion.div
            ...
            initial={{
              y: 0,
              boxShadow:
                "0 1px 2px rgba(35,37,30,0.02), inset 0px 1.5px 1px rgba(255,255,255,0.95)",
            }}
            whileHover={{
              y: 1.5,
              boxShadow:
                "0 0.5px 1px rgba(35,37,30,0.01), inset 0px 1.5px 1px rgba(255,255,255,0.95)",
            }}
            whileTap={{
              y: 5,
      ...
      <div className="flex-shrink-0 w-full max-w-[340px] mx-auto pt-0 mb-2">
        {/* 7. Disclaimer Card */}
        <div
          className="text-center space-y-1 bg-white/30 border border-border-main/8 rounded-[12px] py-2.5 px-4 shadow-[0_1.5px_8px_rgba(35,37,30,0.01)] backdrop-blur-xs flex flex-col items-center justify-center"
          id="refined-disclaimer-card"
        >
...
function PulsatingProfileButton({
  onClick,
  language,
  size = 48,
  iconSize = 20,
    ...
    >
      ...
      >
        <svg
          ...
          className="w-full h-full select-none pointer-events-none overflow-hidden rounded-full"
        >
          <defs>
          ...
          <circle
            r="88"
            fill="url(#orbButtonReflection)"
            className="pointer-events-none mix-blend-overlay"
          />
          <circle
            r="88"
            fill="url(#orbButtonSapphire)"
...
function HomeScreen({
  ...
  const [openedEnvelope, setOpenedEnvelope] = useState<
    "sunset" | "gastronomy" | "after_hours" | null
  >(null);
  ...
  const getMysteryRec = (
    type: "sunset" | "gastronomy" | "after_hours" | null,
  ) => {
    ...
    return (
      recommendations.find((item: any) => {
        ...
        if (type === "sunset") {
          return (
            cats.includes("nature") ||
            cats.includes("travel") ||
            cats.includes("history") ||
            title.includes("lookout") ||
            title.includes("fortress") ||
            title.includes("view") ||
            title.includes("peak") ||
          ...
          return (
            cats.includes("gastronomy") ||
            title.includes("kafana") ||
            title.includes("dining") ||
            title.includes("distillery")
          );
        } else {
          // after_hours
          return (
            cats.includes("clubbing") ||
  ...
  return (
    ...
    >
      <header className="flex justify-between items-start">
        <div className="space-y-1">
          <h2 className="text-4xl font-serif text-brand-charcoal tracking-tighter">
            {t.welcome}
          </h2>
          <p className="text-[13px] text-accent-red font-bold flex items-center gap-2 tracking-normal leading-tight">
            ...
            <span className="max-w-[220px] sm:max-w-md">
              {t.curated_for_you}
            </span>
          </p>
        </div>
      ...
      <section id="recommendations-section" className="relative -mx-6">
        <PremiumCarousel
          ...
          renderItem={(item, isCenter) => (
            <div
              className={`relative h-[480px] rounded-[40px] overflow-hidden shadow-xl transition-all duration-300 ease-out border border-border-main cursor-pointer ${isCenter ? "bg-brand-charcoal" : "bg-brand-charcoal/80"}`}
            >
              {lowSignalMode ? (
                <div className="w-full h-full bg-[#FAF9F5] border-b border-[#E5E3DB] flex flex-col items-center justify-center p-6 text-center select-none">
                  <span className="text-[24px] mb-2 font-serif opacity-35 text-brand-charcoal/80">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        triggerHaptic(8);
                        setActiveTooltipId(
                          activeTooltipId === item.id ? null : item.id,
                        );
                      }}
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="flex items-center gap-2 text-white/90">
                        ...
                        <span className="truncate">
                          <strong>{t.travel_time || "Travel Time"}:</strong>{" "}
                          {item.travelTime}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-white/90">
                        ...
                        <span className="truncate">
                          <strong>{t.primary_transport || "Transport"}:</strong>{" "}
                          {item.preferredTransport}
                        </span>
                      </div>
                    </div>
        ...
        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5 z-30">
          {recommendations.map((_: any, i: number) => (
            <div
              key={i}
              className={`w-1 h-1 rounded-full transition-all duration-500 ${i === activeRecIndex ? "bg-accent-red w-3" : "bg-brand-charcoal/10"}`}
            />
          ))}
        </div>
      </section>
      ...
      <section className="space-y-4 pt-2 border-t border-border-main/20">
        ...
        <div className="grid grid-cols-3 gap-3 pt-1">
          ...
          >
            ...
            <div>
              <span className="block text-[12px] uppercase tracking-wider text-accent-red font-black mb-0.5">
                Summit & Vapour
              </span>
              <span className="block text-[13px] font-serif text-brand-charcoal font-extrabold leading-tight">
                Secret Vista
              </span>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <span className="block text-[12px] uppercase tracking-wider text-[#5C5A4D] font-black mb-0.5">
                Kafana Ritual
              </span>
              <span className="block text-[13px] font-serif text-brand-charcoal font-extrabold leading-tight">
                Curated Dining
          ...
          >
            ...
            <div>
              <span className="block text-[12px] uppercase tracking-wider text-accent-teal font-black mb-0.5">
                After-Hours
              </span>
              <span className="block text-[13px] font-serif text-brand-charcoal font-extrabold leading-tight">
                Midnight Secrets
            ...
            >
              ...
              {!envelopeRevealed ? (
                <div className="space-y-6 py-6 w-full flex flex-col items-center">
                  ...
                  <div className="space-y-2">
                    <span className="text-[10px] uppercase tracking-[0.3em] font-black text-accent-red">
                      SECRET NO:{" "}
                      {openedEnvelope === "sunset"
                        ? "01"
                        : openedEnvelope === "gastronomy"
                          ? "02"
                          : "03"}
                    </span>
                    <h4 className="text-xl font-serif text-brand-charcoal">
                      {openedEnvelope === "sunset"
                        ? "The Summit & Vapour Link"
                        : openedEnvelope === "gastronomy"
                          ? "The Kafana Code Link"
                          : "The Gilded After-Hours Link"}
                    </h4>
                    <p className="text-xs text-brand-charcoal/60 leading-relaxed max-w-[240px] mx-auto">
                      This sealed dispatch contains a personalized
                      recommendation aligned with your active vibe match matrix
                      and the Belgrade Waterfront spring calendar.
                    </p>
                  </div>
                ...
                >
                  <div className="border-b border-border-main/50 pb-4">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[8px] uppercase tracking-[0.2em] font-black text-accent-red">
                        REVEALED DISPATCH
                      </span>
                      <span className="text-[9px] font-mono text-[#8C8A7D]">
                        🌿 DYNAMIC CALIBRATION MATCH
                      </span>
                    </div>
                  ...
                  <div className="space-y-3.5">
                    <div className="p-4 bg-[#F5F4EE] rounded-2xl border border-border-main/40 text-xs text-[#3A3D32] leading-relaxed relative">
                      <p className="font-serif italic text-brand-charcoal/80 mb-2 font-medium font-bold text-accent-red">
                        Concierge Secret Note:
                      </p>
                      <p className="font-sans font-light">
                        {CYRILLIC_DICTIONARY[getMysteryRec(openedEnvelope).id]
                          ?.tip ||
                          getMysteryRec(openedEnvelope).shortDescription}
                      </p>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[10px] text-brand-charcoal/70 bg-white/40 border border-[#EBEBE6] p-3 rounded-2xl">
                      <div>
                        <span className="block text-[#8C8A7D] uppercase tracking-wider text-[7.5px] font-bold">
                          Duration
                        </span>
                        <span className="font-medium text-brand-charcoal">
                          {getMysteryRec(openedEnvelope).duration}
                        </span>
                      </div>
                      <div>
                        <span className="block text-[#8C8A7D] uppercase tracking-wider text-[7.5px] font-bold">
                        ...
                        <span className="font-medium text-brand-charcoal">
                          {getMysteryRec(openedEnvelope).preferredTransport}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="max-h-[300px] overflow-y-auto pr-1 text-[16.5px] text-brand-charcoal/80 leading-relaxed font-normal space-y-3 font-sans">
                  <p>
                    {getLocalizedValue(selectedTip, "description", language)}
                  </p>
                  {selectedTip.equivalentPhrases && (
                    <div className="pt-3 border-t border-border-main/20 mt-3 bg-brand-pearl/25 p-3.5 rounded-2xl border border-border-main/10">
                      <span className="block text-[11px] uppercase tracking-wider text-[#8C8A7D] font-black mb-1">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="max-h-[350px] overflow-y-auto pr-1 space-y-4">
                  ...
                  <div className="pt-4 border-t border-border-main">
                    <p className="text-[12px] uppercase tracking-[0.3em] text-accent-red font-black mb-1.5">
                      {t.revelation_label}
                    </p>
      ...
      <section className="space-y-4 pt-2 border-t border-border-main/20">
        <div className="space-y-1">
          ...
          <p className="text-[12px] uppercase tracking-[0.2em] font-extrabold text-[#5C5A4D]">
            {language === "sr"
              ? "SVE ŠTO TREBA DA ZNATE O SRBIJI"
              : language === "zh"
                ? "帮助您深入了解塞尔维亚的专属窗口"
                : "WHAT SHOULD I KNOW ABOUT SERBIA?"}
          </p>
        </div>
              ...
              <div className="relative -mx-6">
                <PremiumCarousel
                  ...
                  renderItem={(tip, isCenter) => (
                    <div
                      className={`h-[400px] p-6 rounded-[32px] flex flex-col justify-between items-start text-left group transition-all duration-300 ${
                        isCenter
                    ...
                    >
                      <div className="w-full">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-[12px] uppercase tracking-widest text-accent-red font-bold">
                            {formatCategory(tip.category, t)}
                          </span>
                          <span className="text-[15px] font-serif italic text-brand-charcoal/40">
                            {tip.id.padStart(2, "0")}
                          </span>
                        </div>
                        <h4 className="text-[24px] font-serif font-bold leading-tight text-brand-charcoal mb-2">
                          {getLocalizedValue(tip, "title", language)}
                        </h4>
                        <p className="text-[21px] text-brand-charcoal/70 leading-relaxed font-normal line-clamp-4">
                          {getLocalizedValue(tip, "description", language)}
                        </p>
            ...
            >
              <div className="relative -mx-6">
                <PremiumCarousel
                  ...
                  renderItem={(item, isCenter) => (
                    <div
                      className={`h-[400px] p-6 rounded-[32px] space-y-4 flex flex-col justify-between transition-all duration-300 ${
                        isCenter
                          ? "bg-white border-2 border-brand-charcoal/25 shadow-[0_12px_28px_rgba(45,48,37,0.14)]"
                          : "bg-[#EDE9DE] border border-border-main shadow-[0_4px_12px_rgba(0,0,0,0.06)]"
                      }`}
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <div className="w-2.5 h-2.5 bg-accent-red rounded-sm rotate-45" />
                          <p className="text-[13.5px] uppercase font-black tracking-[0.2em] text-[#8C8A7D]">
                            {t.mystery_label}
                          </p>
                        </div>
                      </div>
                      <p className="text-[21px] font-serif text-brand-charcoal leading-snug flex-1">
                        {getLocalizedValue(item, "fact", language)}
                      </p>
                      <div className="pt-4 border-t border-border-main flex flex-col justify-between items-start gap-2.5 w-full">
                        <div className="w-full">
                          <p className="text-[12px] uppercase tracking-[0.3em] text-accent-red font-black mb-1">
                            {t.revelation_label}
                          </p>
                          <p className="text-[16.5px] text-[#5C5E54] leading-relaxed italic line-clamp-2">
                            {getLocalizedValue(item, "whyItMatters", language)}
                          </p>
                        </div>
                        <span className="text-[18px] uppercase tracking-[0.1em] text-[#8A1F1F] font-black opacity-90">
                          {language === "sr"
        ...
        <div className="flex items-end justify-between">
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-[0.4em] text-accent-red font-black">
              {language === "sr"
                ? "AKTUELNA SEZONA U SRBIJI"
                : language === "zh"
                  ? "本季限定游览精选"
                  : "THIS SEASON IN SERBIA"}
            </p>
            ...
            renderItem={(tip, isCenter) => (
              ...
              >
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-between text-white backdrop-blur-[1px]">
                  <div className="space-y-1">
                    <p className="text-[11px] uppercase tracking-[0.4em] font-black opacity-80">
                      {tip.subtitle}
                    </p>
                    <h3 className="text-4xl font-serif tracking-tighter leading-none font-bold">
                      {tip.title}
                    </h3>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-3 border-t border-white/20 flex items-center justify-between">
                      <p className="text-[10px] uppercase tracking-[0.3em] font-bold opacity-60 italic">
                        {tip.mood}
                      </p>
                    </div>
                  </div>
...
const FEEDBACK_TAGS = [
  {
    id: "authentic",
    label: {
      en: "Authentic Curation",
      sr: "Autentična priča",
      es: "Curación auténtica",
      de: "Authentisch",
      ru: "Аутентично",
      zh: "正宗精选",
    },
  },
  {
    id: "peaceful",
    label: {
      en: "Serene Atmosphere",
      sr: "Spokojna atmosfera",
      es: "Ambiente sereno",
      de: "Ruhig & Gelassen",
      ru: "Умиротворенно",
      zh: "幽静宁谧",
...
export const CONCIERGE_T: Record<string, any> = {
  en: {
    ...
    placeholder:
      "e.g., Preferred time, party size, dietary bounds, language needs...",
    ...
    confirmed_msg:
      "Your brokerage request has been recorded. To retain priority VIP status and premium pricing, please complete this scheduling through our designated secure desk lines:",
  ...
  sr: {
    ...
    placeholder:
      "нпр. Жељено време, број особа, дијететске преференције, језик...",
    ...
    confirmed_msg:
      "Ваш упут је забележен. Да бисте задржали ВИП статус и повлашћене цене, молимо вас да довршите разговор путем наших заштићених каналских линија:",
    ...
    warning:
      "Директан контакт са објектом ван нашег система поништава ВИП погодности.",
    temp_gastronomy: [
  ...
  es: {
    ...
    placeholder:
      "ej., Hora de preferencia, número de personas, alergias, idioma...",
    ...
    confirmed_msg:
      "Su solicitud exclusiva ha sido registrada de forma segura. Para mantener los privilegios VIP y las tarifas de convenio, finalice el acuerdo a través de nuestros canales oficiales:",
    ...
    warning:
      "El contacto directo sin nuestro código de intermediación invalida las facilidades VIP.",
    temp_gastronomy: [
  ...
  de: {
    ...
    placeholder:
      "z.B. Bevorzugte Uhrzeit, Personenanzahl, Ernährung, Sprache...",
    ...
    confirmed_msg:
      "Ihre exklusive Anfrage wurde erfasst. Um den VIP-Status und die ermäßigten Preise zu behalten, schließen Sie die Buchung bitte über unsere sicheren Leitungen ab:",
    ...
    warning:
      "Direktbuchungen ohne unseren Vermittlungscode können VIP-Vorteile aufheben.",
    temp_gastronomy: [
  ...
  ru: {
    ...
    placeholder:
      "например, желаемое время, количество гостей, предпочтения по меню...",
    ...
    confirmed_msg:
      "Ваш эксклюзивный запрос успешно оформлен. Для сохранения закрытых тарифов и VIP-привилегий завершите бронирование через наши безопасные каналы связи:",
  ...
  zh: {
    ...
    confirmed_msg:
      "您的礼宾中介专属委托已安全登记成功。为了确保您的贵宾特权、优先权及协议价不被覆盖，请不要绕过平台直接联系商户，点击下方专属通道完成确认：",
    via_whatsapp: "通过 WhatsApp 专线发送",
    via_instagram: "通过 Instagram 私信发送",
    via_call: "发起私密电话确认",
    close: "返回浏览",
    warning:
      "若绕过中介代理自行直接联系商家，将无法享受平台提供的贵宾级增值待遇。",
    temp_gastronomy: [
      "申请贵宾专属桌位预订",
      "安排定制主厨品鉴菜单",
      "咨询奢华VIP专座空位",
...
export const getCompactTransportIndicator = (
  transportStr: string,
  lang: string,
): string => {
    ...
    <div className="bg-[#FAF9F5]/40 border border-border-main rounded-2.5xl overflow-hidden transition-all duration-200">
      ...
      >
        <div className="flex items-start gap-4 pr-2">
          <div className="text-[22px] shrink-0 mt-0.5 text-[#1C1E18]">
            {icon}
          </div>
          <div className="space-y-1">
...
function getQuickFacts(rec: any, lang: string) {
  ...
  // 8. Pet Friendly
  const isPetFriendly =
    cat.includes("nature") || cat.includes("travel") || id === "29";
  if (isPetFriendly) {
    facts.push({
...
function getConciergeButtonText(rec: any, lang: string) {
  const cat = (rec.category || "").toLowerCase();
  const title = (
    getLocalizedValue(rec, "title", lang) ||
    rec.title ||
    ""
  ).toLowerCase();
  ...
  if (
    cat.includes("nature") ||
    title.includes("hike") ||
    title.includes("meanders") ||
    title.includes("gorge") ||
  ...
  if (
    ...
    title.includes("muzej") ||
    title.includes("monastery") ||
    title.includes("manastir")
  ) {
...
function DetailsScreen({
  ...
  const [conciergeStep, setConciergeStep] = useState<
    "form" | "submitting" | "confirmed"
  >("form");
  ...
  const isResourceCached = React.useMemo(() => {
    ...
    const isLocalAsset =
      ...
      path.startsWith("assets/images/");
    const isStandardId =
      recommendation.id && !recommendation.id.toString().startsWith("cur_");
      ...
      <div className="relative h-[38vh]">
        {lowSignalMode ? (
          <div className="w-full h-full bg-[#FAF9F5] flex flex-col items-center justify-center p-6 text-center select-none border-b border-border-main">
            <span className="text-[32px] mb-3 font-serif opacity-35 text-[#1E2E20]">
              ✦
            </span>
            <span className="text-xl font-serif text-brand-charcoal max-w-xs leading-tight">
              {getLocalizedValue(recommendation, "title", language)}
            </span>
            <span className="text-[8px] font-mono tracking-wider text-[#8C8A7D] uppercase mt-3 bg-[#EAE8DF]/50 px-3 py-1 rounded-full border border-border-main/20">
              Media Preserved • Low Signal Mode
            </span>
          </div>
        ) : isAdminPreview ? (
          <LazyImage
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={async () => {
                ...
                const loc = getLocalizedValue(
                  recommendation,
                  "location",
                  language,
                );
                const coords = recommendation.coordinates
          ...
          <div className="flex gap-2 items-center mb-2">
            ...
            >
              <span className="text-[8px] uppercase font-black tracking-[0.2em]">
                {formatCategory(recommendation.category, t)}
              </span>
            </motion.div>
          </div>
          <motion.h2
      ...
      <div className="px-8 pt-10 pb-10 -mt-6 bg-brand-bg rounded-t-[48px] relative z-20 space-y-6 flex-1 border-t border-border-main">
        ...
        {(() => {
          const isCalibrated =
            ...
            typeof recommendation.coordinates.lat === "number" &&
            typeof recommendation.coordinates.lng === "number" &&
            recommendation.coordinates.lat !== 0 &&
            recommendation.coordinates.lng !== 0 &&
            !(
              recommendation.id &&
              recommendation.id.toString().startsWith("draft-")
            );
          const recCoords = recommendation.coordinates || {
            lat: 44.8154,
            lng: 20.4607,
          };
          const repSquare = BASE_HUBS?.find(
            (h) => h.id === "republic_square",
          ) || { coordinates: { lat: 44.8154, lng: 20.4607 } };
          const dist = calculateDistance(
            repSquare.coordinates.lat,
            repSquare.coordinates.lng,
            recCoords.lat,
            recCoords.lng,
          );
          return (
            <div className="grid grid-cols-4 gap-1 sm:gap-2 border-y border-border-main/50 py-5 mt-4 text-center px-0.5">
              ...
              <div className="flex flex-col items-center justify-between min-h-[74px] leading-tight">
                ...
                <div className="flex flex-col items-center gap-1 w-full">
                  ...
                  <span className="text-[11px] sm:text-[12px] font-extrabold text-brand-charcoal break-words leading-tight px-0.5 max-w-full">
                    {getLocalizedValue(recommendation, "duration", language) ||
              ...
              <div className="flex flex-col items-center justify-between min-h-[74px] border-l border-border-main/30 leading-tight">
                <span className="text-[17px] font-sans font-black text-accent-red mb-1.5 shrink-0 leading-none">
                  €
                </span>
                <div className="flex flex-col items-center gap-1 w-full">
                  <span className="text-[8.5px] uppercase tracking-[0.12em] text-[#5C5A4D] font-black leading-none">
              ...
              <div className="flex flex-col items-center justify-between min-h-[74px] border-l border-border-main/30 leading-tight">
                <Navigation
                  size={18}
                  className="text-accent-red mb-2 shrink-0"
                />
                <div className="flex flex-col items-center gap-1 w-full">
                  <span className="text-[8.5px] uppercase tracking-[0.12em] text-[#5C5A4D] font-black leading-none">
                    {language === "sr"
                  ...
                  <span className="text-[11px] sm:text-[12px] font-extrabold text-brand-charcoal break-words leading-tight px-0.5 max-w-full">
                    {getCompactTransportIndicator(
                      recommendation.preferredTransport,
                      language,
                    )}
                  </span>
                </div>
        ...
        <div className="space-y-4 pt-1">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-[11px] uppercase tracking-[0.3em] text-[#6C6A5D] font-black">
                {t.the_curation}
              </h3>
            </div>
            {(() => {
              ...
              const displayedText =
                shouldTruncate && !isNotesExpanded
                        ...
                        {isNotesExpanded
                          ? language === "sr"
                            ...
                            : language === "zh"
                              ...
                              : "Collapse Notes"
                          : language === "sr"
                            ? "Pročitaj cele kustoske beleške"
                            : language === "zh"
                              ? "阅读完整策展笔记"
                              : "Read Full Curator Notes"}
                      </span>
                      <ChevronDown
                        size={14}
          ...
          <div className="py-2.5 relative">
              ...
              onScroll={() => {
                if (factCarouselRef.current) {
                  const { scrollLeft, scrollWidth, clientWidth } =
                    factCarouselRef.current;
                  const totalItems = getQuickFacts(
                    recommendation,
                    language,
                  ).length;
                  if (totalItems > 1) {
            ...
            >
              {getQuickFacts(recommendation, language).map((fact, idx) => (
                ...
                >
                  <span className="text-[#8C8A7D] shrink-0">
                    {fact.iconElement}
                  </span>
                  <div className="flex flex-col text-left leading-tight">
                    <span className="text-[10px] uppercase tracking-wider text-[#8C8A7D] font-extrabold mb-0.5">
                      {fact.label}
                    </span>
                    <span className="text-[13px] font-black text-brand-charcoal">
                      {fact.value}
                    </span>
                  </div>
                </div>
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    if (factCarouselRef.current) {
                      ...
                      const targetScroll =
                        (idx / (totalItems - 1 || 1)) * maxScrollLeft;
                      factCarouselRef.current.scrollTo({
                        left: targetScroll,
                        behavior: "smooth",
                      });
                    }
          ...
          {recommendation.equivalents?.[language] && (
            <div className="pt-3 border-t border-[#E7E4DB]/40">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[9px] uppercase tracking-[0.2em] text-[#8C8A7D] font-black">
                  {t.local_equivalent}
                </span>
              </div>
        ...
        <div className="space-y-4 pt-4 border-t border-border-main/70">
          ...
          <AccordionSection
            ...
            onToggle={() =>
              setActiveAccordion(
                activeAccordion === "transit" ? null : "transit",
              )
            }
          >
            <div className="space-y-4 py-1.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <DetailStatSmall
                  ...
                  label={t.travel_time}
                  value={
                    getLocalizedValue(recommendation, "travelTime", language) ||
                ...
                <DetailStatSmall
                  ...
                  value={getCompactTransportIndicator(
                    recommendation.preferredTransport,
                    language,
                  )}
                />
                {recommendation.website && (
                  <DetailStatSmall
                    ...
                    onClick={() => {
                      setSelectedTemplate(
                        CONCIERGE_T[language]?.temp_default?.[0] ||
                          "Inquire details",
                      );
              ...
              {(() => {
                const walkInfo = getRecommendationWalkability(
                  recommendation,
                  language,
                );
                return (
                  <div className="bg-white border border-[#E7E4DB] rounded-2xl p-4 flex gap-3.5 items-start shadow-sm">
                    ...
                    <div className="space-y-0.5">
                      <span className="text-[10px] uppercase tracking-wider text-[#8C8A7D] font-extrabold block">
                        {language === "sr"
                          ? "PODACI O PEŠAČENJU"
                          : "WALKABILITY & TERRAIN"}
                      </span>
                      ...
                      <p className="text-[11.5px] text-[#5C5A4D] font-medium leading-tight pt-0.5">
                        {language === "sr"
                          ? "Prohodnost pešaka:"
                          : "Pedestrian zone friendliness:"}{" "}
                        <span className="font-bold text-[#155e5b]">
                          {walkInfo.friendliness}
                        </span>
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <span className="text-[9px] text-white/50 block tracking-normal font-sans">
                      {language === "sr"
                        ? "Prikažite direktno taksisti"
                        : "Hand screen directly to taxi driver"}
                    </span>
                  </div>
                </div>
          ...
          <AccordionSection
            ...
            onToggle={() =>
              setActiveAccordion(
                activeAccordion === "preparation" ? null : "preparation",
              )
            }
          >
            <div className="space-y-4 py-1.5">
          ...
          <AccordionSection
            ...
            onToggle={() =>
              setActiveAccordion(
                activeAccordion === "anti_advice" ? null : "anti_advice",
              )
            }
          >
            <div className="py-1">
          ...
          {nearbyCompanions.length > 0 && (
            <AccordionSection
              ...
              onToggle={() =>
                setActiveAccordion(
                  activeAccordion === "companions" ? null : "companions",
                )
              }
            >
              <div className="space-y-4 py-1.5">
                <div className="grid grid-cols-1 gap-3">
                  {nearbyCompanions.map((comp: any) => {
                    const compTitle = getLocalizedValue(
                      comp,
                      "title",
                      language,
                    );
                    const compDesc = getLocalizedValue(
                      comp,
                      "shortDescription",
                      language,
                    );
                    return (
                      ...
                      >
                        ...
                        <ChevronRight
                          size={16}
                          className="text-brand-charcoal/30 group-hover:text-accent-red transition-all"
                        />
                      </div>
                    );
                  })}
          ...
          {CYRILLIC_DICTIONARY[recommendation.id] && (
            <AccordionSection
              ...
              onToggle={() =>
                setActiveAccordion(
                  activeAccordion === "pronunciation" ? null : "pronunciation",
                )
              }
            >
          ...
          {(recommendation.category?.toLowerCase() === "history" ||
            ...
            <AccordionSection
              ...
              onToggle={() =>
                setActiveAccordion(
                  activeAccordion === "history" ? null : "history",
                )
              }
            >
              <div className="p-5 bg-white border border-[#E7E4DB] rounded-2xl shadow-sm space-y-3.5">
                ...
                <div className="font-serif italic text-brand-charcoal text-[13.5px] leading-relaxed space-y-2.5">
                  {(() => {
                    ...
                    if (
                      recId === "7" ||
                      recommendation.title?.toLowerCase().includes("tesla")
                    ) {
                      return language === "sr" ? (
                        <p>
                          Nikola Tesla, vizionar i genije srpskog porekla,
                          zaveštao je svetu osnove moderne elektrotehnike.
                          Njegov lični arhiv, koji se čuva u ovom beogradskom
                          muzeju, uvršten je u UNESCO-ov registar „Pamćenje
                          sveta” i svedoči o neizmernom doprinosu čovečanstvu.
                        </p>
                      ) : language === "zh" ? (
                        <p>
                          尼古拉·特斯拉（Nikola
                          Tesla）是塞尔维亚裔科学远见者和发明巨匠。他在该博物馆内保存的私人珍贵档案，已被列入联合国教科文组织（UNESCO）的《世界记忆名录》，见证了他对全人类做出的永恒贡献。
                        </p>
                      ) : (
                        <p>
                          Nikola Tesla, a scientific visionary of Serbian
                          origin, bequeathed the foundations of modern
                          electricity to the world. His personal archive,
                          preserved in this Belgrade museum, is inscribed in
                          UNESCO's Memory of the World Register, witnessing his
                      ...
                      recommendation.title
                        ?.toLowerCase()
                        .includes("fortress") ||
                      recommendation.title?.toLowerCase().includes("kalemegdan")
                    ) {
                      return language === "sr" ? (
                        <p>
                          Beogradska tvrđava (Kalemegdan) predstavlja istorijsko
                          srce grada, svedočeći o preko dva milenijuma sukoba i
                          ponovnog rađanja. Smeštena na ušću Save u Dunav, ova
                          monumentalna vojna fortifikacija branila je granice
                          carstava i služila kao kulturno čvorište Balkana.
                        </p>
                      ) : language === "zh" ? (
                        <p>
                        ...
                        <p>
                          The Belgrade Fortress (Kalemegdan) stands as the
                          historical heart of the capital, bearing witness to
                          over two millennia of clashes and rebirths. Situated
                          at the confluence of the Sava and Danube, this
                          monumental fortification guarded imperial borders and
                          served as the cultural hub of the Balkans.
                        </p>
                      );
                    } else {
                      return language === "sr" ? (
                        <p>
                          Beograd je jedan od najstarijih kontinuirano
                          naseljenih gradova u Evropi. Njegovo istorijsko
                          nasleđe predstavlja raskošnu mešavinu vizantijskih,
                          habzburških, osmanskih i jugoslovenskih uticaja koji
                          stvaraju jedinstven i neponovljiv duh savremenog
                          Beograda.
                        </p>
                      ) : language === "zh" ? (
                        <p>
                        ...
                        <p>
                          Belgrade is one of the oldest continuously inhabited
                          cities in Europe. Its historical legacy is a rich
                          tapestry of Byzantine, Habsburg, Ottoman, and Yugoslav
                          influences, shaping the unique, resilient, and
                          charming character of modern Belgrade.
                        </p>
                      );
                    }
                  })()}
              ...
              >
                ...
                <div className="space-y-1.5">
                  <h3 className="font-serif text-lg font-bold text-brand-charcoal">
                    {language === "sr"
                      ? "Istaknuto partnersko iskustvo"
                      : "Featured Partner Experience"}
                  </h3>
                ...
                <div className="p-6 overflow-y-auto no-scrollbar flex-1 space-y-5">
                  ...
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-[10px] text-brand-charcoal/40 font-mono">
                      {CONCIERGE_T[language]?.sub || CONCIERGE_T["en"].sub}{" "}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-[#EAE8DF]/65 p-3 rounded-2xl border border-border-main flex flex-col items-center justify-center text-center">
                        <span className="text-[12px] uppercase tracking-[0.2em] font-mono text-[#5C5A4D] font-extrabold">
                          {CONCIERGE_T[language]?.inquiry_reference ||
                            CONCIERGE_T["en"].inquiry_reference}
                        </span>
                        ...
                        <span className="text-[11px] text-brand-charcoal/70 mt-1 font-sans">
                          {CONCIERGE_T[language]?.regarding ||
                            CONCIERGE_T["en"].regarding}
                          :{" "}
                          {getLocalizedValue(recommendation, "title", language)}
                        </span>
                      </div>
                      ...
                      <div className="space-y-2">
                        <label className="text-[13px] uppercase tracking-[0.1em] text-[#5C5A4D] font-bold">
                          {CONCIERGE_T[language]?.select_template ||
                            CONCIERGE_T["en"].select_template}
                        </label>
                        <div className="grid gap-2">
                          {(() => {
                            ...
                            const ct =
                              CONCIERGE_T[language] || CONCIERGE_T["en"];
                            const templates =
                              cat.includes("food") ||
                              cat.includes("gastronomy") ||
                              cat.includes("clubbing")
                                ? ct.temp_gastronomy
                                : cat.includes("medical") ||
                                    cat.includes("dental") ||
                                    cat.includes("wellbeing") ||
                      ...
                      <div className="space-y-2">
                        <label className="text-[13px] uppercase tracking-[0.1em] text-[#5C5A4D] font-bold">
                          {CONCIERGE_T[language]?.custom_details ||
                            CONCIERGE_T["en"].custom_details}
                        </label>
                        <textarea
                          rows={3}
                          value={customNote}
                          ...
                          placeholder={
                            CONCIERGE_T[language]?.placeholder ||
                            CONCIERGE_T["en"].placeholder
                          }
                      ...
                      >
                        ...
                        <span>
                          {CONCIERGE_T[language]?.despatch ||
                            CONCIERGE_T["en"].despatch}
                        </span>
                      </button>
                    </div>
                  )}
                  ...
                  {conciergeStep === "submitting" && (
                    <div className="py-10 flex flex-col items-center justify-center text-center space-y-6">
                      <div className="relative w-16 h-16 flex items-center justify-center">
                        ...
                        <Compass
                          size={24}
                          className="text-accent-teal animate-bounce"
                        />
                      </div>

                      <div className="space-y-2">
                        <h4 className="font-serif text-sm font-bold text-brand-charcoal">
                          {CONCIERGE_T[language]?.submitting ||
                            CONCIERGE_T["en"].submitting}
                        </h4>
                    ...
                    <div className="space-y-5 py-2">
                      <div className="text-center space-y-2">
                        ...
                        <h4 className="font-serif text-base font-black tracking-tight text-[#1E2E20]">
                          {CONCIERGE_T[language]?.confirmed_title ||
                            CONCIERGE_T["en"].confirmed_title}
                        </h4>
                        <p className="text-[11px] leading-relaxed text-brand-charcoal/70">
                          {CONCIERGE_T[language]?.confirmed_msg ||
                            CONCIERGE_T["en"].confirmed_msg}
                        </p>
                      </div>
                      ...
                      <div className="p-4 bg-white rounded-2xl border border-border-main/40 text-center font-mono space-y-1 shadow-sm select-all">
                        ...
                        <div className="text-[8px] italic text-[#8C8A7D] mt-1.5">
                          Show this credential at checkout for priority benefits
                        </div>
                      </div>
...
Language: ${getLanguageFullName(language)}`;
                          ...
                          const getWhatsappLabel = () => {
                            if (whatsappCopied) {
                              switch (language) {
                                case "sr":
                                  return "Zahtev kopiran. Otvaram WhatsApp — nalepite ga u polje za poruku i pošaljite.";
                                case "es":
                                  return "Solicitud copiada. Abriendo WhatsApp — péguela en el campo de mensaje y envíe.";
                                case "de":
                                  return "Anfrage kopiert. WhatsApp wird geöffnet — in das Nachrichtenfeld einfügen und senden.";
                                case "ru":
                                  return "Запрос скопирован. Открываем WhatsApp — вставьте в поле сообщения и отправьте.";
                                case "zh":
                                  return "预约已复制。正在跳转 WhatsApp — 请粘贴至对话框并发送。";
                                default:
                                  return "Request copied. Opening WhatsApp — paste it into the message field and send.";
                              }
                            }
                            return (
                              CONCIERGE_T[language]?.via_whatsapp ||
                              CONCIERGE_T["en"].via_whatsapp
                            );
                          };
                          ...
                          return (
                            ...
                            >
                              <span className="text-[#25D366] text-sm shrink-0">
                                💬
                              </span>
...
Language: ${getLanguageFullName(language)}`;
                          ...
                          const getInstagramLabel = () => {
                            if (instagramCopied) {
                              switch (language) {
                                case "sr":
                                  return "Zahtev kopiran. Otvaram Instagram — nalepite ga u polje za poruku i pošaljite.";
                                case "es":
                                  return "Solicitud copiada. Abriendo Instagram — péguela en el campo de mensaje y envíe.";
                                case "de":
                                  return "Anfrage kopiert. Instagram wird geöffnet — in das Nachrichtenfeld einfügen und senden.";
                                case "ru":
                                  return "Запрос скопирован. Открываем Instagram — вставьте в поле сообщения и отправьте.";
                                case "zh":
                                  return "预约已复制。正在跳转 Instagram — 请粘贴至对话框并发送。";
                                default:
                                  return "Request copied. Opening Instagram — paste it into the message field and send.";
                              }
                            }
                            return (
                              CONCIERGE_T[language]?.via_instagram ||
                              CONCIERGE_T["en"].via_instagram
                            );
                          };
                        ...
                        {(() => {
                          const getCallLabel = () => {
                            if (callCopied) {
                              switch (language) {
                                case "sr":
                                  return "Kopirano! Pozivam...";
                                case "es":
                                  return "¡Copiado! Llamando...";
                                case "de":
                                  return "Kopiert! Rufe an...";
                                case "ru":
                                  return "Скопировано! Звоним...";
                                case "zh":
                                  return "已复制！正在拨号...";
                                default:
                                  return "Number Copied! Calling...";
                              }
                            }
                            return (
                              CONCIERGE_T[language]?.via_call ||
                              CONCIERGE_T["en"].via_call
                            );
                          };
                          ...
                          return (
                            <a
                              ...
                              onClick={(e) => {
                                const rawPhone =
                                  getConciergePhone(recommendation);
                                if (
                                  navigator.clipboard &&
                                  navigator.clipboard.writeText
                                ) {
                            ...
                            >
                              <Phone
                                size={14}
                                className={
                                  callCopied ? "text-white" : "text-accent-teal"
                                }
                              />
                      ...
                      <div className="pt-2 text-center text-[9px] text-[#8C8A7D] italic border-t border-border-main/50 font-sans flex flex-col gap-1">
                        <p>
                          {CONCIERGE_T[language]?.warning ||
                            CONCIERGE_T["en"].warning}
                        </p>
                        <p className="text-[7.5px] opacity-75">
                          Concierge desk routing hotline:{" "}
                          {getConciergePhone(recommendation)} • Belgrade support
                        </p>
                      </div>
                      ...
                      >
                        {CONCIERGE_T[language]?.close ||
                          CONCIERGE_T["en"].close}
                      </button>
                    </div>
                  )}
                </div>
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-3.5 border-b border-brand-charcoal/15 pb-4">
                  ...
                  <div>
                    <h4 className="text-[11.5px] uppercase tracking-[0.3em] font-black text-[#5C5A4D]">
                      {language === "sr"
                        ? "TAKSI KARTICA VOZAČA"
                        : language === "zh"
                          ? "出租车地址名片"
                          : "CAB DRIVER ADDRESS CARD"}
                    </h4>
                    <p className="text-[13px] text-brand-charcoal/60 font-mono">
                      IDEMO Premium Digital Concierge
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-1">
                  <div className="space-y-1">
                    ...
                    <p className="text-[25px] font-serif font-black tracking-tight text-brand-charcoal leading-tight">
                      {getLocalizedValue(recommendation, "title", "sr") ||
                        recommendation.title}
                    </p>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[19px] font-semibold leading-relaxed text-[#1a1c13]">
                      {getLocalizedValue(recommendation, "location", "sr") ||
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-1">
                        <span className="text-[11px] uppercase tracking-wider text-[#8C8A7D] font-extrabold font-mono block">
                          {language === "sr"
                            ? "SATELITSKE KOORDINATE (GPS):"
                            : language === "zh"
                              ? "卫星卫星导航坐标 (GPS):"
                              : "SATELLITE COORDINATES (GPS):"}
                        </span>
                        <p className="font-mono text-xs font-bold text-accent-red tracking-wide uppercase bg-accent-red/5 border border-accent-red/15 inline-block px-2.5 py-1 rounded-md">
                          {recommendation.coordinates.lat}° N,{" "}
                          {recommendation.coordinates.lng}° E
                        </p>
                      </div>
                    );
                  })()}
                  ...
                  <button
                    onClick={() => {
                      ...
                      const srAddr =
                        ...
                        recommendation.location;
                      const gps =
                        recommendation.coordinates && isCalibrated
                          ? `${recommendation.coordinates.lat}, ${recommendation.coordinates.lng}`
                          : "TBD";
                  ...
                  >
                    {taxiCopied ? (
                      <>
                        ...
                        <span>
                          {language === "sr"
                            ? "KOPIRANO U PRIVREMENU MEMORIJU!"
                            : language === "zh"
                              ? "名片内容已复制！"
                              : "COPIED TO CLIPBOARD!"}
                        </span>
                ...
                <div className="text-center text-[10.5px] text-[#8C8A7D] font-medium leading-snug">
                  {language === "sr"
                    ? "Ova kartica je dizajnirana sa namerom da se telefon direktno preda vozaču u ruke, radi lakše koordinacije."
                    : language === "zh"
                      ? "本名片专为解决语言障碍而设计，支持高对比度离线出示。"
...
function PlanScreen({
  ...
  const [calendarToast, setCalendarToast] = React.useState<{
  ...
  const renderJourneyBundles = () => {
    const JOURNEY_BUNDLES = [
      {
        id: "old_belgrade",
        title: {
          en: "3 Hours in Old Belgrade",
          sr: "3 sata u starom Beogradu",
          zh: "老贝尔格莱德3小时游",
        },
        tag: "2 Hours / 3 Hours",
        duration: "3h",
      ...
      },
      {
        ...
        title: {
          en: "Belgrade for Architects",
          sr: "Beograd za arhitekte",
          zh: "建筑师的贝尔格莱德",
        },
        tag: "Half-Day",
        duration: "Half-Day",
      ...
      },
      {
        id: "rakija_river",
        title: {
          en: "Rakija & River Evenings",
          sr: "Rakija i rečne večeri",
          zh: "拉基亚与河畔之夜",
        },
        tag: "Weekend",
        duration: "Weekend",
    ...
    const handleDownloadBundleICS = (b: any) => {
      ...
      b.itemIds.forEach((id: string, index: number) => {
        ...
        icsContent.push(
          `UID:bundle-${b.id}-${item.id}-${Date.now()}@idemo.com`,
        );
        icsContent.push(
        ...
        icsContent.push(
          `SUMMARY:${getLocalizedValue(item, "title", language)}`,
        );
        icsContent.push(
          `DESCRIPTION:Curated Itinerary Step for ${b.title[language] || b.title["en"]}. Vibe: ${b.vibe}. Cost: ${item.estimatedCost || "N/A"}`,
        );
        icsContent.push(
          `LOCATION:${getLocalizedValue(item, "location", language)}`,
        );
      ...
      <div className="space-y-4 pt-10 border-t border-border-main/50 mt-10">
        <div className="space-y-1">
          ...
          <h3 className="text-xl font-serif text-brand-charcoal tracking-tight">
            {language === "sr"
              ? "Predefinisane rute (Offline)"
              : language === "zh"
                ? "离线品质行程包"
                : "Offline Journey Bundles"}
          </h3>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {JOURNEY_BUNDLES.map((b) => {
            const isAdded = b.itemIds.every((id) =>
              scheduledItems.some((s: any) => s.id === id),
            );
            return (
              <div
                key={b.id}
                ...
                <div className="mt-5 pt-3 border-t border-[#EAE8DF] flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    {isAdded ? (
                      ...
                      >
                        {language === "sr"
                          ? "Sačuvaj u planer događaja"
                          : language === "zh"
                            ? "保存到我的活动计划器"
                            : "Save to My Event Planner"}
                      </button>
                    )}
                  </div>
                </div>
  ...
  const handleSharePDF = async () => {
    const shareData = {
      title:
        language === "sr"
          ? "Moj EXPO 2027 Planer Događaja"
          : "My EXPO 2027 Event Planner",
      text:
        language === "sr"
          ? "Pogledaj moj skrojeni planer događaja za EXPO 2027 u Beogradu!"
          : "Check out my custom event planner for EXPO 2027 Belgrade!",
      url: window.location.href,
    };
  ...
  const handleSyncCalendar = () => {
    ...
    try {
      ...
      scheduledItems.forEach((item: any) => {
        ...
        icsContent.push(
          `DESCRIPTION:${t.event_description(formatCategory(item.category, t), getLocalizedValue(item, "location", language))}`,
        );
        icsContent.push(
          `LOCATION:${getLocalizedValue(item, "location", language)}`,
        );
  ...
  const createPhrasesCard = (lang: string): string => {
    ...
    ctx.fillText(
      lang === "sr"
        ? "KORISNE LOKALNE FRAZE (SRBIJA)"
        : "USEFUL LINGUISTIC PHRASES (SERBIA)",
      20,
      26,
    );
  ...
  const drawCalendar = (
    ...
    daysHeader.forEach((day, idx) => {
      pdfDoc.text(
        day,
        x + 4 + idx * cellWidth + cellWidth / 2 - pdfDoc.getTextWidth(day) / 2,
        y + 10,
      );
    });
  ...
  const findCompanionRecommendation = (currentItem: any) => {
    if (
      ...
      typeof currentItem.coordinates.lat !== "number" ||
      typeof currentItem.coordinates.lng !== "number"
    ) {
      return null;
    }
  ...
  const generatePdfDocument = async () => {
    ...
    > = {
      en: {
        ...
        afterYourVisitCopy:
          "Help IDEMO keep this recommendation accurate. After visiting, update the Mood Orb if your pace or preferred atmosphere changed, then leave a short note about what was accurate, what changed, or what future visitors should know.",
      },
      sr: {
        ...
        afterYourVisitCopy:
          "Pomozite IDEMO-u da ova preporuka ostane tacna. Nakon posete, azurirajte Mood Orb ako su se vas tempo ili zeljena atmosfera promenili, a zatim ostavite kratku belesku o tome sta je bilo tacno, sta se promenilo ili sta bi buduci posetioci trebali da znaju.",
      },
      de: {
      ...
      ru: {
        ...
        afterYourVisitCopy:
          "Pomogite IDEMO sohranit` etu rekomendaciyu tochnoj. Posle poseweniya nastrojte Mood Orb, esli vash temp ili predpochitaemaya atmosfera izmenilis`, a zatem ostav'te korotkuyu zametku o tom, chto bylo tochno, chto izmenilos` ili chto sleduet znat` buduwim posetitelyam.",
      },
      es: {
        ...
        afterYourVisitCopy:
          "Ayude a IDEMO a mantener esta recomendacion precisa. Despues de su visita, actualice el Mood Orb si su ritmo o la atmosfera de su preferencia cambiaron, luego deje una breve nota sobre lo que fue preciso, lo que cambio o lo que los futuros visitantes deberian saber.",
      },
    };
    ...
    const getKnowBeforeYouGoBullets = (recItem: any, lang: string) => {
      ...
      if (recItem.preferredTransport) {
        if (isSR) {
          bulletsList.push(
            `Preporuceni nacin transporta: ${recItem.preferredTransport}.`,
          );
        } else if (isDE) {
          bulletsList.push(
            `Empfohlenes Transportmittel: ${recItem.preferredTransport}.`,
          );
        } else if (isRU) {
          bulletsList.push(
            `Rekomenduemyj transport: ${recItem.preferredTransport}.`,
          );
        } else if (isES) {
          bulletsList.push(
            `Transporte recomendado: ${recItem.preferredTransport}.`,
          );
        } else {
          bulletsList.push(
            `Recommended mode of transport: ${recItem.preferredTransport}.`,
          );
        }
      ...
      if (isSR) {
        bulletsList.push(
          `Ova lokacija se nalazi u regiji ${recItem.location || "Srbija"}. Planirajte u skladu sa tim.`,
        );
      } else if (isDE) {
        bulletsList.push(
          `Dieser Ort befindet sich in der Region ${recItem.location || "Serbien"}. Planen Sie entsprechend.`,
        );
      } else if (isRU) {
        bulletsList.push(
          `Eto mesto nahoditsya v regione ${recItem.location || "Serbiya"}. Zaplanirujte zaranee.`,
        );
      } else if (isES) {
        bulletsList.push(
          `Este lugar se encuentra en la region de ${recItem.location || "Serbia"}. Planifique en consecuencia.`,
        );
      } else {
        bulletsList.push(
          `This location is situated in ${recItem.location || "Serbia"}. Please plan accordingly.`,
        );
      }
    ...
    const drawMoodField = (
      ...
      doc.text(sanitizePdfText(labelUrban), x + w / 2, y - 1, {
        align: "center",
      });
      // Nature: centered below the box
      doc.text(sanitizePdfText(labelNature), x + w / 2, y + w + 2.2, {
        align: "center",
      });
      // Calm: right-aligned left of the box
      doc.text(sanitizePdfText(labelCalm), x - 0.8, y + w / 2 + 0.6, {
        align: "right",
      });
      // Energetic: left-aligned right of the box
      doc.text(sanitizePdfText(labelEnergetic), x + w + 0.8, y + w / 2 + 0.6, {
        align: "left",
      });
      ...
      const wrappedCaption = doc.splitTextToSize(
        sanitizePdfText(caption.toUpperCase()),
        w + 8,
      );
      ...
      if (isUserMoodField) {
        ...
        for (let i = 0; i < numSegments; i++) {
          ...
          doc.triangle(
            plotX,
            plotY,
            plotX + dialR * Math.cos(angle1),
            plotY + dialR * Math.sin(angle1),
            plotX + dialR * Math.cos(angle2),
            plotY + dialR * Math.sin(angle2),
            "F",
          );
        }
        ...
        doc.line(
          plotX,
          plotY,
          plotX + dialR * 0.48 * Math.cos(hrAngle),
          plotY + dialR * 0.48 * Math.sin(hrAngle),
        );
        // Mercedes circular emblem
        doc.setFillColor(244, 244, 245);
        doc.circle(
          plotX + dialR * 0.32 * Math.cos(hrAngle),
          plotY + dialR * 0.32 * Math.sin(hrAngle),
          0.18,
          "F",
        );
        ...
        doc.line(
          plotX,
          plotY,
          plotX + dialR * 0.72 * Math.cos(minAngle),
          plotY + dialR * 0.72 * Math.sin(minAngle),
        );
        ...
        doc.line(
          plotX,
          plotY,
          plotX + dialR * 0.8 * Math.cos(secAngle),
          plotY + dialR * 0.8 * Math.sin(secAngle),
        );
        // Lollipop dot
        doc.setFillColor(0, 240, 255);
        doc.circle(
          plotX + dialR * 0.58 * Math.cos(secAngle),
          plotY + dialR * 0.58 * Math.sin(secAngle),
          0.14,
          "F",
        );
    ...
    for (let idx = 0; idx < scheduledItems.length; idx++) {
      ...
      const testDate = item.scheduledDate
        ? new Date(item.scheduledDate)
        : new Date(2027, 5, 15);
      ...
      doc.text(
        sanitizePdfText(
          `Plan item ${idx + 1} of ${scheduledItems.length}`.toUpperCase(),
        ),
        20,
        27,
      );

      let titleVal = sanitizePdfText(
        getLocalizedValue(item, "title", pdfLang) || item.title || "",
      );
      if (item.isAvailable === false) {
        const unavailableTag =
          pdfLang === "sr"
            ? "[Arhivirano / Nije dostupno]"
            : "[Archived / Unavailable]";
        titleVal = `${unavailableTag} ${titleVal}`;
      }
      const locationVal = sanitizePdfText(
        getLocalizedValue(item, "location", pdfLang) || item.location || "",
      );
      const categoryVal = sanitizePdfText(
        formatCategory(item.category, activeT) || item.category || "",
      );
      ...
      doc.text(
        sanitizePdfText(`Location: ${locationVal}`),
        80,
        51 + (titleHeightOffset > 4 ? 4 : 0),
      );
      doc.text(
        sanitizePdfText(`Journey Time: ${item.travelTime || "Flexible"}`),
        80,
        56 + (titleHeightOffset > 4 ? 4 : 0),
      );
      doc.text(
        sanitizePdfText(
          `Transit Mode: ${item.preferredTransport || "Car/Walk"}`,
        ),
        80,
        61 + (titleHeightOffset > 4 ? 4 : 0),
      );
      doc.text(
        sanitizePdfText(`Est. Investment: ${item.estimatedCost || "Curated"}`),
        80,
        66 + (titleHeightOffset > 4 ? 4 : 0),
      );
      ...
      if (item.coordinates) {
        ...
        doc.setTextColor(140, 138, 125);
        doc.text(
          `GPS: Lat ${item.coordinates.lat.toFixed(4)}, Lng ${item.coordinates.lng.toFixed(4)}`,
          80,
          71 + (titleHeightOffset > 4 ? 4 : 0),
        );
      }
      ...
      const hasTravelTime =
        !!item.travelTime && item.travelTime.trim().length > 0;
      const travelTimeText = hasTravelTime ? item.travelTime : pL.notSpecified;

      const hasVisitDuration =
        (typeof item.recommendedVisitDuration === "number" &&
          item.recommendedVisitDuration > 0) ||
        (!!item.duration && item.duration.trim().length > 0);
      ...
      if (hasTravelTime) {
        doc.text(
          sanitizePdfText(`${pL.approx} ${travelTimeText}`),
          45,
          budgetY,
        );
      } else {
      ...
      if (hasVisitDuration) {
        doc.text(
          sanitizePdfText(`${pL.approx} ${visitDurationText}`),
          45,
          budgetY,
        );
      } else {
      ...
      if (hasTravelTime) {
        doc.text(
          sanitizePdfText(`${pL.allowApprox} ${travelTimeText}`),
          45,
          budgetY,
        );
      } else {
      ...
      if (hasNumericDurations) {
        const totalMins =
          item.travelTimeMinutes * 2 + item.recommendedVisitDuration;
        ...
        doc.text(
          sanitizePdfText(`${pL.approx} ${totalDurationText}`),
          45,
          budgetY,
        );
      ...
      const wrappedFocusText = doc.splitTextToSize(
        sanitizePdfText(focusText),
        54,
      );
      ...
      if (hasMoodData) {
        ...
        doc.setFontSize(7.2);
        doc.setTextColor(92, 94, 84);
        const wrappedMood = doc.splitTextToSize(
          sanitizePdfText(moodText),
          textWidth,
        );
        ...
        if (hasRecCoords) {
          ...
          const caption1 =
            pdfLang === "sr"
              ? "Trenutna kalibracija"
              : pdfLang === "de"
                ? "Aktuelle Kalibrierung"
                : pdfLang === "ru"
                  ? "Tekushchaya kalibrovka"
                  : pdfLang === "es"
                    ? "Calibracion actual"
          ...
          const caption2 =
            pdfLang === "sr"
              ? "Pozicija preporuke"
              : pdfLang === "de"
                ? "Position dieser Empfehlung"
                : pdfLang === "ru"
                  ? "Poziciya rekomendacii"
                  : pdfLang === "es"
                    ? "Posicion de esta recomendacion"
                    : "This recommendation's position";
      ...
      doc.setFontSize(7.2);
      doc.setTextColor(92, 94, 84);
      const wrappedAfterVisit = doc.splitTextToSize(
        sanitizePdfText(pL.afterYourVisitCopy),
        85,
      );
      ...
      const descVal =
        getLocalizedValue(item, "shortDescription", pdfLang) ||
        getLocalizedValue(item, "longDescription", pdfLang) ||
        item.shortDescription ||
        item.longDescription ||
        "";
      ...
      const hasVerifiedFact = !!(
        ...
        item.fact.trim() !== "" &&
        !item.fact.includes("[fact]") &&
        !item.fact.includes("[") &&
        item.whyItMatters &&
        typeof item.whyItMatters === "string" &&
        item.whyItMatters.trim() !== "" &&
        !item.whyItMatters.includes("[whyItMatters]") &&
        !item.whyItMatters.includes("[")
      );
      ...
      if (hasVerifiedFact) {
        ...
        let factText = sanitizePdfText(
          getLocalizedValue(item, "fact", pdfLang) || item.fact || "",
        );
        let whyText = sanitizePdfText(
          getLocalizedValue(item, "whyItMatters", pdfLang) ||
            item.whyItMatters ||
            "",
        );
        ...
        const boxHeight =
          8 + wrappedFact.length * 3.5 + wrappedWhy.length * 3.5;
        ...
        doc.text(
          sanitizePdfText(pL.culturalContext.toUpperCase()),
          113,
          rightY + 4,
        );
      ...
      doc.setTextColor(30, 46, 32);
      doc.text(
        sanitizePdfText(`Average temp: ${wx.tempRange}`),
        110,
        rightY + 10,
      );
      ...
      const wrappedEmergency = doc.splitTextToSize(
        sanitizePdfText(emergencyDesc),
        35,
      );
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        sanitizePdfText(item.preferredTransport || pL.notSpecified),
        colX4 + 3,
        colY + 12.5,
      );
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        sanitizePdfText(item.travelTime || pL.notSpecified),
        colX4 + 3,
        colY + 19.5,
      );
      ...
      doc.setFont("helvetica", "normal");
      const wrappedLoc = doc.splitTextToSize(
        sanitizePdfText(item.location || pL.notSpecified),
        35,
      );
      ...
      const footerText2 = sanitizePdfText(
        `Plan item ${idx + 1} of ${scheduledItems.length}`,
      );
      doc.text(footerText1, 15, pageHeight - 10);
      doc.text(
        footerText2,
        pageWidth - 15 - doc.getTextWidth(footerText2),
        pageHeight - 10,
      );
    }
  ...
  const handlePrintPDF = async () => {
    ...
    try {
      ...
      const pdfFile = new File([pdfBlob], "idemo-event-planner.pdf", {
      ...
      if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
        try {
          await navigator.share({
            ...
            title:
              language === "sr"
                ? "IDEMO Planer Događaja"
                : "IDEMO Event Planner",
            text:
              language === "sr"
                ? "Pogledaj moj skrojeni plan za Srbiju!"
                : "Check out my custom itinerary for Serbia!",
          });
          return;
        } catch (shareErr) {
    ...
    try {
      ...
      console.log(
        "Failed to open new tab or blob, downloading instead",
        tabErr,
      );
      doc.save("idemo-travel-plan.pdf");
    }
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-2 text-accent-red">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">
              {scheduledItems.length} {t.events_scheduled}
            </span>
          </div>
            ...
            {showLivePreview && (
              ...
              >
                ...
                >
                  ...
                  <div
                    id="print-preview-document"
                    className="flex-1 overflow-y-auto p-5 pb-24 relative select-text no-scrollbar scroll-smooth"
                  >
                    {/* Absolute Svg Artistic Watermarks */}
                    ...
                    <div className="absolute bottom-[10%] left-[8%] opacity-[0.035] pointer-events-none z-0 w-[130px]">
                      ...
                      >
                        ...
                        <text
                          ...
                          letterSpacing="1"
                        >
                          <textPath href="#roman_txt_preview" startOffset="5%">
                            IMP CONSTANTINVS P F AVG
                          </textPath>
                        </text>
                      </svg>
                    ...
                    >
                      {
                        (
                          ITINERARY_LOCALIZATIONS[language] ||
                          ITINERARY_LOCALIZATIONS["en"]
                        ).watermark
                      }
                    </div>
                      ...
                      <div className="bg-white border-2 border-[#D5D3C8] rounded-[20px] p-5 space-y-3.5 shadow-xs">
                        ...
                        <div className="space-y-2 pt-2 border-t border-[#FAF9F5]">
                          {dynamicStyle.whyBullets.map((bullet, idx) => (
                            <div
                              key={idx}
                              className="flex items-start gap-2 text-[12px] text-brand-charcoal font-semibold leading-relaxed"
                            >
                              <span className="w-2 h-2 rounded-full bg-accent-teal flex-shrink-0 mt-1.5" />
                              <span>{bullet}</span>
                            </div>
                        ...
                        {[...scheduledItems]
                          ...
                          .map((item: any, idx: number) => {
                            ...
                            return (
                              <div
                                key={item.id}
                                className="bg-white border border-[#E5E3DB] rounded-xl overflow-hidden shadow-xs"
                              >
                                <div className="bg-[#F8F7F4] border-b border-[#E5E3DB] px-3 py-1.5 flex justify-between items-center">
                                  <span className="font-mono text-[9px] font-bold text-[#2E7D32]">
                                    #{idx + 1}
                                  </span>
                                  <span className="text-[8.5px] text-[#1E2E20] font-bold uppercase tracking-wide">
                                    {dateFormatted}
                                  </span>
                                </div>
                                <div className="p-3 space-y-2">
                                  <div>
                                    ...
                                    <p className="text-[8px] text-[#8C8A7D] mt-1">
                                      📍{" "}
                                      {getLocalizedValue(
                                        item,
                                        "location",
                  ...
                  <div className="bg-white border-t border-[#E5E3DB] p-4 flex flex-col gap-2 z-10 shadow-lg">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        ...
                        {modalDownloadLabels[language] ||
                          modalDownloadLabels["en"]}
                      </button>
                      <button
                        onClick={() => {
...
const PAIRINGS_TRANSLATIONS: Record<string, any> = {
  en: {
    ...
    subtitle:
      "Bespoke professional partnerships combining active culture with elite recovery.",
  ...
  sr: {
    ...
    subtitle:
      "Namenska i pažljivo usklađena iskustva koja spajaju aktivnu kulturu sa oporavkom.",
    both_title: "Usklađeni par",
    all_discoveries: "Sva odabrana otkrića",
    click_to_view:
      "Kliknite na bilo koje mesto ispod da pogledate detalje plana.",
  },
  zh: {
    title: "高管定制尊享组合",
    subtitle:
      "为世博商务精英和高层代表度身定制的完美行程，合力倍增社交深度与身心静治。",
    both_title: "经典双生组合",
  ...
  es: {
    ...
    subtitle:
      "Asociaciones profesionales a la medida que combinan cultura activa y recuperación.",
    both_title: "Dúo seleccionado",
    all_discoveries: "Todos los descubrimientos",
    click_to_view:
      "Haga clic en cualquiera de las opciones para ver sus detalles.",
  },
  de: {
    title: "Executive Kuratierte Paare",
    subtitle:
      "Maßgeschneiderte professionelle Erlebnisse, die aktive Kultur mit Erholung verbinden.",
  ...
  ru: {
    ...
    subtitle:
      "Специально разработанные комбинации, сочетающие культуру высшего уровня со спа-восстановлением.",
    both_title: "Эксклюзивный дуэт",
    all_discoveries: "Все кураторские открытия",
    click_to_view:
      "Нажмите на любую карточку ниже, чтобы изучить подробности маршрута.",
  },
};
...
function ExploreScreen({
  ...
  const isCultural = useMemo(() => {
    ...
    return (
      styleName.toLowerCase().includes("cultural") ||
      styleName.toLowerCase().includes("kulturn")
    );
  }, [dynamicConciergeStyle]);

  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [exploreMode, setExploreMode] = useState<"categories" | "areas">(
    "categories",
  );
  ...
  const [sortBy, setSortBy] = useState<"featured" | "shortest" | "longest">(
    "featured",
  );
  ...
  const searchFilteredRecs = useMemo(() => {
    ...
    return finalFilteredRecs.filter((r: any) => {
      const searchTarget =
        `${getLocalizedValue(r, "title", language)} ${getLocalizedValue(r, "shortDescription", language)} ${getLocalizedValue(r, "longDescription", language)} ${r.location} ${r.category}`.toLowerCase();
    ...
    >
      ...
      >
        <header className="flex justify-between items-start">
          <div className="space-y-1">
            ...
            <div className="pt-2 select-none">
              ...
              >
                <div
                  className={`w-1 h-1 rounded-full ${lowSignalMode ? "bg-amber-600 animate-pulse" : "bg-green-600"}`}
                />
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2 items-center">
            <div className="flex-1 flex bg-[#EAE8DF]/40 p-1.5 rounded-2xl border border-border-main shadow-inner h-13 items-center">
              <button
                ...
                className={`flex-1 py-1.5 text-[12px] uppercase tracking-widest font-black rounded-xl transition-all cursor-pointer h-[40px] flex items-center justify-center ${
                  exploreMode === "categories"
                    ? "bg-white text-brand-charcoal shadow-sm border border-border-main/20"
                    : "text-[#5C5A4D]"
                }`}
              >
                {t.categories_label}
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-[12px] uppercase tracking-widest font-black rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer h-[40px] ${
                  exploreMode === "areas"
                    ? "bg-white text-brand-charcoal shadow-sm border border-[#D5D3C8] border-opacity-30"
                    : "text-[#5C5A4D]"
                }`}
              >
            ...
            >
              <Heart
                size={14}
                fill={selectedVibes.length > 0 ? "#8A1F1F" : "none"}
                className="text-accent-red transition-all duration-300"
              />
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    >
                      <Heart
                        size={13}
                        fill={
                          selectedVibes.includes("like") ? "#8A1F1F" : "none"
                        }
            ...
            <button
              ...
              className={`px-5 py-3 rounded-full text-[12px] font-black uppercase tracking-widest border transition-all whitespace-nowrap shadow-sm active:scale-95 cursor-pointer min-h-[44px] ${
                !selectedCategory
                  ...
                  : "bg-white text-[#5C5A4D] border-border-main"
              }`}
            >
              {t.all}
            </button>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 -mx-6 px-6 no-scrollbar touch-pan-x">
            ...
            {REGIONS.map((reg) => (
              ...
              >
                ...
                <span>
                  {reg.name[language as keyof typeof reg.name] || reg.name.en}
                </span>
              </button>
            ))}
          </div>
            ...
            <section className="space-y-4 pt-1 pb-2">
              <div className="space-y-1">
                <h3 className="text-xl font-serif text-brand-charcoal tracking-tight">
                  {
                    (
                      PAIRINGS_TRANSLATIONS[language] ||
                      PAIRINGS_TRANSLATIONS["en"]
                    ).title
                  }
                </h3>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 no-scrollbar touch-pan-x">
                {BUSINESS_PAIRINGS.map((p) => {
                  const rec1 = recommendations.find(
                    (r: any) => r.id === p.rec1Id,
                  );
                  const rec2 = recommendations.find(
                    (r: any) => r.id === p.rec2Id,
                  );
                  const pairingsLocal =
                    PAIRINGS_TRANSLATIONS[language] ||
                      ...
                      <div className="mt-4 flex flex-col gap-2 bg-[#F6F5F2] p-2 rounded-2xl relative border border-[#ECEAE6]">
                        ...
                        >
                          {lowSignalMode ? (
                            ...
                            <LazyImage
                              src={rec1.image}
                              containerClassName="w-8 h-8 rounded-lg flex-shrink-0"
        ...
        <div className="flex justify-between items-center gap-4 pt-1 relative">
          <h3 className="text-xl font-serif text-brand-charcoal tracking-tight">
            {showEverything
              ? language === "sr"
                ? "Sve preporuke (Poređane po kontekstu)"
                : "All Experiences (Context-Ranked)"
              : exploreMode === "categories"
                ? selectedCategory === null
                  ? (
                      PAIRINGS_TRANSLATIONS[language] ||
                  ...
                  : t["category_" + selectedCategory.toLowerCase()] ||
                    selectedCategory
                : selectedAreaId === null
                  ? language === "sr"
                    ? "Sve lokacije"
                    : language === "zh"
                      ? "全部地理发现"
                      : "All Curated Districts"
                  : REGIONS.find((reg) => reg.id === selectedAreaId)?.name[
                      language as any
                    ] || "Curated district"}
          </h3>
          ...
          <div className="relative shrink-0 z-[60]">
            ...
            >
              ...
              <span>
                {
                  (SORT_TRANSLATIONS[language] || SORT_TRANSLATIONS["en"])[
                    sortBy
                  ]
                }
              </span>
              ...
              {sortOpen && (
                <>
                  ...
                  >
                    ...
                    {(["featured", "shortest", "longest"] as const).map(
                      (opt) => (
                        <button
                          key={opt}
                          onClick={() => {
                        ...
                        >
                          <span>
                            {
                              (SORT_TRANSLATIONS[language] ||
                                SORT_TRANSLATIONS["en"])[opt]
                            }
                          </span>
                          {sortBy === opt && (
                            <span className="w-1.5 h-1.5 rounded-full bg-accent-teal" />
                ...
                <div className="w-[102px] h-full overflow-hidden border-r border-border-main relative animate-fade-in flex items-center justify-center bg-neutral-100">
                  {lowSignalMode ? (
                    <div className="w-full h-full bg-[#FAF9F5] flex flex-col items-center justify-center p-2 text-center select-none">
                      <span className="text-xs uppercase font-serif opacity-30 text-brand-charcoal">
                        ✦
                      </span>
                      <span className="text-[7.5px] font-mono tracking-wider text-[#8C8A7D] uppercase mt-1">
                        Preserved
                      </span>
                    </div>
                  ) : (
                ...
                <div className="flex-1 py-2.5 px-3.5 flex flex-col justify-between min-w-0">
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1 pt-1.5 border-t border-border-main/40">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      />
                      <span className="text-[9px] font-bold text-brand-charcoal truncate">
                        {item.estimatedCost}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 min-w-0">
      ...
      <AnimatePresence>
        {showCorrelationModal && (
          <div
            className="fixed inset-0 z-[160] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm"
                    ...
                    <>
                      <p>
                        {isSr ? (
                          ...
                          <>
                            Your <strong>Dominant Persona</strong> represents
                            your core, long-term travel style (defined by your
                            Mood Orbit position), while{" "}
                            <strong>Vibe & Atmosphere</strong> reflects your
                            active interest categories right now.
                          </>
                        )}
                      </p>
                      <p className="border-t border-[#2D3025]/10 pt-3 text-xs font-bold text-accent-teal">
                        {isSr ? (
                          <>
                            <strong>Novo na IDEMO?</strong> Posetite{" "}
                            <strong>Profil</strong> i kalibrišite Vašu Mood
                            Orbitu za još personalizovanije preporuke.
                          </>
                        ) : isZh ? (
                          <>
                            <strong>新使用 IDEMO？</strong> 访问{" "}
                            <strong>个人资料</strong> 校准您的 Mood
                            Orbit，获取更个性化的推荐。
                          </>
                        ) : (
                          <>
                            <strong>New to IDEMO?</strong> Visit{" "}
                            <strong>Profile</strong> and calibrate your Mood
                            Orbit for more personalized recommendations.
                          </>
                        )}
                      </p>
                    </>
...
const ONBOARDING_TRANSLATIONS: Record<string, any> = {
  en: {
    cards: [
      {
        ...
        description:
          "Every journey begins differently. Tell IDEMO what you enjoy, how much time you have and how you feel today. In less than a minute, your personal guide is ready to curate experiences that fit you—not the average traveller.",
      ...
      },
      {
        ...
        description:
          "Your mood changes. Your recommendations should too. IDEMO carefully matches places and experiences to your interests, available time, budget and current location, so every suggestion feels relevant when you need it.",
      ...
      },
      {
        ...
        description:
          "Save favourite places, build your personal travel plan and enjoy Serbia with confidence. Whenever you want local insight or assistance, IDEMO Concierge and trusted local experts are ready to help.",
    ...
    skip: "Skip",
    trust_line:
      "No registration required. Your preferences stay private on your device.",
    trust_ribbon:
      "PRIVATE BY DESIGN • CURATED EXPERIENCES • TRUSTED LOCAL EXPERTS",
    card_indicator: "Step",
  },
  sr: {
    cards: [
      {
        ...
        description:
          "Svako putovanje počinje drugačije. Recite IDEMO aplikaciji šta volite, koliko vremena imate i kako se danas osećate. Za manje od minuta, vaš lični vodič je spreman da odabere iskustva koja odgovaraju vama—a ne prosečnom putniku.",
      ...
      },
      {
        ...
        description:
          "Vaše raspoloženje se menja. Vaše preporuke bi takođe trebalo. IDEMO pažljivo usklađuje mesta i iskustva sa vašim interesovanjima, vremenom, budžetom i trenutnom lokacijom.",
      ...
      },
      {
        ...
        description:
          "Sačuvajte omiljena mesta, kreirajte lični plan putovanja i uživajte u Srbiji sa samopouzdanjem. Kada god poželite lokalni uvid ili pomoć, IDEMO konsijerž i provereni lokalni stručnjaci su tu da pomognu.",
    ...
    skip: "Preskoči",
    trust_line:
      "Bez registracije. Vaše preferencije ostaju privatne na vašem uređaju.",
    trust_ribbon:
      "PRIVATNOST PO DIZAJNU • PAŽLJIV IZBOR • PROVERENI LOKALNI STRUČNJACI",
    ...
    cards: [
      {
        ...
        title: "您的资料。您的起点。",
        description:
          "每一段旅程各有不同。告知 IDEMO 您的喜好、可用时间与当下心境。不到一分钟，您的专属随行指南即可为您精准甄选体验。",
      ...
      },
      {
        ...
        title: "发现当下契合之选。",
        description:
          "心境在变，推荐亦随之演进。IDEMO 根据您的兴趣、可用时间、预算及实时位置精细匹配，让每一项建议都恰到好处。",
      ...
      },
      {
        ...
        title: "从容自信探索。",
        description:
          "收藏心仪目的地，定制专属行程，从容自信探索塞尔维亚。如需深入本土建议，IDEMO 礼宾与值得信赖的本地专家随时为您提供支持。",
...
function OnboardingOverlay({
  ...
  const renderChipIcon = (iconName: string) => {
    const props = {
      size: 12,
      className: "text-brand-charcoal stroke-[1.5] shrink-0 opacity-80",
    };
    switch (iconName) {
      case "Shield":
        return <Shield {...props} />;
      ...
      >
        ...
        <div className="flex-shrink-0 w-full space-y-2">
          ...
          <div className="flex justify-between items-center text-[10px] font-mono font-bold uppercase tracking-wider text-brand-charcoal/50">
            <span>
              {t.card_indicator} {cardIndex + 1} / 3
            </span>
            {cardIndex < 2 && (
              <button
        ...
        <div className="flex-shrink-0 space-y-2 text-left">
          ...
          <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1.5 pt-2 border-t border-border-main/20">
            {current.chips.map(
              (chip: { icon: string; label: string }, idx: number) => (
                <div
                  key={idx}
                  className="inline-flex items-center gap-1.5 text-brand-charcoal/80"
                >
                  {renderChipIcon(chip.icon)}
                  <span className="text-[10.5px] font-sans font-medium text-brand-charcoal/85 tracking-tight">
                    {chip.label}
                  </span>
                </div>
              ),
        ...
        <div className="flex-shrink-0 w-full space-y-2 pt-0.5">
          ...
          >
            ...
            <ChevronRight
              size={14}
              className="group-hover:translate-x-0.5 transition-transform text-white/80"
            />
          </button>
