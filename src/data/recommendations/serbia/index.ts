...
import { streamFActiveSerbiaRecommendations } from './stream_f_active_serbia';
import { streamGLivingSerbiaRecommendations } from './stream_g_living_serbia';
...
export const CANONICAL_EXPANSION_IDS = new Set<string>([
  ...
  '133', '134', '135', '137', '138', '139', '140', '141', '142', '145', '146', '148',
  'PKG-006', 'PKG-007', 'PKG-008', 'PKG-016', 'PKG-017', 'PKG-018', 'PKG-024'
]);

export const EDITORIAL_IMPROVEMENT_IDS = new Set<string>(['103', '109', '116', '126', '136']);
export const ADDITIONAL_RESEARCH_IDS = new Set<string>(['117', '144']);
export const DEFERRED_IDS = new Set<string>(['113', '125', '129', '132', '143', '147']);

export function getRecommendationLifecycleStatus(id: string, existingStatus?: RecommendationLifecycleStatus): RecommendationLifecycleStatus {
...
const rawRecommendations: Recommendation[] = [
  ...
  ...draftExpansionPool,
  ...streamFActiveSerbiaRecommendations,
  ...streamGLivingSerbiaRecommendations
];

export const INITIAL_RECOMMENDATIONS: Recommendation[] = rawRecommendations.map(rec => {
  const status = getRecommendationLifecycleStatus(rec.id, rec.publicationStatus);
