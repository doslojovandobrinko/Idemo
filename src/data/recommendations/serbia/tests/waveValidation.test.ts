...
export function validateWave(lang: LanguageCode, waveName: string): WaveValidationResult {
  console.log(`\n==================================================`);
  console.log(`RUNNING VALIDATION FOR WAVE: ${waveName} [Lang: ${lang.toUpperCase()}]`);
  console.log(`==================================================`);

  const canonicalRecs = INITIAL_RECOMMENDATIONS.filter(r => r.publicationStatus === 'CANONICAL');
  const nonCanonicalRecs = INITIAL_RECOMMENDATIONS.filter(r => r.publicationStatus !== 'CANONICAL');
  ...
  for (const rec of canonicalRecs) {
    ...
    if (!trans) {
      unresolvedCount++;
      emptyFieldErrors.push(`Canonical ID ${rec.id}: Missing translation object for '${lang}'`);
      continue;
    }
    ...
    if (!title) {
      emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Missing '${lang}' title`);
      recComplete = false;
    }
    if (!shortDesc) {
      emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Missing '${lang}' shortDescription`);
      recComplete = false;
    }
    if (!longDesc) {
      emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Missing '${lang}' longDescription`);
      recComplete = false;
    }
    if (!location) {
      emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Missing '${lang}' location`);
      recComplete = false;
    }
    ...
    if ((lang as string) !== 'en') {
      if (shortDesc && shortDesc === rec.shortDescription.trim()) {
        emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Detected English fallback in '${lang}' shortDescription`);
        recComplete = false;
      }
      if (longDesc && longDesc === rec.longDescription.trim()) {
        emptyFieldErrors.push(`Canonical ID ${rec.id} [${rec.title}]: Detected English fallback in '${lang}' longDescription`);
        recComplete = false;
      }
    }
    ...
    if (checkText.includes('undefined') || checkText.includes('null') || checkText.includes('[object Object]')) {
      placeholderErrors.push(`Canonical ID ${rec.id}: Contains invalid token/placeholder in '${lang}'`);
      recComplete = false;
    }
  ...
  const success = unresolvedCount === 0 && emptyFieldErrors.length === 0 && placeholderErrors.length === 0;
  ...
  console.log(`- Verdict: ${success ? 'PASSED SUCCESSFUL' : 'FAILED - STRUCTURAL DISCREPANCIES'}`);
