...
const SPECIFIC_TRUTH_DATA: Record<
  string,
  {
...
const CATEGORY_TRUTH_DATA: Record<
  string,
  {
    desiredOutcome: Record<string, string>;
    whyItsWorthYourTime: Record<string, string>;
 ...
 */
export function getTruthCurationForRecommendation(
  item: Recommendation,
  lang: string,
): TruthCurationResult {
  ...
  if (!item) {
    const defaultRec =
      DEFAULT_TRUTH_DATA.whyItsWorthYourTime[l] ||
      DEFAULT_TRUTH_DATA.whyItsWorthYourTime.en;
    const defaultUnlearn =
      DEFAULT_TRUTH_DATA.unlearn[l] || DEFAULT_TRUTH_DATA.unlearn.en;
    const defaultWorth =
      DEFAULT_TRUTH_DATA.worthKnowing[l] || DEFAULT_TRUTH_DATA.worthKnowing.en;
    const defaultIdeal =
      DEFAULT_TRUTH_DATA.whoWillEnjoy[l] || DEFAULT_TRUTH_DATA.whoWillEnjoy.en;
    const defaultNotIdeal =
      NOT_IDEAL_PROFILES.default[l] || NOT_IDEAL_PROFILES.default.en;

    return {
      whyRecommended:
        DEFAULT_TRUTH_DATA.whyRecommended[l] ||
        DEFAULT_TRUTH_DATA.whyRecommended.en,
      whatMakesItSpecial:
        DEFAULT_TRUTH_DATA.whatMakesItSpecial[l] ||
        DEFAULT_TRUTH_DATA.whatMakesItSpecial.en,
      whoWillEnjoy: defaultIdeal,
      antiAdvice: defaultWorth,
      coreValue:
        DEFAULT_TRUTH_DATA.coreValue[l] || DEFAULT_TRUTH_DATA.coreValue.en,
      desiredOutcome:
        DEFAULT_TRUTH_DATA.desiredOutcome[l] ||
        DEFAULT_TRUTH_DATA.desiredOutcome.en,
      ...
      shortRecommendation:
        "Explore the raw, sovereign heritage and escape commercial clichés.",
  ...
  if (item.shortDescription) {
    if (typeof item.shortDescription === "object") {
      shortRec =
        (item.shortDescription as any)[l] ||
        (item.shortDescription as any)["en"] ||
        shortRec;
    } else {
  ...
  if (
    ...
    const wineNotIdeal =
      NOT_IDEAL_PROFILES.wine[l] || NOT_IDEAL_PROFILES.wine.en;
    ...
    return {
      ...
      coreValue:
        "Cultivating slow, authentic conversations with passionate winemakers in historic family cellars.",
  ...
  const rawWho = d.idealFor
    ? d.idealFor[l] || d.idealFor.en
    : d.whoWillEnjoy
      ? d.whoWillEnjoy[l] || d.whoWillEnjoy.en
      : "Mindful discoverers";
  const rawUnlearn = d.unlearn[l] || d.unlearn.en;
  const rawWorth = d.worthKnowing[l] || d.worthKnowing.en;
  const rawNotIdeal = NOT_IDEAL_PROFILES[catKey]
    ? NOT_IDEAL_PROFILES[catKey][l] || NOT_IDEAL_PROFILES[catKey].en
    : NOT_IDEAL_PROFILES.default[l] || NOT_IDEAL_PROFILES.default.en;
  const rawTime = d.whyItsWorthYourTime
    ? d.whyItsWorthYourTime[l] || d.whyItsWorthYourTime.en
    : rawWhyRec;
  ...
  return {
    ...
    whoWillEnjoy: rawWho,
    antiAdvice: rawWorth,
    coreValue: d.coreValue
      ? d.coreValue[l] || d.coreValue.en
      : "Preserving unhurried local presence.",
    desiredOutcome: d.desiredOutcome
      ? d.desiredOutcome[l] || d.desiredOutcome.en
      : "Connect with authentic, pristine heritage.",
