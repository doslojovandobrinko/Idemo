...
import { DestinationPackage, DestinationManifest, SyncStatus, Recommendation, EditorialCollection, Partner, PublicationWorkflowStage } from '../types';
 ...
 */
export async function calculatePackageHash(data: { recommendations: Recommendation[]; collections: EditorialCollection[]; partners: Partner[] }): Promise<string> {
  ...
  if (typeof crypto !== 'undefined' && crypto.subtle) {
    try {
      ...
      console.warn('[DestinationPackageManager] WebCrypto SHA-256 failed, fallback hash used', e);
    }
  }
...
/**
 * Filters and returns all valid mapped recommendations in the active discovery catalogue.
...
export async function buildCanonicalSerbiaPackage(): Promise<DestinationPackage> {
  ...
  const payloadHash = await calculatePackageHash({ recommendations, collections, partners });
 ...
 */
export async function validateDestinationPackage(pkg: DestinationPackage): Promise<{ valid: boolean; reason?: string }> {
  if (!pkg || !pkg.manifest) {
    return { valid: false, reason: 'INVALID_PACKAGE: Missing manifest header.' };
  }
  ...
  if (!manifest.destinationId || !manifest.packageVersion || !manifest.sha256) {
    return { valid: false, reason: 'MANIFEST_CORRUPT: Missing required manifest fields.' };
  }

  if (manifest.status !== 'published') {
    return { valid: false, reason: `UNPUBLISHED_PACKAGE: Package status is "${manifest.status}". Only published packages may be activated.` };
  }
  ...
  const computedHash = await calculatePackageHash({ recommendations, collections: editorialCollections, partners });
  if (manifest.sha256 && !manifest.sha256.startsWith('sha256-fallback-') && computedHash !== manifest.sha256) {
    ...
    if (
      ...
      return { valid: false, reason: 'INTEGRITY_FAILED: Package SHA-256 checksum mismatch and item counts differ.' };
    }
  }
...
export async function getActiveDestinationPackage(): Promise<DestinationPackage> {
  ...
  if (prevJson) {
    try {
      ...
      if (prevVal.valid) {
        console.warn('[DestinationPackageManager] Rolled back to previous valid package version:', parsedPrev.manifest.packageVersion);
        return parsedPrev;
      }
    } catch (e) {
      console.error('[DestinationPackageManager] Failed to parse previous package JSON:', e);
    }
  }
 ...
 */
export async function activateDestinationPackage(pkg: DestinationPackage, createRollbackBackup = true): Promise<boolean> {
  const validation = await validateDestinationPackage(pkg);
  if (!validation.valid) {
    console.error('[DestinationPackageManager] Cannot activate package:', validation.reason);
    return false;
  }
  ...
  console.log(`[DestinationPackageManager] Activated destination package ${pkg.manifest.destinationId} v${pkg.manifest.packageVersion} (${pkg.manifest.sha256.substring(0, 10)}...)`);
  return true;
}
...
export async function rollbackToPreviousPackage(): Promise<boolean> {
  ...
  try {
    ...
    if (val.valid) {
      ...
      console.log('[DestinationPackageManager] Rollback successful! Reverted to version', parsed.manifest.packageVersion);
      return true;
    }
  } catch (e) {
 ...
 */
export async function checkAndSyncDestinationPackage(destinationId: string = 'serbia'): Promise<SyncStatus> {
  ...
  if (!isOnline) {
    return {
      ...initialStatus,
      syncState: 'idle',
      syncError: 'OFFLINE_MODE: Device is offline. Using local destination package.',
    };
  }
  ...
  try {
    ...
    const remoteVersion = latestManifestRow.package_version || latestManifestRow.content_version || '1.0.0';
    ...
    console.log(`[DestinationSync] Newer package v${remoteVersion} available for ${destinationId}. Syncing...`);
    ...
    if (recRows && recRows.length > 0) {
      ...
      const newPkg: DestinationPackage = {
        manifest: {
          ...
          publishedAt: latestManifestRow.published_at || new Date().toISOString(),
          minSupportedAppVersion: latestManifestRow.min_supported_app_version || '1.0.0',
          sha256: latestManifestRow.sha256 || await calculatePackageHash({ recommendations: updatedRecs, collections: activePkg.editorialCollections, partners: activePkg.partners }),
