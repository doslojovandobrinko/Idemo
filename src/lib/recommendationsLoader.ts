...
export const loadRecommendations = async (): Promise<LoadRecommendationsResult> => {
  if (!isSupabaseConfigured()) {
    return {
      data: null,
      error: 'MISSING_CONFIG: Supabase environment variables (VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY) are not set.',
      isLive: false,
    };
  }
  ...
  if (!supabase) {
    return {
      data: null,
      error: 'CLIENT_INIT_FAILED: Unable to initialize Supabase client instance.',
      isLive: false,
    };
  }
  ...
  try {
    ...
    if (error) {
      ...
      return {
        ...
        error: isPermissionErr
          ? `PERMISSION_OR_RLS_FAILURE: ${error.message} (code: ${error.code || 'N/A'})`
          : `QUERY_FAILED: ${error.message} (code: ${error.code || 'N/A'})`,
        isLive: false,
      };
    }
    ...
    if (!data || data.length === 0) {
      return {
        data: null,
        error: 'EMPTY_TABLE: Query executed successfully but returned 0 published rows from public.recommendations.',
        isLive: false,
      };
    }
