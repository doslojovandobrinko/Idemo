...
export const loadRecommendations =
  async (): Promise<LoadRecommendationsResult> => {
    ...
    try {
      ...
      if (error || !data || data.length === 0) {
        ...
        return {
          data: pkg.recommendations,
          error: error
            ? `FALLBACK_TO_PACKAGE: Database error (${error.message}). Loaded package v${pkg.manifest.packageVersion}.`
            : null,
          isLive: false,
          packageVersion: pkg.manifest.packageVersion,
        };
      ...
      for (const row of data as SupabaseRecommendationRow[]) {
        ...
        const coordX =
          (row as any).coordinate_x ??
          (row as any).coordinateX ??
          staticMatch?.coordinateX ??
          0;
        const coordY =
          (row as any).coordinate_y ??
          (row as any).coordinateY ??
          staticMatch?.coordinateY ??
          0;
        ...
        mappedRecommendations.push({
          ...
          travelTimeMinutes:
            row.travel_time_minutes ?? staticMatch?.travelTimeMinutes ?? 60,
          location: row.location_en || staticMatch?.location || "Serbia",
          estimatedCost:
            row.estimated_cost || staticMatch?.estimatedCost || "Moderate",
          preferredTransport:
            row.preferred_transport || staticMatch?.preferredTransport || "Car",
          coordinateX: coordX,
          coordinateY: coordY,
          ...
          translations: {
            sr: {
              ...
              shortDescription:
                row.short_description_sr ||
                staticMatch?.translations?.sr?.shortDescription,
              longDescription:
                row.long_description_sr ||
                staticMatch?.translations?.sr?.longDescription,
              location:
                row.location_sr || staticMatch?.translations?.sr?.location,
            },
          },
        });
      }
