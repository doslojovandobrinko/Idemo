...
export interface UserPreferences {
  ...
  ratings?: Record<
    string,
    { vibe: "like" | "intrigue" | "dislike"; tags?: string[] }
  >;
...
const isOutdoorRecommendation = (rec: Recommendation): boolean => {
  ...
  if (
    ...
    (category.includes("clubbing") && locationText.includes("riverfront"))
  ) {
    return true;
  }
...
export function scoreRecommendation(
  ...
  if (prefs.orbitX !== undefined && prefs.orbitY !== undefined) {
    ...
    const spatialProximity = Math.max(
      0,
      100 - (euclideanDistance / maxPossibleDist) * 100,
    );
    ...
    const combinedSpaceSemantic =
      spatialProximity * 0.6 + semanticSimilarity * 0.4;
  ...
  const recCats =
    typeof rec.category === "string"
      ? rec.category.split(",").map((s) => s.trim())
      : [rec.category];
  const matchingCatsCount = recCats.filter((cat) =>
    ...
    matchingCatsCount > 0
      ? Math.min(
          ...
          0.7 +
            0.3 *
              (matchingCatsCount /
                Math.max(1, prefs.selectedCategories.length)),
        )
      : 0.0;
  ...
  const hasEveningCat =
    recCats.includes(Category.CLUBBING) ||
    recCats.includes(Category.GASTRONOMY);
  ...
  if (weather === "Sunny") {
    ...
    const isWinterResort =
      rec.title.toLowerCase().includes("kopaonik") ||
      rec.location.toLowerCase().includes("kopaonik");
    if (isWinterResort) {
      score += 65;
    } else if (isOutdoor) {
  ...
  if (isWeekend) {
    ...
    if (
      recCats.includes(Category.CLUBBING) ||
      recCats.includes(Category.TRAVEL) ||
      recCats.includes(Category.NATURE) ||
    ...
    if (
      recCats.includes(Category.HISTORY) ||
      recCats.includes(Category.WELLBEING) ||
      (recCats.includes(Category.GASTRONOMY) && totalDurationHours <= 3)
    ) {
      score += 25;
    }
  }
  ...
  if (activeHour >= 5 && activeHour < 11) {
    if (
      isOutdoor &&
      (rec.title.toLowerCase().includes("viewpoint") ||
        rec.location.toLowerCase().includes("tara") ||
        rec.title.toLowerCase().includes("fortress"))
    ) {
      score += 25;
    }
  ...
  if (activeHour >= 11 && activeHour < 16) {
    if (
      ...
      (recCats.includes(Category.HISTORY) ||
        recCats.includes(Category.WELLBEING) ||
        recCats.includes(Category.MEDICAL))
    ) {
      score += 20;
    } else if (isOutdoor) {
      score -= 15;
  ...
  if (activeHour >= 17 && activeHour <= 20) {
    if (
      isOutdoor &&
      (rec.title.toLowerCase().includes("sunset") ||
        rec.title.toLowerCase().includes("kayak") ||
        rec.location.toLowerCase().includes("riverfront") ||
        rec.title.toLowerCase().includes("fortress"))
    ) {
      score += 35;
    }
  ...
  if (selectedRef) {
    ...
    if (
      prefs.maxWalkingDistanceKm !== undefined &&
      prefs.maxWalkingDistanceKm > 0
    ) {
...
export function getRankedRecommendations(
  ...
  const hasNoImplicitTastes =
    !prefs.implicitTastes || Object.keys(prefs.implicitTastes).length === 0;
  const isDefaultBudgetAndTime = prefs.budget === 100 && prefs.time === 24;

  const isDefault =
    hasNoRatings && hasNoImplicitTastes && isDefaultBudgetAndTime;
