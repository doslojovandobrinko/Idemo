...
import {
  EditorialCollection,
  LoadEditorialCollectionsResult,
  SupabaseEditorialCollectionRow,
} from "../types";
import { getSupabaseClient, isSupabaseConfigured } from "./supabaseClient";
import { INITIAL_EDITORIAL_COLLECTIONS } from "../data/editorialCollections";

const staticCollectionsMap = new Map(
  INITIAL_EDITORIAL_COLLECTIONS.map((c) => [c.id, c]),
);
 ...
 */
export const loadEditorialCollections =
  async (): Promise<LoadEditorialCollectionsResult> => {
    if (!isSupabaseConfigured()) {
      return {
        data: INITIAL_EDITORIAL_COLLECTIONS,
        error:
          "MISSING_CONFIG: Supabase environment variables not set. Using local static baseline.",
        isLive: false,
      };
    }
    ...
    if (!supabase) {
      return {
        data: INITIAL_EDITORIAL_COLLECTIONS,
        error:
          "CLIENT_INIT_FAILED: Unable to initialize Supabase client instance. Using local static baseline.",
        isLive: false,
      };
    }
    ...
    try {
      ...
      for (const row of data as SupabaseEditorialCollectionRow[]) {
        ...
        mappedCollections.push({
          ...
          introductionEn:
            row.introduction_en || staticMatch?.introductionEn || "",
          ...
          estimatedDuration:
            row.estimated_duration || staticMatch?.estimatedDuration,
          visitorProfile: row.visitor_profile || staticMatch?.visitorProfile,
          recommendedSeason:
            row.recommended_season || staticMatch?.recommendedSeason,
          ...
          recommendationIds:
            row.recommendation_ids || staticMatch?.recommendationIds || [],
          recommendedOrder:
            row.recommended_order || staticMatch?.recommendedOrder,
          ...
          translations: {
            sr: {
              ...
              subtitle:
                row.subtitle_sr || staticMatch?.translations?.sr?.subtitle,
              introduction:
                row.introduction_sr ||
                staticMatch?.translations?.sr?.introduction,
              estimatedDuration:
                row.estimated_duration ||
                staticMatch?.translations?.sr?.estimatedDuration,
              geographicScope:
                row.geographic_scope ||
                staticMatch?.translations?.sr?.geographicScope,
              estimatedBudget:
                row.estimated_budget ||
                staticMatch?.translations?.sr?.estimatedBudget,
            },
            zh: {
              title: row.title_zh || staticMatch?.translations?.zh?.title,
              subtitle:
                row.subtitle_zh || staticMatch?.translations?.zh?.subtitle,
              introduction:
                row.introduction_zh ||
                staticMatch?.translations?.zh?.introduction,
            },
          },
        });
      }
