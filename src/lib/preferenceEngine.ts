...
export interface LocalPreferenceProfile {
  ...
  history: Array<{
    timestamp: string;
    type: 'favorite' | 'search' | 'category_view' | 'qr_scan' | 'map_open' | 'calendar_export' | 'view_details';
    targetId?: string;
    details?: string;
  }>;
...
export function savePreferenceProfile(profile: LocalPreferenceProfile) {
  ...
  try {
    ...
    if (profile.searchTerms.length > 30) {
      profile.searchTerms = profile.searchTerms.slice(profile.searchTerms.length - 30);
    }
...
function extractRecAttributes(rec: Recommendation) {
  const categories = typeof rec.category === 'string'
  ...
  const keywordsText = `${rec.title} ${rec.shortDescription} ${rec.longDescription}`.toLowerCase();
  const candidateTags = ['exclusive', 'authentic', 'relaxing', 'adventure', 'historic', 'romantic', 'traditional', 'viewpoint', 'gourmet', 'waterfront', 'nature'];
 ...
 */
export function decayPreferenceScores(profile: LocalPreferenceProfile, factor = 0.95): LocalPreferenceProfile {
  const updated = { ...profile };
  
  // Decay items
  for (const key in updated.itemAffinities) {
    updated.itemAffinities[key] = parseFloat((updated.itemAffinities[key] * factor).toFixed(2));
    if (updated.itemAffinities[key] < 0.1) delete updated.itemAffinities[key];
  }
  
  // Decay categories
  for (const key in updated.categoryAffinities) {
    updated.categoryAffinities[key] = parseFloat((updated.categoryAffinities[key] * factor).toFixed(2));
    if (updated.categoryAffinities[key] < 0.1) delete updated.categoryAffinities[key];
  }

  // Decay tags
  for (const key in updated.tagAffinities) {
    updated.tagAffinities[key] = parseFloat((updated.tagAffinities[key] * factor).toFixed(2));
...
export function trackFavoriteSignal(rec: Recommendation, isSaved: boolean) {
  ...
  const itemId = rec.id;
  const weight = isSaved ? SIGNAL_WEIGHTS.FAVORITE_ADD : SIGNAL_WEIGHTS.FAVORITE_REMOVE;
  const propWeight = isSaved ? PROPAGATION_WEIGHTS.FAVORITE : -PROPAGATION_WEIGHTS.FAVORITE / 2;
  
  profile.itemAffinities[itemId] = (profile.itemAffinities[itemId] || 0) + weight;
  
  categories.forEach(cat => {
    profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + propWeight;
  });
...
export function trackSearchSignal(query: string, allRecs: Recommendation[]) {
  ...
  allRecs.forEach(rec => {
    const searchText = `${rec.title} ${rec.shortDescription} ${rec.longDescription} ${rec.category} ${rec.location}`.toLowerCase();
    if (searchText.includes(cleanQuery)) {
      // Direct item boost
      profile.itemAffinities[rec.id] = (profile.itemAffinities[rec.id] || 0) + SIGNAL_WEIGHTS.SEARCH_MATCH;
      
      const { categories, tags } = extractRecAttributes(rec);
      categories.forEach(cat => {
        profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + PROPAGATION_WEIGHTS.SEARCH;
      });
      tags.forEach(tag => {
        profile.tagAffinities[tag] = (profile.tagAffinities[tag] || 0) + PROPAGATION_WEIGHTS.SEARCH;
      });
    }
  });
...
export function trackCategoryViewSignal(category: string) {
  ...
  profile.categoryAffinities[category] = (profile.categoryAffinities[category] || 0) + SIGNAL_WEIGHTS.CATEGORY_VIEW;
...
export function trackQRScanSignal(rec: Recommendation) {
  ...
  profile.itemAffinities[rec.id] = (profile.itemAffinities[rec.id] || 0) + SIGNAL_WEIGHTS.QR_SCAN;
  
  categories.forEach(cat => {
    profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + PROPAGATION_WEIGHTS.QR_SCAN;
  });
  
  tags.forEach(tag => {
    profile.tagAffinities[tag] = (profile.tagAffinities[tag] || 0) + PROPAGATION_WEIGHTS.QR_SCAN;
  });
...
export function trackMapOpenSignal(rec: Recommendation) {
  ...
  profile.itemAffinities[rec.id] = (profile.itemAffinities[rec.id] || 0) + SIGNAL_WEIGHTS.MAP_OPEN;
  
  categories.forEach(cat => {
    profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + PROPAGATION_WEIGHTS.MAP_OPEN;
  });
  
  tags.forEach(tag => {
    profile.tagAffinities[tag] = (profile.tagAffinities[tag] || 0) + PROPAGATION_WEIGHTS.MAP_OPEN;
  });
...
export function trackCalendarExportSignal(rec: Recommendation) {
  ...
  profile.itemAffinities[rec.id] = (profile.itemAffinities[rec.id] || 0) + SIGNAL_WEIGHTS.CALENDAR_EXPORT;
  
  categories.forEach(cat => {
    profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + PROPAGATION_WEIGHTS.CALENDAR_EXPORT;
  });
  
  tags.forEach(tag => {
    profile.tagAffinities[tag] = (profile.tagAffinities[tag] || 0) + PROPAGATION_WEIGHTS.CALENDAR_EXPORT;
  });
...
export function trackViewDetailsSignal(rec: Recommendation) {
  ...
  profile.itemAffinities[rec.id] = (profile.itemAffinities[rec.id] || 0) + SIGNAL_WEIGHTS.VIEW_DETAILS;
  
  categories.forEach(cat => {
    profile.categoryAffinities[cat] = (profile.categoryAffinities[cat] || 0) + PROPAGATION_WEIGHTS.VIEW_DETAILS;
  });
  
  tags.forEach(tag => {
    profile.tagAffinities[tag] = (profile.tagAffinities[tag] || 0) + PROPAGATION_WEIGHTS.VIEW_DETAILS;
  });
 ...
 */
export function getLocalPreferenceBoost(rec: Recommendation, profile?: LocalPreferenceProfile): number {
