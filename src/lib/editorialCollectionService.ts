...
export const getRecommendationsForCollection = (
  ...
  if (
    ...
    collection.recommendationIds.length === 0
  ) {
    return [];
  }
...
export const getCollectionsForRecommendation = (
  ...
  return collections.filter(
    (c) =>
      c.recommendationIds && c.recommendationIds.includes(recommendationId),
  );
};
...
export const getLocalizedCollectionTitle = (
  ...
  if (
    ...
    collection.translations[lang]?.title
  ) {
 ...
 */
export const getLocalizedCollectionSubtitle = (
  collection: EditorialCollection,
  lang: string = "en",
): string => {
  if (!collection) return "";
  ...
  if (
    lang !== "en" &&
    collection.translations &&
    collection.translations[lang]?.subtitle
  ) {
 ...
 */
export const getLocalizedCollectionIntroduction = (
  collection: EditorialCollection,
  lang: string = "en",
): string => {
  if (!collection) return "";
  if (lang === "sr" && collection.introductionSr)
    return collection.introductionSr;
  if (lang === "zh" && collection.introductionZh)
    return collection.introductionZh;
  if (
    lang !== "en" &&
    collection.translations &&
    collection.translations[lang]?.introduction
  ) {
...
export const getCollectionMapCoordinates = (
  ...
  const linkedRecs = getRecommendationsForCollection(
    collection,
    allRecommendations,
  );
  const coords: Array<{
