...
import { 
  ...
  ShieldCheck, Plus, Send, Check, Users, MessageCircle, Eye, ChevronRight, Briefcase, Loader2
} from 'lucide-react';
...
export default function PartnersScreen({ language, triggerHaptic, onNavigateToProfile, onSelectRec, onNavigate }: any) {
  ...
  const pin5001 = [53, 48, 48, 49].map(c => String.fromCharCode(c)).join('');
    ...
    fetchPartnerOpportunities('new').then(res => {
      if (res.success) {
        if (session.mustChangePin) {
          ...
          setActivePartnerId(session.partnerId);
  ...
  const handlePortalLogin = async () => {
    ...
    if (pin === pin9999 || code === pin9999) {
      ...
      if (res.success && res.partner) {
        triggerHaptic([30, 20, 40]);
        setActivePartnerId(res.partner.id);
        setPartnerCodeInput('');
        setPinInput('');
        if (res.partner.must_change_pin) {
          setMustChangePinMode(true);
  ...
  const handleVerifyPin = async () => {
    ...
    if (inputHash === pinTargetPartner.pinHash) {
              ...
              {!networkUnlocked ? (
                ...
                <div className="bg-white border border-[#2D3025]/10 rounded-[32px] shadow-[0_8px_30px_rgba(35,37,30,0.03)] overflow-hidden text-left flex flex-col animate-fade-in max-w-md mx-auto">
                  <div className="p-8 space-y-6">
                    ...
                    <div className="flex justify-center">
                      <IdemoLogo showBg={false} className="w-20 text-brand-charcoal" />
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="text-center space-y-2 py-2">
                    <span className="text-[9px] font-mono font-bold tracking-[0.25em] text-[#8A1F1F] block uppercase">
                      {isSr ? 'IDEMO PARTNERSKA MREŽA' : 'IDEMO PARTNER NETWORK'}
                    </span>
                    <h2 className="text-xl font-serif font-black text-brand-charcoal tracking-tight">
                  ...
                  <div className="bg-white border border-[#2D3025]/10 rounded-[32px] p-6 shadow-sm text-left space-y-4">
                    ...
                    <div className="space-y-3.5 text-[11px] leading-relaxed text-brand-charcoal/80 font-medium">
                      <div className="flex gap-2 items-start">
                        ...
                        <p>
                          <strong>{isSr ? 'Objavljena nova preporuka:' : 'New recommendation published:'}</strong>
                          {isSr ? ' Carska palata Felix Romuliana ažurirana bogatim audio sadržajem.' : ' Felix Romuliana Roman Palace updated with rich audio curation.'}
                        </p>
                      </div>
                      <div className="flex gap-2 items-start">
                        <span className="text-[#8A1F1F] font-bold select-none">•</span>
                        <p>
                          <strong>{isSr ? 'Istaknuta preporuka:' : 'Featured recommendation:'}</strong>
                          {isSr ? ' Krstarenje meandrima Uvca je zvanično otvoreno.' : ' Uvac Meanders prime summer boating is officially open.'}
                        </p>
                      </div>
                      <div className="flex gap-2 items-start">
                        <span className="text-[#8A1F1F] font-bold select-none">•</span>
                        <p>
                          <strong>{isSr ? 'Sezonska preporuka aktivirana:' : 'Seasonal recommendation activated:'}</strong>
                          {isSr ? ' Premijum vinski putevi Srbije aktivni za jesenju berbu.' : ' Premium Serbian Wine Trails active for autumn harvest.'}
                        </p>
                      </div>
                      <div className="flex gap-2 items-start">
                        <span className="text-[#8A1F1F] font-bold select-none">•</span>
                ...
                <div className="bg-white border border-[#2D3025]/10 rounded-[32px] p-6 shadow-sm text-left space-y-4">
                  <div className="flex items-center gap-2 border-b border-[#2D3025]/5 pb-3">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <h3 className="text-xs uppercase tracking-wide font-black text-brand-charcoal">MY OPPORTUNITIES</h3>
                    </div>
                  </div>
                  ...
                  {assignedInquiries.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {assignedInquiries.map(inq => {
                        const isActive = inq.status === 'Locked / Accepted' || inq.status === 'Alternative Proposed';
                        return (
                          <div key={inq.id} className="border border-[#2D3025]/5 rounded-2xl p-4 bg-[#FAF9F5]/50 space-y-3">
                            <div className="flex justify-between items-start gap-2">
                              <div className="text-left">
                                <h4 className="text-xs font-serif font-black text-brand-charcoal">{inq.visitorName}</h4>
                                <p className="text-[9px] font-mono text-brand-charcoal/50 uppercase">{inq.recTitle}</p>
                              </div>
                              <span className={`text-[8.5px] font-mono font-bold uppercase px-2 py-0.5 rounded border ${
                                inq.status === 'Locked / Accepted'
                                  ? 'bg-amber-500/5 text-amber-700 border-amber-500/15 animate-pulse'
                                  : inq.status === 'Alternative Proposed'
                                  ? 'bg-[#8A1F1F]/5 text-[#8A1F1F] border-[#8A1F1F]/15'
                                  : 'bg-[#2D3025]/5 text-[#2D3025]/50 border-transparent'
                              }`}>
                                {inq.status === 'Locked / Accepted'
                                  ? 'Active Lead'
                                  : inq.status === 'Alternative Proposed'
                            ...
                            <div className="bg-white border border-[#2D3025]/5 rounded-xl p-3 text-[11.5px] text-brand-charcoal/80 leading-relaxed italic text-left">
                              "${inq.query}"
                            </div>

                            {/* Display Metadata Fields */}
                            <div className="grid grid-cols-2 gap-2 text-[9px] font-mono text-brand-charcoal/60 bg-white/40 p-2.5 rounded-xl border border-[#2D3025]/5 text-left">
                              <div><span className="text-brand-charcoal/40 uppercase font-bold">Geography:</span> {inq.geography || 'N/A'}</div>
                              <div><span className="text-brand-charcoal/40 uppercase font-bold">Language:</span> {inq.language || 'English'}</div>
                              <div><span className="text-brand-charcoal/40 uppercase font-bold">Budget:</span> {inq.budget || 'N/A'}</div>
                              <div><span className="text-brand-charcoal/40 uppercase font-bold">Available Time:</span> {inq.availableTime || 'N/A'}</div>
                            </div>

                            {/* Show Reply Thread inside lead */}
                            {inq.replies.length > 0 && (
                              <div className="space-y-1.5 pt-1.5 border-t border-[#2D3025]/5 text-left">
                                <span className="text-[8px] font-mono uppercase font-black text-brand-charcoal/40 block">Submitted Responses:</span>
                                {inq.replies.map((rep, idx) => (
                                  <div key={idx} className="bg-emerald-500/5 text-emerald-900 border border-emerald-500/10 rounded-xl p-2.5 text-[10.5px] leading-relaxed">
                                    "${rep}"
                                  </div>
                                ))}
                              </div>
                            ...
                            {/* Submit Reply / Proposal Form */}
                            {isActive && (
                              <div className="space-y-2 pt-2 border-t border-[#2D3025]/5 text-left">
                                <span className="text-[8px] font-mono uppercase font-bold text-brand-charcoal/40 block">Submit Professional Response</span>
                                <textarea 
                                  placeholder="Write your professional offer, itinerary details, or booking conditions..."
                                  value={activeAnswerText[inq.id] || ''}
                                  onChange={e => {
                                    setActiveAnswerText(prev => ({ ...prev, [inq.id]: e.target.value }));
                                  }}
                                  className="w-full p-3 bg-white border border-[#2D3025]/10 rounded-xl text-xs focus:ring-1 focus:ring-[#8A1F1F]/20 focus:outline-none text-brand-charcoal"
                                  rows={3}
                                />
                                <div className="flex justify-end gap-2">
                                  <button 
                                    ...
                                    className="px-4 py-2 bg-[#8A1F1F] hover:bg-[#8A1F1F]/90 text-white text-[9px] font-black uppercase tracking-wider rounded-xl cursor-pointer"
                                  >
                                    Transmit Proposal
                                  </button>
                                </div>
