...
import {
  getInquiryByRecommendationId,
  getVisitorCredential,
  removeVisitorCredential,
  removeInquiryRecordV2,
...
import {
  ...
  confirmProposal,
  declineProposal,
  requestAlternativeProposal,
  fetchPartnerIntroduction,
  PartnerIntroductionResult,
...
> = {
  ...
  de: {
    ...
    apple_desc:
      "Universelle Datei für Apple Kalender (iPhone/Mac) und Outlook.",
...
export default function PlanCard({
  ...
  const [statusFeedback, setStatusFeedback] = React.useState<string | null>(
    null,
  );
  ...
  const [introData, setIntroData] =
    React.useState<PartnerIntroductionResult | null>(null);
    ...
    try {
      ...
      if (existing) {
        return {
          ...
          referenceCode:
            existing.public_reference_code || `IDEMO-REC${item.id}`,
  ...
  const handleRequestArrangement = async () => {
    ...
    const res = await submitInquiry({
      ...
      phoneNumber:
        !isEmail && contactInfoInput.trim().length > 0
          ? contactInfoInput.trim()
          : "+381621873260",
      visitorNotes: notes.trim() || "Arrangement request",
      preferredDate: item.scheduledDate
  ...
  const handleCheckStatus = async () => {
    ...
    if (!serverId) {
      setStatusFeedback(
        language === "sr"
          ? "Identifikator upita nije dostupan."
          : "Inquiry ID is not available.",
      );
      return;
    }
    triggerHaptic(5);
      ...
      setInquiry((prev: any) =>
        prev
          ? {
              ...prev,
              visitorStatusLabel:
                statusRes.visitor_status_label || statusRes.status,
            }
          : prev,
      );
    }
    ...
    if (proposalRes.success && proposalRes.proposal_found) {
      ...
      setStatusFeedback(
        language === "sr"
          ? "Pristigla je ponuda partnera."
          : "A proposal from the partner is available.",
      );
    } else {
      ...
      if (statusRes.success) {
        setStatusFeedback(
          language === "sr"
            ? "Status je osvežen. Čeka se odgovor partnera."
            : "Status updated. Awaiting partner response.",
        );
      } else {
        setStatusFeedback(
          statusRes.error ||
            (language === "sr"
              ? "Neuspešno osvežavanje statusa."
      ...
      setInquiry((prev: any) =>
        prev
          ? {
              ...
              visitorStatusLabel:
                language === "sr"
                  ? "Potvrđeno sa partnerom"
                  : "Confirmed with Partner",
            }
          : prev,
      );
      setActiveProposal(null);
  ...
  const handleDeclineProposal = async () => {
    ...
    const res = await declineProposal(
      serverId,
      activeProposal.match_id,
      proposalReason,
    );
    setActionInProgress(false);
    ...
    if (res.success) {
      ...
      setProposalReason("");
      setStatusFeedback(
        language === "sr" ? "Zahtev je otkazan." : "Inquiry request canceled.",
      );
    } else {
  ...
  const handleRequestAlternative = async () => {
    ...
    const res = await requestAlternativeProposal(
      serverId,
      activeProposal.match_id,
      proposalReason,
    );
    ...
    if (res.success) {
      ...
      setStatusFeedback(
        language === "sr"
          ? "Zatražena je alternativa od partnera."
          : "Alternative option requested from partner.",
      );
    } else {
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            ...
            {(isExpanded || inquiry) && (
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1.5">
                ...
                {item.travelTime && (
                  <span className="flex items-center gap-1 text-[9px] text-brand-charcoal/70 font-medium bg-[#F3F1ED] px-2 py-0.5 rounded leading-none select-none">
                    <Zap size={9} className="text-[#2E7D32]" />{" "}
                    {item.travelTime}
                  </span>
                )}
                <span className="flex items-center gap-1 text-[9px] text-brand-charcoal/70 font-medium bg-[#F3F1ED] px-2 py-0.5 rounded leading-none select-none">
                  <MapPin size={9} className="text-[#2E7D32]" />{" "}
                  {getLocalizedValue(item, "location", language)}
                </span>
                {item.preferredTransport && (
                  <span className="flex items-center gap-1 text-[9px] text-brand-charcoal/70 font-medium bg-[#F3F1ED] px-2 py-0.5 rounded leading-none select-none">
                    <Car size={9} className="text-[#2E7D32]" />{" "}
                    {item.preferredTransport}
                  </span>
                )}
                {item.estimatedCost && (
                  <span className="flex items-center gap-1 text-[9px] text-brand-charcoal/70 font-medium bg-[#F3F1ED] px-2 py-0.5 rounded leading-none select-none">
                    <Euro size={9} className="text-[#2E7D32]" />{" "}
                    {item.estimatedCost}
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#3C3A2E] font-bold select-none">
              {item.scheduledDate
                ...
                : (PLAN_CARD_TR[language] || PLAN_CARD_TR["en"]).set_date}
            </span>
          </div>
          ...
          (() => {
            ...
            const isOutdoor = (() => {
              ...
              if (
                ...
                (category.includes("clubbing") &&
                  locationText.includes("riverfront"))
              ) {
                return true;
              }
              ...
              return outdoorKeywords.some(
                (kw) => titleText.includes(kw) || locationText.includes(kw),
              );
            })();
            ...
            const isBelgradeOrVojvodina =
              locLow.includes("belgrade") ||
              locLow.includes("beograd") ||
              locLow.includes("novi sad") ||
              locLow.includes("fruška gora") ||
              locLow.includes("pančevo") ||
              locLow.includes("vojvodina");
            ...
            if (!isOutdoor) {
              const indoorTr: Record<string, any> = {
                en: {
                  tempText: "Climate-Controlled",
                  condition: "Indoor Sanctuary",
                  advice:
                    "Fully protected from external elements. A perfect itinerary choice regardless of the season.",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
                sr: {
                  tempText: "Kontrolisana klima",
                  condition: "Zatvoreni prostor",
                  advice:
                    "Potpuno zaštićeno od spoljašnjih uticaja. Savršen izvor aktivnosti bez obzira na godišnje doba.",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
                de: {
                  tempText: "Klimatisiert",
                  condition: "Innenbereich",
                  advice:
                    "Von jeglicher Witterung geschützt. Eine perfekte Wahl für Ihre Reiseplanung zu jeder Jahreszeit.",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
                es: {
                  tempText: "Climatizado",
                  condition: "Espacio Cubierto",
                  advice:
                    "Protegido por completo del clima exterior. Elección perfecta independientemente del mes.",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
                ru: {
                  tempText: "Контролируемый микроклимат",
                  condition: "Крытое помещение",
                  advice:
                    "Объект полностью защищен от внешних условий. Замечательно подходит для любого сезона.",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
                zh: {
                  ...
                  condition: "室内场所",
                  advice:
                    "完全不受外界天气变化干扰。无论何时到访，均为极致惬意的全天候完美选择。",
                  color: "bg-[#F6F5F2] border-[#E5E3DB] text-brand-charcoal/70",
                  icon: "🏛️",
                },
              };
              ...
              if (month === 11 || month === 0 || month === 1) {
                const winterTr: Record<string, any> = {
                  en: {
                    tempText: "-2°C to 4°C",
                    condition: "Winter Ice Advisory",
                    advice:
                      "Exposed path may have sleet or dense fog. Heavy thermal layers and rugged footwear recommended.",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                  sr: {
                    tempText: "-2°C do 4°C",
                    condition: "Zimski uslovi i mraz",
                    advice:
                      "Otvorene staze mogu biti zaleđene ili maglovite. Obavezni su termo slojevi i duboka nepromočiva obuća.",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                  de: {
                    tempText: "-2°C bis 4°C",
                    condition: "Winter-Frostwarnung",
                    advice:
                      "Erhöhte Frost- und Nebelgefahr auf Außenbühnen. Thermo-Schichten & absolut rutschfestes Schuhwerk ratsam.",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                  es: {
                    tempText: "-2°C a 4°C",
                    condition: "Aviso de Helada Invernal",
                    advice:
                      "Pistas y senderos expuestos pueden tener hielo o niebla densa. Capas térmicas gruesas aconsejadas.",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                  ru: {
                    tempText: "-2°C до 4°C",
                    condition: "Зимние заморозки",
                    advice:
                      "На открытом воздухе возможен гололед или густой туман. Рекомендуются теплые слои и высокая обувь.",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                  zh: {
                    ...
                    condition: "严寒与结冰预警",
                    advice:
                      "户外行道可能出现湿滑结冰或浓雾。请穿着高保暖防寒服以及防滑耐磨鞋。",
                    color: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
                    icon: "❄️",
                  },
                };
              ...
              // Chilly transition months showing Wind / Autumn chill
                ...
                if (isBelgradeOrVojvodina) {
                  const kosavaTr: Record<string, any> = {
                    en: {
                      tempText: "6°C to 13°C",
                      condition: "Exposed Košava Gale Warning",
                      advice:
                        "Strong, gusty cold southeasterly Košava wind sweeps across riverfronts. Secure light baggage and wear windbreakers.",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                    sr: {
                      tempText: "6°C do 13°C",
                      condition: "Udari hladne Košave",
                      advice:
                        "Jak, hladan jugoistočni vetar (Košava) duva duž reka. Obavezna je vetrovka i izbegavanje kapa na otvorenom.",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                    de: {
                      tempText: "6°C bis 13°C",
                      condition: "Košava-Sturmwarnung",
                      advice:
                        "Starke, böige Kaltwinde aus Südosten peitschen über Flussufer. Windfeste Kleidung unbedingt erforderlich.",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                    es: {
                      tempText: "6°C a 13°C",
                      condition: "Vientos Fuertes Košava",
                      advice:
                        "Ráfagas secas e intensas del sureste soplan en la ribera del río. Use chubasquero o cortavientos firme.",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                    ru: {
                      tempText: "6°C до 13°C",
                      condition: "Холодные порывы ветра Кошава",
                      advice:
                        "Очень сильный, порывистый ветер с Дуная на открытых набережных. Рекомендуются ветрозащитные куртки.",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                    zh: {
                      tempText: "6°C 至 13°C",
                      condition: "Košava 强风防风警示",
                      advice:
                        "多瑙河与萨瓦河汇流区特有强风。阵风凛冽，体感寒冷，请务必穿着密闭型防风战衣。",
                      color: "bg-[#FEF3C7] border-[#FDE68A] text-[#92400E]",
                      icon: "💨",
                    },
                  };
                  ...
                  const transitionalTr: Record<string, any> = {
                    en: {
                      tempText: "5°C to 12°C",
                      condition: "Chilly Mountain Climate",
                      advice:
                        "Volatile mountain temperatures. Bring layers and high-quality boots; prepare for damp trails.",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                    sr: {
                      tempText: "5°C do 12°C",
                      condition: "Sveže vreme u planinama",
                      advice:
                        "Brze promene temperature na planinskom vazduhu. Ponesite tople slojeve i čvrstu obuću za vlažne staze.",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                    de: {
                      tempText: "5°C bis 12°C",
                      condition: "Berg-Klimawandel",
                      advice:
                        "Instabile Gebirgstemperaturen. Bringen Sie wärmende Schichten mit und stellen Sie sich auf feuchte Wege ein.",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                    es: {
                      tempText: "5°C a 12°C",
                      condition: "Clima Frío de Altura",
                      advice:
                        "Cambios drásticos de temperatura. Póngase capas interiores térmicas y use calzado firme repelente al agua.",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                    ru: {
                      tempText: "5°C до 12°C",
                      condition: "Переменная горная прохлада",
                      advice:
                        "Быстрая смена погоды на высотах. Рекомендуется брать с собой теплые кофты и непромокаемые ботинки.",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                    zh: {
                      ...
                      condition: "山区多变湿冷气候",
                      advice:
                        "林区与高地温度容易迅速下挫。建议携带多层可穿脱衣物，行道可能伴有薄泥。",
                      color: "bg-[#F1F5F9] border-[#E2E8F0] text-[#475569]",
                      icon: "🌬️",
                    },
                  };
              ...
              // Peak Rainfall months (May & June are historically the wettest in Serbia, high convective showers)
                ...
                const wetTr: Record<string, any> = {
                  en: {
                    tempText: "14°C to 24°C",
                    condition: "High Thunderstorm Risk",
                    advice:
                      "May & June is Serbia's peak seasonal rainfall window. Suddden heavy afternoon local summer storm likely; please bring an umbrella.",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                  sr: {
                    tempText: "14°C do 24°C",
                    condition: "Rizik od popodnevnih nepogoda",
                    advice:
                      "Maj i jun imaju najveću količinu padavina. Iznenadni i jaki pljuskovi sa grmljavinom su česti popodne; ponesite kišobran.",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                  de: {
                    tempText: "14°C bis 24°C",
                    condition: "Erhöhte Gewitterwahrscheinlichkeit",
                    advice:
                      "Mai und Juni sind die nassesten Monate Serbiens. Plötzlicher Starkregen am Nachmittag ist üblich; Schirm empfehlenswert.",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                  es: {
                    tempText: "14°C a 24°C",
                    condition: "Riesgo de Tormentas Estivales",
                    advice:
                      "Mayo y junio representan el máximo de precipitaciones en Serbia. Tormentas locales fuertes por la tarde; lleve paraguas.",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                  ru: {
                    tempText: "14°C до 24°C",
                    condition: "Сезон гроз и ливней",
                    advice:
                      "Май и июнь — пик годовых осадков в Сербии. Вероятны внезапные сильные грозы во второй половине дня; захватите зонт.",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                  zh: {
                    tempText: "14°C 至 24°C",
                    condition: "午后特大对流雷雨预警",
                    advice:
                      "5月与6月是塞尔维亚雨水最充沛的历史高峰。极易遭遇短时特大雷雨暴风，出行必携折叠伞。",
                    color: "bg-[#EFF6FF] border-[#DBEAFE] text-[#1D4ED8]",
                    icon: "⛈️",
                  },
                };
              ...
              // Scorching Summer months
                ...
                const hotTr: Record<string, any> = {
                  en: {
                    tempText: "30°C to 38°C+",
                    condition: "Extreme Summer Heat Advisory",
                    advice:
                      "Intense midday Balkan sun. Avoid peak open solar exposure between 11:00 - 15:30. Apply sunscreen and maintain high hydration.",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                  sr: {
                    tempText: "30°C do 38°C+",
                    condition: "Ekstremno visoke temperature",
                    advice:
                      "Jako sunčevo zračenje i temperature. Izbegavajte direktnu izloženost suncu između 11:00 i 15:30. Nosite kapu i pijte dosta vode.",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                  de: {
                    tempText: "30°C bis 38°C+",
                    condition: "Extreme Sommerhitze-Warnung",
                    advice:
                      "Sehr starke UV-Belastung. Vermeiden Sie pralle Sonne zwischen 11:00 - 15:30 Uhr. Verwenden Sie Sonnenschutz & trinken ausreichend Wasser.",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                  es: {
                    tempText: "30°C a 38°C+",
                    condition: "Alerta por Ola de Calor",
                    advice:
                      "Sol y radiación balcánica alta. Se desaconseja caminar directamente al descubierto de 11:00 a 15:30. Use SPF alto y beba agua.",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                  ru: {
                    tempText: "30°C до 38°C+",
                    condition: "Предупреждение об аномальной жаре",
                    advice:
                      "Высокая солнечная активность. Избегайте прямых лучей с 11:00 до 15:30. Обязательны солнцезащитный крем и частая вода.",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                  zh: {
                    tempText: "30°C 至 38°C+",
                    condition: "极端夏季暴汗日灼预警",
                    advice:
                      "巴尔干夏季紫外线照射猛烈。请尽量规避 11:00 至 15:30 强日光暴晒，出游请抹防晒霜并及时充足补水。",
                    color: "bg-[#FEF2F2] border-[#FEE2E2] text-[#991B1B]",
                    icon: "🥵",
                  },
                };
              ...
              // Golden Autumn (Indian Summer / Miholjsko Leto)
                ...
                const autumnTr: Record<string, any> = {
                  en: {
                    tempText: "16°C to 23°C",
                    condition: "Pleasant Golden Autumn",
                    advice:
                      "Stunning mild conditions ('Miholjsko Leto'). Excellent sky visibility and crisp local breeze. Ideal for open hikes and fortress exploration.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                  sr: {
                    tempText: "16°C do 23°C",
                    condition: "Predivno zlatno Miholjsko Leto",
                    advice:
                      "Izuzetno suvo i prijatno vreme sa blagim povetarcem. Savršeni panoramski vidikovci, idealno za tvrđave i izlete.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                  de: {
                    tempText: "16°C bis 23°C",
                    condition: "Perfektes Miholjsko Leto",
                    advice:
                      "Wunderbarer, trockener Altweibersommer. Spektakuläre Fernsicht auf Aussichtspunkten. Erstklassige Bedingungen für Wanderungen.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                  es: {
                    tempText: "16°C a 23°C",
                    condition: "Excelente 'Veranillo de San Miguel'",
                    advice:
                      "Brisa templada fantástica y cielos despejados. Época ideal para visitas de altura, fortalezas y caminatas largas sin agobio.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                  ru: {
                    tempText: "16°C до 23°C",
                    condition: "Превосходная золотая осень",
                    advice:
                      "Шикарные сухие и ясные дни бабьего лета. Идеальная видимость на высотах. Рекомендуется для долгих походов и крепостей.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                  zh: {
                    tempText: "16°C 至 23°C",
                    condition: "明亮宜人金秋（小阳春）",
                    advice:
                      "传统高能舒适期（Miholjsko Leto）。绝佳晴空能见度伴有清脆微风。极力推荐露天遗址观光与高峦徒步。",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🍂",
                  },
                };
                ...
                const springTr: Record<string, any> = {
                  en: {
                    tempText: "11°C to 19°C",
                    condition: "Vibrant Temperate Spring",
                    advice:
                      "Generally pleasing and dynamic weather. A light windproof outer jacket or hood is recommended.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                  sr: {
                    tempText: "11°C do 19°C",
                    condition: "Umereno prolećno vreme",
                    advice:
                      "Uglavnom prijatno i promenljivo vreme. Preporučuje se lagana vetrovka za prohladno veče.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                  de: {
                    tempText: "11°C bis 19°C",
                    condition: "Angenehmer Frühling",
                    advice:
                      "Sehr mildes, dynamisches Frühlingswetter. Eine leichte winddichte Jacke wird für die kühlen Abendstunden empfohlen.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                  es: {
                    tempText: "11°C a 19°C",
                    condition: "Primavera Templada",
                    advice:
                      "Clima templado cambiante y agradable. Un cortavientos ligero es ideal para las caminatas vespertinas.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                  ru: {
                    tempText: "11°C до 19°C",
                    condition: "Теплая весенняя погода",
                    advice:
                      "Обычно приятная весенняя атмосфера. Легкая плотная куртка будет кстати для вечерних прогулок.",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                  zh: {
                    tempText: "11°C 至 19°C",
                    condition: "晴和温润初春",
                    advice:
                      "气候普遍多变温暖。随身常备薄款抗风防泼水风衣或连帽衫即是完美的户外穿搭方案。",
                    color: "bg-[#F0FDF4] border-[#DCFCE7] text-[#166534]",
                    icon: "🌱",
                  },
                };
            ...
            const activeNotice =
              forecastNotice[language] || forecastNotice["en"];

            return (
              <div
                className={`p-3.5 rounded-xl border flex flex-col gap-1.5 transition-all duration-300 ${climateObj.color}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-black uppercase tracking-widest">
                      {climateObj.condition}
                    </span>
                  </div>
                  <span className="font-mono text-[12px] font-bold opacity-95 bg-white bg-opacity-60 px-2 py-0.5 rounded-full">
                    {climateObj.tempText}
                  </span>
                </div>
        ...
        {isExpanded && !submitting && !inquiry && (
          <div className="space-y-3 p-3.5 bg-brand-sage/5 rounded-xl border border-brand-sage/20">
            ...
            <div className="space-y-1">
              <label className="text-[9px] uppercase tracking-wider text-[#5C5A4D] font-bold block">
                {ARRANGE_TR[language]?.preferred_time ||
                  ARRANGE_TR["en"].preferred_time}
              </label>
              <input
                type="text"
                value={preferredTime}
                ...
                placeholder={
                  ARRANGE_TR[language]?.time_placeholder ||
                  ARRANGE_TR["en"].time_placeholder
                }
                className="w-full px-3 py-2 text-xs bg-white border border-border-main rounded-xl focus:outline-none focus:border-accent-teal font-sans text-brand-charcoal transition-all"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="text-[9px] uppercase tracking-wider text-[#5C5A4D] font-bold block">
                {ARRANGE_TR[language]?.personal_notes ||
                  ARRANGE_TR["en"].personal_notes}
              </label>
              <textarea
                rows={2}
                value={notes}
                ...
                placeholder={
                  ARRANGE_TR[language]?.notes_placeholder ||
                  ARRANGE_TR["en"].notes_placeholder
                }
            ...
            {submissionError && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-800 space-y-1">
                ...
                <p className="text-[10px] text-red-600">
                  You can contact our VIP Concierge team directly via WhatsApp
                  or Instagram.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3.5 pt-1.5">
              ...
              >
                <span>
                  {ARRANGE_TR[language]?.request_arrangement ||
                    ARRANGE_TR["en"].request_arrangement}
                </span>
              </button>
            </div>
          </div>
        ...
        {submitting && (
          <div className="py-7 flex flex-col items-center justify-center text-center space-y-3.5 bg-brand-sage/5 rounded-xl border border-brand-sage/20 p-4">
            <div className="relative w-11 h-11 flex items-center justify-center">
              ...
              />
              <span className="text-accent-teal text-base animate-bounce">
                🧭
              </span>
            </div>
            ...
            <div className="space-y-1 w-full">
              <h5 className="font-serif text-[11px] font-bold text-brand-charcoal">
                {language === "sr"
                  ...
                  : language === "zh"
                    ? "正在传送专属申请..."
                    : "Dispatching Arrangement Request..."}
              </h5>
          ...
          <div className="p-4 bg-[#F2F5F3] rounded-2xl border border-brand-sage/30 space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5 bg-[#E8EFE9] border border-brand-sage/40 px-2.5 py-0.5 rounded-full">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wider text-[#3E5037]">
                    {inquiry.visitorStatusLabel ||
                      inquiry.status ||
                      ARRANGE_TR[language]?.assistance_requested ||
                      ARRANGE_TR["en"].assistance_requested}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between px-1.5 pt-1">
                <div className="flex flex-col items-center gap-1">
                  <div className="w-5 h-5 rounded-full bg-[#3E5037] text-white flex items-center justify-center text-[9px] font-black">
                    ✓
                  </div>
                  <span className="text-[7.5px] font-black text-brand-charcoal uppercase leading-none">
                    Requested
                  </span>
                </div>
                ...
                <div className="flex flex-col items-center gap-1">
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold border ${activeProposal ? "bg-[#3E5037] text-white border-[#3E5037]" : "bg-white text-brand-charcoal/30 border-brand-charcoal/10"}`}
                  >
                    {activeProposal ? "✓" : "2"}
                  </div>
                  <span className="text-[7.5px] font-bold text-brand-charcoal/30 uppercase leading-none">
                    Review
                  </span>
                </div>
                <div className="h-0.5 flex-1 bg-brand-charcoal/10 mx-1 -translate-y-2" />
                <div className="flex flex-col items-center gap-1">
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold border ${inquiry.status === "Arrangement Confirmed" ? "bg-[#3E5037] text-white border-[#3E5037]" : "bg-white text-brand-charcoal/30 border-brand-charcoal/10"}`}
                  >
            ...
            <div className="p-3 bg-white rounded-xl border border-brand-sage/20 text-center font-mono space-y-1 shadow-sm select-all">
              <span className="text-[7px] uppercase tracking-[0.2em] font-mono text-[#8C8A7D] block">
                {ARRANGE_TR[language]?.inquiry_reference ||
                  ARRANGE_TR["en"].inquiry_reference}
              </span>
            ...
            {inquiry.serverInquiryId &&
              getVisitorCredential(inquiry.serverInquiryId) && (
                <div className="space-y-2.5 pt-1 border-t border-brand-sage/20">
                  ...
                  {activeProposal && activeProposal.proposal_found && (
                    <div className="p-3 bg-white rounded-xl border-2 border-[#3E5037]/30 space-y-2.5 shadow-sm">
                      ...
                      {(activeProposal.proposed_start_at ||
                        activeProposal.proposed_end_at) && (
                  ...
                  <div className="pt-1">
                    ...
                    {introOpen && (
                      <div className="mt-2 p-3 bg-white rounded-xl border border-[#3E5037]/20 space-y-2 text-left">
                        {introLoading ? (
                          <p className="text-[9px] font-mono text-[#8C8A7D] animate-pulse">
                            {language === "sr"
                              ? "Učitavanje predstavljanja..."
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between border-b border-[#3E5037]/10 pb-1.5">
                              <span className="text-[9px] font-bold font-serif text-[#1E2E20]">
                            ...
                            {introData.photo_url && (
                              <div className="flex justify-center my-1.5">
                                <img
                                  ...
                                  alt={
                                    introData.partner_name || "Partner Profile"
                                  }
                          ...
                          <p className="text-[9px] font-sans text-[#8C8A7D] italic">
                            {language === "sr"
                              ? "Predstavljanje trenutno nije dostupno."
...
Language: ${language === "sr" ? "Serbian" : language === "zh" ? "Chinese" : "English"}`;
              ...
              >
                <span>
                  {copiedChannel === "whatsapp" ? "COPIED!" : "WHATSAPP"}
                </span>
              </a>
              <a
...
Language: ${language === "sr" ? "Serbian" : language === "zh" ? "Chinese" : "English"}`;
              ...
              >
                <span>
                  {copiedChannel === "instagram" ? "COPIED!" : "INSTAGRAM"}
                </span>
              </a>
            </div>
            ...
            <div className="pt-2 text-center border-t border-brand-sage/10">
              ...
              >
                {language === "sr"
                  ? "Ukloni upit sa ovog uređaja"
                  : "Remove request from this device"}
              </button>
            </div>
          </div>
        )}
        ...
        {item.isAvailable === false ? (
          <div className="mt-1 pt-1">
            <div className="p-3 bg-[#FEF3C7]/40 border border-[#F59E0B]/30 rounded-xl text-center text-[10px] font-bold text-[#B45309]">
              {language === "sr"
                ? "Arhivirana lokacija — slanje upita onemogućeno."
                : "Archived recommendation — assistance request is disabled."}
            </div>
          </div>
        ) : (
          !inquiry &&
          !isExpanded && (
            <div className="mt-1 pt-1">
              ...
              >
                <span>
                  {ARRANGE_TR[language]?.arrange_this ||
                    ARRANGE_TR["en"].arrange_this}
                </span>
              </button>
            </div>
          )
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-brand-charcoal/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-serif text-[16px] font-bold tracking-tight text-brand-charcoal">
                    {
                      (PLAN_CARD_TR[language] || PLAN_CARD_TR["en"])
                        .reschedule_item
                    }
                  </h3>
                </div>
              ...
              <div className="flex items-center justify-between bg-white/50 border border-brand-charcoal/5 px-3 py-2 rounded-xl">
                ...
                <span className="font-serif font-bold text-sm select-none">
                  {(MONTH_NAMES[language] || MONTH_NAMES["en"])[viewMonth]}{" "}
                  {viewYear}
                </span>
                <button
              ...
              <div className="grid grid-cols-7 gap-1 text-center font-bold text-[10px] uppercase tracking-wider text-brand-charcoal/50">
                {(WEEKDAY_NAMES[language] || WEEKDAY_NAMES["en"]).map(
                  (day, idx) => (
