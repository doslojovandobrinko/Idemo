...
export const ARCHETYPE_INTERESTS_MAP: Record<
  string,
  {
...
function ProfileDetailTile({
  ...
  const finalAria =
    ariaLabel || `${label}: ${value}. ${onClick ? "Tap to view details." : ""}`;
  return (
    <div
      onClick={onClick}
...
export default function ProfileScreen({
  ...
  const getResetSuccessMessage = () => {
    if (isSr)
      return "Vodič je uspešno resetovan. Pri sledećem pokretanju biće Vam ponovo prikazan.";
  ...
  const handleCopyLink = () => {
    ...
    const shareData = {
      title: isSr
        ? "IDEMO Putna Propusnica"
        : isZh
          ? "专属旅游通票"
          : "IDEMO Travel Pass",
      text: isSr
        ? "Moje sačuvane rute i preferencije za Beograd:"
        : isZh
          ? "您在贝尔格莱德的专属旅行通票："
          : "My curated travel preferences and saved routes for Belgrade:",
      url: shareUrl,
    };
  ...
  const isCultural = useMemo(() => {
    ...
    return (
      activeArchId === "cultural_strategist" ||
      vibe.toLowerCase().includes("cultural") ||
      vibe.toLowerCase().includes("kulturn")
    );
  ...
  const explanationData = useMemo(() => {
    ...
    > = {
      persona: {
        title: isSr
          ? "VAŠA PREOVLAĐUJUĆA PERSONA"
          : isZh
            ? "您的核心旅行人格"
            : "YOUR PREVAILING PERSONA",
      ...
      vibe: {
        ...
        action: () => {
          ...
          playHaptic(10);
          document
            .getElementById("mood-orbit-section")
      ...
      tempo: {
        ...
        value: getTempoTag(
          language,
          currentArchetype.targetVibe.activeVSrelaxed,
          orbitY,
        ),
        meaning: isSr
      ...
      budget: {
        ...
        action: () => {
          ...
          setTimeout(() => {
            document
              .getElementById("budget-slider-container")
              ?.scrollIntoView({ behavior: "smooth", block: "center" });
          }, 150);
        },
      },
      time: {
        ...
        action: () => {
          ...
          setTimeout(() => {
            document
              .getElementById("time-slider-container")
              ?.scrollIntoView({ behavior: "smooth", block: "center" });
          }, 150);
        },
      },
      interests: {
        title: isSr
          ...
          : isZh
            ...
            : "PRIMARY INTERESTS",
        value: selectedCats
          .map((cat: string) => t[cat.toLowerCase()] || cat)
          .join(", "),
        meaning: isSr
  ...
  useEffect(() => {
    ...
    for (const key of keys) {
      ...
      for (const w of weights) {
        const val =
          w.arch.targetVibe[key] !== undefined
            ? w.arch.targetVibe[key]
            : DEFAULT_VIBE_SETTINGS[key as keyof VibeSettings];
      ...
      for (const w of weights) {
        ...
        const recCats =
          typeof rec.category === "string"
            ...
            : [rec.category];
        const categoryOverlap = recCats.some((c: any) =>
          arch.categories.includes(c),
        );
        if (categoryOverlap) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const scoredForInterest = scored
          .filter((item: any) => {
            ...
            return (
              (!feedback || feedback.vibe !== "dislike") &&
              !pickedIds.has(item.rec.id)
            );
          })
          .map((item: any) => {
            const textToSearch =
              `${item.rec.title} ${item.rec.shortDescription} ${item.rec.longDescription} ${item.rec.location} ${item.rec.category}`.toLowerCase();
            let kwBonus = 0;
  ...
  const pendingCandidates = accuracyCandidates.filter(
    (exp) => !confirmedAccuracyRecs[exp.id],
  );

  return (
    <div
      className="flex-1 flex flex-col min-h-0 relative overflow-hidden"
      id="profile-screen-wrapper"
    >
      <motion.div
      ...
      >
        ...
        <section
          className="bg-brand-pearl rounded-3xl border border-[#2D3025]/10 p-5 space-y-4 shadow-[0_2px_8px_rgba(35,37,30,0.02)]"
          id="mood-orbit-section"
        >
        ...
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs uppercase tracking-[0.25em] font-black text-brand-charcoal">
                {isSr
                  ...
                  : isZh
                    ? "我的旅行画像"
                    : "WHAT KIND OF TRAVELER AM I TODAY?"}
              </h2>
            </div>
            <span
              onClick={() => {
          ...
          >
            ...
            <p className="text-[8px] uppercase tracking-widest font-black text-accent-teal/85 leading-tight">
              {currentArchetype.tagline[language] ||
                currentArchetype.tagline.en}
            </p>
          ...
          <div className="grid grid-cols-2 gap-3" id="traveler-parameter-grid">
            ...
            <ProfileDetailTile
              label={
                isSr ? "Željeni Tempo" : isZh ? "偏好节奏" : "Preferred Tempo"
              }
              value={getTempoTag(
                language,
                currentArchetype.targetVibe.activeVSrelaxed,
            ...
            <ProfileDetailTile
              label={
                isSr ? "Tipičan Budžet" : isZh ? "典型预算" : "Typical Budget"
              }
              value={`${getBudgetTag(language, budget)} (€${budget}${budget >= 500 ? "+" : ""})`}
              onClick={() => {
                ...
                {refineOpen
                  ? isSr
                    ...
                    : isZh
                      ...
                      : "CLOSE TUNING PANEL"
                  : isSr
                    ? "FINO PODEŠAVANJE PROFILA"
                    : isZh
                      ? "微调旅行偏好设置"
                      : "FINE-TUNE TRAVEL PROFILE"}
              </span>
            </button>
                  ...
                  <div className="pt-4 space-y-5">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                          ...
                          <span className="text-[11px] font-extrabold text-brand-charcoal">
                            {lowSignalMode
                              ? isSr
                                ? "Ušteda signala (Offline-first)"
                                : isZh
                                  ? "离线优先节能模式"
                                  : "Low Signal Mode"
                              : isSr
                                ? "Standardni režim (Full Online)"
                                : isZh
                                  ? "标准在线渲染模式"
                        ...
                        >
                          <div
                            className={`w-5 h-5 rounded-full bg-brand-pearl shadow-sm transition-transform duration-200 transform ${
                    ...
                    <div className="space-y-2 bg-white/60 rounded-2xl border border-[#2D3025]/5 p-4">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (parseInt(e.target.value) % 12 === 0)
                            playHaptic(3);
                        }}
                        className="w-full h-1 bg-[#2D3025]/10 rounded-lg appearance-none cursor-pointer accent-[#2D3025]"
                      />
                      <div className="flex justify-between text-[9px] text-[#2D3025]/40 font-mono">
                        ...
                        <span>
                          {isSr ? "24 sata" : isZh ? "24小时" : "24h"}
                        </span>
                        <span>
                          {isSr ? "48 sati" : isZh ? "48小时" : "48h"}
                        </span>
                      </div>
                    </div>
                          ...
                          if (dayOpt === "weekday")
                            label = isSr
                              ...
                              : isZh
                                ...
                                : "Weekday";
                          if (dayOpt === "weekend")
                            label = isSr ? "Vikend" : isZh ? "周末" : "Weekend";
                          return (
                            <button
                              key={dayOpt}
                              onClick={() => {
                    ...
                    <div className="space-y-2 bg-white/60 rounded-2xl border border-[#2D3025]/5 p-4">
                      ...
                      <div className="grid grid-cols-4 gap-1.5">
                        {["any", "morning", "afternoon", "night"].map(
                          (tOpt) => {
                            const active = timeOfDay === tOpt;
                            let label = tOpt;
                            if (tOpt === "any")
                              label = isSr ? "Sve" : isZh ? "不限" : "Any";
                            if (tOpt === "morning")
                              label = isSr ? "Jutro" : isZh ? "早晨" : "AM";
                            if (tOpt === "afternoon")
                              label = isSr ? "Podne" : isZh ? "下午" : "PM";
                            if (tOpt === "night")
                              label = isSr ? "Noć" : isZh ? "夜间" : "Night";
                            return (
                              <button
                                key={tOpt}
                                onClick={() => {
                          ...
                          {linkCopied
                            ? isSr
                              ...
                              : isZh
                                ...
                                : "LINK COPIED!"
                            : isSr
                              ? "KOPIRAJ PUTNU PROPUSNICU"
                              : isZh
                                ? "一键生成旅行通行证"
                                : "COPY TRAVEL PASS LINK"}
                        </span>
                      </button>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="pt-1">
                        ...
                        >
                          {isSr
                            ? "Resetuj uvodni vodič"
                            : isZh
                              ? "重置新手引导流程"
                              : "Reset Introduction Guide"}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 pt-4">
                      ...
                      <div className="border-t border-[#2D3025]/5 pt-3 space-y-2">
                        <span className="text-[8.5px] uppercase tracking-wider text-accent-red font-black block leading-none">
                          {isSr
                            ? "CRVENA ZONA: BRISANJE PODATAKA"
                            : isZh
                              ? "危机控制：永久抹除数据"
                              : "DANGER ZONE: DELETE ALL LOCAL MEMORIES"}
                        </span>
                    ...
                    <div className="space-y-2 pt-4">
                      <h3 className="text-[9px] uppercase tracking-[0.2em] font-black text-brand-charcoal">
                        {isSr
                          ? "3. PRAVNE INFORMACIJE I VERZIJA"
                          : isZh
                            ? "3. 法律声明"
                            : "3. Legal Disclaimer"}
                      </h3>
        ...
        {/* Footer Tagline */}
        <footer
          className="pt-2 opacity-30 text-center space-y-0.5"
          id="profile-view-footer"
        >
          <p className="text-[7px] uppercase tracking-[0.3em] font-black text-brand-charcoal">
            {t.footer_tagline}
          </p>
          <p className="text-[6px] uppercase tracking-[0.2em] font-bold text-brand-charcoal">
            v1.2.0
          </p>
        </footer>
                    ...
                    {isCultural && !isSr && !isZh ? (
                      <>
                        <p>
                          ...
                          personalization begins. By adjusting it, you tell
                          IDEMO how you feel today.
                        </p>
                        ...
                        <p className="border-t border-[#2D3025]/10 pt-3 text-xs font-bold text-accent-teal">
                          <strong>Want to change your Vibe?</strong> Visit the
                          category selectors on the <strong>Explore</strong>{" "}
                          page to adjust your active interests and fine-tune
                      ...
                      <>
                        <p>
                          {isSr ? (
                            ...
                            <>
                              ...
                              your overarching travel signature (derived from
                              your Mood Orbit), while your{" "}
                        ...
                        <p className="border-t border-[#2D3025]/10 pt-3">
                          {isSr ? (
                            <>
                              <strong>Uticaj na ponudu:</strong> Persona
                              postavlja osnovni stil kustosiranja (npr.
                              naglašavajući skrivena mesta naspram elitnog
                              nasleđa), dok aktivni Vajb direktno utiče na
                              težinu i redosled preporuka u katalogu,
                              izdvajajući na vrh beogradska mesta koja se
                              najviše poklapaju sa Vašim trenutnim
                              raspoloženjem.
                            </>
                          ) : isZh ? (
                            <>
                            ...
                            <>
                              <strong>Consequence on offer selection:</strong>{" "}
                              The Persona sets the qualitative curation
                              threshold (e.g., prioritizing hidden gems versus
                              historic heritage), while the active Vibe directly
                              weights and prioritizes the list of Belgrade
                              spots—bringing locations that maximize both your
                              active mindset and your selected interests
                              straight to the top of your catalog.
                            </>
                          )}
                        </p>
                      </>
