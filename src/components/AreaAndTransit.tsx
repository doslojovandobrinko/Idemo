...
export const BASE_HUBS: Hub[] = [
  {
    ...
    name: {
      ...
      sr: "Trg Republike (Centar grada)",
      es: "Plaza de la República (Centro)",
      de: "Republiksplatz (Stadtzentrum)",
  ...
  },
  {
    id: "expo_hub",
    name: {
      en: "EXPO 2027 Belgrade Site",
      sr: "Kompleks EXPO 2027 Beograd",
      es: "Recinto de la EXPO 2027",
      de: "EXPO 2027 Belgrad Gelände",
      ru: "Площадка ЭКСПО 2027",
      zh: "2027年贝尔格莱德世博会场",
    },
  ...
  },
  {
    ...
    name: {
      ...
      es: "Barrio Antiguo de Zemun",
      de: "Zemun Altstadt",
      ru: "Старый квартал Земун",
      zh: "泽蒙老城区",
    },
...
export const REGIONS = [
  {
    id: "belgrade_historic",
    name: {
      en: "Historic Center",
      sr: "Istorijsko jezgro",
      es: "Centro Histórico",
  ...
  },
  {
    ...
    name: {
      en: "Sava Waterfront",
      sr: "Savski kej i pristan",
      es: "Ribeira del Sava",
      de: "Sava-Uferpromenade",
      ru: "Набережная Савы",
      zh: "萨瓦河滨",
    },
    icon: "🌊",
    locations: [
      "Belgrade Waterfront Clubbing",
      "Sava Confluence",
      "Beton Hala",
      "Savamala",
    ],
  },
  {
    id: "zemun_novi",
    name: {
      en: "Zemun & Novi Beograd",
      sr: "Zemun i Novi Beograd",
      es: "Zemun y Nuevo Belgrado",
      de: "Zemun & Neu-Belgrad",
      ru: "Земун и Нови Белград",
      zh: "泽蒙与新贝尔格莱德",
    },
    icon: "🧱",
    locations: ["Zemun Quay", "Novi Beograd", "Zemun"],
  },
  {
    id: "fruska_gora",
    name: {
      en: "Vojvodina & North",
      sr: "Vojvodina i Fruška gora",
      es: "Vojvodina y el Norte",
      de: "Vojvodina & Norden",
      ru: "Воеводина и север",
      zh: "伏伊伏丁那与北部",
    },
    icon: "🍷",
    locations: ["Sremski Karlovci", "Fruška Gora", "Novi Sad"],
  },
  {
    id: "wilderness",
    name: {
      en: "Serbian Wilderness",
      sr: "Divljina i planine Srbije",
      es: "Naturaleza Serbia",
      de: "Serbische Wildnis",
      ru: "Заповедная Сербия",
      zh: "塞尔维亚荒野",
    },
    icon: "🦅",
    locations: ["Sjenica", "Despotovac", "Western Serbia", "Tara", "Uvac"],
  },
];
...
export function getTaxiEstimation(distanceKm: number): {
  rsd: string;
  eur: string;
} {
...
export function LocalTransitCard({
  ...
  const activeHubCoords = isLiveActive
    ? deviceCoords!
    : (BASE_HUBS.find((h) => h.id === selectedHubId) || BASE_HUBS[0])
        .coordinates;
  ...
  const labels: Record<string, any> = {
    en: {
      ...
      warning_scam:
        "Insist on the taximeter being active. Official city starts at 270 RSD. Avoid airport solicitors.",
    ...
    sr: {
      ...
      warning_scam:
        "Uvek insistirajte na uključenom taksimetru. Početna cena je 270 RSD. Izbegavajte neovlašćene taksiste na aerodromu.",
    ...
    es: {
      ...
      warning_scam:
        "Exija siempre el taxímetro encendido. Tarifa inicial oficial 270 RSD. Evite captadores en el aeropuerto.",
    ...
    de: {
      ...
      warning_scam:
        "Bestehen Sie immer auf ein eingeschaltetes Taxameter. Grundgebühr 270 RSD. Keine Flughafen-Werber annehmen.",
    ...
    ru: {
      ...
      warning_scam:
        "Всегда требуйте включения таксометра. Официальная посадка — 270 RSD. Избегайте частников в аэропорту.",
    ...
    zh: {
      ...
      gps_copied: "坐标已复制！",
      warning_scam:
        "坚持要求开启计价器。官方起步价 270 RSD。避免接受机场黑车司机的兜售。",
      tip_label: "专属管家提示",
      coord_system: "安全坐标位置",
      walk_score: "步行指数评价",
  ...
  return (
    <div className="bg-[#FAF9F5] border border-[#E7E4DB] rounded-[32px] p-6 space-y-6 shadow-sm">
      ...
      <div className="space-y-3">
        <label className="text-[13px] uppercase tracking-[0.15em] text-[#5C5A4D] font-extrabold block">
          {l.transit_from}
        </label>
        <div className="grid grid-cols-4 gap-1.5">
          {BASE_HUBS.map((hub) => (
            ...
            >
              {hub.id === "republic_square"
                ? "📍"
                : hub.id === "expo_hub"
                  ? "✦"
                  : "🧱"}{" "}
              {hub.name[language as keyof typeof hub.name]?.split(" ")[0] ||
                hub.name.en.split(" ")[0]}
            </button>
          ))}
          <button
      ...
      <div
        className={`p-4 rounded-2xl border flex items-center gap-3.5 select-none transition-all duration-300 ${walkability.color}`}
      >
        <div
          className={`w-2.5 h-2.5 rounded-full shrink-0 ${walkability.badge}`}
        />
        <div className="min-w-0 flex-1">
          <p className="text-[12px] uppercase tracking-wider font-extrabold opacity-80 leading-none mb-1">
            {l.walk_score}
          </p>
          <p className="text-[14px] font-black uppercase tracking-wider truncate leading-tight">
            {walkability.text}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3.5">
        <div className="bg-white border border-[#E7E4DB] p-4 rounded-2xl flex flex-col justify-center shadow-xs">
          <p className="text-[13px] uppercase tracking-wider text-[#5C5A4D] font-extrabold">
            {l.estimated_dist}
          </p>
          <p className="text-2xl font-serif font-black text-brand-charcoal mt-1">
            {distance} km
          </p>
        </div>
        <div className="bg-white border border-[#E7E4DB] p-4 rounded-2xl flex flex-col justify-center shadow-xs">
      ...
      <div className="bg-white border border-[#E7E4DB] p-4 rounded-2xl space-y-2.5 shadow-xs relative overflow-hidden">
        <p className="text-[13px] uppercase tracking-wider text-[#155e5b] font-black">
          {l.scam_free_taxi}
        </p>
        <div className="flex items-baseline justify-between select-none">
          ...
          <span className="text-sm font-extrabold text-[#5C5A4D]">
            ({taxiEst.eur})
          </span>
        </div>
      ...
      <div className="bg-white border border-[#D5D3C8] p-5 rounded-2xl space-y-3.5 shadow-sm">
        ...
        <div className="bg-brand-pearl p-4 rounded-xl border border-[#FAF9F5] select-all">
          <p className="text-[12px] font-mono font-bold text-[#5C5A4D]/70 uppercase mb-1">
            Serbian Prompt:
          </p>
      ...
      <div className="pt-1 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-[13px] uppercase tracking-widest text-[#5C5A4D] font-extrabold">
              {l.coord_system}
            </span>
