...
export function StudioPartners({ targetPartnerId }: StudioPartnersProps) {
  ...
  const filteredPartners = useMemo(() => {
    return partnerList.filter((p) => {
      ...
      if (selectedStageFilter === "ACTIVE")
        return (
          p.conciergeRoutingEligible === "Yes" &&
          !vStatus.includes("suspended") &&
          !vStatus.includes("archived")
        );
      if (selectedStageFilter === "CANDIDATE")
        return (
          vStatus.includes("candidate") ||
          p.conciergeRoutingEligible === "Pending qualification"
        );
      if (selectedStageFilter === "VERIFICATION")
        return vStatus.includes("unverified") || vStatus.includes("review");
      if (selectedStageFilter === "APPROVED")
        return (
          vStatus.includes("verified") && p.conciergeRoutingEligible !== "Yes"
        );
      if (selectedStageFilter === "SUSPENDED")
        return vStatus.includes("suspended");
      if (selectedStageFilter === "ARCHIVED")
        return vStatus.includes("archived");
  ...
  const activePartner = useMemo(() => {
    return (
      partnerList.find((p) => p.id === selectedPartnerId) ||
      partnerList[0] ||
      INITIAL_PARTNERS[0]
    );
  ...
  const getPartnerStageLabel = (
    ...
    if (p.conciergeRoutingEligible === "Yes")
      return {
    ...
    if (vStatus.includes("unverified") || vStatus.includes("review"))
      return {
        label: "Verification",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#8C8A7D] text-xs sm:text-sm font-sans mt-0.5">
            Operational desk for experience provider onboarding, recommendation
            mapping, and inquiry routing.
          </p>
        </div>
      ...
      <div className="bg-white border border-[#E5E3DB] rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-80">
          <Search
            size={15}
            className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8C8A7D]"
          />
          <input
            type="text"
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span
                        className={`text-[8.5px] font-mono px-1.5 py-0.5 rounded-md font-bold border ${
                      ...
                      <span
                        className={`text-[9px] font-mono px-2 py-0.5 rounded-full uppercase font-bold tracking-wider border ${stage.color}`}
                      >
                        {stage.label}
                      </span>
                    </div>
        ...
        {activePartner && (
          <div className="lg:col-span-2 bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-[#E5E3DB] pb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[9.5px] font-mono px-2 py-0.5 rounded-full font-bold border ${getPartnerStageLabel(activePartner).color}`}
                  >
                ...
                <div className="p-4 bg-[#FAF9F5] border border-[#E5E3DB] rounded-2xl space-y-3 font-mono">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 text-[11px]">
                    <div className="flex items-center gap-1.5 text-[#2E7D32] font-bold">
                      ...
                      <span>
                        {readiness.completedItems.length} Satisfied Criteria
                      </span>
                    </div>
                    ...
                    {readiness.missingItems.length > 0 && (
                      <div className="flex items-center gap-1.5 text-[#8A1F1F] font-bold">
                        ...
                        <span>
                          {readiness.missingItems.length} Missing Items (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs uppercase font-bold text-[#1E2E20]">
                    Mapped Recommendations (
                    {activePartner.linkedRecommendations?.length || 0})
                  </span>
                </div>
              </div>

              {activePartner.linkedRecommendations &&
              activePartner.linkedRecommendations.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {activePartner.linkedRecommendations.map((recTitle, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-white border border-[#E5E3DB] rounded-xl text-xs font-sans text-[#1E2E20] font-medium"
                    >
                      {recTitle}
                    </span>
                  ))}
                ...
                <p className="text-xs text-[#8C8A7D] italic">
                  No recommendations mapped to this partner yet.
                </p>
              )}
            </div>
            ...
            <div className="p-5 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() =>
                    alert(
                      `Copied QR Attribution link for ${activePartner.nameEn}`,
                    )
                  }
            ...
            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-2 font-mono text-xs">
              <div className="flex items-center justify-between">
                <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D]">
                  Verification Record & Audit Notes
                </span>
                <span className="text-[10px] text-[#8C8A7D]">
                  Last Verified: {activePartner.lastVerified || "2026-08-01"}
                </span>
              </div>
              <p className="text-xs text-[#1E2E20] font-sans">
                {activePartner.verificationDetails ||
                  "Verified in official directory registry."}
              </p>
            </div>
          </div>
        )}
