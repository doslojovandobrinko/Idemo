...
export function StudioDestinations() {
  const [destinations, setDestinations] =
    useState<DestinationItem[]>(INITIAL_DESTINATIONS);
  const [selectedDestId, setSelectedDestId] =
    useState<string>("serbia-canonical");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#8C8A7D] text-xs sm:text-sm font-sans mt-0.5">
            Manage active travel destinations, package release statuses, and
            spatial scope.
          </p>
        </div>
        ...
        <button
          onClick={() =>
            alert("Destination creation workflow is open in Studio Foundation.")
          }
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <MapPin
                      size={16}
                      className={
                        isSelected ? "text-[#8A1F1F]" : "text-[#8C8A7D]"
                      }
                    />
        ...
        <div className="lg:col-span-2 bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-6">
          <div className="flex items-center justify-between border-b border-[#E5E3DB] pb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono font-bold text-[#1E2E20]">
                {activeDest.curator}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              ...
              <span className="text-[10.5px] text-[#2E7D32] font-mono">
                SHA-256 Verified
              </span>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              ...
              <span className="text-sm font-mono font-bold text-[#1E2E20] block mt-2 truncate">
                {activeDest.lastPublishedAt !== "Pending"
                  ? new Date(activeDest.lastPublishedAt).toLocaleDateString()
                  : "Pending Release"}
              </span>
              <span className="text-[10.5px] text-[#8C8A7D]">
          ...
          <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-2">
            ...
            <p className="text-xs text-[#8C8A7D] leading-relaxed">
              Every recommendation in this destination adheres strictly to the
              IDEMO Editorial Luxury Design Language and Privacy-First Local
              State Engine. Releases are published as immutable bundles signed
              by SHA-256 hashes.
            </p>
          </div>
        </div>
