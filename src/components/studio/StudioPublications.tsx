...
interface StudioPublicationsProps {
  onNavigateTab?: (tab: 'dashboard' | 'recommendations' | 'collections' | 'partners' | 'publications' | 'destinations' | 'operations' | 'settings', itemId?: string) => void;
}

export function StudioPublications({ onNavigateTab }: StudioPublicationsProps) {
  const [selectedDestinationId, setSelectedDestinationId] = useState<string>('serbia');
  const [activePkg, setActivePkg] = useState<DestinationPackage | null>(null);
  const [integrityStatus, setIntegrityStatus] = useState<string>('Validating...');
  const [history, setHistory] = useState<PackageReleaseRecord[]>([]);

  // Generation & Publishing State
  const [versionIncrement, setVersionIncrement] = useState<'patch' | 'minor' | 'major' | 'custom'>('minor');
  ...
  const [draftPackage, setDraftPackage] = useState<DestinationPackage | null>(null);
  ...
  const [validationFilter, setValidationFilter] = useState<'all' | 'errors' | 'warnings' | 'passed'>('all');

  // Rollback Modal State
  const [showRollbackModal, setShowRollbackModal] = useState<boolean>(false);
  const [targetRollbackRecord, setTargetRollbackRecord] = useState<PackageReleaseRecord | null>(null);

  // Drill-Down Modal State (WP-08 Requirement)
  const [isDrillDownModalOpen, setIsDrillDownModalOpen] = useState<boolean>(false);
  const [drillDownCategory, setDrillDownCategory] = useState<'all' | DrillDownCategory>('all');
  const [drillDownSearch, setDrillDownSearch] = useState<string>('');

  // Calculate overall release readiness summary
  const readinessSummary: OverallReleaseReadinessResult = calculateOverallReleaseReadiness(
  ...
  const loadPackageData = async () => {
    try {
      ...
      if (active) {
        ...
        setIntegrityStatus(valid ? 'SHA-256 Integrity Verified' : `Integrity Alert: ${reason}`);
      } else {
    ...
    const initialHistory: PackageReleaseRecord[] = [
      {
        ...
        releaseNotes: 'Canonical Serbia Baseline package bundle with 113 curated items and 5 QR-verified partners.',
        status: 'ACTIVE',
      ...
      },
      {
        ...
        releaseNotes: 'Early Curation Beta release with initial Belgrade & Vojvodina experience maps.',
        status: 'SUPERSEDED',
  ...
  const getTargetVersionString = () => {
    ...
    if (versionIncrement === 'patch') return `${parts[0]}.${parts[1]}.${parts[2] + 1}`;
  ...
  const handlePreparePackageGeneration = async () => {
    ...
    try {
      ...
      const computedHash = await calculatePackageHash({ recommendations, collections, partners });
      const rawSize = JSON.stringify({ recommendations, collections, partners }).length;

      const newManifest = {
        destinationId: selectedDestinationId,
        destinationName: DESTINATIONS.find(d => d.id === selectedDestinationId)?.name || 'Serbia Canonical Baseline',
    ...
    try {
      ...
      if (activated) {
        ...
        const updatedHistory = [
          newRecord,
          ...history.map(h => ({ ...h, status: (h.status === 'ACTIVE' ? 'SUPERSEDED' : h.status) as PackageReleaseRecord['status'] }))
        ];

        safeStorage.setItem(HISTORY_STORAGE_KEY, JSON.stringify(updatedHistory));
        ...
        alert(`Destination Package v${draftPackage.manifest.packageVersion} published & activated atomically!`);
      } else {
  ...
  const handleExecuteRollback = async () => {
    try {
      ...
      if (success) {
        ...
        alert(`Successfully rolled back active destination package to version v${refreshed.manifest.packageVersion}!`);
      } else {
        alert("Rollback failed: No valid previous backup package found in SafeStorage.");
        setShowRollbackModal(false);
      }
    } catch (e) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#8C8A7D] text-xs sm:text-sm font-sans mt-1">
            Operational release control desk. Authoritative workflow for destination package validation, SHA-256 integrity verification, atomic activation, and rollback governance.
          </p>
        </div>
        ...
        <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="font-serif text-xl sm:text-2xl font-bold text-[#1E2E20] mt-0.5">
                  {selectedDestination.name} (v{activePkg.manifest?.packageVersion || '1.2.0'})
                </h2>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs font-mono">
            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D] block">Semantic Version</span>
              <span className="text-xl font-serif font-bold text-[#1E2E20] mt-1 block">v{activePkg.manifest?.packageVersion || '1.2.0'}</span>
              <span className="text-[10px] text-[#8C8A7D] mt-0.5 block">Published: {new Date(activePkg.manifest?.publishedAt || Date.now()).toLocaleDateString()}</span>
            </div>

            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D] block">Package Content Items</span>
              ...
              <span className="text-[10px] text-[#8C8A7D] mt-0.5 block">
                {activePkg.editorialCollections?.length || 3} Collections • {activePkg.partners?.length || 5} Partners
              </span>
            </div>

            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D] block">Payload Memory Size</span>
              <span className="text-xl font-serif font-bold text-[#1E2E20] mt-1 block">
                {Math.round((activePkg.manifest?.packageSizeBytes || 184520) / 1024)} KB
              </span>
              <span className="text-[10px] text-[#2E7D32] mt-0.5 block">Optimized for offline cache</span>
            </div>

            <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB]">
              <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D] block">SHA-256 Checksum Signature</span>
              <span className="text-[10.5px] font-bold text-[#1E2E20] mt-1.5 block truncate font-mono" title={activePkg.manifest?.sha256}>
                {activePkg.manifest?.sha256 || '6867830f04884464af58d98946dd340c'}
              </span>
              <span className="text-[10px] text-[#8C8A7D] mt-0.5 block">Cryptographic integrity lock</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E5E3DB] pb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`px-3 py-1 rounded-full font-mono text-[10px] font-bold uppercase border ${
                  ...
                  {readinessSummary.isReleaseReady ? 'RELEASE READY' : 'NEEDS AUDIT REVIEW'}
                </span>
              </div>
            </div>
            ...
            <div className="p-5 bg-[#FAF9F5] border border-[#E5E3DB] rounded-2xl space-y-3 font-mono">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="text-[10px] uppercase font-bold text-[#8C8A7D] block">Destination</span>
                  <span className="font-bold text-xs text-[#1E2E20]">{selectedDestination.name}</span>
                </div>
              </div>
              ...
              <div className="w-full bg-[#E5E3DB] h-2.5 rounded-full overflow-hidden">
                <div 
                  className={`h-full transition-all duration-300 ${
                    readinessSummary.overallScorePercentage >= 85 ? 'bg-[#2E7D32]' :
                    readinessSummary.overallScorePercentage >= 70 ? 'bg-[#C5A059]' :
                    'bg-[#8A1F1F]'
                  }`}
                  style={{ width: `${readinessSummary.overallScorePercentage}%` }}
                />
              </div>
              ...
              <div className="pt-2 border-t border-[#E5E3DB]/60">
                <span className="text-[9px] uppercase font-bold text-[#8C8A7D] block mb-2">
                  RELEASE READINESS AUDIT DRILL-DOWN CATEGORIES (CLICK TO INSPECT)
                </span>
                <div className="flex flex-wrap gap-1.5 text-[10px]">
                  <button
                    onClick={() => { setDrillDownCategory('preventing_publication'); setIsDrillDownModalOpen(true); }}
                  ...
                  >
                    <AlertCircle size={12} />
                    <span>Preventing Publication: {readinessSummary.categoryCounts.preventing_publication}</span>
                  </button>

                  <button
                    onClick={() => { setDrillDownCategory('partner_not_ready'); setIsDrillDownModalOpen(true); }}
                  ...
                  >
                    <Users size={12} />
                    <span>Partners Not Ready: {readinessSummary.categoryCounts.partner_not_ready}</span>
                  </button>

                  <button
                    onClick={() => { setDrillDownCategory('incomplete_translations'); setIsDrillDownModalOpen(true); }}
                  ...
                  >
                    <Globe2 size={12} />
                    <span>Incomplete Translations: {readinessSummary.categoryCounts.incomplete_translations}</span>
                  </button>

                  <button
                    onClick={() => { setDrillDownCategory('blocking_validation'); setIsDrillDownModalOpen(true); }}
                    className="px-2.5 py-1 rounded-lg bg-white border border-[#E5E3DB] hover:border-[#8A1F1F] text-[#8A1F1F] font-bold flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <AlertTriangle size={12} />
                    <span>Blocking Errors: {readinessSummary.categoryCounts.blocking_validation}</span>
                  </button>

                  <button
                    onClick={() => { setDrillDownCategory('warnings'); setIsDrillDownModalOpen(true); }}
                  ...
                  >
                    <Info size={12} />
                    <span>Advisory Warnings: {readinessSummary.categoryCounts.warnings}</span>
                  </button>

                  <button
                    onClick={() => { setDrillDownCategory('image_or_metadata'); setIsDrillDownModalOpen(true); }}
                  ...
                  >
                    <Compass size={12} />
                    <span>Image & Metadata: {readinessSummary.categoryCounts.image_or_metadata}</span>
                  </button>
                </div>
              </div>
              ...
              <p className="text-[10px] text-[#8C8A7D] italic flex items-center gap-1.5 pt-1">
                ...
                <span>Note: This summary score is informational only and does not bypass validation checks.</span>
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 font-mono text-xs">
              ...
              <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[#1E2E20] flex items-center gap-1.5 uppercase text-[10px]">
                    <Compass size={14} className="text-[#C5A059]" /> Recommendation Completeness
                  </span>
                  <span className="font-bold text-[#2E7D32]">{readinessSummary.recommendationScorePercentage}%</span>
                </div>
                <div className="text-[11px] text-[#8C8A7D] space-y-1 pt-1 border-t border-[#E5E3DB]">
                  <div className="flex justify-between">
                    <span>Approved Recommendations:</span>
                    <span className="font-bold text-[#1E2E20]">{readinessSummary.approvedRecsCount} items</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Awaiting Review (Candidates):</span>
                    <span className="font-bold text-[#F57F17]">{readinessSummary.draftCandidatesCount} items</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[#1E2E20] flex items-center gap-1.5 uppercase text-[10px]">
                    <Users size={14} className="text-[#2E7D32]" /> Partner Readiness
                  </span>
                  <span className="font-bold text-[#2E7D32]">{readinessSummary.partnerScorePercentage}%</span>
                </div>
                <div className="text-[11px] text-[#8C8A7D] space-y-1 pt-1 border-t border-[#E5E3DB]">
                  <div className="flex justify-between">
                    <span>Active & Routable Partners:</span>
                    <span className="font-bold text-[#2E7D32]">{readinessSummary.activePartnersCount} partners</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Pending Verification:</span>
                    <span className="font-bold text-[#F57F17]">{readinessSummary.pendingPartnersCount} partner</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5E3DB] space-y-2 sm:col-span-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[#1E2E20] flex items-center gap-1.5 uppercase text-[10px]">
                    <Globe2 size={14} className="text-[#8A1F1F]" /> Multi-Language Translation Completeness
                  </span>
                  <span className="font-bold text-[#1E2E20]">{readinessSummary.translationScorePercentage}% Localized</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px] pt-1 border-t border-[#E5E3DB]">
                  {readinessSummary.translationCoverage.map((t, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white p-2 rounded-xl border border-[#E5E3DB]">
                      <span className="text-[#8C8A7D] truncate">{t.lang}:</span>
                      <span className="font-bold text-[#1E2E20]">{t.percentage}%</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-4">
              ...
              <div className="flex items-center gap-1 bg-[#FAF9F5] p-1 border border-[#E5E3DB] rounded-xl font-mono text-[10px]">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                    validationFilter === 'all' ? 'bg-[#23251E] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  All Checks
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                    validationFilter === 'errors' ? 'bg-[#8A1F1F] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  Blocking Errors ({readinessSummary.validationErrors.filter(e => e.severity === 'error').length})
                </button>
                <button
                  onClick={() => setValidationFilter('warnings')}
                  className={`px-2.5 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                    validationFilter === 'warnings' ? 'bg-[#F57F17] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  Warnings ({readinessSummary.validationErrors.filter(e => e.severity === 'warning').length})
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2.5 font-mono text-xs">
              ...
              {/* Passed Core Checks */}
              {(validationFilter === 'all' || validationFilter === 'passed') && (
                <>
                  <div className="p-3 bg-[#E8F5E9]/50 border border-[#C8E6C9] rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <span className="font-bold text-[#1E2E20]">Schema & Minimum Curations Criteria</span>
                        <p className="text-[10.5px] text-[#2E7D32] mt-0.5">Approved items ({readinessSummary.approvedRecsCount}) satisfy minimum density threshold for {selectedDestination.name}.</p>
                      </div>
                    </div>
                    <span className="text-[9.5px] font-bold text-[#2E7D32] bg-[#E8F5E9] px-2 py-0.5 rounded-md">PASSED</span>
                  </div>
                  ...
                  <div className="p-3 bg-[#E8F5E9]/50 border border-[#C8E6C9] rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <CheckCircle2 size={16} className="text-[#2E7D32]" />
                      <div>
                        <span className="font-bold text-[#1E2E20]">Mood Orbit 2D Vector Spatial Calibration</span>
                        <p className="text-[10.5px] text-[#2E7D32] mt-0.5">100% of approved recommendations have valid normalized X/Y spatial vector coordinates.</p>
                      </div>
                    </div>
                    <span className="text-[9.5px] font-bold text-[#2E7D32] bg-[#E8F5E9] px-2 py-0.5 rounded-md">PASSED</span>
                  </div>
              ...
              {readinessSummary.validationErrors
                .filter(e => validationFilter === 'all' || (validationFilter === 'errors' && e.severity === 'error') || (validationFilter === 'warnings' && e.severity === 'warning'))
                .map((err) => (
                  <div 
                    key={err.id}
                    className={`p-3.5 border rounded-xl flex items-center justify-between ${
                      err.severity === 'error' ? 'bg-[#FFEBEE] border-[#FFCDD2]' : 'bg-[#FFF8E1] border-[#FFE082]'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      {err.severity === 'error' ? (
                        ...
                        <AlertTriangle size={16} className="text-[#F57F17] shrink-0" />
                      )}
                      <div>
                        <span className={`font-bold ${err.severity === 'error' ? 'text-[#C62828]' : 'text-[#8C6D00]'}`}>
                          {err.title}
                        </span>
                        <p className={`text-[10.5px] mt-0.5 ${err.severity === 'error' ? 'text-[#B71C1C]' : 'text-[#6D5600]'}`}>
                          {err.detail}
                        </p>
                      </div>
                    </div>

                    <span className={`text-[9.5px] font-bold px-2 py-0.5 rounded-md uppercase shrink-0 ${
                      err.severity === 'error' ? 'bg-[#FFCDD2] text-[#C62828]' : 'bg-[#FFE082] text-[#8C6D00]'
                    }`}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-5">
            ...
            <div className="space-y-2 font-mono text-xs">
              ...
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-[#FAF9F5] border border-[#E5E3DB] rounded-xl text-[11px]">
                <button
                  ...
                  className={`py-1.5 rounded-lg font-bold cursor-pointer transition-all ${
                    versionIncrement === 'patch' ? 'bg-[#23251E] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  Patch
                </button>
                <button
                  ...
                  className={`py-1.5 rounded-lg font-bold cursor-pointer transition-all ${
                    versionIncrement === 'minor' ? 'bg-[#23251E] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  Minor
                </button>
                <button
                  ...
                  className={`py-1.5 rounded-lg font-bold cursor-pointer transition-all ${
                    versionIncrement === 'major' ? 'bg-[#23251E] text-white' : 'text-[#8C8A7D] hover:text-[#1E2E20]'
                  }`}
                >
                  Major
                </button>
              ...
              <div className="p-3 bg-[#FAF9F5] border border-[#E5E3DB] rounded-xl flex items-center justify-between text-xs">
                ...
                <span className="font-serif font-bold text-[#1E2E20] text-sm">v{getTargetVersionString()}</span>
              </div>
            </div>
            ...
            >
              <RefreshCw size={16} className={`text-[#C5A059] ${isBuilding ? 'animate-spin' : ''}`} />
              <span>{isBuilding ? 'Generating SHA-256 Checksum...' : 'Generate Package & Verify SHA-256'}</span>
            </button>

            <p className="text-[10px] text-[#8C8A7D] font-mono italic text-center">
              * Note: Package creation is non-destructive until confirmed in the SHA-256 Verification step.
            </p>
          </div>
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-4">
            ...
            <p className="text-xs text-[#8C8A7D] leading-relaxed">
              If an active destination package exhibits operational flaws, perform instant atomic rollback to restore the previously valid backup package.
            </p>
            ...
            <button
              onClick={() => {
                ...
                else alert("No previous valid backup package found for rollback.");
              }}
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-[9.5px] font-bold uppercase border ${
                    rec.status === 'ACTIVE' ? 'bg-[#E8F5E9] text-[#2E7D32] border-[#C8E6C9]' :
                    rec.status === 'ROLLED_BACK' ? 'bg-[#FFEBEE] text-[#C62828] border-[#FFCDD2]' :
                ...
                <div className="flex items-center gap-2 text-[10.5px] text-[#8C8A7D]">
                  <span>Published: {new Date(rec.publishedAt).toLocaleString()}</span>
                  <span>•</span>
              ...
              <div className="mt-3 pt-2 border-t border-[#E5E3DB] flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[10px] text-[#8C8A7D]">
                <div className="flex items-center gap-4">
                  ...
                  <span>Size: {Math.round(rec.packageSizeBytes / 1024)} KB</span>
                </div>

                <div className="flex items-center gap-3 font-mono">
                  <span title={rec.sha256}>SHA-256: {rec.sha256.substring(0, 16)}...</span>
                  {rec.status !== 'ACTIVE' && (
                    <button
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div className="p-4 bg-[#E8F5E9] border border-[#C8E6C9] rounded-2xl space-y-3 font-mono text-xs">
              <div className="flex items-center justify-between text-[#2E7D32] font-bold">
                ...
                <span className="bg-[#2E7D32] text-white text-[9px] px-2 py-0.5 rounded-md uppercase">100% MATCH</span>
              </div>

              <div className="space-y-1 text-[11px] text-[#1E2E20] pt-1">
                <div className="flex justify-between">
                  <span className="text-[#8C8A7D]">Destination:</span>
                  <span className="font-bold">{draftPackage.manifest.destinationName} ({draftPackage.manifest.destinationId})</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#8C8A7D]">Release Version:</span>
                  <span className="font-serif font-bold text-sm">v{draftPackage.manifest.packageVersion}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#8C8A7D]">Payload Items:</span>
                  <span className="font-bold">{draftPackage.recommendations.length} Recs • {draftPackage.editorialCollections.length} Collections • {draftPackage.partners.length} Partners</span>
                </div>
              </div>

              <div className="p-2.5 bg-white rounded-xl border border-[#C8E6C9] space-y-1">
                <span className="text-[9.5px] uppercase font-bold text-[#8C8A7D] block">SHA-256 Digest Signature:</span>
                <span className="font-mono text-[10.5px] font-bold text-[#1E2E20] block break-all">{draftSha256}</span>
              </div>
            </div>
            ...
            <div className="p-4 bg-[#FAF9F5] border border-[#E5E3DB] rounded-2xl space-y-2">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setConfirmedCheckbox(e.target.value === 'true' || e.target.checked)}
                  className="mt-1 accent-[#23251E] cursor-pointer"
                />
                <span className="text-xs text-[#1E2E20] font-sans leading-relaxed">
                  I confirm that I have reviewed validation reports and authorize the <strong>atomic activation</strong> of Destination Package <strong>v{draftPackage.manifest.packageVersion}</strong>.
                </span>
              </label>
            </div>
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div className="p-4 bg-[#FFEBEE] border border-[#FFCDD2] rounded-2xl space-y-2 text-xs font-mono">
              <div className="flex items-center gap-2 text-[#C62828] font-bold uppercase">
                ...
                <span>Rollback Target: Version v{targetRollbackRecord.packageVersion}</span>
              </div>
              <p className="text-[#B71C1C] text-[11px] leading-relaxed font-sans">
                This operation will atomically replace the currently active package with <strong>v{targetRollbackRecord.packageVersion}</strong> ({targetRollbackRecord.destinationName}). Offline client caches will automatically synchronize with this restored baseline.
              </p>
            </div>
          ...
          <div className="bg-white border border-[#E5E3DB] rounded-3xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="p-5 sm:p-6 border-b border-[#E5E3DB] bg-[#FAF9F5] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
              <div>
                ...
                <p className="text-xs text-[#8C8A7D] mt-0.5">
                  Detailed inspection of blocking recommendations, unready partners, translation gaps, and validation issues preventing v1.3.0 publication.
                </p>
              </div>
            ...
            <div className="p-4 border-b border-[#E5E3DB] bg-white flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0 font-mono text-xs">
              ...
              <div className="flex items-center gap-1 overflow-x-auto pb-1 md:pb-0 scrollbar-none text-[10.5px]">
                ...
                >
                  Canonical Included ({readinessSummary.categoryCounts.canonical_included || 0})
                </button>
                ...
                >
                  Non-Canonical Excluded ({readinessSummary.categoryCounts.non_canonical_excluded || 0})
                </button>
                ...
                >
                  Preventing Publication ({readinessSummary.categoryCounts.preventing_publication})
                </button>
                ...
                >
                  Partners Not Ready ({readinessSummary.categoryCounts.partner_not_ready})
                </button>

                <button
                  onClick={() => setDrillDownCategory('incomplete_translations')}
                  className={`px-3 py-1.5 rounded-xl font-bold whitespace-nowrap cursor-pointer transition-colors ${
                ...
                >
                  Translations ({readinessSummary.categoryCounts.incomplete_translations})
                </button>
                ...
                >
                  Blocking Errors ({readinessSummary.categoryCounts.blocking_validation})
                </button>
                ...
                >
                  Image / Metadata ({readinessSummary.categoryCounts.image_or_metadata})
                </button>
              </div>

              {/* Search Box */}
              <div className="relative shrink-0 md:w-64">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8C8A7D]" />
                <input
                  type="text"
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-3 font-sans">
              {(() => {
                const filtered = readinessSummary.drillDown.filter(item => {
                  const catMatch = drillDownCategory === 'all' || item.category === drillDownCategory;
                  const searchMatch = !drillDownSearch || 
                    item.title.toLowerCase().includes(drillDownSearch.toLowerCase()) ||
                    item.recordId.toLowerCase().includes(drillDownSearch.toLowerCase()) ||
                    item.reason.toLowerCase().includes(drillDownSearch.toLowerCase());
                ...
                if (filtered.length === 0) {
                  return (
                    <div className="p-12 text-center bg-[#FAF9F5] border border-dashed border-[#E5E3DB] rounded-2xl">
                      <CheckCircle2 size={36} className="mx-auto text-[#2E7D32] mb-3" />
                      ...
                      <p className="text-xs text-[#8C8A7D] mt-1 font-mono">
                        All items meet the readiness standards for this audit filter.
                      </p>
                    </div>
                  );
                }
                ...
                return filtered.map(item => (
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 rounded-md font-mono text-[9px] font-bold uppercase ${
                          item.severity === 'error' ? 'bg-[#FFEBEE] text-[#8A1F1F] border border-[#FFCDD2]' :
                          item.severity === 'warning' ? 'bg-[#FFF8E1] text-[#F57F17] border border-[#FFE082]' :
                          'bg-[#E8F5E9] text-[#2E7D32] border border-[#C8E6C9]'
                        }`}>
                          {item.severity === 'error' ? 'BLOCKING ERROR' : item.severity === 'warning' ? 'WARNING' : 'AUDIT CHECK'}
                        </span>

                        {/* Score percentage if present */}
                        {item.scorePercentage !== undefined && (
                          <span className="font-mono text-[10px] font-bold text-[#8C8A7D]">
                            Completeness: <strong className="text-[#1E2E20]">{item.scorePercentage}%</strong>
                          </span>
                        )}
                      </div>
                      ...
                      {item.missingFields && item.missingFields.length > 0 && (
                        <div className="flex flex-wrap gap-1 pt-1">
                          {item.missingFields.slice(0, 4).map((mf, idx) => (
                            <span key={idx} className="text-[9.5px] font-mono bg-white px-2 py-0.5 rounded border border-[#E5E3DB] text-[#8A1F1F]">
                              • {mf}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="shrink-0">
                      <button
                        onClick={() => {
                          const targetTab = item.type === 'recommendation' ? 'recommendations' : 'partners';
                          if (onNavigateTab) {
                      ...
                      >
                        <span>Navigate to {item.type === 'recommendation' ? 'Recommendation' : 'Partner'}</span>
            ...
            <div className="p-4 border-t border-[#E5E3DB] bg-[#FAF9F5] flex items-center justify-between font-mono text-xs shrink-0">
              <span className="text-[#8C8A7D]">
                Showing {readinessSummary.drillDown.length} audit issues in Canonical Serbia Dataset.
              </span>
              <button
                onClick={() => setIsDrillDownModalOpen(false)}
