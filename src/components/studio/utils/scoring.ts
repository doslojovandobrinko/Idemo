...
export function calculateRecommendationCompleteness(
  ...
  const items: ScoreItem[] = [
    ...
    },
    {
      ...
      isComplete: Boolean(
        rec.shortDescription && rec.shortDescription.trim().length >= 15,
      ),
      weight: 10,
    ...
    },
    {
      ...
      isComplete: Boolean(
        rec.longDescription && rec.longDescription.trim().length >= 30,
      ),
      weight: 10,
    ...
    },
    {
      ...
      isComplete: Boolean(
        status && status !== "CANDIDATE" && status !== "RETIRED",
      ),
      weight: 10,
    ...
    },
    {
      ...
      isComplete: Boolean(
        rec.translations?.sr?.title && rec.translations?.sr?.shortDescription,
      ),
      weight: 10,
  ...
  const missingItems = items
    .filter((i) => !i.isComplete)
    .map((i) => i.missingNote || i.label);

  const criticalMissing = items.filter(
    (i) =>
      ["title", "shortDescription", "location", "image"].includes(i.key) &&
      !i.isComplete,
  );
  const isPublicationEligible =
    status === "APPROVED" &&
    scorePercentage >= 80 &&
...
export function calculatePartnerReadiness(
  ...
  const isEmailPresent = Boolean(
    ...
    !partner.email.includes("NOT VERIFIED"),
  );
  const isPhonePresent = Boolean(
    (partner.phone && !partner.phone.includes("NOT VERIFIED")) ||
    (partner.whatsApp && partner.whatsApp !== "N/A"),
  );
  ...
  const items: ScoreItem[] = [
    {
      ...
      isComplete: Boolean(
        partner.id && partner.nameEn && partner.nameEn.trim().length >= 2,
      ),
      weight: 10,
    ...
    },
    {
      ...
      isComplete: Boolean(
        ...
        partner.linkedRecommendations.length > 0,
      ),
      weight: 10,
    ...
    },
    {
      ...
      isComplete: Boolean(
        partner.directContactAvailable === "Yes" || isEmailPresent,
      ),
      weight: 10,
  ...
  const isActivationEligible =
    scorePercentage >= 80 && isRoutingReady && Boolean(partner.nameEn);
...
export function calculateReleaseReadinessDrillDown(
  ...
  recommendations.forEach((r) => {
    ...
    items.push({
      ...
      reason:
        "Approved Canonical Serbia Baseline v2 item included in release package payload",
      severity: "info",
    });
    ...
    const hasSrTitle = Boolean(
      r.translations?.sr?.title && r.translations.sr.title.trim().length > 0,
    );
    const hasSrDesc = Boolean(
      r.translations?.sr?.shortDescription &&
      r.translations.sr.shortDescription.trim().length > 0,
    );
    const isSrIdenticalToEn = Boolean(
      r.translations?.sr?.shortDescription &&
      r.translations.sr.shortDescription === r.shortDescription,
    );
    ...
    if (!hasSrTitle || !hasSrDesc || isSrIdenticalToEn) {
      items.push({
        ...
        reason: !hasSrTitle
          ? "Missing Serbian title translation"
          : !hasSrDesc
            ? "Missing Serbian short description"
            : "Serbian description is untranslated English text",
        severity: "warning",
      });
    }
    ...
    const hasValidImage = Boolean(
      ...
      (r.image.startsWith("http") ||
        ...
        r.image.startsWith("/src/assets")),
    );
    const isPlaceholderImage = Boolean(
      r.image &&
      (r.image.includes("placeholder") || r.image.includes("via.placeholder")),
    );
    const hasMoodOrbit =
    ...
    if (
      ...
      const issues: string[] = [];
      if (!hasValidImage || isPlaceholderImage)
        issues.push("Hero image missing or placeholder URL");
      if (!hasMoodOrbit)
        issues.push("Mood Orbit 2D spatial vector uncalibrated");
  ...
  partners.forEach((p) => {
    ...
    if (read.scorePercentage < 90 || p.conciergeRoutingEligible !== "Yes") {
      const isUnverified =
        (p.verificationStatus || "").toLowerCase().includes("unverified") ||
        (p.verificationStatus || "").toLowerCase().includes("candidate");
      items.push({
    ...
    if (
      p.conciergeRoutingEligible !== "Yes" &&
      p.linkedRecommendations &&
      p.linkedRecommendations.length > 0
    ) {
      items.push({
...
export function calculateOverallReleaseReadiness(
  ...
  const canonicalRecs = recommendations.filter(
    (r) =>
      r.publicationStatus === "CANONICAL" ||
      r.publicationStatus === "PUBLISHED" ||
      Number(r.id) <= 102,
  ...
  const recommendationScorePercentage =
    canonicalRecs.length > 0
  ...
  const partnerScorePercentage =
    partners.length > 0 ? Math.round(partnerScoreSum / partners.length) : 0;
  ...
  const enCount = canonicalRecs.filter((r) =>
    Boolean(r.title && r.shortDescription),
  ).length;
  const srCount = canonicalRecs.filter((r) =>
    Boolean(
  ...
  const deCount = canonicalRecs.filter((r) =>
    Boolean(
      r.translations?.de?.title &&
      r.translations?.de?.shortDescription &&
      r.translations.de.shortDescription !== r.shortDescription,
    ),
  ).length;
  const ruCount = canonicalRecs.filter((r) =>
    Boolean(
      r.translations?.ru?.title &&
      r.translations?.ru?.shortDescription &&
      r.translations.ru.shortDescription !== r.shortDescription,
    ),
  ).length;
  const esCount = canonicalRecs.filter((r) =>
    Boolean(
      r.translations?.es?.title &&
      r.translations?.es?.shortDescription &&
      r.translations.es.shortDescription !== r.shortDescription,
    ),
  ).length;
  const zhCount = canonicalRecs.filter((r) =>
    Boolean(
      r.translations?.zh?.title &&
      r.translations?.zh?.shortDescription &&
      r.translations.zh.shortDescription !== r.shortDescription,
    ),
  ).length;
  ...
  const translationScorePercentage = Math.round(
    (enPercentage +
      srPercentage +
      dePercentage +
      ruPercentage +
  ...
  const activePartnersCount = partners.filter(
    (p) => p.conciergeRoutingEligible === "Yes",
  ).length;
  const pendingPartnersCount = partners.filter(
    (p) => p.conciergeRoutingEligible !== "Yes",
  ).length;
  ...
  if (activePartnersCount < 1) {
    validationErrors.push({
      ...
      detail:
        "At least 1 active partner with Concierge Routing enabled is required for release.",
      severity: "error",
    });
  }
  ...
  const drillDown = calculateReleaseReadinessDrillDown(
    recommendations,
    partners,
  );

  const categoryCounts: Record<DrillDownCategory, number> = {
    canonical_included: drillDown.filter(
      (d) => d.category === "canonical_included",
    ).length,
    non_canonical_excluded: drillDown.filter(
      (d) => d.category === "non_canonical_excluded",
    ).length,
    preventing_publication: drillDown.filter(
      (d) => d.category === "preventing_publication",
    ).length,
    partner_not_ready: drillDown.filter(
      (d) => d.category === "partner_not_ready",
    ).length,
    incomplete_translations: drillDown.filter(
      (d) => d.category === "incomplete_translations",
    ).length,
    blocking_validation: drillDown.filter(
      (d) => d.category === "blocking_validation",
    ).length,
    warnings: drillDown.filter((d) => d.category === "warnings").length,
    image_or_metadata: drillDown.filter(
      (d) => d.category === "image_or_metadata",
    ).length,
  };
