...
export function StudioRecommendations({
  ...
  const handleStatusChange = (recId: string, newStatus: 'CANDIDATE' | 'NEEDS RESEARCH' | 'APPROVED' | 'MERGE CANDIDATE' | 'RETIRED') => {
  ...
  const handleSaveFromEditor = (savedRec: Recommendation, status: 'CANDIDATE' | 'NEEDS RESEARCH' | 'APPROVED' | 'RETIRED') => {
    ...
    if (onUpdateEditorialStatuses) {
      onUpdateEditorialStatuses({ ...editorialStatuses, [savedRec.id]: status });
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#8C8A7D] text-xs sm:text-sm font-sans mt-0.5">
            Manage, review, and audit 148 curated recommendations across Serbia destinations (135 Canonical Baseline v2).
          </p>
        </div>
                ...
                >
                  ...
                  <div className="shrink-0 flex items-center gap-2">
                    <span className={`text-[9px] font-mono px-2 py-0.5 rounded-md font-bold border ${
                      completeness.scorePercentage >= 90 ? 'bg-[#E8F5E9] text-[#2E7D32] border-[#C8E6C9]' :
                      completeness.scorePercentage >= 70 ? 'bg-[#FFF8E1] text-[#F57F17] border-[#FFE082]' :
                      'bg-[#FFEBEE] text-[#C62828] border-[#FFCDD2]'
                    }`}>
                      {completeness.scorePercentage}% Complete
                    </span>

                    <span className={`text-[9.5px] font-mono px-2.5 py-1 rounded-full uppercase font-bold border ${
        ...
        {activeRec && (() => {
          ...
          return (
            <div className="bg-white border border-[#E5E3DB] rounded-3xl p-6 shadow-xs space-y-5">
              ...
              <div className="p-4 bg-[#FAF9F5] border border-[#E5E3DB] rounded-2xl space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full font-mono text-[9.5px] font-bold border ${
                    completeness.scorePercentage >= 90 ? 'bg-[#E8F5E9] text-[#2E7D32] border-[#C8E6C9]' :
                    completeness.scorePercentage >= 70 ? 'bg-[#FFF8E1] text-[#F57F17] border-[#FFE082]' :
                    'bg-[#FFEBEE] text-[#C62828] border-[#FFCDD2]'
                  }`}>
                ...
                <div className="w-full bg-[#E5E3DB] h-2 rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-300 ${
                      completeness.scorePercentage >= 90 ? 'bg-[#2E7D32]' :
                      completeness.scorePercentage >= 70 ? 'bg-[#C5A059]' :
                      'bg-[#8A1F1F]'
                    }`}
                ...
                <div className="space-y-1.5 font-mono text-[11px] pt-1">
                  <div className="flex items-center justify-between text-[#2E7D32]">
                    ...
                    <span className="font-bold">{completeness.completedItems.length} / {completeness.items.length}</span>
                  </div>
                  ...
                  {completeness.missingItems.length > 0 && (
                    <div className="mt-2 text-[#8A1F1F] bg-[#FFEBEE]/60 p-2 rounded-xl border border-[#FFCDD2] space-y-1">
                      <span className="flex items-center gap-1 font-bold text-[10px] uppercase">
                        <AlertTriangle size={12} /> Missing ({completeness.missingItems.length}):
                      </span>
                      <ul className="list-disc list-inside text-[10.5px] space-y-0.5">
                        {completeness.missingItems.slice(0, 3).map((m, idx) => (
                  ...
                  <div className="pt-2 flex items-center justify-between text-[10px]">
                    <span className="text-[#8C8A7D]">Package Release Eligibility:</span>
                    <span className={`font-bold px-2 py-0.5 rounded-md ${
                      completeness.isPublicationEligible
                        ? 'bg-[#E8F5E9] text-[#2E7D32]'
                        : 'bg-[#FFF8E1] text-[#F57F17]'
                    }`}>
                      {completeness.isPublicationEligible ? 'RELEASE READY' : 'INELIGIBLE'}
                    </span>
                  </div>
                </div>
              </div>

              {/* Traveler Preview Action */}
              {onPreviewInTravelerApp && (
                <button
                  onClick={() => onPreviewInTravelerApp(activeRec.id)}
                  className="w-full py-2.5 px-4 bg-[#23251E] hover:bg-[#32352B] text-[#C5A059] border border-[#C5A059]/30 rounded-xl font-mono text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs active:scale-98"
                >
                  <Eye size={14} />
            ...
            <div className="pt-3 border-t border-[#E5E3DB] grid grid-cols-2 gap-2 text-[10.5px] font-mono text-[#8C8A7D]">
              <div>
                <span>Duration:</span> <strong className="text-[#1E2E20]">{activeRec.duration || '2-3 hours'}</strong>
              </div>
              <div>
                <span>Est. Cost:</span> <strong className="text-[#1E2E20]">{activeRec.estimatedCost || 'Free / Standard'}</strong>
              </div>
            </div>
          </div>
        );
