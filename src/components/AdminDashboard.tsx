...
import {
  ...
  checkAndSyncDestinationPackage,
...
const ADMIN_PIN = import.meta.env.DEV
  ? import.meta.env.VITE_DEV_ADMIN_PIN || ""
  : "";

export function generatePremiumCurationImage(
  title: string,
  category: string,
): string {
...
export function AdminDashboard({
  ...
  const [activeTab, setActiveTab] = useState<
    "curations" | "editorial" | "tech" | "analytics" | "packages"
  >("curations");
  const [selectedCountry, setSelectedCountry] = useState<
    "Serbia" | "Greece" | "Italy" | "Montenegro" | "Japan" | "All"
  >("Serbia");
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // DESTINATION PACKAGE MANAGER STATES
  const [activePackage, setActivePackage] = useState<DestinationPackage | null>(
    null,
  );
  const [syncStatus, setSyncStatus] = useState<SyncStatus | null>(null);
  const [isSyncing, setIsSyncing] = useState<boolean>(false);
  const [isGeneratingPackage, setIsGeneratingPackage] =
  ...
  const handleRunPackageSync = async () => {
    ...
    try {
      ...
      setToastMessage(
        `Sync Complete: Destination Package v${updatedPkg.manifest.packageVersion} active.`,
      );
    } catch (e: any) {
  ...
  const handleGenerateNewReleasePackage = async () => {
    ...
    try {
      ...
      const currentVerParts = basePkg.manifest.packageVersion
        .split(".")
        .map(Number);
      ...
      const newRecs =
        allRecommendations && allRecommendations.length > 0
          ? allRecommendations
          : basePkg.recommendations;
      const newHash = await calculatePackageHash({
        recommendations: newRecs,
        collections: basePkg.editorialCollections,
      ...
      if (activated) {
        ...
        setToastMessage(
          `Package Generated & Activated: Serbia v${newVersion} (SHA-256: ${newHash.substring(0, 12)}...)`,
        );
      }
    } catch (err: any) {
      setToastMessage(
        `Package Generation Failed: ${err?.message || String(err)}`,
      );
    } finally {
  ...
  const handleRollbackPackage = async () => {
    ...
    if (success) {
      ...
      setToastMessage(
        `Rollback Successful: Reverted to Serbia Destination Package v${restored.manifest.packageVersion}`,
      );
    } else {
      setToastMessage(
        "Rollback Failed: No previous valid package available in local storage.",
      );
    }
    setTimeout(() => setToastMessage(null), 4000);
  };
  ...
  const saveSubmissions = (newSubs: any[]) => {
    ...
    try {
      safeStorage.setItem(
        "idemo_editorial_observations_v1",
        JSON.stringify(newSubs),
      );
    } catch {}
  };

  const [editorialSubTab, setEditorialSubTab] = useState<
    "reports" | "candidates"
  >("candidates");
  ...
  const saveGovernanceLog = (
    ...
    try {
      safeStorage.setItem(
        "idemo_governance_audit_logs_v1",
  ...
  const [rationaleText, setRationaleText] = useState(
    "Verified operational details and compliance with IDEMO premium standard.",
  );

  const [candidateSearchQuery, setCandidateSearchQuery] = useState("");
  const [candidateStatusFilter, setCandidateStatusFilter] = useState<
    "ALL" | "CANDIDATE" | "NEEDS RESEARCH" | "APPROVED" | "RETIRED"
  >("ALL");

  // Redesigned Curation Review Desk states
  const [candidateReviewId, setCandidateReviewId] = useState("draft-1");
  const [activeReviewMode, setActiveReviewMode] = useState<
    "visitor" | "editorial"
  >("visitor");
  const [isDetailsExpanded, setIsDetailsExpanded] = useState(false);
  const [candidateCategoryFilter, setCandidateCategoryFilter] =
    useState<string>("ALL");
  const [candidateImageReadinessFilter, setCandidateImageReadinessFilter] =
    useState<string>("ALL");
  const [showFullScreenPreview, setShowFullScreenPreview] = useState(false);

  const [imageReadinessStatuses, setImageReadinessStatuses] = useState<
  ...
  const handleUpdateImageReadiness = (recId: string, status: string) => {
    ...
    const prov = Array.isArray(imageProvenance)
      ? imageProvenance.find((p: any) => p.draftId === recId)
      : null;
    if (prov) {
      const isRejectedOrMissing =
        prov.status.includes("REJECTED") || prov.status === "IMAGE MISSING";
      if (
        isRejectedOrMissing &&
        (status === "READY FOR CREATOR REVIEW" ||
          status === "VERIFIED PRODUCTION IMAGE")
      ) {
        showToast(
          "⚠️ Compliance: Rejected or missing images cannot be set to review/production state!",
        );
        return;
      }
    }
    ...
    try {
      safeStorage.setItem(
        "idemo_image_readiness_statuses_v1",
  ...
  const [curationAction, setCurationAction] = useState<
    "add" | "modify" | "delete"
  >("add");
  ...
  const [activeQualityFilter, setActiveQualityFilter] = useState<string | null>(
    null,
  );
  ...
  const [explorerSortOrder, setExplorerSortOrder] = useState<"asc" | "desc">(
    "asc",
  );
  ...
  const [datasets, setDatasets] = useState<
    Record<
      string,
      {
        ...
        releaseDate: string;
        status:
          "Draft" | "Review" | "Release Candidate" | "Production" | "Archived";
        lastModified: string;
        maintainer: string;
      }
    >
  ...
  const [recMetadata, setRecMetadata] = useState<any>(() => {
    ...
    allRecommendations.forEach((r) => {
      ...
      initial[r.id] = {
        ...
        qualityScore: score,
        reviewer:
          charSum % 2 === 0 ? "Lead Auditor Milan" : "Senior Reviewer Jelena",
  ...
  const [relationships, setRelationships] = useState<any>(() => {
    ...
    const initial: Record<string, any> = {};
    allRecommendations.forEach((r, idx) => {
      const sameCat = allRecommendations.filter(
        (o) => o.id !== r.id && o.category === r.category,
      );
      const diffCat = allRecommendations.filter(
        (o) => o.id !== r.id && o.category !== r.category,
      );
      ...
      initial[r.id] = {
        ...
        luxuryUpgrade:
          sameCat[0]?.badge && sameCat[0]?.badge !== "none"
            ? sameCat[0]?.id
            : "",
  ...
  const [simulatedReason, setSimulatedReason] = useState<string>(
    "Optimized location mapping density",
  );
  ...
  useEffect(() => {
    if (curationAction === "modify" && selectedCurationId) {
      const selectedRec = allRecommendations.find(
        (r) => r.id === selectedCurationId,
      );
      if (selectedRec) {
  ...
  useEffect(() => {
    if (candidateReviewId) {
      ...
      if (selectedRec) {
        ...
        setManTransport(selectedRec.preferredTransport || "Taxi");
        setManLat(
          selectedRec.latitude || (selectedRec.coordinates?.lat ?? 44.8125),
        );
        setManLng(
          selectedRec.longitude || (selectedRec.coordinates?.lng ?? 20.4612),
        );
        setManCoordX(selectedRec.coordinateX ?? 0);
        setManCoordY(selectedRec.coordinateY ?? 0);
        setManBadge(selectedRec.badge || "none");
  ...
  const diagnosticsList = useMemo(() => {
    ...
    if (curationAction === "add" || curationAction === "modify") {
      ...
      const isXValid =
        typeof manCoordX === "number" &&
        !isNaN(manCoordX) &&
        manCoordX >= -5 &&
        manCoordX <= 5;
      const isYValid =
        typeof manCoordY === "number" &&
        !isNaN(manCoordY) &&
        manCoordY >= -5 &&
        manCoordY <= 5;
      ...
      if (curationAction === "add") {
        const titleExists = allRecommendations.some(
          (r) => r.title.toLowerCase() === titleVal.toLowerCase(),
        );
        list.push({
          id: "uniqueness",
  ...
  const BUNDLED_IMAGES = [
    {
      value: "dynamic_generate",
      label: "✨ Auto-Generate Premium Photo (Real-time AI Selection)",
    },
    {
      value:
        "/src/assets/images/silosi_belgrade_industrial_night_1778842947193.png",
      label: "Silos Belgrade Industrial",
    },
    {
      value: "/src/assets/images/ada_ciganlija_lifestyle_1778848256252.png",
      label: "Ada Ciganlija Riverfront",
    },
    {
      value: "/src/assets/images/ambar_belgrade_riverfront_1778844046663.png",
      label: "Ambar Beton Hala",
    },
    {
      value: "/src/assets/images/saint_sava_temple_interior_1778845911761.png",
      label: "Sava Temple Mosaic",
    },
    {
      value: "/src/assets/images/avala_tower_panoramic_view_1778846516735.png",
      label: "Avala Panoramic Tower",
    },
    {
      value: "/src/assets/images/homa_restaurant_interior_1778843469455.png",
      label: "Homa Michelin Design",
    },
    {
      value: "/src/assets/images/comunale_beton_hala_1778845784310.png",
      label: "Comunale Beton Hala",
    },
    {
      value:
        "/src/assets/images/serbian_boutique_distillery_rakija_1778846500524.png",
      label: "Rakija Boutique Cellars",
    },
    {
      value: "/src/assets/images/uvac_meanders_1778841048759.png",
      label: "Uvac Emerald Meanders",
    },
    {
      value: "/src/assets/images/banjska_stena_outlook_1778841232535.png",
      label: "Banjska Stena Outlook",
    },
  ];
  ...
  const isCustomImage =
    manImage &&
    (manImage.startsWith("data:image/") ||
      (!BUNDLED_IMAGES.some((img) => img.value === manImage) &&
        manImage !== "dynamic_generate"));
  ...
  const handleSimulateScan = (partnerId: string) => {
    ...
    refreshStats();
    const partnerLabel =
      PARTNERS.find((p) => p.id === partnerId)?.label || partnerId;
  ...
  const handleRestoreBaseline = () => {
    if (
      confirm(
        "Would you like to restore the realistic EXPO 2027 campaign baseline statistics?",
      )
    ) {
      restoreBaselineMetrics();
      refreshStats();
  ...
  const handleParseAICuration = () => {
    ...
    if (!text) {
      setAiValidationFeedback({
        status: "err",
        msg: "Please paste some text first!",
      });
      return;
    }
    ...
    try {
      ...
      if (!jsonMatch) {
        setAiValidationFeedback({
          status: "err",
          msg: "Failed to extract a valid JSON structure from your pasted AI text. Ensure it is surrounded by { } brackets.",
        });
        return;
      }
      ...
      if (!parsed.title || typeof parsed.title !== "string") {
        throw new Error(
          "Curation 'title' is required and must be a valid text string.",
        );
      }
      if (!parsed.shortDescription || parsed.shortDescription.length < 10) {
        throw new Error(
          "A valid 'shortDescription' (~50 words) is required for explorer previews.",
        );
      }
      if (!parsed.longDescription || parsed.longDescription.length < 20) {
        throw new Error(
          "A detailed 'longDescription' (up to 200 words) is required for full visitor view sheets.",
        );
      }
      if (!parsed.category) {
        throw new Error(
          "'category' is required (e.g. Gastronomy, Nature, History, Wellbeing, Medical, Travel, Clubbing).",
        );
      }
      if (
        parsed.travelTimeMinutes === undefined ||
        typeof parsed.travelTimeMinutes !== "number"
      ) {
        throw new Error(
          "'travelTimeMinutes' is required as a numeric integer (e.g., 30) for route optimization.",
        );
      }
      if (!parsed.location) {
      ...
      if (
        ...
        parsed.image ===
          "/src/assets/images/silosi_belgrade_industrial_night_1778842947193.png" ||
        parsed.image === "dynamic_generate"
      ) {
        parsed.image = generatePremiumCurationImage(
          parsed.title,
          parsed.category,
        );
      }
      ...
      setAiValidationFeedback({
        status: "success",
        msg: "Curation layout compiled and fully validated for dynamic app injection!",
      });
    } catch (e: any) {
      setAiValidationFeedback({
        status: "err",
        msg: "Schema Integration Compile Error: " + e.message,
      });
    }
  };
  ...
  const handlePublishAICuration = () => {
    ...
    if (customRecommendations.length >= 50) {
      alert(
        "Maximum limit of 50 custom curations reached. Please delete standard trials before completing more.",
      );
      return;
    }
    ...
    showToast(
      `Successfully published "${aiParsedRec.title}" to dynamic Belgrade list!`,
    );
    setAiParsedRec(null);
    setAiText("");
    setAiValidationFeedback({ status: "idle", msg: "" });
  ...
  const handleUnifiedPublish = () => {
    ...
    if (curationAction === "add") {
      ...
      const newRec: any = {
        ...
        coordinates: {
          lat: Number(manLat) || 44.8125,
          lng: Number(manLng) || 20.4612,
        },
      ...
      const oldRec = allRecommendations.find(
        (r) => r.id === selectedCurationId,
      );
      ...
      const nextLogs = {
        ...changesSinceValidation,
        updated: Array.from(
          new Set([...changesSinceValidation.updated, selectedCurationId]),
        ),
  ...
  const handleDeleteCuration = (id: string) => {
    if (
      confirm(
        "Are you sure you want to remove this recommendation from active traveler dashboards? It can be retrieved at any time by resetting overrides.",
      )
    ) {
  ...
  const handleRestoreDefaults = () => {
    if (
      confirm(
        "Restore dynamic curations to fresh default status? This action will recover your deleted items and discard localized manual adjustments.",
      )
    ) {
...
Schema:
{
      ...
      <header className="p-5 border-b border-border-main flex items-center justify-between shrink-0 bg-white shadow-sm">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] uppercase tracking-[0.25em] font-black text-accent-teal">
              EXPO 2027 CAMPAIGN COMMAND
            </span>
          </div>
          <h2 className="text-lg font-serif tracking-tight font-black text-brand-sage">
            IDEMO Curations Control
          </h2>
        </div>
        <button
          onClick={onClose}
          ...
          >
            ...
            {activeTab === tab.id && (
              <motion.div
                ...
                className="absolute bottom-0 left-3 right-3 h-0.5 bg-accent-teal"
              />
            )}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar bg-brand-bg">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border-main">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-serif text-lg font-black text-brand-sage">
                      IDEMO Core Engine — Destination Package Publisher
                    </h3>
                  </div>
                  <p className="text-xs text-brand-charcoal/70 max-w-2xl leading-relaxed">
                    Operates under the Dynamic Content Platform Directive.
                    Content is versioned and signed as atomic packages, enabling
                    offline-first execution with zero App Store releases.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1.5 rounded-full text-[10px] uppercase tracking-wider font-extrabold flex items-center gap-1.5 ${
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full ${(syncStatus?.isOnline ?? true) ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`}
                    />
                    {(syncStatus?.isOnline ?? true)
                      ? "Network Online"
                      : "Offline Mode"}
              ...
              {activePackage ? (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
                  <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-border-main/60 space-y-1">
                    <span className="text-[10px] uppercase tracking-widest text-brand-charcoal/50 font-extrabold block">
                      Active Destination
                    </span>
                    <div className="font-serif text-base font-bold text-brand-charcoal flex items-center gap-2">
                      ...
                      {activePackage.manifest.destinationName} (
                      {activePackage.manifest.destinationId})
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-border-main/60 space-y-1">
                    ...
                    <div className="font-mono text-base font-bold text-brand-sage flex items-center gap-2">
                      <Layers size={16} className="text-accent-teal" />v
                      {activePackage.manifest.packageVersion}
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-border-main/60 space-y-1">
                    ...
                    <div
                      className="font-mono text-xs font-bold text-emerald-700 truncate flex items-center gap-1.5"
                      title={activePackage.manifest.sha256}
                    >
                      <ShieldCheck
                        size={14}
                  ...
                  <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-border-main/60 space-y-1">
                    ...
                    <div className="text-xs font-bold text-brand-charcoal space-x-2">
                      <span className="bg-white px-2 py-0.5 rounded border border-border-main">
                        {activePackage.manifest.itemCount.recommendations} Recs
                      </span>
                      <span className="bg-white px-2 py-0.5 rounded border border-border-main">
                        {activePackage.manifest.itemCount.collections}{" "}
                        Collections
                      </span>
                    </div>
                  </div>
                ...
                <div className="p-6 text-center text-xs text-brand-charcoal/60">
                  Loading active package manifest...
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm space-y-5">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-serif text-sm font-bold text-brand-sage uppercase tracking-wider">
                    Mobile Synchronisation Engine
                  </h4>
                </div>
                <p className="text-xs text-brand-charcoal/70 leading-relaxed">
                  Queries backend manifest table for newly published destination
                  releases. Performs SHA-256 integrity validation before atomic
                  hot-swap.
                </p>
                ...
                <div className="p-4 rounded-2xl bg-brand-pearl/50 border border-border-main space-y-3">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono font-bold uppercase text-accent-teal">
                      {syncStatus?.syncState || "IDLE / ACTIVE"}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-brand-charcoal/70">
              ...
              <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm space-y-5">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-serif text-sm font-bold text-brand-sage uppercase tracking-wider">
                    Editorial Publication Pipeline
                  </h4>
                </div>
                <p className="text-xs text-brand-charcoal/70 leading-relaxed">
                  Governance pipeline governing content promotion into the
                  canonical destination release payload.
                </p>
                ...
                <div className="space-y-2.5">
                  {[
                    {
                      stage: "Drafting",
                      count: `${allRecommendations.length} Items`,
                      desc: "Curators assemble text, GPS, & metadata.",
                      status: "complete",
                    },
                    {
                      stage: "Editorial Review",
                      count: "100% Passed",
                      desc: "Verified against Canonical Editorial Standard.",
                      status: "complete",
                    },
                    {
                      stage: "Engineering QA",
                      count: "Passed",
                      desc: "SHA-256 hash validation & offline test.",
                      status: "complete",
                    },
                    {
                      stage: "Approved & Active",
                      count: `v${activePackage?.manifest.packageVersion || "1.0.0"} Installed`,
                      desc: "Loaded into Core Engine local state.",
                      status: "active",
                    },
                  ].map((step, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 rounded-2xl bg-[#FAF9F5] border border-border-main/50 text-xs"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-brand-charcoal/60">
                            {step.desc}
                          </div>
                        </div>
                      </div>
                ...
                <div className="pt-2">
                  ...
                  >
                    <Code size={14} />
                    {showManifestJson
                      ? "Hide Atomic Package Manifest JSON"
                      : "Inspect Atomic Package Manifest JSON"}
                  </button>
                </div>
              </div>
            </div>
            ...
            {showManifestJson && activePackage && (
              <div className="bg-slate-900 text-slate-100 p-6 rounded-3xl border border-slate-800 space-y-3 font-mono text-xs overflow-x-auto">
                <div className="flex justify-between items-center text-slate-400 text-[10px] uppercase tracking-wider pb-2 border-b border-slate-800">
                  ...
                  <span>
                    Destination Package v{activePackage.manifest.packageVersion}
                  </span>
                </div>
          ...
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-brand-charcoal/60">
                Simulated campaign scale tracking total conversion rates from
                printed QR codes to phone layouts.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1 bg-white p-3 rounded-2xl border border-border-main shadow-sm">
                <div className="flex justify-between text-[11px] font-bold text-brand-charcoal">
                  <span className="flex items-center gap-1.5">
                    <QrCode size={12} className="text-accent-teal" /> Printed QR
                    Scans
                  </span>
                  <span className="font-mono text-brand-charcoal">
                    {metrics.totalQrScans.toLocaleString()} (100%)
                  </span>
                </div>
              ...
              <div className="space-y-1 bg-white p-3 rounded-2xl border border-border-main shadow-sm">
                <div className="flex justify-between text-[11px] font-bold text-brand-charcoal">
                  <span className="flex items-center gap-1.5">
                    🟢 Store Visits / Device Launches
                  </span>
                  <span className="font-mono text-brand-charcoal">
                    {metrics.storeClicks.toLocaleString()} (
                    {metrics.conversionRates.scanToStorePercent}%)
                  </span>
                </div>
                <div className="w-full h-3 bg-[#FAF9F5] border border-border-main/50 rounded-full overflow-hidden mt-1.5">
                  <div
                    className="h-full bg-brand-charcoal/30 rounded-full"
                    style={{
                      width: `${metrics.conversionRates.scanToStorePercent}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1 bg-white p-3 rounded-2xl border border-border-main shadow-sm">
                <div className="flex justify-between text-[11px] font-bold text-brand-charcoal">
                  ...
                  <span className="font-mono text-brand-charcoal">
                    {metrics.installs.toLocaleString()} (
                    {metrics.conversionRates.scanToInstallPercent}%)
                  </span>
                </div>
                <div className="w-full h-3 bg-[#FAF9F5] border border-border-main/50 rounded-full overflow-hidden mt-1.5">
                  <div
                    className="h-full bg-accent-teal rounded-full"
                    style={{
                      width: `${metrics.conversionRates.scanToInstallPercent}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm">
              <h4 className="text-[10px] uppercase font-mono tracking-widest text-brand-charcoal/50">
                Language Selections Distribution
              </h4>
              <div className="grid grid-cols-2 gap-4">
                {Object.entries(metrics.languages).map(([lang, val]) => {
                  const total = Object.values(metrics.languages).reduce(
                    (acc, curr) => (acc as number) + (curr as number),
                    0,
                  ) as number;
                  const p = Math.round(
                    ((val as number) / Math.max(1, total)) * 100,
                  );
                  return (
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm">
              ...
              <div className="grid gap-3">
                {["<1m", "1m-5m", "5m-15m", ">15m"].map((bucket) => {
                  ...
                  const total = Object.values(metrics.durations).reduce(
                    (acc, curr) => (acc as number) + (curr as number),
                    0,
                  ) as number;
                  const p = Math.round(
                  ...
                  return (
                    ...
                    >
                      <span className="w-14 text-brand-charcoal/70 font-bold">
                        {bucket}
                      </span>
                      <div className="flex-1 h-3 bg-[#FAF9F5] rounded-md overflow-hidden relative border border-border-main/50">
                        <div
                          className="h-full bg-brand-charcoal/10"
                          style={{ width: `${p}%` }}
                        />
                        <span className="absolute inset-y-0 right-2 flex items-center text-[8px] font-black text-brand-charcoal/70">
                          {p}%
                        </span>
                      </div>
                    </div>
        ...
        {activeTab === "partners" && (
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide">
                QR campaigns & Partners performance
              </h3>
              <p className="text-[10px] text-brand-charcoal/60">
                Tracking scans and conversion metrics attributed strictly to
                designated physical QR placements across Belgrade.
              </p>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-border-main overflow-hidden shadow-sm">
              <div className="p-4 bg-[#FAF9F5] border-b border-border-main flex justify-between items-center">
                <span className="text-[10px] uppercase font-mono tracking-widest text-brand-charcoal/60 font-black">
                  Designated Partners List
                </span>
                <span className="text-[8px] uppercase tracking-widest font-bold bg-accent-teal/10 text-accent-teal px-2 py-0.5 rounded-full">
                  REALTIME DELEGATES
                </span>
              </div>
              ...
              <div className="divide-y divide-border-main">
                {PARTNERS.map((partner) => {
                  ...
                  const conversion =
                    Math.round((scans / metrics.totalQrScans) * 100) || 0;

                  return (
                    <div
                      key={partner.id}
                      className="p-4 flex justify-between items-center gap-2 bg-white"
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-bold text-brand-charcoal truncate">
                          {partner.label}
                        </p>
                        <p className="text-[9px] text-brand-charcoal/50 uppercase tracking-wider font-mono mt-0.5">
                          {partner.type} • ID:{" "}
                          <span className="font-bold text-brand-charcoal/70 select-all font-sans">
                            ?src={partner.id}
                          </span>
                        </p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-xs font-mono font-bold text-accent-teal">
                          {scans.toLocaleString()}
                        </p>
                        <p className="text-[8.5px] font-mono text-brand-charcoal/50 mt-0.5">
                          {conversion}% of pool
                        </p>
                      </div>
                    </div>
            ...
            <div className="p-4 bg-white border border-border-main rounded-2xl space-y-2 text-[10px] text-brand-charcoal/60 leading-normal font-sans shadow-sm">
              ...
              <p>
                All partner codes are parsed directly on launch. This
                attribution occurs purely inside the client device, mapping
                actions to local performance benchmarks. No cross-site profiling
                trackers, Google identifiers, or telemetry payloads leave the
                secure client.
              </p>
            </div>
          </div>
        ...
        {activeTab === "retention" && (
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide">
                Cohort App Retention
              </h3>
              <p className="text-[10px] text-brand-charcoal/60">
                Tracking visitor loyalty cohorts over extended durations,
                critical for gauging the continuous utility of concierge assets
                across the EXPO timeframe.
              </p>
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-5 shadow-sm">
              ...
              <div className="grid grid-cols-4 gap-2.5 text-center font-mono select-none">
                <div className="bg-[#FAF9F5] p-3 rounded-2xl border border-border-main relative">
                  <div className="absolute top-1.5 left-1.5 text-[7px] text-brand-charcoal/40 font-sans font-bold">
                    D1
                  </div>
                  <p className="text-xl font-black text-accent-teal mt-1">
                    {metrics.retention.d1}%
                  </p>
                  <p className="text-[7.5px] text-brand-charcoal/50 font-sans mt-0.5">
                    Next Day Open
                  </p>
                </div>
                <div className="bg-[#FAF9F5] p-3 rounded-2xl border border-border-main relative">
                ...
                <div className="bg-[#FAF9F5] p-3 rounded-2xl border border-border-main relative">
                  ...
                  <p className="text-xl font-black text-brand-charcoal/60 mt-1">
                ...
                <div className="bg-[#FAF9F5] p-3 rounded-2xl border border-border-main relative">
                  ...
                  <p className="text-xl font-black text-brand-charcoal/40 mt-1">
                    {metrics.retention.d90}%
                  </p>
                  <p className="text-[7.5px] text-brand-charcoal/50 font-sans mt-0.5">
                    EXPO Duration
                  </p>
              ...
              <p className="text-[10px] text-brand-charcoal/60 leading-relaxed">
                * Based on the anonymous installation identifier `sb-uuid`
                logged initially with a localized timestamp. Every active day
                logged under `activeDates` counts towards D1, D7, or D30
                criteria.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-fade-in text-left">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-brand-charcoal/60 font-medium">
                Review visitor factual verification signals, correct
                inaccuracies, and update our master curations dataset. Remember:
                Visitor submissions NEVER modify recommendations automatically.
              </p>
            </div>
            ...
            <div className="flex border-b border-border-main/50 pb-px">
              ...
              >
                ⚖ Candidate Curation review desk (
                {
                  allRecommendations.filter((r) => r.id.startsWith("draft-"))
                    .length
                }
                )
              </button>
            ...
            {editorialSubTab === "reports" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-4 gap-3 bg-white p-4 rounded-2xl border border-border-main text-[10px] shadow-sm">
                  <div>
                    <span className="font-bold text-brand-charcoal/50 uppercase text-[8px] tracking-wider block">
                      Total Inbox Reports
                    </span>
                    <span className="font-mono text-brand-charcoal font-black text-sm">
                      {submissions.length}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono text-[#D84315] font-black text-sm">
                      {
                        submissions.filter((s: any) => s.status === "New")
                          .length
                      }
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono text-accent-teal font-black text-sm">
                      {
                        submissions.filter((s: any) => s.status === "Reviewing")
                          .length
                      }
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono text-[#2E7D32] font-black text-sm">
                      {
                        submissions.filter((s: any) => s.status === "Updated")
                          .length
                      }
                    </span>
                  </div>
                ...
                {editingRec && (
                  <div className="bg-white p-5 rounded-3xl border border-[#2E7D32] space-y-4 shadow-lg animate-fade-in">
                    <div className="flex items-center justify-between border-b border-border-main/50 pb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono font-black uppercase tracking-wider text-brand-charcoal">
                          Update Recommendation Dataset: {editingRec.title} (ID:{" "}
                          {editingRec.id})
                        </span>
                      </div>
                      <button
                    ...
                    <div className="grid grid-cols-2 gap-3.5 text-[11px]">
                      <div className="space-y-1">
                        <label className="font-extrabold text-brand-charcoal/70">
                          TITLE
                        </label>
                        <input
                          type="text"
                    ...
                    <div className="flex gap-2 justify-end pt-2 flex-wrap">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setEditingSubId(null);
                          setToastMessage(
                            `Draft edits for "${edTitle}" successfully saved. Draft is ready for review.`,
                          );
                        }}
                      ...
                      <button
                        onClick={() => {
                          ...
                          setEditingSubId(null);
                          setToastMessage(
                            `Dataset for "${edTitle}" successfully published. Submission marked as "Updated".`,
                          );
                        }}
                  ...
                  <div className="p-4 border-b border-border-main/50 bg-[#FAF9F5] flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        if (
                          window.confirm(
                            "Are you sure you want to clear all submissions in the local Editorial Inbox?",
                          )
                        ) {
                          saveSubmissions([]);
                  ...
                  {submissions.length === 0 ? (
                    <div className="p-8 text-center text-brand-charcoal/40 text-[11px] font-medium space-y-1">
                      ...
                      <p className="text-[9.5px]">
                        When users select "Confirm Accuracy" on the Profile
                        page, their factual verification reports will appear
                        here.
                      </p>
                    </div>
                  ) : (
                    <div className="divide-y divide-border-main/50 max-h-[500px] overflow-y-auto no-scrollbar">
                      {submissions.map((sub: any) => {
                        const rec = allRecommendations.find(
                          (r: any) => r.id === sub.recId,
                        );
                        const statusColor =
                          sub.status === "New"
                        ...
                        return (
                          <div
                            key={sub.id}
                            className="p-4 space-y-3 transition-colors hover:bg-[#FAF9F5]/40 text-left"
                          >
                            <div className="flex flex-wrap justify-between items-start gap-2">
                              <div className="space-y-0.5">
                                ...
                                <h4 className="text-[12.5px] font-serif font-black text-brand-charcoal">
                                  {sub.recTitle}{" "}
                                  <span className="text-[10px] font-mono text-brand-charcoal/40 font-normal">
                                    ({sub.recId})
                                  </span>
                                </h4>
                              </div>
                              <span
                                className={`px-2 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider border ${statusColor}`}
                              >
                                {sub.status || "New"}
                              </span>
                            </div>
                            ...
                            <div className="bg-[#FAF9F5] p-3 rounded-xl border border-border-main/40 text-[11px] space-y-2">
                              <div className="grid grid-cols-2 gap-2 text-[10px]">
                                <div>
                                  <span className="text-brand-charcoal/40 uppercase font-bold text-[8px] tracking-wide block">
                                    Feedback Category
                                  </span>
                                ...
                                <div>
                                  ...
                                  <span className="font-extrabold text-brand-charcoal">
                                    Language:{" "}
                                    {sub.language?.toUpperCase() || "EN"} • App
                                    v{sub.appVersion || "1.0.0"}
                                  </span>
                                </div>
                              </div>
                            ...
                            {sub.draft && (
                              <div className="bg-[#E3F2FD]/30 border border-[#0D47A1]/20 p-3 rounded-xl text-[10px] space-y-1.5 animate-fade-in">
                                <div className="flex items-center justify-between">
                                  <span className="font-extrabold text-[#0D47A1] uppercase text-[8.5px] tracking-wide block">
                                    ACTIVE DRAFT EDITS (UNPUBLISHED)
                                  </span>
                                  <span className="px-1.5 py-0.5 rounded bg-[#E3F2FD] text-[#0D47A1] text-[7.5px] font-extrabold uppercase">
                                    DRAFT
                                  </span>
                                </div>
                                <div className="grid grid-cols-2 gap-1.5 text-[9.5px] border-l-2 border-[#0D47A1]/40 pl-2">
                                  {sub.draft.title !== rec?.title && (
                                    <div>
                                      <span className="font-bold text-brand-charcoal/50">
                                        Title:
                                      </span>{" "}
                                ...
                                <div className="flex justify-end gap-2 pt-1.5 border-t border-dashed border-[#0D47A1]/10 mt-1">
                                  <button
                                    onClick={() => {
                                      ...
                                      setToastMessage(
                                        `Draft edits for Report #${sub.id} discarded.`,
                                      );
                                    }}
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      saveSubmissions(updated);
                                      setToastMessage(
                                        `Draft edits for "${sub.draft.title}" published successfully!`,
                                      );
                                    }}
                              ...
                              (() => {
                                ...
                                return (
                                  <div className="bg-[#FFF3E0] border border-[#FFE0B2] p-3 rounded-xl text-[10px] space-y-2 mt-2 animate-fade-in">
                                    <span className="font-extrabold text-[#E65100] uppercase text-[8px] tracking-wide block">
                                      SELECT AN ACTIVE OBSERVER REPORT TO MERGE
                                      THIS NOTE INTO:
                                    </span>
                                    {otherReportsOfSameRec.length === 0 ? (
                                      <p className="text-brand-charcoal/50 italic text-[9.5px]">
                                        No other active, unresolved reports
                                        exist for this location.
                                      </p>
                                    ) : (
                                      <div className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                                        {otherReportsOfSameRec.map(
                                          (targetSub: any) => (
                                            <button
                                              key={targetSub.id}
                                              onClick={() => {
                                                ...
                                                setToastMessage(
                                                  `Report #${sub.id} successfully merged into Report #${targetSub.id}.`,
                                                );
                                              }}
                                            ...
                                            >
                                              ...
                                              {targetSub.note ? (
                                                <span className="truncate text-brand-charcoal/60 italic font-medium">
                                                  "{targetSub.note}"
                                                </span>
                                              ) : (
                                                <span className="text-brand-charcoal/40 italic">
                                                  No notes (Confirmed OK)
                                                </span>
                                              )}
                                            </button>
                                          ),
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-dashed border-border-main/30">
                                ...
                                {sub.status === "New" && (
                                  <button
                                    onClick={() => {
                                      ...
                                      saveSubmissions(updated);
                                      setToastMessage(
                                        `Report #${sub.id} marked as "Reviewing".`,
                                      );
                                    }}
                                  ...
                                  sub.status !== "Merged" && (
                                    <button
                                      onClick={() => {
                                        ...
                                        saveSubmissions(updated);
                                        setToastMessage(
                                          `Report #${sub.id} marked as "Closed / Ignored".`,
                                        );
                                      }}
                              ...
                              {rec ? (
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      if (sub.draft) {
                                        ...
                                        setEdShortDesc(
                                          sub.draft.shortDescription || "",
                                        );
                                        setEdLongDesc(
                                          sub.draft.longDescription || "",
                                        );
                                ...
                                <span className="text-[9px] text-[#C62828] font-bold">
                                  Error: Parent Recommendation deleted or not
                                  found
                                </span>
                              )}
                            </div>
            ...
            {editorialSubTab === "candidates" &&
              (() => {
                ...
                const filteredReviewCandidates = allRecommendations.filter(
                  (rec) => {
                    ...
                    if (candidateSearchQuery.trim()) {
                      ...
                      const matchesTitle = rec.title
                        ?.toLowerCase()
                        .includes(query);
                      const matchesLoc = rec.location
                        ?.toLowerCase()
                        .includes(query);
                      const matchesDesc =
                        rec.shortDescription?.toLowerCase().includes(query) ||
                        rec.longDescription?.toLowerCase().includes(query);
                    ...
                    if (
                      ...
                      rec.category !== candidateCategoryFilter
                    ) {
                      return false;
                    }
                    ...
                    const imgReadiness =
                      imageReadinessStatuses[rec.id] ||
                      "IMAGE RESEARCH REQUIRED";
                    if (
                      candidateImageReadinessFilter !== "ALL" &&
                      imgReadiness !== candidateImageReadinessFilter
                    ) {
                      return false;
                    }
                ...
                // Active candidate
                const currentCandidateRec =
                  allRecommendations.find((r) => r.id === candidateReviewId) ||
                  filteredReviewCandidates[0];
                const currentCandidateIndex = currentCandidateRec
                  ? filteredReviewCandidates.findIndex(
                      (r) => r.id === currentCandidateRec.id,
                    )
                  : -1;
                ...
                const handlePrevCandidate = () => {
                  ...
                  const prevIdx =
                    (currentCandidateIndex -
                      1 +
                      filteredReviewCandidates.length) %
                    filteredReviewCandidates.length;
                ...
                const candidateValidation = (() => {
                  ...
                  if (title.length < 3 || title.length > 60) {
                    errors.push(
                      `Title length must be between 3 and 60 chars. Currently: ${title.length} chars.`,
                    );
                  }

                  // 2. No [CONCEPT DRAFT] marker
                  const hasConceptDraftMarker =
                    title.includes("[CONCEPT DRAFT]") ||
                    shortDesc.includes("[CONCEPT DRAFT]") ||
                    longDesc.includes("[CONCEPT DRAFT]");
                  if (hasConceptDraftMarker) {
                    errors.push(
                      "Contains [CONCEPT DRAFT] unverified market placeholder tag.",
                    );
                  }

                  // 3. No 'To be verified' required fields
                  const hasToBeVerified =
                    /to be verified/i.test(title) ||
                    /to be verified/i.test(shortDesc) ||
                    /to be verified/i.test(longDesc) ||
                    /to be verified/i.test(currentCandidateRec.location || "");
                  if (hasToBeVerified) {
                    errors.push(
                      "Contains unverified information labeled 'To be verified'.",
                    );
                  }

                  // 4. Factual content complete (Long description word count >= 20)
                  const lDescWords = longDesc.trim()
                    ? longDesc.trim().split(/\s+/).length
                    : 0;
                  if (lDescWords < 20) {
                    errors.push(
                      `Long description is too brief (${lDescWords} words). Must be at least 20 words for editorial depth.`,
                    );
                  }
                  ...
                  if (!isLatValid || !isLngValid) {
                    errors.push(
                      `Invalid or uncalibrated GPS bounds (Lat: ${currentCandidateRec.latitude}, Lng: ${currentCandidateRec.longitude}).`,
                    );
                  }
                  ...
                  if (x === 0 && y === 0) {
                    warnings.push(
                      "Mood Orbit placement coordinates are set to exactly 0.0 (Uncalibrated).",
                    );
                  }
                  ...
                  if (!isXBounded || !isYBounded) {
                    errors.push(
                      `Mood Orbit coordinates out of bounds (X: ${x}, Y: ${y}). Must be within [-5, 5].`,
                    );
                  }

                  // 7. Verified production image
                  const imgReadiness =
                    imageReadinessStatuses[currentCandidateRec.id] ||
                    "IMAGE RESEARCH REQUIRED";
                  const isImageVerified =
                    imgReadiness === "VERIFIED PRODUCTION IMAGE";
                  if (!isImageVerified) {
                    errors.push(
                      `Hero image status is classified as '${imgReadiness}'. Only 'VERIFIED PRODUCTION IMAGE' is eligible for Release.`,
                    );
                  }

                  // 8. Practical information reviewed
                  if (
                    !currentCandidateRec.duration ||
                    currentCandidateRec.duration
                      .toLowerCase()
                      .includes("placeholder")
                  ) {
                    errors.push(
                      "Practical field 'Duration' contains placeholder values.",
                    );
                  }
                  if (
                    !currentCandidateRec.travelTime ||
                    currentCandidateRec.travelTime
                      .toLowerCase()
                      .includes("placeholder")
                  ) {
                    errors.push(
                      "Practical field 'Travel Time' contains placeholder values.",
                    );
                  }
                ...
                const handleTransitionStatus = (
                  ...
                  if (nextSt === "APPROVED") {
                    ...
                    if (val && !val.isValid) {
                      setToastMessage(
                        `❌ Approval blocked: Candidate fails release safety gates.`,
                      );
                      return;
                    }
                  }
                  ...
                  saveGovernanceLog(
                    ...
                    nextSt,
                    rationaleText ||
                      `Status manually transitioned to ${nextSt}.`,
                    curatorName,
                  );
                  ...
                  setToastMessage(
                    `✅ Updated "${currentCandidateRec.title}" status to ${nextSt}. Audit logged.`,
                  );
                };
                ...
                const handleUpdateCandidateDataLocal = () => {
                  ...
                  setToastMessage(
                    `Saved local modifications for candidate "${manTitle}".`,
                  );
                };

                // Link image queue specifications
                const queueSpecs = imageProductionQueue.find(
                  (q) => q.id === currentCandidateRec?.id,
                );
                  ...
                  <div className="space-y-6 animate-fade-in text-brand-charcoal">
                    ...
                    <div className="bg-[#FAF9F5] p-5 rounded-3xl border border-border-main/60 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="space-y-1 text-left">
                        <span className="text-[8px] uppercase tracking-[0.2em] font-black text-brand-charcoal/40 block">
                          Editorial workspace
                        </span>
                        <h2 className="font-serif text-lg font-black text-brand-charcoal uppercase tracking-wider">
                          Curation Review Desk
                        </h2>
                        <p className="text-[10px] text-brand-charcoal/60 font-medium">
                          Reconciling Belgrade Secret drafts to
                          production-grade, visitor-faithful publishing
                          readiness.
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2 shrink-0 bg-white/50 p-1.5 rounded-xl border border-border-main/40">
                        <div className="text-center px-3 py-1.5 border-r border-border-main/40">
                          <span className="text-[8px] block uppercase font-bold text-brand-charcoal/40">
                            Total Candidates
                          </span>
                          ...
                          <span className="font-serif text-sm font-black text-emerald-700">
                            {
                              allRecommendations.filter(
                                (r) =>
                                  ...
                                  editorialStatuses[r.id] === "APPROVED",
                              ).length
                            }
                          </span>
                        </div>
                          ...
                          <span className="font-serif text-sm font-black text-amber-700">
                            {
                              allRecommendations.filter(
                                (r) =>
                                  r.id.startsWith("draft-") &&
                                  editorialStatuses[r.id] === "NEEDS RESEARCH",
                              ).length
                            }
                          </span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-[330px_1fr] gap-6 items-start">
                      ...
                      <div className="bg-white p-4 rounded-3xl border border-border-main space-y-4 shadow-sm h-auto xl:max-h-[1000px] xl:overflow-y-auto">
                        <div className="border-b border-border-main/40 pb-3 text-left">
                          <h3 className="font-serif text-xs font-black uppercase tracking-wider text-brand-charcoal">
                            Candidates Registry
                          </h3>
                          <span className="text-[8px] text-brand-charcoal/40 block mt-0.5">
                            {filteredReviewCandidates.length} of 49 match filter
                            criteria
                          </span>
                        </div>
                        ...
                        <div className="space-y-2.5 text-[9.5px]">
                          {/* Search Bar */}
                          <div className="space-y-1 text-left">
                            <label className="font-bold text-brand-charcoal/60 uppercase text-[8px] tracking-wider block">
                              Search Candidate
                            </label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1 text-left">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                setCandidateImageReadinessFilter(
                                  e.target.value,
                                );
                                setIsDetailsExpanded(false);
                              }}
                            ...
                            >
                              ...
                              <option value="VERIFIED PRODUCTION IMAGE">
                                VERIFIED PRODUCTION IMAGE
                              </option>
                              <option value="READY FOR CREATOR REVIEW">
                                READY FOR CREATOR REVIEW
                              </option>
                              <option value="IMAGE RESEARCH REQUIRED">
                                IMAGE RESEARCH REQUIRED
                              </option>
                              <option value="IMAGE REJECTED">
                                IMAGE REJECTED
                              </option>
                            </select>
                          </div>
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-border-main/30 max-h-[450px] xl:max-h-[600px] overflow-y-auto pr-1">
                          {filteredReviewCandidates.length === 0 ? (
                            ...
                            filteredReviewCandidates.map((rec) => {
                              const isSelected =
                                currentCandidateRec &&
                                rec.id === currentCandidateRec.id;
                              const status =
                                editorialStatuses[rec.id] || "CANDIDATE";
                              const imgStatus =
                                imageReadinessStatuses[rec.id] ||
                                "IMAGE RESEARCH REQUIRED";
                              const isConcept = [
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0 space-y-1">
                                    <div className="flex justify-between items-center gap-2">
                                      <span className="font-mono text-[7.5px] text-brand-charcoal/45 font-bold">
                                        {rec.id}
                                      </span>
                                      <span
                                        className={`px-1.5 py-0.2 rounded font-black text-[6.5px] uppercase tracking-wider ${
                                          status === "APPROVED"
                                            ? "bg-[#E8F5E9] text-[#2E7D32] border border-[#2E7D32]/10"
                                            : status === "NEEDS RESEARCH"
                                              ? "bg-amber-100 text-amber-800 border border-amber-800/10"
                                              : status === "RETIRED"
                                                ? "bg-red-100 text-red-800 border border-red-800/10"
                                    ...
                                    <h4 className="font-bold text-[11px] text-brand-charcoal leading-tight line-clamp-2">
                                      {rec.title}
                                    </h4>
                                    ...
                                    {isConcept && (
                                      <span className="inline-block text-[6.5px] bg-purple-100 text-purple-800 px-1 py-0.2 rounded font-bold uppercase mt-1">
                                        Concept
                                      </span>
                                    )}
                      ...
                      {currentCandidateRec ? (
                          ...
                          <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                              <div className="space-y-1 min-w-0 text-left">
                                <div className="flex items-center gap-2">
                                  <span className="font-mono text-[9px] font-bold text-accent-teal bg-accent-teal/5 px-2 py-0.5 rounded-md">
                                    Candidate Inspection
                                  </span>
                                  <span className="text-[9.5px] uppercase font-bold text-brand-charcoal/40">
                                    {currentCandidateRec.category} •{" "}
                                    {currentCandidateRec.location}
                                  </span>
                                </div>
                                <h3 className="font-serif text-sm font-black text-brand-charcoal truncate pr-4">
                                  {currentCandidateRec.title}
                                </h3>
                              </div>
                              ...
                              <div className="flex items-center gap-1.5 shrink-0 self-end md:self-center bg-brand-pearl p-1 rounded-xl border border-border-main/50">
                                ...
                                <span className="text-[9px] font-mono font-extrabold px-2 text-brand-charcoal/70">
                                  {currentCandidateIndex !== -1
                                    ? `${currentCandidateIndex + 1} of ${filteredReviewCandidates.length}`
                                    : "—"}
                                </span>
                                <button
                          ...
                          {activeReviewMode === "visitor" ? (
                            <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 items-start">
                              ...
                              <div className="space-y-3 flex flex-col items-center">
                                <span className="text-[8px] uppercase tracking-widest font-black text-[#75776B]">
                                  Faithful Device Frame
                                </span>
                                      ...
                                      {isDetailsExpanded ? (
                                        renderDetailsScreen ? (
                                          ...
                                          <div className="p-4 text-center">
                                            Missing details screen renderer
                                          </div>
                                        )
                                      ) : (
                                ...
                                <p className="text-[9px] text-brand-charcoal/50 font-medium text-center max-w-[280px]">
                                  {isDetailsExpanded
                                    ? "💡 Viewing exact production details. Tap back inside the device frame to view the card."
                                    : "💡 Tap on the candidate card to inspect the exact mobile details sheet."}
                                </p>
                              </div>
                              ...
                              <div className="space-y-5 text-left text-[10px]">
                                ...
                                {(() => {
                                  const draftProvenance = Array.isArray(
                                    imageProvenance,
                                  )
                                    ? imageProvenance.find(
                                        (p: any) =>
                                          p.draftId === currentCandidateRec.id,
                                      )
                                    : null;
                                  const isRejectedOrMissing =
                                    draftProvenance &&
                                    (draftProvenance.status.includes(
                                      "REJECTED",
                                    ) ||
                                      draftProvenance.status ===
                                        "IMAGE MISSING");
                                  return (
                                    <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4">
                                      <div className="border-b border-[#EAE8DF] pb-2 flex justify-between items-center">
                                        <h4 className="font-serif text-xs font-black uppercase tracking-wider text-brand-charcoal">
                                          Hero Image Classification
                                        </h4>
                                        <span
                                          className={`px-2 py-0.5 rounded font-mono text-[7px] font-black uppercase tracking-wider ${
                                            (imageReadinessStatuses[
                                              currentCandidateRec.id
                                            ] || "IMAGE RESEARCH REQUIRED") ===
                                            "VERIFIED PRODUCTION IMAGE"
                                      ...
                                      <div className="space-y-3 text-left">
                                        <label className="block text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/50">
                                          Assign Visual Quality Grade:
                                        </label>
                                        <select
                                          value={
                                            imageReadinessStatuses[
                                        ...
                                        >
                                          ...
                                          <option value="IMAGE RESEARCH REQUIRED">
                                            ⚠️ IMAGE RESEARCH REQUIRED (DRAFT
                                            WORKPLACE)
                                          </option>
                                          <option value="IMAGE REJECTED">
                                            ❌ IMAGE REJECTED (NON-COMPLIANT)
                                          </option>
                                          {isRejectedOrMissing && (
                                            <option
                                              value={draftProvenance.status}
                                              disabled
                                            >
                                              {draftProvenance.status}
                                            </option>
                                      ...
                                      {isRejectedOrMissing && (
                                        <div className="bg-red-50 p-4 rounded-2xl border border-red-200 space-y-2 text-left">
                                          ...
                                          <p className="text-[9px] text-brand-charcoal/70 leading-relaxed font-sans">
                                            The image asset is flagged as{" "}
                                            <strong>
                                              {draftProvenance.status}
                                            </strong>{" "}
                                            in the image provenance ledger. It
                                            is strictly blocked from
                                            publication.
                                          </p>
                                        </div>
                                      )}
                                      ...
                                      {draftProvenance && (
                                        <div className="mt-4 pt-4 border-t border-border-main/20 space-y-2.5 text-[9px] text-brand-charcoal/70 bg-[#FAF9F5]/50 p-3 rounded-2xl border border-border-main/10">
                                          <div className="flex justify-between items-center">
                                            <span className="font-semibold uppercase text-[7.5px] tracking-wider text-brand-charcoal/45">
                                              Provenance ID
                                            </span>
                                            <span className="font-mono text-brand-charcoal font-black">
                                              {draftProvenance.draftId}
                                            </span>
                                          </div>
                                          <div className="flex justify-between items-center">
                                            <span className="font-semibold uppercase text-[7.5px] tracking-wider text-brand-charcoal/45">
                                              Geographic Identity Proven
                                            </span>
                                            <span
                                              className={`font-bold uppercase text-[7.5px] ${draftProvenance.geographicIdentityProven === "YES" ? "text-emerald-600" : "text-red-600"}`}
                                            >
                                              {draftProvenance.geographicIdentityProven ||
                                                "UNVERIFIED"}
                                            </span>
                                          </div>
                                          <div className="flex justify-between items-center col-span-2">
                                            ...
                                            <span className="font-mono text-brand-charcoal truncate max-w-[150px]">
                                              {draftProvenance.author ||
                                                "UNKNOWN"}
                                            </span>
                                          </div>
                                          <div className="flex justify-between items-center">
                                            ...
                                            <span className="font-mono text-brand-charcoal truncate max-w-[150px]">
                                              {draftProvenance.license ||
                                                "UNVERIFIED"}
                                            </span>
                                          </div>
                                          {draftProvenance.notes && (
                                            <div className="pt-2 border-t border-border-main/10">
                                              <span className="font-semibold uppercase text-[7.5px] tracking-wider text-brand-charcoal/45 block mb-1">
                                                Curation Audit Notes
                                              </span>
                                              <p className="text-brand-charcoal/80 leading-normal italic text-[8.5px]">
                                                {draftProvenance.notes}
                                              </p>
                                            </div>
                                          )}
                                      ...
                                      {!draftProvenance &&
                                        currentCandidateRec.image?.includes(
                                          "draft_placeholder",
                                        ) && (
                                          <div className="bg-accent-red/5 p-4 rounded-2xl border border-accent-red/20 space-y-2 text-left">
                                            ...
                                            <p className="text-[9px] text-brand-charcoal/70 leading-relaxed">
                                              This candidate relies on{" "}
                                              <code className="bg-white px-1.5 py-0.2 rounded font-mono border border-border-main">
                                                draft_placeholder.png
                                              </code>
                                              . In accordance with the{" "}
                                              <strong>
                                                IDEMO Design Constitution
                                              </strong>
                                              , this asset is forbidden in the
                                              production release. High-end
                                              visual research or custom
                                ...
                                {queueSpecs && (
                                  <div className="bg-[#FAF9F5]/70 p-5 rounded-3xl border border-border-main space-y-4 text-left">
                                    <div className="border-b border-border-main/40 pb-2">
                                      <span className="text-[7.5px] uppercase tracking-[0.2em] font-black text-[#75776B] block">
                                        Belgrade Secret Asset Pipeline
                                      </span>
                                      <h4 className="font-serif text-xs font-black uppercase tracking-wide text-brand-charcoal">
                                        Structured Production Queue
                                        Specifications
                                      </h4>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 text-[9.5px]">
                                      <div className="bg-white p-2.5 rounded-xl border border-border-main/40">
                                        <span className="text-[7px] text-brand-charcoal/45 uppercase font-bold block">
                                          Proposed Filename
                                        </span>
                                        <span className="font-mono text-[8.5px] font-bold text-brand-charcoal break-all">
                                          {queueSpecs.proposedFilename}
                                        </span>
                                      </div>
                                      <div className="bg-white p-2.5 rounded-xl border border-border-main/40">
                                        <span className="text-[7px] text-brand-charcoal/45 uppercase font-bold block">
                                    ...
                                    <div className="space-y-2.5 text-[9.5px]">
                                      <div>
                                        <span className="font-bold text-brand-charcoal/50 uppercase text-[7px] tracking-wider block">
                                          Visual Subject Description
                                        </span>
                                        <p className="text-brand-charcoal/80 leading-relaxed font-medium">
                                          {queueSpecs.visualSubject}
                                        </p>
                                      </div>
                                      <div>
                                        <span className="font-bold text-brand-charcoal/50 uppercase text-[7px] tracking-wider block">
                                          Factual Geological Characteristics
                                        </span>
                                        <p className="text-brand-charcoal/80 leading-relaxed font-medium">
                                          {queueSpecs.factualCharacteristics}
                                        </p>
                                      </div>
                                      <div className="grid grid-cols-2 gap-2">
                                        <div>
                                          ...
                                          <p className="text-brand-charcoal/80 leading-snug">
                                            {queueSpecs.recommendedViewpoint}
                                          </p>
                                        </div>
                                        <div>
                                          <span className="font-bold text-brand-charcoal/50 uppercase text-[7px] tracking-wider block">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-brand-charcoal/80 leading-relaxed">
                                          {queueSpecs.peopleDescription}
                                        </p>
                                      </div>
                                      <div className="bg-accent-red/5 p-3 rounded-xl border border-accent-red/10">
                                        <span className="font-bold text-[#A82B2B] uppercase text-[7.5px] tracking-widest block mb-0.5">
                                          ⚠️ Rigid Reality Gate (Do Not Invent)
                                        </span>
                                        <p className="text-[#A82B2B] leading-relaxed text-[9px] font-semibold">
                                          {queueSpecs.mustNotInvent}
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                ...
                                <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4 text-left">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div className="space-y-1 text-left">
                                      <label className="text-[7.5px] font-black uppercase text-brand-charcoal/50 block">
                                        Authorized Curator Name:
                                      </label>
                                      <input
                                        type="text"
                                        value={curatorName}
                                        onChange={(e) =>
                                          setCuratorName(e.target.value)
                                        }
                                    ...
                                    <button
                                      ...
                                      className={`px-4 py-2 text-white rounded-xl text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 ${
                                        candidateValidation?.isValid
                                          ? "bg-emerald-700 hover:bg-emerald-800 shadow-md"
                                          : "bg-brand-charcoal/30 cursor-not-allowed opacity-50"
                                      }`}
                                      disabled={!candidateValidation?.isValid}
                                      title={
                                        candidateValidation?.isValid
                                          ? "Elevate to production stream"
                                          : "Fails safety release requirements"
                                      }
                                    >
                            ...
                            <div className="space-y-6 text-left">
                              ...
                              {candidateValidation && (
                                <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4">
                                  <div className="border-b border-border-main/40 pb-2 flex justify-between items-center">
                                    <h4 className="font-serif text-xs font-black uppercase tracking-wider text-brand-charcoal">
                                      Release Diagnostic Safety Gate
                                    </h4>
                                    <span
                                      className={`px-2.5 py-0.5 rounded-full font-sans text-[8px] font-extrabold uppercase tracking-widest ${
                                        candidateValidation.isValid
                                          ? "bg-[#E8F5E9] text-[#2E7D32]"
                                    ...
                                    >
                                      {candidateValidation.isValid
                                        ? "🛡️ PASSED ALL GATES"
                                        : "⚠️ COMPLIANCE ALERTS OUTSTANDING"}
                                    </span>
                                  </div>

                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[9.5px]">
                                    <div className="space-y-2">
                                      <span className="font-black uppercase tracking-wider text-[8px] text-brand-charcoal/50">
                                        Rigid Technical Compliance Results:
                                      </span>
                                      ...
                                      <div className="space-y-1.5">
                                        <div className="flex items-center justify-between p-2 rounded-xl bg-brand-pearl/50 border border-border-main/35">
                                          <span className="font-bold">
                                            No Concept Draft tags in metadata
                                          </span>
                                          <span
                                            className={`font-black text-[8px] ${!(currentCandidateRec.title?.includes("[CONCEPT DRAFT]") || currentCandidateRec.shortDescription?.includes("[CONCEPT DRAFT]") || currentCandidateRec.longDescription?.includes("[CONCEPT DRAFT]")) ? "text-emerald-700" : "text-accent-red"}`}
                                          >
                                            {!(
                                              currentCandidateRec.title?.includes(
                                                "[CONCEPT DRAFT]",
                                              ) ||
                                              currentCandidateRec.shortDescription?.includes(
                                                "[CONCEPT DRAFT]",
                                              ) ||
                                              currentCandidateRec.longDescription?.includes(
                                                "[CONCEPT DRAFT]",
                                              )
                                            )
                                              ? "✔ PASS"
                                        ...
                                        <div className="flex items-center justify-between p-2 rounded-xl bg-brand-pearl/50 border border-border-main/35">
                                          <span className="font-bold">
                                            No "To be verified" required field
                                            marks
                                          </span>
                                          <span
                                            className={`font-black text-[8px] ${!(/to be verified/i.test(currentCandidateRec.title || "") || /to be verified/i.test(currentCandidateRec.shortDescription || "") || /to be verified/i.test(currentCandidateRec.longDescription || "")) ? "text-emerald-700" : "text-accent-red"}`}
                                          >
                                            {!(
                                              /to be verified/i.test(
                                                currentCandidateRec.title || "",
                                              ) ||
                                              /to be verified/i.test(
                                                currentCandidateRec.shortDescription ||
                                                  "",
                                              ) ||
                                              /to be verified/i.test(
                                                currentCandidateRec.longDescription ||
                                                  "",
                                              )
                                            )
                                              ? "✔ PASS"
                                        ...
                                        <div className="flex items-center justify-between p-2 rounded-xl bg-brand-pearl/50 border border-border-main/35">
                                          <span className="font-bold">
                                            Verified production hero image grade
                                          </span>
                                          <span
                                            className={`font-black text-[8px] ${imageReadinessStatuses[currentCandidateRec.id] === "VERIFIED PRODUCTION IMAGE" ? "text-emerald-700" : "text-accent-red"}`}
                                          >
                                            {imageReadinessStatuses[
                                              currentCandidateRec.id
                                            ] === "VERIFIED PRODUCTION IMAGE"
                                        ...
                                        <div className="flex items-center justify-between p-2 rounded-xl bg-brand-pearl/50 border border-border-main/35">
                                          <span className="font-bold">
                                            Factual text complete (&gt;20 words
                                            in long)
                                          </span>
                                          <span
                                            className={`font-black text-[8px] ${(currentCandidateRec.longDescription || "").trim().split(/\s+/).length >= 20 ? "text-emerald-700" : "text-accent-red"}`}
                                          >
                                            {(
                                              currentCandidateRec.longDescription ||
                                              ""
                                        ...
                                        <div className="flex items-center justify-between p-2 rounded-xl bg-brand-pearl/50 border border-border-main/35">
                                          <span className="font-bold">
                                            Valid GPS bounds inside Serbia range
                                          </span>
                                          <span
                                            className={`font-black text-[8px] ${!isNaN(Number(currentCandidateRec.latitude)) && Number(currentCandidateRec.latitude) >= 42.0 ? "text-emerald-700" : "text-accent-red"}`}
                                          >
                                            {!isNaN(
                                              Number(
                                                currentCandidateRec.latitude,
                                              ),
                                            ) &&
                                            Number(
                                              currentCandidateRec.latitude,
                                    ...
                                    <div className="space-y-3 bg-brand-pearl/40 p-4 rounded-2xl border border-border-main/50 text-left">
                                      <span className="font-bold uppercase text-[8px] text-[#75776B] block">
                                      ...
                                      {candidateValidation.errors.length ===
                                        ...
                                        candidateValidation.warnings.length ===
                                          ...
                                          <p className="text-emerald-700 font-semibold italic text-[9px]">
                                            ● Excellent. All production and
                                            design standards satisfied. This
                                            curation is fully authorized for
                                            Release.
                                          </p>
                                        )}
                                      ...
                                      {candidateValidation.errors.map(
                                        (err, i) => (
                                          <div
                                            key={i}
                                            className="flex items-start gap-1.5 text-accent-red text-[9px] font-medium leading-relaxed"
                                          >
                                      ...
                                      {candidateValidation.warnings.map(
                                        (warn, i) => (
                                          <div
                                            key={i}
                                            className="flex items-start gap-1.5 text-amber-700 text-[9px] font-medium leading-relaxed"
                                          >
                                            <span className="font-bold shrink-0">
                                              ⚠ Warning:
                                            </span>
                              ...
                              <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm text-left space-y-4">
                                <div className="border-b border-border-main/40 pb-2">
                                  <h4 className="font-serif text-xs font-black uppercase tracking-wider text-brand-charcoal">
                                    Factual Metadata Calibration Form
                                  </h4>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[9.5px]">
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-left">
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        value={manTravelTime}
                                        onChange={(e) =>
                                          setManTravelTime(e.target.value)
                                        }
                                        className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                      />
                                    </div>
                                  ...
                                  <div className="space-y-1 md:col-span-2 text-left">
                                    ...
                                    <input
                                      ...
                                      value={manShortDesc}
                                      onChange={(e) =>
                                        setManShortDesc(e.target.value)
                                      }
                                      className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                    />
                                  </div>
                                  ...
                                  <div className="space-y-1 md:col-span-2 text-left">
                                    <label className="font-bold text-brand-charcoal/60 uppercase text-[8px] tracking-wider block">
                                      Traveler Detailed Description (Long)
                                    </label>
                                    <textarea
                                      value={manLongDesc}
                                      onChange={(e) =>
                                        setManLongDesc(e.target.value)
                                      }
                                      rows={4}
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-left">
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        value={manLat}
                                        onChange={(e) =>
                                          setManLat(Number(e.target.value))
                                        }
                                        className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        value={manLng}
                                        onChange={(e) =>
                                          setManLng(Number(e.target.value))
                                        }
                                        className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                      />
                                    </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-left">
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        value={manCoordX}
                                        onChange={(e) =>
                                          setManCoordX(Number(e.target.value))
                                        }
                                        className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        value={manCoordY}
                                        onChange={(e) =>
                                          setManCoordY(Number(e.target.value))
                                        }
                                        className="w-full bg-[#FAF9F5] border border-border-main py-1.5 px-3 rounded-lg text-[10px]"
                                      />
                                    </div>
                        ...
                        <div className="bg-white py-16 px-4 rounded-3xl border border-border-main text-center italic text-brand-charcoal/50 text-[10.5px]">
                          Select a draft candidate from the left panel registry
                          to begin high-end curation review.
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm text-left">
                      <div className="border-b border-border-main/50 pb-3">
                        <span className="text-[8px] uppercase tracking-[0.2em] font-black text-[#75776B] block">
                          Permanent Registry Desk
                        </span>
                        <h4 className="font-serif text-xs font-black text-brand-charcoal uppercase tracking-wider mt-0.5">
                          Global Editorial Governance Log
                        </h4>
                      </div>

                      {governanceLogs.length === 0 ? (
                        <div className="py-8 text-center text-brand-charcoal/40 italic text-[9.5px]">
                          No permanent status changes registered in this
                          session. The audit ledger remains blank.
                        </div>
                      ) : (
                        <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                          {governanceLogs.map((log: any) => {
                            ...
                            return (
                              <div
                                key={log.id}
                                className="bg-[#FAF9F5]/50 border border-border-main/50 p-3 rounded-2xl flex flex-col md:flex-row md:items-start justify-between gap-3 text-[10px]"
                              >
                                <div className="space-y-1 text-left flex-1">
                                  <div className="flex flex-wrap items-center gap-2">
                                    <span className="font-mono text-[8px] text-brand-charcoal/50">
                                      Transition Event
                                    </span>
                                    <span className="text-[8px] font-mono font-bold text-accent-teal">
                                  ...
                                  <div>
                                    <span className="font-bold text-brand-charcoal">
                                      {rec
                                        ? rec.title
                                        : `Deleted Curation (ID: ${log.recId})`}
                                    </span>
                                    <span className="text-brand-charcoal/40 font-mono text-[8.5px] ml-1">
                                      ({log.recId})
                                    </span>
                                  </div>
                                  <p className="text-brand-charcoal/70 italic font-medium">
                                    "{log.rationale}"
                                  </p>
                                </div>
                      ...
                      <div className="fixed inset-0 z-[1100] bg-brand-pearl/95 backdrop-blur-md flex flex-col overflow-y-auto p-4 md:p-8 animate-fade-in font-sans text-brand-charcoal">
                        ...
                        <div className="max-w-6xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-4 pb-4 border-b border-border-main/50 mb-6 sticky top-0 bg-brand-pearl/90 backdrop-blur-xs z-10 py-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-[9px] uppercase tracking-wider font-extrabold text-brand-charcoal/50">
                                  Production Preview Mode
                                </span>
                                <span className="bg-accent-teal/10 font-mono text-[8px] text-accent-teal px-1.5 py-0.5 rounded font-black uppercase tracking-wider">
                                  {currentCandidateRec.id}
                                </span>
                              </div>
                              <h2 className="font-serif text-sm font-black uppercase tracking-wider text-brand-charcoal">
                                {currentCandidateRec.title}
                              </h2>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono font-bold text-brand-charcoal/50">
                              Current Status:
                            </span>
                            <span
                              className={`px-2 py-0.5 rounded font-black text-[8px] uppercase tracking-wider ${
                                (editorialStatuses[currentCandidateRec.id] ||
                                  "CANDIDATE") === "APPROVED"
                                  ? "bg-[#E8F5E9] text-[#2E7D32] border border-[#2E7D32]/10"
                                  : (editorialStatuses[
                                        currentCandidateRec.id
                        ...
                        <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 items-start pb-12">
                          ...
                          <div className="flex flex-col items-center space-y-4">
                            <span className="text-[9px] uppercase tracking-[0.2em] font-black text-[#75776B]">
                              Interactive Visitor Viewport
                            </span>
                          ...
                          <div className="space-y-6 text-left">
                            ...
                            <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4">
                              <div className="border-b border-border-main/40 pb-3">
                                <span className="text-[8px] uppercase tracking-widest text-[#75776B] font-black block">
                                  Safety Gate Diagnostic
                                </span>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-1 bg-[#FAF9F5]/40 border border-border-main/50 p-3 rounded-2xl">
                                  <span className="text-[8px] uppercase tracking-wider text-brand-charcoal/50 font-bold block">
                                    Assigned WebP Asset
                                  </span>
                                  <div className="font-mono text-[9px] text-brand-charcoal font-black break-all flex items-center gap-1.5 pt-0.5">
                                    <span>
                                      {currentCandidateRec.image
                                        ?.split("/")
                                        .pop() || "No image"}
                                    </span>
                                    <span className="text-[8px] font-sans px-1.5 py-0.2 rounded bg-emerald-100 text-emerald-800 font-bold">
                                      WebP Loaded
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-1 bg-[#FAF9F5]/40 border border-border-main/50 p-3 rounded-2xl">
                                  ...
                                  <div className="font-sans text-[10px] font-extrabold text-[#2E7D32] flex items-center gap-1 pt-0.5">
                                    {candidateValidation?.isValid
                                      ? "✅ PASSES ALL SAF GRADES"
                                      : "⚠️ OUTSTANDING METADATA WARNINGS"}
                                  </div>
                                </div>

                                {(() => {
                                  const isCalibrated =
                                    currentCandidateRec.coordinateX !==
                                      undefined &&
                                    currentCandidateRec.coordinateX !== null &&
                                    typeof currentCandidateRec.coordinateX ===
                                      "number" &&
                                    !isNaN(currentCandidateRec.coordinateX) &&
                                    isFinite(currentCandidateRec.coordinateX) &&
                                    currentCandidateRec.coordinateY !==
                                      undefined &&
                                    currentCandidateRec.coordinateY !== null &&
                                    typeof currentCandidateRec.coordinateY ===
                                      "number" &&
                                    ...
                                    <div className="space-y-1 bg-[#FAF9F5]/40 border border-border-main/50 p-3 rounded-2xl">
                                      ...
                                      <div className="font-sans text-[10px] font-extrabold flex items-center gap-1 pt-0.5">
                                        {isCalibrated ? (
                                          ...
                                          <span className="text-amber-800 bg-amber-100 px-1.5 py-0.5 rounded font-black text-[8px] uppercase tracking-wider animate-pulse">
                                            ⚠️ NOT YET CALIBRATED
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                ...
                                !candidateValidation.isValid && (
                                  <div className="bg-amber-500/5 border border-amber-500/10 p-3 rounded-2xl space-y-1.5 text-[9.5px]">
                                    <span className="font-black text-amber-800 uppercase tracking-wider block">
                                      ⚠️ Outstanding Corrections Needed:
                                    </span>
                            ...
                            <div className="bg-white p-5 rounded-3xl border border-border-main shadow-sm space-y-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1 text-left">
                                  <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60">
                                    Curator Signature
                                  </label>
                                  <input
                                    type="text"
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                  <button
                                    onClick={() => {
                                      handleTransitionStatus("NEEDS RESEARCH");
                                      setToastMessage(
                                        `Moved ${currentCandidateRec.id} to NEEDS RESEARCH.`,
                                      );
                                    }}
                                  ...
                                  <button
                                    onClick={() => {
                                      handleTransitionStatus("RETIRED");
                                      setToastMessage(
                                        `Retired candidate ${currentCandidateRec.id}.`,
                                      );
                                    }}
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      setToastMessage(
                                        `Reverted ${currentCandidateRec.id} to CANDIDATE status.`,
                                      );
                                    }}
                                  ...
                                  <button
                                    onClick={() => {
                                      if (
                                        ...
                                        setToastMessage(
                                          `❌ Approval blocked: Candidate does not satisfy Safety Gates.`,
                                        );
                                        return;
                                      }
                                      handleTransitionStatus("APPROVED");
                                      setToastMessage(
                                        `🎉 Approved & Published candidate ${currentCandidateRec.id}!`,
                                      );
                                    }}
                            ...
                            <div className="bg-[#FAF9F5]/40 p-5 rounded-3xl border border-border-main/50 space-y-4">
                              <div className="flex justify-between items-center pb-2 border-b border-border-main/30">
                                <div>
                                  <span className="text-[8px] uppercase tracking-widest text-brand-charcoal/50 font-black block">
                                    Factual Metadata Review
                                  </span>
                                  <h4 className="font-serif text-[11px] font-black text-brand-charcoal uppercase tracking-wide mt-0.5">
                                    Manual Metadata Calibration Editor
                                  </h4>
                                </div>
                                <button
                                  onClick={() => {
                                    ...
                                    const targetEl = document.getElementById(
                                      "curation-factual-calibration-form",
                                    );
                                    if (targetEl)
                              ...
                              <p className="text-[9.5px] text-brand-charcoal/60 leading-relaxed">
                                Verify or recalibrate geo-coordinates, Mood
                                Orbit mapping density, categories, badges,
                                travel durations, translations, and curation
                                descriptions directly in the primary workspace
                                editor to keep candidates aligned with reality.
                              </p>
                            </div>
                          </div>
                        </div>
        ...
        {activeTab === "tech" && (
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide">
                Developer Technical Manual
              </h3>
              <p className="text-[10px] text-brand-charcoal/60">
                Unified architecture documentation, schemas, payloads, and
                hosting evaluation.
              </p>
            </div>
            ...
            <div className="bg-[#FAF9F5]/80 border border-border-main p-4 rounded-2xl flex items-center justify-between text-left shadow-sm">
              <div className="space-y-1">
                ...
                <p className="text-[9px] text-brand-charcoal/50 font-medium">
                  Relocated from traveler interface to preserve editorial luxury
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm text-left">
              <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal flex items-center gap-1.5">
                <Database size={14} className="text-accent-teal" /> General
                Setup & Custom Branding
              </h4>
              <p className="text-[10px] text-brand-charcoal/60 leading-normal">
                Configure primary global assets used across onboarding or
              ...
              <div className="p-4 bg-[#FAF9F5]/60 border border-border-main/50 rounded-2xl space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-brand-charcoal">
                      Language Selection Card Custom Image
                    </h4>
                    <p className="text-[8.5px] text-brand-charcoal/50 font-sans">
                      Replace the central welcome screen graphic with your own
                      ...
                      onChange={(e) => {
                        ...
                        reader.onload = (event) => {
                          ...
                          if (base64 && onUpdateLandingImage) {
                            ...
                            showToast(
                              "Language Selection Card image uploaded successfully!",
                            );
                          }
                        };
                        reader.readAsDataURL(file);
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (onUpdateLandingImage) {
                          onUpdateLandingImage("");
                          showToast(
                            "Reset language card image to default suitcase.",
                          );
                        }
                      }}
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm">
              <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal/60 flex items-center gap-1.5">
                <Server size={14} className="text-accent-teal" /> Analytics
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-[9px] font-mono border-collapse divide-y divide-border-main">
                  ...
                  <tbody className="divide-y divide-border-main/50 text-brand-charcoal/80">
                    <tr>
                      <td className="py-2.5 font-bold text-accent-teal">
                        Plausible
                      </td>
                    ...
                    <tr className="text-accent-teal bg-accent-teal/5">
                      <td className="py-2.5 font-bold px-1 text-accent-teal">
                        ★ Custom App Core
                      </td>
                      <td className="py-2.5">Strict (None)</td>
              ...
              <p className="text-[9.5px] text-brand-charcoal/60 leading-normal pt-1 italic font-sans">
                ** Recommendation: For EXPO 2027 scale (10,000+ active
                visitors), we recommend our **Custom Offline-Safe App Core**
                because it requires 0€ hosting overhead, guarantees complete
                GDPR anonymity automatically, and complies perfectly with mobile
                platform privacy privacy guidelines without Cookie Banners.
              </p>
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3 shadow-sm">
              <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal/60 flex items-center gap-1.5">
                <Code size={14} className="text-accent-teal" /> Event Payload
                Schema (JSON)
              </h4>
              <p className="text-[10px] text-brand-charcoal/60">
                Standard anonymized campaign payload layout dispatched to local
                client collectors:
              </p>
  ...
  "properties": {
    ...
    "user_agent_pwa": "SafariStandaloneEXPO2027"
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3 shadow-sm">
              <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal/60 flex items-center gap-1.5">
                <Database size={14} className="text-accent-teal" /> Database
  ...
  last_day_active DATE
        ...
        {activeTab === "analytics" &&
          (() => {
            ...
            const filteredByCountry =
              selectedCountry === "All"
                ...
                : allRecommendations.filter((r) =>
                    selectedCountry === "Serbia" ? true : false,
                  ); // Only Serbia has active dataset items

            const totalCurationsCount = filteredByCountry.length;

            // Quality checks on country-filtered recommendations
            const duplicateIdList = filteredByCountry.filter(
              (r, i, arr) => arr.findIndex((item) => item.id === r.id) !== i,
            );
            const duplicateIdsCount = duplicateIdList.length;

            const missingCoordsList = filteredByCountry.filter(
              (r) =>
                typeof r.coordinateX !== "number" ||
                typeof r.coordinateY !== "number",
            );
            const missingCoordsCount = missingCoordsList.length;

            const missingImagesList = filteredByCountry.filter(
              (r) =>
                !r.image || r.image === "" || r.image === "dynamic_generate",
            );
            const missingImagesCount = missingImagesList.length;

            const missingTranslationsList = filteredByCountry.filter(
              (r) =>
                !r.translations ||
                !r.translations.sr ||
                !r.translations.zh ||
            ...
            const missingAttributesList = filteredByCountry.filter(
              (r) =>
                !r.estimatedCost ||
                !r.travelTime ||
            ...
            const invalidCategoriesList = filteredByCountry.filter(
              (r) => !ALL_CATEGORIES.includes(r.category),
            );
            const invalidCategoriesCount = invalidCategoriesList.length;

            const coordinateConflictsList = filteredByCountry.filter(
              (r) =>
                typeof r.coordinateX === "number" &&
                typeof r.coordinateY === "number" &&
                filteredByCountry.some(
                  (item) =>
                    item.id !== r.id &&
                    item.coordinateX === r.coordinateX &&
                    item.coordinateY === r.coordinateY,
                ),
            );
            const coordinateConflictsCount = coordinateConflictsList.length;

            const validationErrorsList = filteredByCountry.filter(
              (r) =>
                (typeof r.coordinateX === "number" &&
                  (r.coordinateX < -5 || r.coordinateX > 5)) ||
                (typeof r.coordinateY === "number" &&
                  (r.coordinateY < -5 || r.coordinateY > 5)),
            );
            ...
            const getQualityScore = (r: any) => {
              ...
              if (
                ...
                score -= 20;
              if (!r.image || r.image === "" || r.image === "dynamic_generate")
                score -= 15;
              if (!r.translations || !r.translations.sr || !r.translations.zh)
                score -= 20;
            ...
            const avgQualityScore =
              totalCurationsCount > 0
                ? Math.round(
                    (filteredByCountry.reduce(
                      (sum, r) => sum + getQualityScore(r),
                      0,
                    ) /
                      totalCurationsCount) *
                      10,
            ...
            const lastValidationDateStr = new Date().toLocaleDateString(
              undefined,
              {
                year: "numeric",
                month: "short",
            ...
            const activeRecsForGrid = filteredByCountry.filter((r) => {
              ...
              const matchesCategory =
                ocCategoryFilter === "All" || r.category === ocCategoryFilter;
              const matchesClass =
                ocClassFilter === "All" ||
                (ocClassFilter === "Premium" &&
                  (r.badge === "silver" ||
                    r.badge === "gold" ||
                    r.badge === "platinum")) ||
                (ocClassFilter === "Standard" &&
                  (!r.badge || r.badge === "none"));
              const matchesSeason =
                ocSeasonFilter === "All" || r.seasonality === ocSeasonFilter;

              // Indoor/Outdoor heuristic
              const isOutdoor =
                r.category === "Nature" ||
                r.category === "Clubbing" ||
                r.preferredTransport?.toLowerCase().includes("hike");
              const matchesEnvironment =
                ocEnvironmentFilter === "All" ||
            ...
            for (let rIdx = 0; rIdx < 10; rIdx++) {
              for (let cIdx = 0; cIdx < 10; cIdx++) {
                ...
                const itemsInCell = activeRecsForGrid.filter(
                  (r) =>
                    typeof r.coordinateX === "number" &&
                    typeof r.coordinateY === "number" &&
                    Math.abs(r.coordinateX - sx) <= 0.5 &&
                    Math.abs(r.coordinateY - sy) <= 0.5,
                );
            ...
            const getClustersCount = () => {
              ...
              activeRecsForGrid.forEach((r) => {
                if (
                  ...
                  const size = activeRecsForGrid.filter(
                    (other) =>
                      typeof other.coordinateX === "number" &&
                      typeof other.coordinateY === "number" &&
                      Math.sqrt(
                        Math.pow(r.coordinateX - other.coordinateX, 2) +
                          Math.pow(r.coordinateY - other.coordinateY, 2),
                      ) <= 1.5,
                  ).length;
            ...
            const handleTriggerBuildAudit = () => {
              ...
              const newAudit = {
                ...
                issuesFixed:
                  duplicateIdsCount + validationErrorsCount === 0 ? 1 : 0,
              ...
              try {
                safeStorage.setItem(
                  "idemo_validation_history_v2",
                  JSON.stringify(updatedHistory),
                );
              } catch {}
              showToast(
                "Comprehensive Dataset & Build Audit completed successfully!",
              );
            };
            ...
            const explorerFilteredRecs = filteredByCountry.filter((r) => {
              ...
              switch (activeQualityFilter) {
                ...
                case "coordinate_conflicts":
                  return coordinateConflictsList.some(
                    (item) => item.id === r.id,
                  );
            ...
            // Sorting Explorer
            const sortedExplorerRecs = [...explorerFilteredRecs].sort(
              (a, b) => {
            ...
            const potentialBlindSpots = [
              {
                ...
                priority: "HIGH",
                improvement: "6.4%",
                description: "Upper-left wilderness-comfort gap",
              },
              {
                x: 2.5,
                y: -4.5,
                priority: "MEDIUM",
                improvement: "4.2%",
                description: "Bottom-right luxury-action gap",
              },
              {
                x: -4.5,
                y: -1.5,
                priority: "HIGH",
                improvement: "7.8%",
                description: "Extreme left budget-serene gap",
              },
              {
                x: 3.5,
                y: 3.5,
                priority: "LOW",
                improvement: "2.5%",
                description: "Top-right high-action luxury gap",
              },
            ].map((bs) => {
              // Find existing nearby recommendations
              const nearby = filteredByCountry.filter(
                (r) =>
                  typeof r.coordinateX === "number" &&
                  typeof r.coordinateY === "number" &&
                  Math.sqrt(
                    Math.pow(r.coordinateX - bs.x, 2) +
                      Math.pow(r.coordinateY - bs.y, 2),
                  ) <= 2.5,
              );
              return {
                ...bs,
              ...
              <div className="space-y-8 animate-fade-in text-left">
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border-main/50 pb-5 mb-5">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] uppercase tracking-wider font-extrabold text-accent-teal">
                          MANAGEMENT SYSTEM
                        </span>
                        <span className="text-[8px] bg-accent-teal/10 text-accent-teal px-1.5 py-0.5 rounded font-mono font-bold">
                          REAL-TIME
                        </span>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] uppercase tracking-wider font-bold text-brand-charcoal/50">
                          Curation Country:
                        </span>
                        <select
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left">
                      <p className="text-[8.5px] uppercase font-bold text-brand-charcoal/40">
                        Active Country
                      </p>
                    ...
                    <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left">
                      ...
                      <p className="text-[12px] font-mono font-bold text-brand-charcoal mt-1">
                        {datasetVersion}{" "}
                        <span className="text-[9.5px] font-normal text-brand-charcoal/50">
                          {releaseCandidate}
                        </span>
                      </p>
                    </div>
                    ...
                    <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left">
                      <p className="text-[8.5px] uppercase font-bold text-brand-charcoal/40">
                        Total Curated Locations
                      </p>
                      <p className="text-[14px] font-mono font-black text-accent-teal mt-0.5">
                        {totalCurationsCount}
                      </p>
                    </div>
                    ...
                    <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left">
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <p className="text-[14px] font-mono font-black text-brand-charcoal">
                          {avgQualityScore}%
                        </p>
                        <span className="text-[8px] text-[#2E7D32] font-semibold">
                          ({totalCurationsCount > 0 ? "Optimal" : "N/A"})
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left col-span-2 md:col-span-1">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        {overallHealth === "GREEN" ? (
                          <span className="bg-[#E8F5E9] text-[#2E7D32] text-[8.5px] font-mono font-black px-2 py-0.5 rounded border border-[#2E7D32]/10 uppercase">
                            Certified
                          </span>
                        ) : overallHealth === "YELLOW" ? (
                          <span className="bg-amber-50 text-amber-800 text-[8.5px] font-mono font-black px-2 py-0.5 rounded border border-amber-500/10 uppercase">
                            Warning
                          </span>
                        ) : (
                          <span className="bg-red-50 text-accent-red text-[8.5px] font-mono font-black px-2 py-0.5 rounded border border-accent-red/10 uppercase">
                            Errors
                          </span>
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-border-main/30">
                    <div className="flex items-center gap-2.5 text-[11px]">
                      <span className="text-[8.5px] uppercase font-bold text-brand-charcoal/40">
                        Build Status:
                      </span>
                      <span
                        className={`font-mono font-black text-[10px] ${buildStatus === "PASSED" ? "text-[#2E7D32]" : "text-accent-red"}`}
                      >
                        ⚙ {buildStatus}
                      </span>
                    </div>
                  ...
                  <div className="mt-5 pt-5 border-t border-border-main/30 space-y-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-brand-pearl p-4.5 rounded-2xl border border-border-main/50 text-[10px]">
                      <div className="space-y-1">
                        <label className="text-[8px] uppercase font-bold text-brand-charcoal/50 block text-left font-sans">
                          Lifecycle Status
                        </label>
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            onChange={(e) =>
                              handleUpdateDatasetField("status", e.target.value)
                            }
                        ...
                        <span className="text-[7.5px] text-brand-charcoal/40 font-mono block text-left">
                          Production status locks edits.
                        </span>
                      </div>
                      ...
                      <div className="space-y-1 text-left">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            onClick={() => {
                              const parts = currentDataset.version
                                .replace("v", "")
                                .split(".");
                        ...
                        <span className="text-[7.5px] text-brand-charcoal/40 font-mono block text-left">
                          Current: {currentDataset.semanticVersion}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1 text-left">
                        ...
                        />
                        <span className="text-[7.5px] text-brand-charcoal/40 font-mono block text-left">
                          Release Date: {currentDataset.releaseDate}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 text-brand-charcoal/70 font-sans">
                        ...
                        <span>
                          {currentDataset.status === "Production" ? (
                            <span className="font-extrabold text-accent-teal">
                              Production Active — Dataset is safe under
                              read-only lock.
                            </span>
                          ) : (
                            <span className="font-medium text-brand-charcoal/60">
                              Dataset is in an editable state. Always perform
                      ...
                      {currentDataset.status === "Production" && (
                        <button
                          onClick={() => {
                            if (
                              confirm(
                                `Unlock ${activeCountryKey} dataset and transition back to 'Draft' for calibration?`,
                              )
                            ) {
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="flex justify-between items-start gap-4 border-b border-border-main/50 pb-4 mb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-brand-charcoal/60">
                        Precision density analysis on the 10×10 Grid mapped from
                        Adrenaline vs Comfort indices. Hover cells to audit
                        clusters.
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-3">
                      <span className="text-[9px] uppercase tracking-wider font-extrabold text-[#75776B] block">
                        Interactive 10×10 Grid (Comfort y-axis vs Adrenaline
                        x-axis)
                      </span>
                      ...
                      <div className="space-y-3">
                        <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/50 space-y-2">
                          ...
                          <div className="flex justify-between items-center text-[10.5px]">
                            ...
                            <span className="font-mono font-black text-accent-teal">
                              {coveragePercentage}%
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/50 space-y-2 text-[10px]">
                          <h5 className="font-bold uppercase tracking-wider text-[8px] text-accent-teal">
                            Cluster & Jitter Analysis
                          </h5>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-mono font-bold">
                              {occupiedCellsCount > 0
                                ? (
                                    totalCurationsCount / occupiedCellsCount
                                  ).toFixed(2)
                                : "0"}{" "}
                              recs/cell
                            </span>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-mono font-bold">
                              {largestClusterSize} recommendations
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-brand-charcoal/60">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-semibold ${emptyCellsCount > 70 ? "text-accent-red" : "text-[#2E7D32]"}`}
                            >
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="space-y-1 border-b border-border-main/50 pb-4 mb-4">
                    ...
                    <p className="text-[10px] text-brand-charcoal/60">
                      Live structural, metadata, translation, and validation
                      diagnostics. Click any metric to filter affected
                      recommendations in the Explorer below.
                    </p>
                  </div>
                    ...
                    {[
                      {
                        id: "duplicate_ids",
                        label: "Duplicate IDs",
                        count: duplicateIdsCount,
                        desc: "Identical unique primary keys",
                        style:
                          duplicateIdsCount > 0
                      ...
                      },
                      {
                        ...
                        desc: "Coordinate values unassigned",
                        style:
                          missingCoordsCount > 0
                      ...
                      },
                      {
                        ...
                        desc: "Placeholder image or empty values",
                        style:
                          missingImagesCount > 0
                            ? "border-amber-500/20 bg-amber-500/5"
                            : "border-border-main",
                      },
                      {
                        ...
                        desc: "Missing Serbian (sr) or Chinese (zh) translations",
                        style:
                          missingTranslationsCount > 0
                            ? "border-amber-500/20 bg-amber-500/5"
                            : "border-border-main",
                      },
                      {
                        ...
                        label: "Missing Semantic Attributes",
                        count: missingAttributesCount,
                        desc: "No cost, time, mode, or duration metadata",
                        style:
                          missingAttributesCount > 0
                            ? "border-amber-500/20 bg-amber-500/5"
                            : "border-border-main",
                      },
                      {
                        ...
                        count: invalidCategoriesCount,
                        desc: "Category doesn't match standard list",
                        style:
                      ...
                      {
                        id: "coordinate_conflicts",
                        label: "Coordinate Conflicts",
                        count: coordinateConflictsCount,
                        desc: "Identical locations inside the Mood Orbit",
                        style:
                          coordinateConflictsCount > 0
                            ? "border-amber-500/20 bg-amber-500/5"
                            : "border-border-main",
                      },
                      {
                        ...
                        desc: "Out-of-bounds coordinates (must be -5 to 5)",
                        style:
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <span className="text-[10px] font-black text-brand-charcoal uppercase leading-tight">
                            {metric.label}
                          </span>
                          <span
                            className={`font-mono text-xs font-black ${metric.count > 0 ? "text-accent-red" : "text-[#2E7D32]"}`}
                          >
                            {metric.count}
                          </span>
                        </div>
                        <p className="text-[8.5px] text-brand-charcoal/50 font-sans mt-1">
                          {metric.desc}
                        </p>
                        ...
                        {metric.count > 0 ? (
                          <button
                            onClick={() => {
                              if (activeQualityFilter === metric.id) {
                                ...
                                // Smooth scroll down
                                const explorerEl =
                                  document.getElementById("oc-explorer-anchor");
                                if (explorerEl)
                                  explorerEl.scrollIntoView({
                                    behavior: "smooth",
                                  });
                          ...
                          >
                            {activeQualityFilter === metric.id
                              ? "✕ Clear Filter"
                              : "🔍 Link to affected ➔"}
                          </button>
                        ) : (
                          <span className="text-[7.5px] text-[#2E7D32] font-bold block mt-3">
                            ✔ ALL CLEAR
                          </span>
                        )}
                      </div>
                ...
                <div
                  id="oc-explorer-anchor"
                  className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border-main/50 pb-4 mb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-brand-charcoal/60">
                        Search, sort, and inspect active curation datasets.
                        Inspect metadata, status flags, and trace validation
                        errors directly.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="relative">
                        ...
                        />
                        <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-brand-charcoal/40 text-[10px]">
                          🔍
                        </span>
                      </div>
                      ...
                      {activeQualityFilter && (
                        <span className="bg-accent-teal/10 text-accent-teal border border-accent-teal/15 text-[8.5px] px-2 py-1 rounded-xl font-bold uppercase tracking-wider flex items-center gap-1.5">
                          ...
                          <button
                            onClick={() => setActiveQualityFilter(null)}
                            className="hover:text-accent-red font-mono"
                          >
                            ✕
                          </button>
                        </span>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                      ...
                      <table className="w-full text-left text-[9.5px] font-sans border-collapse divide-y divide-border-main select-none">
                        <thead>
                          <tr className="text-brand-charcoal/50 uppercase tracking-wider font-extrabold">
                            <th
                              className="pb-2.5 font-bold cursor-pointer hover:text-brand-charcoal"
                              onClick={() => {
                                setExplorerSortKey("id");
                                setExplorerSortOrder(
                                  explorerSortOrder === "asc" ? "desc" : "asc",
                                );
                              }}
                            >
                              ID{" "}
                            ...
                            <th
                              className="pb-2.5 font-bold cursor-pointer hover:text-brand-charcoal"
                              onClick={() => {
                                setExplorerSortKey("coordinates");
                                setExplorerSortOrder(
                                  explorerSortOrder === "asc" ? "desc" : "asc",
                                );
                              }}
                        ...
                        <tbody className="divide-y divide-border-main/40 text-brand-charcoal/80">
                          {sortedExplorerRecs.length === 0 ? (
                            <tr>
                              <td
                                colSpan={7}
                                className="py-6 text-center text-brand-charcoal/40 italic"
                              >
                                No recommendations found matching search/filter
                                constraints.
                              </td>
                            </tr>
                          ) : (
                            sortedExplorerRecs.map((rec) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="py-2.5">
                                    <span className="bg-brand-pearl text-[8px] font-bold tracking-widest uppercase px-1.5 py-0.5 rounded">
                                      {rec.category}
                                    </span>
                                  </td>
                                  <td className="py-2.5 font-mono">
                                    {typeof rec.coordinateX === "number"
                                      ? `(${rec.coordinateX}, ${rec.coordinateY})`
                                      : "Missing"}
                                  </td>
                                  ...
                                  <td className="py-2.5">
                                    {hasWarnings ? (
                                      <span className="text-[7.5px] uppercase font-black bg-amber-500/10 text-amber-700 px-1 py-0.5 rounded">
                                        Warning
                                      </span>
                                    ) : (
                                      <span className="text-[7.5px] uppercase font-black bg-[#E8F5E9] text-[#2E7D32] px-1 py-0.5 rounded">
                                        Certified
                                      </span>
                                    )}
                                  </td>
                    ...
                    <div
                      id="curation-inspector-panel-top"
                      className="bg-[#FAF9F5] p-5 rounded-2xl border border-border-main/50 space-y-4"
                    >
                      <span className="text-[9px] uppercase tracking-wider font-extrabold text-[#75776B] block">
                      ...
                      {inspectedRecId ? (
                        (() => {
                          const rec = filteredByCountry.find(
                            (r) => r.id === inspectedRecId,
                          );
                          if (!rec)
                            return (
                              <p className="text-[10px] text-brand-charcoal/50 italic">
                                Error locating curation record.
                              </p>
                            );

                          const score = getQualityScore(rec);
                          const meta = recMetadata[rec.id];
                          const historyForCuration =
                            coordinateHistory[rec.id] || [];

                          return (
                            <div className="space-y-3.5 text-left text-[10px]">
                              <div className="space-y-1">
                                <p className="text-[8px] font-mono font-bold text-accent-teal uppercase">
                                  ID: {rec.id}
                                </p>
                                <h4 className="text-sm font-serif font-black text-brand-charcoal">
                                  {rec.title}
                                </h4>
                                <p className="text-[8px] uppercase tracking-widest font-black text-brand-charcoal/50">
                                  {rec.category}
                                </p>
                              </div>
                              ...
                              <div className="border-t border-border-main/30 pt-2.5 space-y-1.5">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-mono font-bold">
                                    {rec.estimatedCost || "Not specified"}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-brand-charcoal/60">
                                  ...
                                  <span className="font-bold">
                                    {rec.preferredTransport || "Not specified"}
                                  </span>
                                </div>
                              </div>

                              <div className="border-t border-border-main/30 pt-2.5 space-y-1 text-left">
                                <p className="text-[8px] uppercase font-bold text-brand-charcoal/40">
                                  English Description Preview:
                                </p>
                                <p className="text-brand-charcoal/80 leading-relaxed text-[9.5px] italic">
                                  "{rec.shortDescription}"
                                </p>
                              </div>
                              ...
                              <div className="border-t border-border-main/30 pt-2.5 space-y-1.5 text-left text-[9px]">
                                ...
                                <div className="flex gap-2">
                                  <span
                                    className={`px-1.5 py-0.5 rounded font-bold uppercase ${rec.translations?.sr?.title ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-red-50 text-accent-red"}`}
                                  >
                                    sr translation
                                  </span>
                                  <span
                                    className={`px-1.5 py-0.5 rounded font-bold uppercase ${rec.translations?.zh?.title ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-red-50 text-accent-red"}`}
                                  >
                                    zh translation
                                  </span>
                                </div>
                              ...
                              <div className="border-t border-border-main/30 pt-2.5 space-y-1.5 text-left">
                                <p className="text-[8px] uppercase font-bold text-brand-charcoal/40">
                                  Coordinate Mutation Log:
                                </p>
                                {historyForCuration.length === 0 ? (
                                  <p className="text-[8.5px] text-brand-charcoal/40 italic">
                                    No previous coordinate mutations logged.
                                    Base alignment active.
                                  </p>
                                ) : (
                                  <div className="space-y-1.5 max-h-[120px] overflow-y-auto pr-1">
                                    {historyForCuration.map(
                                      (h: any, idx: number) => (
                                        <div
                                          key={idx}
                                          className="bg-white border border-border-main/50 p-2 rounded-lg space-y-1 text-[8.5px]"
                                        >
                                          ...
                                          <div className="flex items-center gap-1">
                                            ...
                                            <span className="font-mono font-bold text-accent-teal">
                                              ({h.current.x}, {h.current.y})
                                            </span>
                                          </div>
                                          <div className="text-brand-charcoal/70 italic">
                                            "{h.reason}"
                                          </div>
                                          <div className="text-[7px] text-brand-charcoal/40 font-mono font-bold">
                                            Scope: {h.datasetVersion}
                                          </div>
                                        </div>
                                      ),
                                    )}
                              ...
                              <div className="border-t border-border-main/30 pt-2.5 space-y-1.5 text-left text-[9px] bg-brand-pearl p-2.5 rounded-xl border border-border-main/30">
                                <p className="text-[8px] uppercase font-bold text-[#75776B] mb-1">
                                  Curation Metadata & Governance:
                                </p>

                                {/* CEMS STATUS SELECTOR CONSOLE */}
                                <div className="bg-white p-2 rounded-lg border border-border-main/50 space-y-1 mb-2">
                                  <span className="text-[7.5px] uppercase font-black text-brand-charcoal/50 block">
                                    CEMS Review & Approval Status:
                                  </span>
                                  <div className="flex items-center gap-1.5">
                                    <select
                                      value={
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setToastMessage(
                                          `Updated "${rec.title}" status to ${nextSt}. Audit logged.`,
                                        );
                                      }}
                                    ...
                                    >
                                      ...
                                      <option value="APPROVED">
                                        APPROVED (LIVE IN STREAM)
                                      </option>
                                      <option value="MERGE CANDIDATE">
                                        MERGE CANDIDATE
                                      </option>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-[8.5px]">
                                  <div>
                                    <span className="text-brand-charcoal/50 block">
                                      Lifecycle State:
                                    </span>
                                    <span className="font-bold text-brand-charcoal">
                                      {editorialStatuses[rec.id] || "CANDIDATE"}
                                  ...
                                  <div>
                                    ...
                                    <span
                                      className={`font-extrabold uppercase ${meta?.qaStatus === "Passed" ? "text-[#2E7D32]" : "text-accent-red"}`}
                                    >
                              ...
                              <div className="border-t border-border-main/30 pt-3 flex gap-2">
                                <button
                                  onClick={() => {
                                    ...
                                    showToast(
                                      `Loaded ${rec.title} inside Curations Manager`,
                                    );
                                  }}
                        ...
                        <div className="py-12 text-center text-brand-charcoal/40 italic text-[10px] space-y-1.5">
                          ...
                          <p className="text-[8.5px] font-sans font-normal text-brand-charcoal/30">
                            Click on any recommendation row to audit attributes,
                            translations, and missing fields.
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="space-y-1 border-b border-border-main/50 pb-4 mb-4">
                    ...
                    <p className="text-[10px] text-brand-charcoal/60">
                      Analytical priority queuing of under-curated areas inside
                      the Mood Orbit. Use these suggested vectors to expand our
                      Belgrade catalogue.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-[11px]">
                    <div className="space-y-3">
                      <span className="text-[9px] uppercase tracking-wider font-extrabold text-[#75776B] block">
                        Current Coverage Priority Queue
                      </span>
                      ...
                      <div className="space-y-2.5">
                        {potentialBlindSpots.map((bs, i) => (
                          <div
                            key={i}
                            className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/50 flex flex-col md:flex-row justify-between gap-3.5 items-start md:items-center"
                          >
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                <span className="font-mono font-black text-accent-teal">
                                ...
                                <span
                                  className={`text-[8px] font-mono font-black px-1.5 py-0.5 rounded ${bs.priority === "HIGH" ? "bg-red-50 text-accent-red border border-accent-red/10" : bs.priority === "MEDIUM" ? "bg-amber-50 text-amber-700 border border-amber-500/10" : "bg-gray-100 text-gray-600"}`}
                                >
                              ...
                              <p className="text-[8.5px] text-brand-charcoal/50 leading-snug">
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-[8.5px] uppercase font-bold text-brand-charcoal/40">
                                Est. Coverage Improvement
                              </p>
                              <p className="text-[14px] font-mono font-black text-[#2E7D32]">
                                {bs.improvement}
                              </p>
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setManId(
                                    `rec-gap-${Math.floor(100 + Math.random() * 900)}`,
                                  );
                                  setManLat(44.81 + bs.x / 100);
                                  setManLng(20.46 + bs.y / 100);
                                  setManLocation(
                                    "Belgrade (Under-curated vector)",
                                  );
                                  setActiveTab("curations");
                                  showToast(
                                    `Loaded blind spot vector ${bs.description} in manual curation editor.`,
                                  );
                                }}
                    ...
                    <div className="bg-[#FAF9F5] p-5 rounded-2xl border border-border-main/50 space-y-3.5 flex flex-col justify-between">
                      <div className="space-y-2">
                        <h4 className="font-bold uppercase tracking-wider text-[10px] text-[#2D2D2D]">
                          Algorithm Coverage Guidelines
                        </h4>
                        <p className="text-[10px] text-brand-charcoal/70 leading-relaxed font-sans">
                          Our predictive coverage index tracks the density and
                          average spacing between recommendations inside the
                          10x10 Mood Orbit. High-priority items indicate
                          quadrants where visitors receive fewer than 2 relevant
                          recommendation matches during extreme sensory
                          filtering (high action vs. maximum comfort).
                        </p>
                        <p className="text-[10px] text-brand-charcoal/70 leading-relaxed font-sans pt-1">
                          To maintain a flawless 100% EXPO coverage index,
                          future agency brokers must insert at least 3 new
                          year-round low-budget nature experiences in order to
                          secure robust digital recommendations.
                        </p>
                      </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="space-y-1 border-b border-border-main/50 pb-4 mb-4">
                    ...
                    <p className="text-[10px] text-brand-charcoal/60">
                      Review and verify database pipelines across expansion
                      territories for regional travel marketing.
                    </p>
                  </div>
                      ...
                      <tbody className="divide-y divide-border-main/40 text-brand-charcoal/80 font-medium">
                        <tr>
                          ...
                          <td className="py-3 font-mono">
                            {totalCurationsCount} locations
                          </td>
                          <td className="py-3 font-mono">
                            {coveragePercentage}%
                          </td>
                          <td className="py-3 font-mono font-black text-[#2E7D32]">
                            {avgQualityScore}%
                          </td>
                          <td className="py-3">
                            <span className="bg-[#E8F5E9] text-[#2E7D32] text-[8px] font-black px-1.5 py-0.5 rounded border border-[#2E7D32]/10 uppercase">
                              Certified
                            </span>
                          </td>
                        </tr>
                        {[
                          ...
                          <tr key={c.name} className="text-brand-charcoal/40">
                            ...
                            <td className="py-3">
                              <span className="bg-gray-100 text-gray-400 text-[8px] font-bold px-1.5 py-0.5 rounded border border-gray-200 uppercase">
                                PLANNED
                              </span>
                            </td>
                          </tr>
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="space-y-1 border-b border-border-main/50 pb-4 mb-4">
                    ...
                    <p className="text-[10px] text-brand-charcoal/60">
                      Permanent chronological record of dataset builds,
                      certification runs, and metadata repairs. Never
                      overwritten.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 max-h-[300px] overflow-y-auto no-scrollbar font-sans text-[10px]">
                    {validationHistory.map((val) => (
                      <div
                        key={val.id}
                        className="p-4 bg-[#FAF9F5] rounded-2xl border border-border-main/50 space-y-2"
                      >
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-border-main/20 pb-2">
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-accent-teal">
                              {new Date(val.date).toLocaleDateString()} at{" "}
                              {new Date(val.date).toLocaleTimeString()}
                            </span>
                            <span className="text-[8px] uppercase tracking-widest font-black bg-accent-teal/10 font-mono text-accent-teal px-1.5 py-0.5 rounded">
                              Engine {val.version}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 font-mono text-[9px]">
                            ...
                            <span
                              className={`ml-2 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${val.buildResult === "SUCCESS" || val.buildResult === "PASSED" ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-red-50 text-accent-red"}`}
                            >
                              {val.buildResult}
                            </span>
                          </div>
                        ...
                        <p className="text-brand-charcoal/80 leading-relaxed italic">
                          "{val.note}"
                        </p>
                        <div className="flex justify-between text-[8.5px] font-mono text-brand-charcoal/40">
                          <span>
                            Repair status: {val.issuesFixed} issues repaired
                          </span>
                ...
                <div className="bg-white p-6 rounded-3xl border border-border-main shadow-sm text-left">
                  <div className="space-y-1 border-b border-border-main/50 pb-4 mb-4">
                    ...
                    <p className="text-[10px] text-brand-charcoal/60">
                      Real-time memory, processing, and rendering telemetry.
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/40 text-left"
                      >
                        <p className="text-[8.5px] uppercase font-bold text-brand-charcoal/40">
                          {perf.label}
                        </p>
                        <p className="text-[12px] font-mono font-black text-brand-charcoal mt-1">
                          {perf.val}
                        </p>
                        {perf.change && (
                          <span className="text-[7.5px] font-mono text-[#2E7D32] font-semibold mt-0.5 block">
                            {perf.change} vs yesterday
                          </span>
                        )}
                      </div>
                    ))}
                ...
                <div className="bg-[#FAF9F5]/50 p-6 rounded-3xl border border-border-main/80 shadow-sm text-left relative">
                  <div className="space-y-1 border-b border-border-main/40 pb-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[8px] bg-brand-charcoal/10 text-brand-charcoal/60 px-2 py-0.5 rounded font-black uppercase">
                        V2 BACKLOG
                      </span>
                    </div>
                    <p className="text-[10px] text-brand-charcoal/50">
                      Expandable analytical tools in development for the active
                      EXPO campaign. Locked until next integration sprint.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {[
                      {
                        name: "Heatmaps",
                        desc: "Continuous spatial density contours mapped onto map visuals",
                      },
                      {
                        name: "User Behavior Analytics",
                        desc: "Privacy-preserving local clickstream and session conversion logging",
                      },
                      {
                        name: "Country Expansion Progress",
                        desc: "Real-time sync progress loops for planned regional datasets",
                      },
                      {
                        name: "Image Quality Audits",
                        desc: "AI-assisted resolution, format, and aspect checking",
                      },
                      {
                        name: "Translation Completion",
                        desc: "Multilingual parity counters for emerging translation locales",
                      },
                      {
                        name: "Seasonal Coverage",
                        desc: "Automatic balancing of winter vs summer adventure options",
                      },
                      {
                        name: "Accessibility Audits",
                        desc: "Physical and sensory ease rating parity metrics",
                      },
                    ].map((m, i) => (
                      <div
                        key={i}
                        className="bg-white/40 p-4 rounded-2xl border border-border-main/30 flex flex-col justify-between items-start opacity-70"
                      >
                        <div className="space-y-1 w-full">
                          <div className="flex justify-between items-center">
                            <h4 className="text-[10px] font-black text-brand-charcoal/60 uppercase">
                              {m.name}
                            </h4>
                            <span className="text-[8px] font-mono font-bold text-brand-charcoal/40">
                              🔒 DISABLED
                            </span>
                          </div>
                          <p className="text-[8.5px] text-brand-charcoal/50 leading-relaxed mt-1">
                            {m.desc}
                          </p>
                        </div>
                      </div>
        ...
        {activeTab === "diagnostics" && (
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide">
                Diagnostics & Privacy Audits
              </h3>
              <p className="text-[10px] text-brand-charcoal/60">
                Tracking operational logs, exceptions, and reviewing our
                zero-leakage compliance structure.
              </p>
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3.5 shadow-sm">
              <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal/80 flex items-center gap-1.5">
                <ShieldCheck size={14} className="text-accent-teal" /> GDPR &
                Platform Guidelines Alignment
              </h4>
              ...
              <div className="grid gap-2 text-[10px]">
                <div className="bg-[#FAF9F5] p-3 rounded-xl border border-border-main/50 flex gap-3.5 items-start">
                  ...
                  <div>
                    <p className="font-bold text-brand-charcoal">
                      Zero-Identity Policy Alignment
                    </p>
                    <p className="text-[9px] text-brand-charcoal/60 leading-snug mt-1">
                      No phone, contacts, name accounts, or cross-site tokens
                      exist inside any storage variable.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-[#FAF9F5] p-3 rounded-xl border border-border-main/50 flex gap-3.5 items-start">
                  ...
                  <div>
                    <p className="font-bold text-brand-charcoal">
                      Apple Platform Privacy Guidelines Alignment
                    </p>
                    <p className="text-[9px] text-brand-charcoal/60 leading-snug mt-1">
                      Does not access IDFA advertising tokens, locations, or
                      perform background network scraping.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-[#FAF9F5] p-3 rounded-xl border border-border-main/50 flex gap-3.5 items-start">
                  ...
                  <div>
                    <p className="font-bold text-brand-charcoal">
                      Google Play Privacy Policy Alignment
                    </p>
                    <p className="text-[9px] text-brand-charcoal/60 leading-snug mt-1 font-mono">
                      No telemetry packets map to device serial, hardware finger
                      print, or MAC trackers.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3 shadow-sm">
              <div className="flex justify-between items-center">
                <h4 className="text-[10.5px] font-black uppercase tracking-widest text-brand-charcoal/60">
                  System Logs & Exception Audits
                </h4>
                <button
                  onClick={() => {
              ...
              <div className="space-y-2 max-h-[220px] overflow-y-auto no-scrollbar font-mono text-[9px] leading-relaxed">
                {(metrics.localRaw.errorLogs || [])
                  .slice()
                  .reverse()
                  .map((log, i) => (
                    <div
                      key={i}
                      className="p-3 bg-[#FAF9F5] rounded-xl border border-border-main/50 flex flex-col gap-1 text-brand-charcoal/70"
                    >
                      <div className="flex justify-between font-bold">
                        <span className="text-brand-charcoal uppercase">
                          [{log.code}]
                        </span>
                        <span>
                      ...
                      <span className="text-brand-charcoal/90">
                        {log.message}
                      </span>
                    </div>
                  ))}
        ...
        {activeTab === "simulator" && (
          <div className="space-y-5 animate-fade-in">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide font-black">
                Live Signal Simulator
              </h3>
              <p className="text-[10px] text-brand-charcoal/60">
                Inject live, mock and actual marketing events instantly into the
                database engine. Watch conversion ratios, funnel graphs, and
                metrics adapt in real-time.
              </p>
            </div>

            {/* Campaign Injects buttons card */}
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm">
              <h4 className="text-[10px] uppercase font-mono tracking-widest text-brand-charcoal/50 font-black">
                Trigger Marketing Signals
              </h4>

              <div className="grid gap-3.5 pt-1.5">
                {/* QR placements loops */}
                <div className="space-y-2">
                  <p className="text-[9px] uppercase tracking-wider font-extrabold text-brand-charcoal/60">
                    Simulate Incoming QR Scans
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    {PARTNERS.map((partner) => (
                      <button
                      ...
                      >
                        <span className="truncate pr-1">
                          ✦ {partner.label.split(" ")[0]}
                        </span>
                        <Play
                          size={10}
                          className="text-accent-teal opacity-60 group-hover:opacity-100 shrink-0 transform translate-x-0 group-active:translate-x-1 transition-transform"
                        />
                      </button>
                    ))}
                  </div>
        ...
        {activeTab === "curations" && (
          <div className="space-y-6 animate-fade-in text-left">
            <div className="space-y-1">
              <h3 className="font-serif text-sm font-black text-accent-teal uppercase tracking-wide">
                Dynamic Belgrade Curations Manager
              </h3>
              <p className="text-[10px] text-brand-charcoal/60 font-medium">
                Register, parse, edit, and audit new recommended locations (up
                to 50 max). Fully validated metadata guarantees a seamless
                integration into existing lists, offline storage, travel time,
                match vectors, and interactive map coordinate networks.
              </p>
            </div>
            ...
            <div className="p-4 bg-white border border-border-main rounded-2xl flex flex-wrap gap-4 justify-between items-center text-[10px]">
              <div className="flex gap-4 flex-wrap text-left">
                <div>
                  <span className="font-bold text-brand-charcoal/60 uppercase text-[8.5px] tracking-wider block">
                    Active Locations
                  </span>
                  <span className="font-mono text-accent-teal font-black text-xs">
                    {allRecommendations.length} positions
                  </span>
                </div>
                <div>
                  <span className="font-bold text-brand-charcoal/60 uppercase text-[8.5px] tracking-wider block">
                  ...
                  <span className="font-mono text-brand-charcoal font-bold text-xs">
                    {customRecommendations.length} items
                  </span>
                </div>
                <div>
                  <span className="font-bold text-brand-charcoal/60 uppercase text-[8.5px] tracking-wider block">
                  ...
                  <span className="font-mono text-brand-charcoal font-bold text-xs">
                    {Object.keys(modifiedRecommendations).length} active
                  </span>
                </div>
                <div>
                  <span className="font-bold text-accent-red/70 uppercase text-[8.5px] tracking-wider block">
                    Suspended / Hidden
                  </span>
                  <span className="font-mono text-accent-red/90 font-bold text-xs">
                    {deletedRecommendationIds.length} cards
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white border border-border-main rounded-3xl overflow-hidden shadow-sm">
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="text-[11px] font-black uppercase tracking-widest text-[#2D2D2D] leading-none">
                      📖 Administrator Image Management Guide
                    </h4>
                    <p className="text-[8.5px] text-brand-charcoal/50 font-sans mt-1">
                      Interactive walkthrough for custom branding and dynamic
                      curations visual assets.
                    </p>
                  </div>
                </div>
                  ...
                  >
                    <div className="p-5 space-y-4 font-sans divide-y divide-border-main/40">
                      ...
                      <div className="space-y-2.5 pb-3.5">
                        ...
                        <h5 className="font-extrabold text-brand-charcoal uppercase tracking-wider text-[10px]">
                          Welcome/Language Selection Card Custom Branding
                        </h5>
                        <p className="text-brand-charcoal/70 leading-normal">
                          This action enables administrators to replace the
                          default suitcase suitcase-styled landing SVG graphic
                          with a tailor-made promotional image or logo.
                        </p>
                        <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/50 space-y-2">
                          <div className="flex gap-2 items-start">
                            ...
                            <p>
                              <strong>Locate Setup Controls:</strong> Click the{" "}
                              <strong>Tech Ref</strong> tab in the navigation
                              bar above.
                            </p>
                          </div>
                          <div className="flex gap-2 items-start">
                            ...
                            <p>
                              <strong>Upload File:</strong> Scroll to the{" "}
                              <strong>General Setup & Custom Branding</strong>{" "}
                              section and click{" "}
                              <strong>📤 Upload Card Image</strong>. Files are
                              compressed to Base64 and stored with low latency.
                            </p>
                          </div>
                          <div className="flex gap-2 items-start">
                            <span className="font-bold text-accent-teal">
                            ...
                            <p>
                              <strong>Instant Preview:</strong> Once uploaded, a
                              preview is shown in the Admin Panel and the
                              central onboarding graphic updates in real-time.
                            </p>
                          </div>
                          <div className="flex gap-2 items-start">
                            <span className="font-bold text-accent-teal">
                            ...
                            <p>
                              <strong>Revert Changes:</strong> Click{" "}
                              <strong>🗑️ Reset to Default</strong> next to the
                              preview to instantly restore the Belgrade suitcase
                              SVG.
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2.5 pt-4">
                        ...
                        <h5 className="font-extrabold text-brand-charcoal uppercase tracking-wider text-[10px]">
                          Custom Belgrade Curations Visual Asset Management
                        </h5>
                        <p className="text-brand-charcoal/70 leading-normal">
                          Replace individual recommendation cover photos with
                          custom photography or branded materials, with a
                          seamless fallback to the generator system.
                        </p>
                        <div className="bg-[#FAF9F5] p-3.5 rounded-2xl border border-border-main/50 space-y-2">
                          <div className="flex gap-2 items-start">
                            <span className="font-bold text-accent-teal">
                            ...
                            <p>
                              <strong>Select Recommendation Target:</strong> In
                              the <strong>Curations Manager</strong>, choose{" "}
                              <strong>Ingest Card</strong> to create a new
                              location, or <strong>Modify Listing</strong> to
                          ...
                          <div className="flex gap-2 items-start">
                            ...
                            <p>
                              <strong>Attach Visual Asset:</strong> Scroll down
                              to the <strong>Visual Asset Image</strong> field
                              and choose <strong>📤 Upload Image</strong> to
                              capture your image.
                            </p>
                          </div>
                          <div className="flex gap-2 items-start">
                            ...
                            <p>
                              <strong>Clear Custom Image:</strong> If the
                              recommendation has a custom uploaded graphic, a
                              trash-can <strong>🗑️ Reset</strong> button is
                              dynamically rendered next to the field. Click this
                              button to instantly restore IDEMO's premium
            ...
            <div className="space-y-2">
              <label className="text-[8.5px] uppercase tracking-widest font-extrabold text-[#75776B] block">
                ...
                <button
                  ...
                  className={`py-2 px-1 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center justify-center gap-1 ${
                    curationAction === "add"
                      ...
                      : "text-brand-charcoal/50 hover:bg-white"
                  }`}
                >
                  ➕ Ingest Card
                </button>
            ...
            {(curationAction === "modify" || curationAction === "delete") && (
              <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3 shadow-sm text-left">
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#2D2D2D] block">
                      Select Registry Target
                    </span>
                    <span className="text-[8px] font-mono text-brand-charcoal/40 uppercase">
                      Search active dynamic lists
                    </span>
                  </div>
                  <select
                  ...
                  >
                    <option value="">
                      -- Choose dynamic curation position to target --
                    </option>
            ...
            {curationAction === "delete" &&
              selectedCurationId &&
              (() => {
                const target = allRecommendations.find(
                  (r) => r.id === selectedCurationId,
                );
                ...
                return (
                  <div className="bg-white p-5 rounded-3xl border border-accent-red/25 bg-accent-red/[0.01] space-y-4 shadow-sm animate-fade-in relative overflow-hidden text-left">
                    ...
                    <div>
                      <span className="text-[9px] uppercase tracking-widest font-black bg-accent-red/10 text-accent-red px-2 py-0.5 rounded-full inline-block font-mono mb-2">
                        Exclusion Block
                      </span>
                      <h4 className="font-serif text-sm font-black text-brand-charcoal leading-tight">
                        Confirm Exclusion for "{target.title}"?
                      </h4>
                      <p className="text-[9.5px] text-brand-charcoal/60 mt-1 font-sans">
                        This item will be safely suspended/hidden from active
                        Belgrade tourist itineraries, category feeds, route
                        builders, and map coordinate markers.
                      </p>
                    </div>
                    ...
                    <div className="p-3 bg-[#FAF9F5] rounded-xl border border-border-main/50 text-[9.5px] text-brand-charcoal/70 flex gap-3.5 items-center">
                      ...
                      <div className="text-left min-w-0 flex-1">
                        <p className="font-bold text-brand-charcoal truncate">
                          {target.title}{" "}
                          <span className="text-[8px] uppercase tracking-wider font-mono bg-accent-teal/10 text-accent-teal px-1 py-0.2 rounded">
                            {target.scoreMatch || target.category}
                          </span>
                        </p>
                        <p className="text-[8px] text-brand-charcoal/40 font-mono mt-0.5 truncate">
                          ID: {target.id} • Registered Area: {target.location}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#FAF9F5] p-4 rounded-2xl border border-border-main/50 space-y-2 text-left">
                      <span className="text-[9px] font-extrabold uppercase tracking-wider text-brand-charcoal/80 block">
                        Exclusion Safety Verification:
                      </span>
                      {diagnosticsList.every((d) => d.isPassed) ? (
                        <div className="text-[9px] font-semibold text-accent-teal flex items-center gap-1.5">
                          ✓ All safety standards verified. Excluding this card
                          does not compromise core offline routing vectors.
                        </div>
                      ) : (
            ...
            {(curationAction === "add" ||
              (curationAction === "modify" && selectedCurationId)) && (
              <div className="space-y-4">
                  ...
                  <div className="space-y-4 animate-fade-in text-left">
                    <div className="bg-white p-5 rounded-3xl border border-border-main space-y-3 shadow-sm text-left">
                      <div className="flex justify-between items-center rounded-xl">
                        ...
                        >
                          {templateCopied ? (
                            <span className="text-accent-teal flex items-center gap-0.5">
                              <Check size={9} /> COPIED!
                            </span>
                          ) : (
                            <span className="flex items-center gap-0.5">
                              <Clipboard size={9} /> COPY PATTERN
                            </span>
                          )}
                        </button>
                      </div>
                      <p className="text-[9.5px] text-brand-charcoal/60 leading-normal font-sans">
                        Generate safe, structured curations in any AI model
                        (such as Gemini 1.5 Pro) with strict metadata alignment.
                        Copy this pattern directly:
                      </p>
                      <div className="bg-[#FAF9F5] p-3 rounded-xl border border-border-main/50 text-[8.5px] font-mono text-brand-charcoal/60 leading-relaxed overflow-x-auto whitespace-pre max-h-36 select-all text-left">
                        This is new curation or Recommendation.{"\n"}
                        Please compile a premium curation about Serbia/Belgrade
                        in strict JSON block.
                      </div>
                    </div>

                    <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm text-left">
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#2D2D2D] block">
                        Ingestion Terminal Input
                      </span>
                      <div className="space-y-1.5 text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60 block text-left">
                          Pasted AI Output Console:
                        </label>
                        <textarea
                          rows={6}
                          value={aiText}
                      ...
                      {aiValidationFeedback.msg && (
                        <div
                          className={`p-3 rounded-xl border flex gap-2 items-start text-[9.5px] leading-snug transition-all ${
                        ...
                        >
                          ...
                          <div className="text-left">
                            ...
                            <p className="mt-0.5 font-sans whitespace-pre-line">
                              {aiValidationFeedback.msg}
                            </p>
                          </div>
                        </div>
                    ...
                    {aiParsedRec && (
                      <div className="space-y-2 text-left">
                        <p className="text-[9px] uppercase tracking-widest font-extrabold text-[#75776B] px-1 flex items-center gap-1">
                          <Eye size={11} /> Realtime Visitor Layout Preview
                        </p>
                        <div className="bg-white rounded-[32px] overflow-hidden border border-border-main p-4 space-y-3.5 shadow-sm">
                          ...
                          <div className="space-y-1.5 px-1">
                            <div className="flex justify-between items-baseline gap-2">
                              <h4 className="text-sm font-serif font-black text-brand-charcoal leading-tight text-left">
                                {aiParsedRec.title}
                              </h4>
                              <span className="font-mono text-[9px] text-[#8A1F1F] font-bold shrink-0">
                                {aiParsedRec.estimatedCost}
                              </span>
                            </div>
                            ...
                            <p className="text-[10px] text-brand-charcoal/70 leading-relaxed pt-1 font-sans text-left">
                              {aiParsedRec.shortDescription}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                {(curationAction === "modify" ||
                  (curationAction === "add" && curationMode === "manual")) && (
                  <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm text-[10.5px] animate-fade-in text-left">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#2D2D2D] block mb-1">
                      Curation Ingestion Form
                    </span>

                    <div className="grid grid-cols-2 gap-3 font-sans w-full">
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60 text-left block">
                          Unique Code Slug
                        </label>
                        <input
                          type="text"
                    ...
                    <div className="grid grid-cols-2 gap-3 font-sans w-full">
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-[#75776B] text-left block">
                          Visual Asset Image
                        </label>
                        <select
                          value={manImage}
                          onChange={(e) => setManImage(e.target.value)}
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[9.5px] focus:outline-none focus:border-accent-teal"
                        >
                          ...
                          {isCustomImage && (
                            <button
                              ...
                              onClick={() => {
                                setManImage("dynamic_generate");
                                showToast(
                                  "Visual asset reset to auto-generation.",
                                );
                              }}
                      ...
                      <div className="space-y-1 font-medium text-left">
                        ...
                        <select
                          ...
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[9.5px] focus:outline-none font-bold"
                        >
                          <option value="Gastronomy">Gastronomy</option>
                          <option value="Nature">Nature</option>
                          <option value="History">History</option>
                    ...
                    <div className="space-y-1 font-medium text-left">
                      <div className="flex justify-between items-baseline">
                        ...
                        <span className="text-[8px] font-mono text-brand-charcoal/40 font-bold">
                          {wordCount(manShortDesc)} words / 8-75 max
                        </span>
                      </div>
                      <textarea
                        rows={2}
                        ...
                        placeholder="Summarize experience for visitor preview (~50 words)..."
                        className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px] focus:outline-none focus:border-accent-teal resize-none leading-relaxed text-left"
                      />
                    </div>
                    ...
                    <div className="space-y-1 font-medium text-left">
                      <div className="flex justify-between items-baseline">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60 block text-left">
                          English Detailed Long Description *
                        </label>
                        <span className="text-[8px] font-mono text-brand-charcoal/40 font-bold">
                          {wordCount(manLongDesc)} words / 20-250 max
                        </span>
                      </div>
                      <textarea
                        rows={3}
                        ...
                        placeholder="Full traveler guidelines, history, timing (up to 200 words)..."
                        className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px] focus:outline-none resize-none leading-relaxed text-left"
                      />
                    </div>

                    <div className="grid grid-cols-3 gap-2 font-sans text-[10px] w-full">
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8px] uppercase tracking-wider font-black text-brand-charcoal/50 block text-left">
                          Mins Driving *
                        </label>
                        <input
                          type="number"
                          value={manTravelTimeMins}
                          onChange={(e) =>
                            setManTravelTimeMins(Number(e.target.value))
                          }
                          className="w-full p-2 rounded-lg border border-border-main bg-[#FAF9F5] font-mono focus:outline-none"
                        />
                      </div>
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8px] uppercase tracking-wider font-black text-brand-charcoal/50 block text-left">
                    ...
                    <div className="grid grid-cols-2 gap-3 font-sans text-[10px] w-full">
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-[#75776B] block text-left">
                          Mood Orbit Coord X *
                        </label>
                        <input
                          type="number"
                          ...
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] font-mono focus:outline-none focus:border-accent-teal"
                          placeholder="e.g. -1.5 (Comfort)"
                        />
                        <span className="text-[7.5px] text-brand-charcoal/40 font-mono block leading-tight">
                          Comfort (negative) vs. Adrenaline (positive)
                        </span>
                      </div>
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-[#75776B] block text-left">
                        ...
                        <input
                          ...
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] font-mono focus:outline-none focus:border-accent-teal"
                          placeholder="e.g. 2.5 (Action)"
                        />
                        <span className="text-[7.5px] text-brand-charcoal/40 font-mono block leading-tight">
                          Serenity (negative) vs. Action (positive)
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 font-sans w-full">
                      <div className="space-y-1 font-medium text-left">
                        ...
                        <input
                          ...
                          placeholder="e.g. Dorćol"
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px]"
                        />
                      </div>
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60 block text-left">
                    ...
                    <div className="grid grid-cols-2 gap-3 font-sans w-full">
                      <div className="space-y-1 font-medium text-left">
                        ...
                        <input
                          type="text"
                          value={manTransport}
                          onChange={(e) => setManTransport(e.target.value)}
                          placeholder="e.g. Taxi / Bus"
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px]"
                        />
                      </div>
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/60 block text-left">
                          Equivalent World Spot
                        </label>
                        <input
                          type="text"
                          value={manEquivalent}
                          onChange={(e) => setManEquivalent(e.target.value)}
                          placeholder="e.g. Clerkenwell (London)"
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px]"
                        />
                      </div>
                    </div>
                    ...
                    <div className="border-t border-border-main/50 pt-3 space-y-3 font-sans w-full text-left">
                      <span className="text-[9.5px] font-black uppercase tracking-widest text-[#75776B] flex items-center gap-1 block text-left">
                        🗺 Serbian Translation Override (Optional)
                      </span>

                      <div className="grid grid-cols-2 gap-3 w-full">
                        <div className="space-y-1 font-medium text-left">
                          <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/50 block text-left">
                            Serbian Curation Title
                          </label>
                          <input
                            type="text"
                            value={manSrTitle}
                            onChange={(e) => setManSrTitle(e.target.value)}
                            placeholder="e.g. Silosi Beograd"
                            className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px]"
                          />
                        </div>
                        <div className="space-y-1 font-medium text-left">
                          ...
                          <input
                            type="text"
                            value={manSrLocation}
                            onChange={(e) => setManSrLocation(e.target.value)}
                            placeholder="e.g. Dorćol"
                            className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px]"
                          />
                        </div>
                      ...
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/55 block text-left">
                          Serbian Short Description
                        </label>
                        <textarea
                          rows={1}
                          value={manSrShortDesc}
                          onChange={(e) => setManSrShortDesc(e.target.value)}
                          placeholder="Kratak opis na srpskom..."
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px] resize-none text-left"
                        />
                      </div>
                      <div className="space-y-1 font-medium text-left">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-brand-charcoal/55 block text-left">
                          Serbian Long Description
                        </label>
                        <textarea
                          rows={2}
                          value={manSrLongDesc}
                          onChange={(e) => setManSrLongDesc(e.target.value)}
                          placeholder="Detaljan opis na srpskom..."
                          className="w-full p-2.5 rounded-xl border border-border-main bg-[#FAF9F5] text-[10px] resize-none text-left"
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-[#FAF9F5] p-5 rounded-3xl border border-border-main/50 space-y-3 font-sans w-full text-left">
                      <div className="flex justify-between items-center text-[10px]">
                        ...
                        {diagnosticsList.every((d) => d.isPassed) ? (
                          ...
                          <span className="text-[8px] uppercase font-black tracking-wider bg-accent-red/10 text-accent-red px-2 py-0.5 rounded-full border border-accent-red/15 font-mono">
                            ● PENDING (
                            {diagnosticsList.filter((d) => !d.isPassed).length}{" "}
                            errors)
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-2 max-h-[160px] overflow-y-auto no-scrollbar pt-1 w-full text-left">
                        {diagnosticsList.map((d) => (
                          <div
                            key={d.id}
                            className="p-2.5 rounded-xl bg-white border border-border-main/40 flex items-start gap-2.5 text-[9px] font-sans w-full loading-normal"
                          >
                            <span
                              className={`text-[10px] font-bold shrink-0 ${d.isPassed ? "text-accent-teal" : "text-accent-red"}`}
                            >
                              {d.isPassed ? "✓" : "✗"}
                            </span>
                            <div className="space-y-0.5 text-left min-w-0 flex-1">
                              ...
                              <p className="text-brand-charcoal/50 font-mono tracking-tight text-[8px] truncate">
                                {d.rule}
                              </p>
                              {!d.isPassed && (
            ...
            <div className="bg-white p-5 rounded-3xl border border-border-main space-y-4 shadow-sm font-sans text-left">
              <div className="flex justify-between items-center text-xs">
                <span className="text-[10.5px] font-black uppercase tracking-widest text-[#2D2D2D] block">
                  Currently Active Belgrade Recommendations
                </span>
              ...
              {allRecommendations.length === 0 ? (
                <div className="py-8 text-center text-brand-charcoal/40 italic flex flex-col items-center gap-1 text-[10px] text-left">
                  ...
                  <span className="text-[9px] font-sans font-normal text-brand-charcoal/30 font-bold">
                    Press Restore Factory Defaults above to instantly reload
                    basic Belgrade curations!
                  </span>
                </div>
              ) : (
                <div className="divide-y divide-border-main/50 max-h-[300px] overflow-y-auto no-scrollbar font-sans text-left">
                  {allRecommendations.map((rec) => {
                    const isCustom = customRecommendations.some(
                      (cr) => cr.id === rec.id,
                    );
                    ...
                    return (
                      <div
                        key={rec.id}
                        className="py-3 flex justify-between items-center gap-2 text-left"
                      >
                        <div className="min-w-0 flex-1 text-left">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[8px] uppercase tracking-widest font-black bg-accent-teal/10 font-mono text-accent-teal px-1.5 py-0.5 rounded-full">
                              {rec.category}
                            </span>
                            {isCustom && (
                          ...
                          <p className="text-[9px] text-brand-charcoal/50 uppercase tracking-widest font-mono mt-1 text-left">
                            ID: {rec.id} • 📍 {rec.location} • 🧭 {rec.duration}
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <button
      ...
      {compareTargetId &&
        (() => {
          const candidate = allRecommendations.find(
            (r) => r.id === compareTargetId,
          );
          if (!candidate) return null;

          // Auto-select first non-draft comparison target if none selected
          const nonDraftRecommendations = allRecommendations.filter(
            (r) => !r.id.startsWith("draft-") && r.id !== compareTargetId,
          );
          const activeSelectedId =
            compareSelectionId || nonDraftRecommendations[0]?.id || "";
          const targetRec = allRecommendations.find(
            (r) => r.id === activeSelectedId,
          );

          // Compute similarity metrics
          const dx =
            (candidate.coordinateX ?? 0) - (targetRec?.coordinateX ?? 0);
          const dy =
            (candidate.coordinateY ?? 0) - (targetRec?.coordinateY ?? 0);
          ...
          if (
            ...
            targetRec?.coordinates?.lat &&
            targetRec?.coordinates?.lng
          ) {
            const R = 6371; // km
            const dLat =
              ((targetRec.coordinates.lat - candidate.coordinates.lat) *
                Math.PI) /
              180;
            const dLon =
              ((targetRec.coordinates.lng - candidate.coordinates.lng) *
                Math.PI) /
              180;
            const a =
              Math.sin(dLat / 2) * Math.sin(dLat / 2) +
              Math.cos((candidate.coordinates.lat * Math.PI) / 180) *
                Math.cos((targetRec.coordinates.lat * Math.PI) / 180) *
                Math.sin(dLon / 2) *
                Math.sin(dLon / 2);
          ...
          const handleResolveComparison = (
            nextStatus:
              "APPROVED" | "NEEDS RESEARCH" | "MERGE CANDIDATE" | "RETIRED",
          ) => {
            ...
            setToastMessage(
              `Candidate "${candidate.title}" status updated to ${nextStatus}. governance audit ledger recorded.`,
            );
          };
            ...
            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-brand-charcoal/40 backdrop-blur-sm overflow-y-auto no-scrollbar font-sans text-left">
              ...
              >
                ...
                <div className="flex justify-between items-start border-b border-border-main/50 pb-3">
                  <div className="space-y-0.5">
                    <span className="text-[8px] uppercase tracking-[0.2em] font-black text-brand-charcoal/50 block">
                      CEMS Verification Tools
                    </span>
                    <h3 className="font-serif text-sm font-black text-brand-charcoal uppercase tracking-wider">
                      Cannibalization & Spatial Similarity Desk
                    </h3>
                  </div>
                  <button
                ...
                <div className="flex-1 overflow-y-auto pr-1 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-brand-pearl/30 border border-border-main/50 p-4 rounded-2xl space-y-3">
                      <div className="flex justify-between items-center border-b border-border-main/30 pb-2">
                        <span className="text-[8px] font-black uppercase tracking-wider text-accent-teal">
                          New Candidate
                        </span>
                      ...
                      <div className="space-y-1">
                        <span className="font-mono text-[8px] text-brand-charcoal/40 block">
                          ID: {candidate.id}
                        </span>
                        <h4 className="font-serif text-xs font-black text-brand-charcoal uppercase tracking-wide">
                          {candidate.title}
                        </h4>
                      ...
                      <div className="text-[10px] space-y-2 text-brand-charcoal/80 leading-relaxed font-medium">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
                          <div className="bg-white border border-border-main/30 p-2 rounded-xl">
                            <span className="text-brand-charcoal/40 block uppercase text-[7.5px] font-bold">
                              Location
                            </span>
                        ...
                        {candidate.coordinates ? (
                          <div className="bg-white border border-border-main/30 p-2 rounded-xl text-[9px] font-mono">
                            ...
                            <span className="text-brand-charcoal font-bold">
                              lat: {candidate.coordinates.lat}, lng:{" "}
                              {candidate.coordinates.lng}
                            </span>
                          </div>
                        ) : (
                    ...
                    <div className="bg-[#FAF9F5]/40 border border-border-main/50 p-4 rounded-2xl space-y-3">
                      <div className="flex justify-between items-center border-b border-border-main/30 pb-2">
                        <span className="text-[8px] font-black uppercase tracking-wider text-brand-charcoal/60">
                          Compare With Live/Approved Curation:
                        </span>
                        <span className="bg-green-100 text-green-800 text-[8px] font-mono px-2 py-0.5 rounded-full uppercase font-black">
                          {targetRec
                            ? editorialStatuses[targetRec.id] || "APPROVED"
                            : "—"}
                        </span>
                      </div>

                      <div className="space-y-1.5">
                        <label className="text-[8px] font-black uppercase tracking-wider text-brand-charcoal/50 block">
                          Select Target Curation to Compare:
                        </label>
                        <select
                          value={activeSelectedId}
                          onChange={(e) =>
                            setCompareSelectionId(e.target.value)
                          }
                      ...
                      {targetRec ? (
                          ...
                          <div className="text-[10px] space-y-2 text-brand-charcoal/80 leading-relaxed font-medium">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
                              ...
                              <div className="bg-white border border-border-main/30 p-2 rounded-xl">
                                ...
                                <span className="text-brand-charcoal font-bold">
                                  X: {targetRec.coordinateX ?? "—"}, Y:{" "}
                                  {targetRec.coordinateY ?? "—"}
                                </span>
                              </div>
                            </div>
                            ...
                            {targetRec.coordinates && (
                              <div className="bg-white border border-border-main/30 p-2 rounded-xl text-[9px] font-mono">
                                ...
                                <span className="text-brand-charcoal font-bold">
                                  lat: {targetRec.coordinates.lat}, lng:{" "}
                                  {targetRec.coordinates.lng}
                                </span>
                              </div>
                            )}
                        ...
                        <div className="py-8 text-center text-brand-charcoal/40 italic">
                          Select a curation target from the list above to begin
                          comparison.
                        </div>
                      )}
                    </div>
                  ...
                  {targetRec && (
                    <div className="bg-brand-pearl p-4 rounded-2xl border border-border-main/50 grid grid-cols-1 md:grid-cols-3 gap-4 text-[10px] leading-relaxed">
                      <div className="space-y-1">
                        <span className="font-extrabold uppercase text-[8px] tracking-wider text-[#75776B] block">
                          Mood Coordinates proximity:
                        </span>
                        <p className="font-medium text-brand-charcoal">
                          Distance:{" "}
                          <strong className="font-mono text-accent-teal text-xs font-black">
                            {moodDistance?.toFixed(3)}
                          </strong>{" "}
                          units.
                        </p>
                        <span className="text-[8px] text-brand-charcoal/50 block">
                          Coordinates reflect spatial affinity in mood profile.
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="font-medium text-brand-charcoal">
                          {physicalDistanceKm ? (
                            ...
                            <span className="text-brand-charcoal/40 italic">
                              Geodetic range uncomputable (missing GPS)
                            </span>
                          )}
                        </p>
                        <span className="text-[8px] text-brand-charcoal/50 block">
                          Haversine geodetic calculation across Belgrade region.
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="font-medium text-brand-charcoal">
                          Categories:{" "}
                          <strong className="font-mono text-brand-charcoal">
                            {candidate.category === targetRec.category
                              ? "MATCHING (OVERLAP)"
                              : "DISTINCT"}
                          </strong>
                        </p>
                        <span className="text-[8px] text-brand-charcoal/50 block">
                          Shared target audience and visitor motivation signals.
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-[#FAF9F5] border border-border-main/50 p-4 rounded-2xl space-y-4">
                    <div>
                      <span className="text-[8px] uppercase tracking-[0.15em] font-black text-[#75776B] block">
                        Cannibalization Evaluation Rules:
                      </span>
                      <h5 className="font-serif text-xs font-black text-brand-charcoal uppercase tracking-wider mt-0.5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[9px] text-brand-charcoal/70 font-medium">
                      <label className="flex items-start gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          defaultChecked
                          className="mt-0.5 rounded border-border-main accent-accent-teal"
                        />
                        <span>
                          Are the experiences distinct in principal activity or
                          traveler rewards?
                        </span>
                      </label>
                      <label className="flex items-start gap-2 cursor-pointer">
                        ...
                        />
                        <span>
                          Does each justify distinct visitor travel effort and
                          situation?
                        </span>
                      </label>
                      <label className="flex items-start gap-2 cursor-pointer">
                        ...
                        />
                        <span>
                          Are there differences in spatial coordinate orbits
                          that prevent customer confusion?
                        </span>
                      </label>
                      <label className="flex items-start gap-2 cursor-pointer">
                        <input
                        ...
                        />
                        <span>
                          Is this curation verified with non-placeholder
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
                      <div className="md:col-span-2 space-y-1">
                        <label className="text-[8.5px] uppercase tracking-wider font-extrabold text-[#75776B] block">
                          Permanent Decision Rationale / Notes *
                        </label>
                        <textarea
                          value={rationaleText}
  ...
  return (
    <div className="fixed inset-0 bg-[#2D3025]/40 backdrop-blur-md z-[998] flex items-center justify-center p-6 text-brand-charcoal font-sans">
      ...
      >
        ...
        <div className="space-y-1">
          <h3 className="font-serif text-sm uppercase tracking-[0.2em] text-accent-red font-bold">
            EXPO 2027 ADMIN SECURITY
          </h3>
          <p className="text-[10px] text-brand-charcoal/60 leading-snug">
            Authorized personal Belgrade concierge agency broker gate.
          </p>
        </div>
        ...
        <div className="py-2.5">
          <div
            className={`h-14 w-full bg-[#FAF9F5] border rounded-2xl flex items-center justify-center gap-2.5 transition-colors ${
              errorStatus
                ? "border-accent-red bg-accent-red/5"
                : "border-border-main focus-within:border-accent-teal"
            }`}
          >
          ...
          {errorStatus && (
            <p className="text-[9.5px] text-accent-red mt-1.5 font-bold tracking-wide transition-all animate-bounce">
              {language === "sr"
                ? "Netačan PIN kod! Pristup zabranjen."
                : "Incorrect PIN! Administrative access denied."}
            </p>
          )}
        </div>
        ...
        <label className="flex items-center gap-2 px-3 py-2 bg-[#FAF9F5] rounded-xl border border-border-main/50 text-left cursor-pointer select-none">
          ...
          <span className="text-[8.5px] italic text-brand-charcoal/70 leading-snug">
            {language === "sr"
              ? "Potvrđujem da sam ovlašćeni menadžer lokacije."
              : "I confirm authorized developer manager state."}
          </span>
        </label>
        {checkboxError && (
          <p className="text-[9px] text-accent-red font-semibold text-center leading-snug transition-all animate-pulse">
            {language === "sr"
              ? "Molimo označite polje za potvrdu ovlašćenja prasat!"
              : "Please check the manager confirmation box above!"}
          </p>
        )}
