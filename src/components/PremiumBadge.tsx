...
export default function PremiumBadge({ type, compact = false, onClick }: PremiumBadgeProps) {
  ...
  if (badgeType === 'silver') {
    ...
    containerStyles = 'bg-gradient-to-r from-[#CAD2C5] via-[#E2E8F0] to-[#94A3B8] border-[#94A3B8] shadow-[0_0_15px_rgba(148,163,184,0.45)]';
    ...
    containerStyles = 'bg-gradient-to-r from-[#FBBF24] via-[#FDE68A] to-[#D97706] border-[#B45309] shadow-[0_0_18px_rgba(245,158,11,0.55)]';
    ...
    containerStyles = 'bg-gradient-to-r from-[#C7D2FE] via-[#E0E7FF] to-[#818CF8] border-[#4338CA] shadow-[0_0_20px_rgba(99,102,241,0.65)] animate-pulse-subtle';
  ...
  return (
    ...
    >
      ...
      <span className={`absolute -inset-y-12 w-6 -left-10 rotate-[25deg] filter blur-md animate-shine pointer-events-none ${shineColor}`} />
      
      <span className={`${textStyles} relative z-10 font-extrabold flex items-center gap-1.5`}>
        {badgeType === 'silver' && <span className="text-[12px] filter drop-shadow">🥈</span>}
        {badgeType === 'gold' && <span className="text-[12px] filter drop-shadow">🥇</span>}
        {badgeType === 'platinum' && <span className="text-[12px] filter drop-shadow">💎</span>}
        <span>{text}</span>
      </span>
    </motion.div>
  );
