...
const GAP_FILLERS: any[] = [
  {
    ...
    shortDescription: 'Tucked away in the isolated peaks of central Serbia, this eco-haven offers pristine forest baths and slow-food gastronomy at 900m altitude.',
    longDescription: 'Gledić Highlands Retreat is designed for travelers seeking complete digital detox and physical restoration. Positioned perfectly in the high-elevation forests, it features cold-plunge mountain springs, private stone saunas, and hand-foraged organic dining led by local shepherds.',
    ...
    translations: {
      sr: {
        ...
        shortDescription: 'Skriveno eko-utočište na Gledićkim planinama sa šumskim kupatilima i organskom hranom.',
        location: 'Gledićke planine'
      },
      zh: {
        title: '格莱迪奇高地静修所',
        shortDescription: '坐落于塞尔维亚中部的幽静山脉中，提供森林浴与原始有机餐饮。',
        location: '格莱迪奇山脉'
      }
    },
  ...
  },
  {
    ...
    shortDescription: 'An exotic turquoise karst spring of surreal beauty, paired with thermal mineral baths and fresh river trout dining.',
    longDescription: 'Krupaj Springs Sanctuary represents a geological masterpiece in eastern Serbia. The thermal spring forms a deep turquoise pool surrounded by dense forest, where visitors can enjoy private mineral therapy and historical monastery visits.',
    ...
    translations: {
      sr: {
        ...
        shortDescription: 'Egzotični kraški izvor tirkizne vode, uparen sa termalnim mineralnim kupatilima.',
        location: 'Homolje'
      },
      zh: {
        title: '库帕伊泉生态保护区',
  ...
  },
  {
    ...
    shortDescription: 'A premium medieval banquet in the outer courtyard of Manasija Fortress, featuring authentic spiced honey wines.',
    longDescription: 'Despot Stefan’s Gilded Heritage merges royal history with medieval gastronomy. Set against the limestone towers of Manasija, guests dine on venison roast and copper-pot wines prepared according to preserved 15th-century court codices.',
    ...
    translations: {
      sr: {
        ...
        shortDescription: 'Ekskluzivni srednjovekovni banket u zidinama tvrđave Manasija sa vinima od meda.',
        location: 'Despotovac'
      },
      zh: {
        title: '斯特凡大公鎏金晚宴',
        shortDescription: '马纳西亚城堡外院的顶级中世纪宴会，配有秘制香料蜜酒。',
        location: '德斯波托瓦茨'
      }
    },
  ...
  },
  {
    ...
    shortDescription: 'A private riverside terrace offering custom molecular cocktails and ambient deep house under the stars.',
    longDescription: 'Zemun Riverfront Midnight Lounge offers premium late-night entertainment on the banks of the Danube. Combining historical tavern elements with modern luxury mixology and high-contrast sound design.',
    ...
    translations: {
      sr: {
        ...
        shortDescription: 'Ekskluzivni bar na Dunavskom keju sa molekularnim koktelima i deep house ritmovima.',
        location: 'Zemunski kej'
      },
      zh: {
        title: '泽蒙河畔午夜沙龙',
        shortDescription: '私密的河畔露台，在星空下提供定制分子鸡尾酒与深度浩室音乐。',
        location: '老泽蒙码头'
      }
    },
...
export default function MoodOrbGridAnalyzer({
  ...
  const getCurationCoords = (rec: any) => {
    if (typeof rec.coordinateX === 'number' && typeof rec.coordinateY === 'number') {
      return { x: rec.coordinateX, y: rec.coordinateY };
    }
    if (rec.customCoords) {
      return rec.customCoords;
    }
    const cats = typeof rec.category === 'string'
    ...
    for (let rowIdx = 0; rowIdx < 10; rowIdx++) {
      ...
      for (let colIdx = 0; colIdx < 10; colIdx++) {
        ...
        for (const cur of activeCurations) {
          const dist = Math.abs(sx - cur.coords.x) + Math.abs(sy - cur.coords.y);
          const matchPercent = Math.max(0, 100 - 5 * dist);
          if (matchPercent > maxMatch) {
            maxMatch = matchPercent;
            closestCurationId = cur.id;
            closestCurationTitle = language === 'sr' ? cur.srTitle : language === 'zh' ? cur.zhTitle : cur.title;
          }
        }
  ...
  return (
    <div className="bg-white rounded-[32px] border border-border-main p-6 shadow-tactile text-left animate-fade-in relative overflow-hidden mt-6" id="mood-orb-grid-analyzer">
      {/* Header */}
      <div className="flex justify-between items-start gap-4 mb-4">
        <div>
          <span className="text-[10px] uppercase tracking-[0.2em] text-accent-teal font-black flex items-center gap-1.5 mb-1">
            ...
            {language === 'sr' ? 'DIJAGNOSTIKA ORB POKRIVENOSTI' : language === 'zh' ? '星轨仪覆盖诊断' : 'ORB COVERAGE DIAGNOSTICS'}
          </span>
          <h3 className="text-lg font-serif text-brand-charcoal font-bold tracking-tight">
            {language === 'sr' ? 'Sistem 100 Kvadrata' : language === 'zh' ? '100 格局微调诊断' : '100-Squares Precision Grid'}
          </h3>
        </div>
      ...
      {showInfo && (
        <div className="bg-[#FAF9F5] rounded-2xl p-4 border border-border-main/60 mb-5 text-[11px] text-brand-charcoal/75 space-y-2 leading-relaxed">
          ...
          <p>
            <strong>{language === 'sr' ? 'Pravilo blizine:' : language === 'zh' ? '邻近匹配原则：' : 'Proximity Decaying Rule:'}</strong>{' '}
            {language === 'sr'
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="bg-[#FAF9F5] border border-border-main/50 px-3 py-2.5 rounded-2xl text-center">
          <p className="text-[8.5px] uppercase font-bold tracking-wider text-brand-charcoal/45 mb-0.5">
            {language === 'sr' ? 'PROSEČNA USKLAĐENOST' : language === 'zh' ? '全局平均适配度' : 'AVG COMPATIBILITY'}
          </p>
        ...
        <div className="bg-[#FAF9F5] border border-border-main/50 px-3 py-2.5 rounded-2xl text-center">
          <p className="text-[8.5px] uppercase font-bold tracking-wider text-brand-charcoal/45 mb-0.5">
            {language === 'sr' ? 'OPTIMALNA POKRIVENOST' : language === 'zh' ? '90%+ 完美覆盖率' : 'OPTIMAL COVERAGE'}
          </p>
          <p className={`text-xl font-mono font-black ${analysis.coveragePercent >= 90 ? 'text-[#2E7D32]' : 'text-[#D32F2F]'}`}>
        ...
        <div className="bg-[#FAF9F5] border border-border-main/50 px-3 py-2.5 rounded-2xl text-center">
          <p className="text-[8.5px] uppercase font-bold tracking-wider text-brand-charcoal/45 mb-0.5">
            {language === 'sr' ? 'BLIND TAČKE' : language === 'zh' ? '覆盖盲点数' : 'BLIND SPOTS'}
          </p>
          <p className={`text-xl font-mono font-black ${analysis.blindSpots.length === 0 ? 'text-[#2E7D32]' : 'text-accent-red'}`}>
      ...
      <div className="flex flex-col md:flex-row gap-5 items-center md:items-start">
        ...
        <div className="relative bg-neutral-900 rounded-[24px] p-4 flex-shrink-0 w-full max-w-[280px] aspect-square shadow-inner flex items-center justify-center">
          <div className="absolute top-2 left-3 text-[8px] font-mono text-white/30 uppercase tracking-widest">
            {language === 'sr' ? 'Senzorna Mreža 10x10' : language === 'zh' ? '10x10 感官坐标网' : '10x10 Sensory Net'}
          </div>
          
          <svg viewBox="0 0 220 220" className="w-full h-full select-none">
            {/* Axis grid lines */}
            <line x1="110" y1="10" x2="110" y2="210" stroke="rgba(255,255,255,0.15)" strokeWidth="1" strokeDasharray="2 2" />
            <line x1="10" y1="110" x2="210" y2="110" stroke="rgba(255,255,255,0.15)" strokeWidth="1" strokeDasharray="2 2" />
            ...
            {gridSquares.map((sq, i) => {
              ...
              const isUserInside = Math.abs(sq.x - userTargetCoords.x) < 0.5 && Math.abs(sq.y - userTargetCoords.y) < 0.5;
        ...
        <div className="flex-1 space-y-4 w-full">
          <div>
            <h4 className="text-[11px] font-mono font-bold text-brand-charcoal/45 uppercase tracking-wider mb-2">
              {language === 'sr' ? 'STANJE DIJAGNOSTIKE' : language === 'zh' ? '诊断报告明细' : 'DIAGNOSTIC STATUS'}
            </h4>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between border-b border-neutral-100 pb-1.5">
                <span className="text-brand-charcoal/60">{language === 'sr' ? 'Pozicija korisnika:' : language === 'zh' ? '当前心境星点:' : 'Current User Target:'}</span>
                <span className="font-mono font-bold text-accent-teal">[{userTargetCoords.x}, {userTargetCoords.y}]</span>
              </div>
              <div className="flex justify-between border-b border-neutral-100 pb-1.5">
                <span className="text-brand-charcoal/60">{language === 'sr' ? 'Zabeležene curacije:' : language === 'zh' ? '可用专属策划数:' : 'Available Curations:'}</span>
                <span className="font-bold">{activeCurations.length}</span>
              </div>
              <div className="flex justify-between border-b border-neutral-100 pb-1.5">
                <span className="text-brand-charcoal/60">{language === 'sr' ? 'Zasićenost pokrivenosti:' : language === 'zh' ? '全局理想覆合度:' : 'Global Coverage Fitness:'}</span>
                <span className={`font-mono font-black ${analysis.coveragePercent >= 90 ? 'text-[#2E7D32]' : 'text-accent-red'}`}>
                  {analysis.coveragePercent}% {analysis.coveragePercent >= 90 ? '✓ SECURE' : '⚠ WEAK'}
                </span>
              </div>
            </div>
          </div>
          ...
          >
            {gapFillersActive ? (
              <>
                ...
                <span>
                  {language === 'sr' ? 'PRODUKCIJA OSLOBOĐENA SLEPIH TAČAKA' : language === 'zh' ? '100% 理想盲区覆盖已启动' : '100% GAP-FILLERS PROVISIONED'}
                </span>
              </>
            ) : (
              <>
                ...
                <span>
                  {language === 'sr' ? 'OTKLONI SLEPE TAČKE (+4 CURACIJE)' : language === 'zh' ? '一键弥补体验盲区 (+4 奢华方案)' : 'CLOSE BLIND SPOTS (+4 CURATIONS)'}
                </span>
              </>
            )}
          </button>
