...
export default function MiniMoodGrid({ coordinateX, coordinateY, className = '' }: MiniMoodGridProps) {
  // Safely parse coordinates with fallback to 0 (Center) so a dot is ALWAYS rendered
  const numX = typeof coordinateX === 'number' && !isNaN(coordinateX) ? coordinateX : Number(coordinateX);
  const numY = typeof coordinateY === 'number' && !isNaN(coordinateY) ? coordinateY : Number(coordinateY);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <rect x="22" y="22" width="28" height="28" fill="#DC2626" fillOpacity="0.04" />
        <rect x="50" y="22" width="28" height="28" fill="#EA580C" fillOpacity="0.04" />
        <rect x="22" y="50" width="28" height="28" fill="#16A34A" fillOpacity="0.04" />
        <rect x="50" y="50" width="28" height="28" fill="#0284C7" fillOpacity="0.04" />

        {/* Outer Reference Guideline */}
        <rect x="22" y="22" width="56" height="56" fill="none" stroke="#D1CEBD" strokeWidth="0.8" strokeDasharray="3,3" />

        {/* Primary Axes Lines */}
        {/* Vertical Axis: Dense Metropolitan (top) ↔ Remote Nature (bottom) */}
        <line x1="50" y1="15" x2="50" y2="85" stroke="#44403C" strokeWidth="1.8" />
        
        {/* Horizontal Axis: Hedonist (left) ↔ Adventurer (right) */}
        <line x1="15" y1="50" x2="85" y2="50" stroke="#44403C" strokeWidth="1.8" />
        ...
        <circle cx="50" cy="50" r="2.5" fill="#FAF9F6" stroke="#44403C" strokeWidth="1.2" />
        ...
        <g className="transition-all duration-300">
          ...
          <circle cx={plotX - 1.8} cy={plotY - 1.8} r="2" fill="#FFFFFF" opacity="0.95" />
        </g>
      </svg>
    </div>
  );
