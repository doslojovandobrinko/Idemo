...
export function getRecommendationStatus(
  ...
  let hour = typeof testTime?.hour === 'number' ? testTime.hour : new Date().getHours();
  // Day of week: 0 = Sunday, 1 = Monday, 2 = Tuesday, etc.
  let day = typeof testTime?.day === 'number' ? testTime.day : new Date().getDay();
  ...
  if (categoryStr.includes('clubbing') || categoryStr.includes('party') || idStr === '15' || idStr === '41') {
    ...
    if (isClubbingNight && isClubbingHour) {
      return {
        ...
        badgeColor: "bg-red-500/10 text-red-500 border border-red-500/20 shadow-xs animate-pulse",
        detailLabel: language === 'sr' ? "Novi Berlin • 23:00 - 05:00 • Rezervacija stola obavezna" :
                    language === 'zh' ? "浮动主场 • 23:00 -次日05:00 • 必须预约卡座" :
      ...
      return {
        ...
        badgeColor: "bg-brand-charcoal/20 text-[#5C5A4D]/80 border border-border-main/50",
        detailLabel: language === 'sr' ? "Zatvoreno • Otvara se četvrtkom u 23:00" :
                    language === 'zh' ? "已关闭 • 周四至周六 23:00 起开放营业" :
  ...
  if (categoryStr.includes('gastronomy') || categoryStr.includes('food') || categoryStr.includes('bar') || idStr === '4' || idStr === '26' || idStr === '33' || idStr === '85') {
    ...
    if (isRestaurantHour) {
      return {
        ...
        detailLabel: language === 'sr' ? "Aktivno kulinarsko carstvo • 12:00 - 00:00" :
                    language === 'zh' ? "精品珍馐时段 • 12:00 - 00:00" :
      ...
      return {
        status: 'closed',
        badgeLabel: lValue('closed'),
        badgeColor: "bg-[#8A1F1F]/10 text-accent-red border border-accent-red/25",
        detailLabel: language === 'sr' ? "Kuhinja je zatvorena • Otvara se u 12:00" :
                    language === 'zh' ? "厨房歇业中 • 每日中午 12:00 准时开业" :
  ...
  if (categoryStr.includes('history') || categoryStr.includes('culture') || categoryStr.includes('museum') || idStr === '2' || idStr === '3' || idStr === '5' || idStr === '9' || idStr === '11') {
    ...
    if (isMonday) {
      return {
        status: 'closed',
        badgeLabel: lValue('closed'),
        badgeColor: "bg-[#8A1F1F]/10 text-accent-red border border-[#8A1F1F]/20",
        detailLabel: language === 'sr' ? "Zatvoreno ponedeljkom (Dan za održavanje)" :
                    language === 'zh' ? "周一闭馆 (日常系统维护日)" :
    ...
    if (isMuseumHour) {
      return {
        ...
        detailLabel: language === 'sr' ? "Otvoreno • 10:00 - 19:00 • Poslednji ulaz 18:30" :
                    language === 'zh' ? "开放中 • 10:00 - 19:00 • 18:30 停止入馆" :
      ...
      return {
        status: 'closed',
        badgeLabel: lValue('closed'),
        badgeColor: "bg-brand-charcoal/30 text-[#4C4E44]/75 border border-border-main",
        detailLabel: language === 'sr' ? "Zatvoreno • Radno vreme: Utorak - Nedelja 10:00 - 19:00" :
                    language === 'zh' ? "已关闭 • 开放时间：周二至周日 10:00 - 19:00" :
  ...
  if (categoryStr.includes('nature') || idStr === '1' || idStr === '10' || idStr === '14' || idStr === '17') {
    ...
    if (isDaylightHour) {
      return {
        ...
        badgeColor: "bg-emerald-500/10 text-emerald-700 border border-emerald-500/20",
        detailLabel: language === 'sr' ? "Prirodni rezervat • Najbolje posetiti od aprila do oktobra" :
                    language === 'zh' ? "大自然旷野 • 极力推荐在4月至10月黄金季节探访" :
      ...
      return {
        ...
        detailLabel: language === 'sr' ? "Niska vidljivost • Izbegavati noćne staze bez lokalnog vodiča" :
                    language === 'zh' ? "视线受阻不建议夜行 • 请避免在无专业向导下夜游" :
  ...
  return {
    status: 'verify',
    badgeLabel: language === 'sr' ? "Proverite radno vreme" : language === 'zh' ? "建议出发前确认" : "Verify Hours",
    badgeColor: "bg-brand-charcoal/10 text-brand-charcoal/70 border border-border-main/40",
