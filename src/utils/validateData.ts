...
export function runDataValidation() {
  ...
  INITIAL_RECOMMENDATIONS.forEach((rec) => {
    ...
    if (rec.coordinateX === undefined || rec.coordinateY === undefined) {
      ...
      if (rec.coordinateX < -5 || rec.coordinateX > 5) {
        errors.push(
          `coordinateX (${rec.coordinateX}) is out of bounds [-5, 5].`,
        );
      }
      if (rec.coordinateY < -5 || rec.coordinateY > 5) {
        errors.push(
          `coordinateY (${rec.coordinateY}) is out of bounds [-5, 5].`,
        );
      }
    }
    ...
    if (rec.category) {
      ...
      splitCats.forEach((cat) => {
        if (!validCategoryList.includes(cat)) {
          errors.push(
            `Invalid category component: "${cat}". Must be one of: ${validCategoryList.join(", ")}`,
          );
        }
      });
    }
    ...
    if (!rec.translations) {
      ...
      if (languages.length === 0) {
        ...
        languages.forEach((lang) => {
          ...
          if (!trans.title)
            warnings.push(`Translation [${lang}] is missing title.`);
          if (!trans.shortDescription)
            warnings.push(`Translation [${lang}] is missing shortDescription.`);
          if (!trans.longDescription)
            warnings.push(`Translation [${lang}] is missing longDescription.`);
        });
      }
    }
    ...
    if (rec.image) {
      if (!rec.image.startsWith("/src/")) {
        errors.push(
          `Image reference path "${rec.image}" should be standard "/src/..." relative format.`,
        );
      } else {
    ...
    semanticAttributes.forEach((attr) => {
      ...
      if (val === undefined || val === null) {
        ...
        errors.push(
          `Semantic attribute "${attr}" value (${val}) must be a number between 0 and 10.`,
        );
      }
    });
    ...
    if (rec.estimatedCost && rec.estimatedCost.length < 2) {
      warnings.push(
        `Suspiciously short estimatedCost value: "${rec.estimatedCost}".`,
      );
    }
  ...
  console.log(`\nAUDIT SUMMARY:`);
  console.log(
    `Total Curated Recommendations: ${INITIAL_RECOMMENDATIONS.length}`,
  );
...
if (
  ...
  process.argv[1]?.endsWith("validateData.ts")
) {
  runDataValidation();
}
