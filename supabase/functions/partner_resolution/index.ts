...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-partner-session",
...
function validateImageBinarySignature(
  bytes: Uint8Array,
  declaredMime: string,
  ...
  if (bytes.length < 12) {
    return {
      valid: false,
      detectedMime: null,
      error: "File binary is too small to contain a valid image header.",
    };
  }
  ...
  if (bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff) {
    if (declaredMime && declaredMime !== "image/jpeg") {
      return {
        ...
        error: `Binary signature is JPEG but declared MIME type is ${declaredMime}`,
      };
    }
  ...
  if (
    ...
    if (declaredMime && declaredMime !== "image/png") {
      return {
        ...
        error: `Binary signature is PNG but declared MIME type is ${declaredMime}`,
      };
    }
  ...
  if (
    ...
    if (declaredMime && declaredMime !== "image/webp") {
      return {
        ...
        error: `Binary signature is WebP but declared MIME type is ${declaredMime}`,
      };
    }
  ...
  return {
    valid: false,
    detectedMime: null,
    error: "Invalid image file signature or unsupported binary magic bytes.",
  };
}
...
serve(async (req: Request) => {
  ...
  try {
      ...
      if (!public_code || !pin) {
        return new Response(
          JSON.stringify({
            ...
            message: "Partner code and PIN are required.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (pathname.endsWith("/admin/profile-review") && req.method === "POST") {
      ...
      if (
        req.headers.has("x-partner-session") &&
        ...
        return new Response(
          JSON.stringify({
            ...
            error: "UNAUTHORIZED",
            message:
              "Partner sessions are not authorized to perform administrative or editorial profile reviews.",
          }),
          {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return new Response(
          JSON.stringify({
            ...
            error: "UNAUTHORIZED",
            message:
              "Studio/admin authentication token is required in Authorization header.",
          }),
          {
            status: 401,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const userRole = String(user.app_metadata?.role || user.role || "")
        .toLowerCase()
        .replace(/[\s_-]+/g, "");
      const isAllowedRole =
        userRole === "editoriallead" || userRole === "superadmin";
      ...
      if (!isAllowedRole) {
        return new Response(
          JSON.stringify({
            ...
            error: "FORBIDDEN",
            message:
              "Insufficient permissions. Only editorial_lead or super_admin roles can review partner profiles.",
          }),
          {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (!partner_id || !action) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "MISSING_FIELDS",
            message: "partner_id and action are required.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (sessionErr || !sessionData || !sessionData.valid) {
      return new Response(
        JSON.stringify({
          ...
          message:
            sessionData?.message || "Invalid or expired partner session.",
        }),
        {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (pathname.endsWith("/me") && req.method === "GET") {
      const { data: partnerRow } = await supabase
        .from("partners")
        .select(
          "id, public_code, name, status, is_open_for_inquiries, contact_preference, must_change_pin",
        )
        .eq("id", partnerId)
        .single();
      ...
      if (!current_pin || !new_pin || !confirm_new_pin) {
        return new Response(
          JSON.stringify({
            ...
            error: "MISSING_FIELDS",
            message:
              "Trenutni PIN, novi PIN i potvrda novog PIN-a su obavezni.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (pathname.endsWith("/opportunities") && req.method === "GET") {
      ...
      const { data, error } = await supabase.rpc(
        "get_partner_opportunities_secure",
        {
          p_partner_id: partnerId,
          p_scope: scope,
        },
    ...
    if (pathname.endsWith("/opportunities/view") && req.method === "POST") {
      ...
      const { data, error } = await supabase.rpc(
        "view_partner_opportunity_secure",
        {
          p_partner_id: partnerId,
          p_match_id: match_id,
        },
    ...
    if (
      pathname.endsWith("/profile-content/upload-authorize") &&
      req.method === "POST"
    ) {
      const body = await req.json().catch(() => ({}));
        ...
        return new Response(
          JSON.stringify({
            ...
            message:
              issueData?.message ||
              issueErr?.message ||
              "Failed to issue upload authorization.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (uploadErr || !uploadData?.signedUrl) {
        ...
        return new Response(
          JSON.stringify({
            ...
            error: "STORAGE_ERROR",
            message:
              uploadErr?.message || "Failed to generate signed upload URL.",
          }),
          {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (pathname.endsWith("/profile-content/draft") && req.method === "POST") {
      const body = await req.json().catch(() => ({}));
      const { intro_draft, draft_photo_path, draft_photo_mime, photo_consent } =
        body;
      ...
      if (draft_photo_path) {
        ...
        if (!draft_photo_mime || !draft_photo_mime.trim()) {
          return new Response(
            JSON.stringify({
              ...
              error: "MIME_REQUIRED",
              message:
                "draft_photo_mime is mandatory when draft_photo_path is provided.",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        if (downloadErr || !blob) {
          return new Response(
            JSON.stringify({
              ...
              message:
                downloadErr?.message ||
                "Failed to download uploaded draft image for verification.",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        const sigResult = validateImageBinarySignature(
          headerBytes,
          draft_photo_mime || "",
        );
        ...
        if (!sigResult.valid) {
          ...
          return new Response(
            JSON.stringify({
              ...
              message:
                sigResult.error ||
                "Uploaded file binary magic bytes failed validation.",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { data, error } = await supabase.rpc(
        "save_partner_profile_draft_with_authorization_secure",
        {
          p_partner_id: partnerId,
    ...
    if (
      pathname.endsWith("/profile-content/withdraw") &&
      req.method === "POST"
    ) {
      const body = await req.json().catch(() => ({}));
      ...
      const { data, error } = await supabase.rpc(
        "withdraw_partner_profile_v2_secure",
        {
          p_partner_id: partnerId,
          p_scope: scope,
        },
    ...
    if (pathname.endsWith("/opportunities/accept") && req.method === "POST") {
      ...
      if (!match_id || !message) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "MISSING_FIELDS",
            message: "match_id and message are required.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { data, error } = await supabase.rpc(
        "accept_partner_opportunity_secure",
        {
          p_partner_id: partnerId,
          p_match_id: match_id,
          p_message: message,
    ...
    if (pathname.endsWith("/opportunities/decline") && req.method === "POST") {
      ...
      const { data, error } = await supabase.rpc(
        "decline_partner_opportunity_secure",
        {
          p_partner_id: partnerId,
          p_match_id: match_id,
    ...
    if (pathname.endsWith("/opportunities/propose") && req.method === "POST") {
      ...
      if (!match_id || !proposed_start_at || !proposed_end_at || !message) {
        return new Response(
          JSON.stringify({
            ...
            error: "MISSING_FIELDS",
            message:
              "match_id, proposed_start_at, proposed_end_at, and message are required.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { data, error } = await supabase.rpc(
        "propose_partner_alternative_secure",
        {
          p_partner_id: partnerId,
          p_match_id: match_id,
