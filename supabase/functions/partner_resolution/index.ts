  ...
  try {
    ...
    if (pathname.endsWith("/me") && req.method === "GET") {
      const { data: partnerRow } = await supabase
        .from("partners")
        .select("id, public_code, name, status, is_open_for_inquiries, contact_preference, must_change_pin")
        .eq("id", partnerId)
        .single();
    ...
    if (pathname.endsWith("/profile-content/draft") && req.method === "POST") {
      const body = await req.json().catch(() => ({}));
      const { intro_draft, draft_photo_path, draft_photo_mime, photo_consent } = body;
