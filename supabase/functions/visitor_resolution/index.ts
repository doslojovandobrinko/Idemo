...
serve(async (req) => {
  ...
  if (!rawIp) {
    return errorResponse(500, "Your request cannot be checked at this moment. Please try again shortly.", "Untrusted or missing client source identity.");
  }

  const clientIp = normalizeIp(rawIp);
  if (!clientIp || clientIp === "unknown" || clientIp === "") {
    return errorResponse(500, "Your request cannot be checked at this moment. Please try again shortly.", "Invalid client source identity.");
  }

  // Check fast in-memory limiter to guard against massive visual spam (secondary optimization)
  if (isRateLimited(clientIp)) {
    return errorResponse(429, "Too many requests. Please slow down.", "In-memory backup rate limit exceeded.");
  }
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return errorResponse(500, "Internal gateway misconfiguration.", "Missing environment keys on edge runtime.");
    }
    ...
    if (!secret) {
      return errorResponse(500, "Your request cannot be checked at this moment. Please try again shortly.", "Server misconfiguration: RECOVERY_LIMIT_SECRET is absent.");
    }
    ...
    if (!inquiryId || !rawToken) {
      return errorResponse(400, "Access denied", "Missing required parameters inquiry_id or token.");
    }

    // UUID format check to prevent SQL or format exceptions
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (!uuidRegex.test(inquiryId)) {
      return errorResponse(400, "Access denied", "Invalid inquiry UUID format.");
    }
    ...
    if (path.endsWith("/status")) {
      ...
      if (!matchId) return errorResponse(400, "Access denied", "Missing match_id for confirmation.");
      rpcName = "confirm_proposal";
      rpcParams.p_match_id = matchId;
    } else if (path.endsWith("/decline")) {
      if (!matchId) return errorResponse(400, "Access denied", "Missing match_id for decline.");
      ...
      const { data: valData, error: valErr } = await supabase.rpc("validate_and_get_inquiry", {
        p_inquiry_id: inquiryId,
        p_raw_token: rawToken,
      });

      if (valErr || !valData) {
        return errorResponse(403, "Access denied", `Visitor validation failed: ${valErr?.message || "Invalid credential"}`);
      }

      // 2. Query eligible match for this inquiry
      const { data: matches, error: matchErr } = await supabase
        .from("inquiry_matches")
        .select("id, partner_id, status, inquiries!inner(status, recommendation_id)")
        .eq("inquiry_id", inquiryId)
        .in("status", ["responded", "selected"])
        .in("inquiries.status", ["awaiting_visitor", "confirmed", "in_progress"]);
      ...
      if (isConfirmedHandoff) {
        const targetRecId = valData?.recommendation_id || (match as any).inquiries?.recommendation_id;
        if (targetRecId) {
          const { data: eligRow } = await supabase
            .from("recommendation_partner_eligibility")
    ...
    if (error) {
      ...
      if (errMsg.includes("Inquiry is not awaiting visitor resolution")) {
        return errorResponse(400, "The request is not in a resolvable state.", `State mismatch: ${errMsg}`);
      }

      if (errMsg.includes("Proposal already accepted")) {
        return errorResponse(400, "This offer has already been accepted.", `Idempotency block: ${errMsg}`);
      }

      if (errMsg.includes("Proposal already declined") || errMsg.includes("Proposal details not found or already resolved")) {
        return errorResponse(400, "This offer has already been resolved.", `Idempotency block: ${errMsg}`);
      }

      if (errMsg.includes("Alternative option already requested")) {
        return errorResponse(400, "An alternative option has already been requested.", `Idempotency block: ${errMsg}`);
      }

      // Default fallback error
      return errorResponse(400, "An error occurred while processing your request. Please try again.", `Database RPC exception: ${errMsg}`);
    }
    ...
    return errorResponse(500, "An unexpected error occurred. Please try again.", `Internal crash: ${err.message}`);
  }
});
