...
* *Constraint*: If the request violates any constitutional principle, the AI Agent MUST refuse implementation.
...
* *Freeze Constraint*: Once frozen, the AI Agent is strictly prohibited from redesigning, reinterpreting, simplifying, substituting, renaming, or introducing "minor improvements". Any conflict with baseline or runtime dependencies requires an immediate STOP. Silent workarounds are prohibited.
