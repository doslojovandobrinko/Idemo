...
langs.forEach((lang) => {
  const complete = canonical.filter((r) => {
    ...
    if (!t) return false;
    if (!t.title || !t.shortDescription || !t.longDescription || !t.location)
      return false;
  ...
  console.log(
    `${lang.toUpperCase()}: ${complete.length}/${canonical.length} authentic translations (${canonical.length - complete.length} missing/fallback)`,
  );
});
