...
console.log(
  `Building complete 5-language translation matrix for ${canonical.length} canonical items...`,
);
...
function generateLanguageRecord(rec: any, lang: string) {
  ...
  const isEng = (s?: string) =>
    s === rec.shortDescription || s === rec.longDescription;
  ...
  if (
    ...
    existing.location &&
    !isEng(existing.shortDescription)
  ) {
    return {
