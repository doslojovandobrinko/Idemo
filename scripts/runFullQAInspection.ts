...
categories.forEach((cat) => {
  const items = canonical.filter(
    (r) => r.category === cat || r.category.includes(cat),
  );
...
sampled.forEach((r, idx) => {
  ...
  ["sr", "zh", "de", "ru", "es"].forEach((lang) => {
    ...
    console.log(
      `  ${lang.toUpperCase()}: Title="${t?.title}" [SameTitle:${sameTitle}] | Loc="${t?.location}"`,
    );
    console.log(
      `      Short: "${t?.shortDescription}" [SameShort:${sameShort}]`,
    );
  });
  console.log(
    `----------------------------------------------------------------\n`,
