...
${JSON.stringify(promptItems, null, 2)}`;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(
        `Attempt ${attempt} failed for ${langCode}: ${err.message || err}`,
      );
...
async function runWaveForLang(target: {
  code: string;
  name: string;
  wave: string;
}) {
  ...
  console.log(`\n============================================`);
  console.log(
    `STARTING ${target.wave}: ${target.name} (${target.code.toUpperCase()})`,
  );
  console.log(`============================================`);
  ...
  for (let i = 0; i < canonicalRecs.length; i += batchSize) {
    ...
    console.log(
      `Translating batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(canonicalRecs.length / batchSize)} (${target.code.toUpperCase()})...`,
    );
    ...
    for (const item of batch) {
      if (
        res[item.id] &&
        res[item.id].title &&
        res[item.id].shortDescription &&
        res[item.id].longDescription &&
        res[item.id].location
      ) {
        langDict[item.id] = {
        ...
        console.warn(
          `Missing generated translation for ID ${item.id} in ${target.code}`,
        );
        langDict[item.id] = {
          title: item.translations?.[target.code]?.title || item.title,
          shortDescription:
            item.translations?.[target.code]?.shortDescription ||
            item.shortDescription,
          longDescription:
            item.translations?.[target.code]?.longDescription ||
...
`;
  ...
  console.log(
    `Successfully generated and saved ${target.code}Translations.ts with ${Object.keys(langDict).length} items.`,
  );
}
